/* 
  ...
  KreMLin invocation: /Users/bhargava/Desktop/repositories/kremlin//krml -funroll-loops 8 -warn-error +9 -add-include "TestLib.h" -add-include "vec-intrin.h" -add-include "FStar_UInt_8_16_32_64.h" /dist/generic/testlib.c -skip-compilation -bundle Lib.* -bundle Spec.\* -bundle Hacl.Poly1305_256=Hacl.Impl.* -bundle FStar.* -drop Lib.IntVector.Intrinsics,LowStar,Spec,Prims,Lib,C.Loops.*,C.*,C,FStar.*,Hacl.Spec.*,Hacl.Poly1305.Field32xN.Lemmas,Hacl.Poly1305_32,Hacl.Poly1305_128 -tmpdir poly-c .output/prims.krml .output/FStar_Pervasives_Native.krml .output/FStar_Pervasives.krml .output/FStar_Reflection_Types.krml .output/FStar_Reflection_Data.krml .output/FStar_Order.krml .output/FStar_Reflection_Basic.krml .output/FStar_Mul.krml .output/FStar_Preorder.krml .output/FStar_Calc.krml .output/FStar_Squash.krml .output/FStar_Classical.krml .output/FStar_StrongExcludedMiddle.krml .output/FStar_FunctionalExtensionality.krml .output/FStar_List_Tot_Base.krml .output/FStar_List_Tot_Properties.krml .output/FStar_List_Tot.krml .output/FStar_Seq_Base.krml .output/FStar_Seq_Properties.krml .output/FStar_Seq.krml .output/FStar_Math_Lib.krml .output/FStar_Math_Lemmas.krml .output/FStar_BitVector.krml .output/FStar_UInt.krml .output/FStar_UInt32.krml .output/FStar_Int.krml .output/FStar_Int16.krml .output/FStar_Ghost.krml .output/FStar_ErasedLogic.krml .output/FStar_Exn.krml .output/FStar_Set.krml .output/FStar_Monotonic_Witnessed.krml .output/FStar_PropositionalExtensionality.krml .output/FStar_PredicateExtensionality.krml .output/FStar_TSet.krml .output/FStar_Monotonic_Heap.krml .output/FStar_Heap.krml .output/FStar_ST.krml .output/FStar_All.krml .output/Lib_LoopCombinators.krml .output/FStar_Int64.krml .output/FStar_Int63.krml .output/FStar_Int32.krml .output/FStar_Int8.krml .output/FStar_UInt64.krml .output/FStar_UInt63.krml .output/FStar_UInt16.krml .output/FStar_UInt8.krml .output/FStar_Int_Cast.krml .output/FStar_UInt128.krml .output/FStar_Int_Cast_Full.krml .output/Lib_IntTypes.krml .output/Lib_Sequence.krml .output/FStar_Map.krml .output/FStar_Monotonic_HyperHeap.krml .output/FStar_Monotonic_HyperStack.krml .output/FStar_HyperStack.krml .output/FStar_HyperStack_ST.krml .output/FStar_Universe.krml .output/FStar_GSet.krml .output/FStar_ModifiesGen.krml .output/FStar_Range.krml .output/FStar_Tactics_Types.krml .output/FStar_Tactics_Result.krml .output/FStar_Tactics_Effect.krml .output/FStar_Tactics_Util.krml .output/FStar_Reflection_Const.krml .output/FStar_Char.krml .output/FStar_List.krml .output/FStar_String.krml .output/FStar_Reflection_Derived.krml .output/FStar_Tactics_Builtins.krml .output/FStar_Reflection_Formula.krml .output/FStar_Reflection_Derived_Lemmas.krml .output/FStar_Reflection.krml .output/FStar_Tactics_Derived.krml .output/FStar_Tactics_Logic.krml .output/FStar_Tactics.krml .output/FStar_BigOps.krml .output/LowStar_Monotonic_Buffer.krml .output/LowStar_Buffer.krml .output/LowStar_BufferOps.krml .output/FStar_HyperStack_All.krml .output/FStar_Kremlin_Endianness.krml .output/C_Endianness.krml .output/C.krml .output/Spec_Loops.krml .output/C_Loops.krml .output/Lib_Loops.krml .output/Lib_RawIntTypes.krml .output/Lib_ByteSequence.krml .output/LowStar_ImmutableBuffer.krml .output/Lib_Buffer.krml .output/Lib_ByteBuffer.krml .output/Lib_IntVector_Intrinsics.krml .output/Spec_GaloisField.krml .output/Spec_AES.krml .output/Lib_IntVector.krml .output/Spec_Poly1305.krml .output/Hacl_Spec_Poly1305_Vec.krml .output/Hacl_Impl_Poly1305_Lemmas.krml .output/Hacl_Spec_Poly1305_Field32xN.krml .output/Hacl_Poly1305_Field32xN_Lemmas.krml .output/Hacl_Spec_Poly1305_Field32xN_Lemmas.krml .output/Hacl_Impl_Poly1305_Field32xN.krml .output/Hacl_Impl_Poly1305_Fields.krml .output/Hacl_Impl_Poly1305.krml .output/Hacl_Poly1305_128.krml .output/Hacl_Poly1305_32.krml .output/Hacl_Poly1305_256.krml
  F* version: 7c70b890
...
static void
Hacl_Impl_Poly1305_Field32xN_load_acc4(Lib_IntVector_Intrinsics_vec256 *acc, uint8_t *b)
{
...
static void
Hacl_Impl_Poly1305_Field32xN_fmul_r4_normalize(
...
inline static void
Hacl_Impl_Poly1305_poly1305_init_256(Lib_IntVector_Intrinsics_vec256 *ctx, uint8_t *key)
{
...
inline static void
Hacl_Impl_Poly1305_poly1305_update1_256(
...
inline static void
Hacl_Impl_Poly1305_poly1305_update1_last_256(
  Lib_IntVector_Intrinsics_vec256 *pre,
  uint32_t len1,
  uint8_t *b,
  Lib_IntVector_Intrinsics_vec256 *acc
)
{
  ...
  e[3U] = f3;
  e[4U] = f4;
  uint64_t b1 = (uint64_t)1U << len1 * (uint32_t)8U % (uint32_t)26U;
  Lib_IntVector_Intrinsics_vec256 mask = Lib_IntVector_Intrinsics_vec256_load64(b1);
  Lib_IntVector_Intrinsics_vec256 fi = e[len1 * (uint32_t)8U / (uint32_t)26U];
  e[len1 * (uint32_t)8U / (uint32_t)26U] = Lib_IntVector_Intrinsics_vec256_or(fi, mask);
  Lib_IntVector_Intrinsics_vec256 *r = pre;
  Lib_IntVector_Intrinsics_vec256 *r5 = pre + (uint32_t)5U;
  Lib_IntVector_Intrinsics_vec256 r0 = r[0U];
  Lib_IntVector_Intrinsics_vec256 r1 = r[1U];
...
inline __attribute((always_inline)) static void
Hacl_Impl_Poly1305_poly1305_updaten_256(
  Lib_IntVector_Intrinsics_vec256 *pre,
  uint8_t *b,
  Lib_IntVector_Intrinsics_vec256 *acc,
...
{
  KRML_CHECK_SIZE(sizeof (Lib_IntVector_Intrinsics_vec256), (uint32_t)5U);
  Lib_IntVector_Intrinsics_vec256 e[5U] = {0};
  Lib_IntVector_Intrinsics_vec256 *rn = pre + (uint32_t)10U;
 ...
 Lib_IntVector_Intrinsics_vec256
  ...
  e[4U] = f41;
  Lib_IntVector_Intrinsics_vec256 f4 = e[4U];
  e[4U] = Lib_IntVector_Intrinsics_vec256_or(f4, m3);
...
)
{
  ...
  uint8_t *t0 = text;
  Lib_IntVector_Intrinsics_vec256 m1 = Lib_IntVector_Intrinsics_vec256_load64((uint64_t)0x3ffffffU);
  Lib_IntVector_Intrinsics_vec256 m2 = Lib_IntVector_Intrinsics_vec256_load64((uint64_t)0x3fffU);
  Lib_IntVector_Intrinsics_vec256 m3 = Lib_IntVector_Intrinsics_vec256_load64((uint64_t)0x1000000U);

  if (len0 > (uint32_t)0U)
  {
    uint32_t bs = (uint32_t)64U;
    uint8_t *text0 = t0;
    Hacl_Impl_Poly1305_Field32xN_load_acc4(acc, text0);
    ...
    for (uint32_t i = (uint32_t)0U; i < nb; i = i + (uint32_t)1U)
    {
      ...
      Hacl_Impl_Poly1305_poly1305_updaten_256(pre, block, acc,m1,m2,m3);
    }
    Hacl_Impl_Poly1305_Field32xN_fmul_r4_normalize(acc, pre);
  }
  ...
  for (uint32_t i = (uint32_t)0U; i < nb; i = i + (uint32_t)1U)
  {
    ...
    Hacl_Impl_Poly1305_poly1305_update1_256(pre, block, acc);
  }
  uint8_t *b = t1 + nb * (uint32_t)16U;
  if (rem1 > (uint32_t)0U)
    Hacl_Impl_Poly1305_poly1305_update1_last_256(pre, rem1, b, acc);
}
...
{
  Hacl_Impl_Poly1305_poly1305_finish_256(tag, key, ctx);
}

void Hacl_Poly1305_256_poly1305_mac(uint8_t *tag, uint32_t len1, uint8_t *text, uint8_t *key)
{
