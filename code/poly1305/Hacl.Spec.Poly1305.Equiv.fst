...
val repeat_blocks_multi_vec_equiv_pre_lemma: #w:lanes -> r:pfelem -> b:block_v w -> acc_v0:elem w -> Lemma
  (let rw = compute_rw #w r in
   ...
   PLoops.repeat_blocks_multi_vec_equiv_pre w size_block (w * size_block) f f_v (normalize_n r) b acc_v0)
...
let poly_update_multi_lemma_v #w text acc_v0 r =
  ...
  PLoops.lemma_repeat_blocks_multi_vec w size_block text f f_v (normalize_n r) acc_v0
...
let poly1305_update_vec_lemma #w text acc0 r =
  ...
  if len0 > 0 then begin
    ...
    PLoops.repeat_blocks_split size_block len0 text f l acc0 end
