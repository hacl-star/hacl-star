...
val lemma_repeat_blocks_multi_load_acc:
  ...
  (ensures
   (let len0 = w * size_block in
    let t0 = Seq.slice inp 0 len0 in
    normalize_n (load_acc t0 acc0) == repeat_blocks_multi size_block t0 f acc0))
...
let lemma_repeat_blocks_multi_vec #a #b #b_vec w size_block inp f f_vec normalize_n load_acc acc0 =
  ...
  let acc1 = load_acc t0 acc0 in
  let acc2 = normalize_n acc1 in
  let acc3 = repeat_blocks_multi size_block t0 f acc0 in
  ...
  calc (==) {
    ...
    Loops.repeati nb repeat_bf_t1 acc2;
    (==) { lemma_repeat_blocks_multi_load_acc w size_block inp f normalize_n load_acc acc0 }
    Loops.repeati nb repeat_bf_t1 acc3;
    (==) { lemma_repeat_blocks_multi size_block t1 f acc3 }
    repeat_blocks_multi size_block t1 f acc3;
