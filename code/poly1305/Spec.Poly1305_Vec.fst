...
let poly_update_multi (#w:lanes) (text:bytes{0 < length text /\ length text % (w * size_block) = 0}) (acc:pfelem) (r:elem w) : pfelem =
...
let poly (#w:lanes) (text:bytes) (acc:pfelem) (r:elem w) : pfelem =
  let len = length text in
  let sz_block = w * size_block in
  let len0 = len / sz_block * sz_block in
  let t0 = Seq.slice text 0 len0 in
  let acc = if len0 > 0 then poly_update_multi t0 acc r else acc in

  let t1 = Seq.slice text len0 len in
  poly_update1 t1 acc (from_elem r)

let poly_update (#w:lanes) (text:bytes) (acc:pfelem) (r:elem w) : pfelem =
  match w with
  | 1 -> poly_update1 text acc (from_elem r)
  | 2 -> poly text acc r
  | 4 -> poly text acc r
...
let poly1305_init (#w:lanes) (k:key) : pfelem & elem w =
