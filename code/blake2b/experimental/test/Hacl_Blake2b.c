...
static void Hacl_Impl_Blake2b_g1(uint64_t *wv, uint32_t a, uint32_t b, uint32_t r)
{
  uint64_t wv_a = wv[a];
  uint64_t wv_b = wv[b];
...
static void Hacl_Impl_Blake2b_g2(uint64_t *wv, uint32_t a, uint32_t b, uint64_t x)
{
  uint64_t wv_a = wv[a];
  uint64_t wv_b = wv[b];
  wv[a] = wv_a + wv_b + x;
...
static void Hacl_Impl_Blake2b_blake2_round(uint64_t *wv, uint64_t *m, uint32_t i)
{
...
Hacl_Impl_Blake2b_blake2b_update_block(uint64_t *hash, FStar_UInt128_uint128 prev, uint8_t *d)
{
  ...
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)16U; i = i + (uint32_t)1U)
  {
    ...
    uint64_t u = lowstar_load64_le(bj);
    uint64_t r = u;
    uint64_t x = r;
    os[i] = x;
  }
...
static void Hacl_Impl_Blake2b_blake2b_finish(uint32_t nn, uint8_t *output, uint64_t *hash)
{
  uint8_t b[64U] = { 0U };
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)8U; i = i + (uint32_t)1U)
    lowstar_store64_le(b + i * (uint32_t)8U, hash[i]);
  uint8_t *final = b;
...
void Hacl_Blake2b_blake2b_init(uint64_t *hash, uint32_t kk, uint8_t *k, uint32_t nn)
{
  KRML_CHECK_SIZE(sizeof (uint8_t), (uint32_t)16U * (uint32_t)8U);
  uint8_t b[(uint32_t)16U * (uint32_t)8U];
  memset(b, 0U, (uint32_t)16U * (uint32_t)8U * sizeof b[0U]);
...
void Hacl_Blake2b_blake2b_update_block(uint64_t *hash, FStar_UInt128_uint128 prev, uint8_t *d)
{
  Hacl_Impl_Blake2b_blake2b_update_block(hash, prev, d);
}

void
Hacl_Blake2b_blake2b_update_last(
  uint64_t *hash,
  FStar_UInt128_uint128 prev,
  uint32_t len,
  uint8_t *last1
...
void Hacl_Blake2b_blake2b_finish(uint32_t output, uint8_t *hash, uint64_t *nn)
{
