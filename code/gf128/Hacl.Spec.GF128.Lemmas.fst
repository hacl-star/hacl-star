...
let add_opp a =
  FStar.UInt.logxor_self #128 (v a);
  v_extensionality (a ^. a) zero


[@canon_attr]
let elem_cr : cr elem =
  CR elem_add_cm elem_mul_cm ( ~% ) add_opp mul_add_distr mul_zero_l

let gf128_semiring () : Tac unit = canon_semiring elem_cr; trefl ()


let gf128_update_multi_mul_add_lemma_load_acc_aux a0 b0 b1 b2 b3 r = admit()
  // add_identity b1;
  // add_identity b2;
  // add_identity b3;
  // assert (
...
let gf128_update_multi_mul_add_lemma_loop_aux a0 a1 a2 a3 b0 b1 b2 b3 r = admit()
  // assert (
