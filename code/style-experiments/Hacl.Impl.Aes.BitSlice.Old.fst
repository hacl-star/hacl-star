...
val aes128_alloc: unit -> StackInline (keyex * state)
			     (requires (fun h -> True))
			     (ensures (fun h0 (x,y) h1 -> live h1 x /\ live h1 y))
let aes128_alloc () = 
  let keyex = alloca (u64 0) 88ul in
  let nonce = alloca (u64 0) 8ul in
  (keyex,nonce)

val aes128_init: keyx:keyex -> nvec:state -> key:skey -> nonce:lbytes 12 -> ST unit
