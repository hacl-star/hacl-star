...
let load_felem f u64s = 
    ...
    let f2l = (u64s.(2ul) &. u64 0x1fff) <<. 38ul in
    let f2h = u64s.(2ul) >>. 13ul in
    let f3l = (u64s.(3ul) &. u64 0x3fffffffff) <<. 13ul in
    let f3h = u64s.(3ul) >>. 38ul in
    f.(size 0) <- f0l;
    f.(size 1) <- f0h ^. f1l;
    f.(size 2) <- f1h ^. f2l;
    f.(size 3) <- f2h ^. f3l;
...
let store_felem u64s f = 
    let f0 = f.(0ul) ^. (f.(1ul) <<. 51ul) in
    let f1 = (f.(1ul) >>. 13ul) ^. (f.(2ul) <<. 38ul) in 
    let f2 = (f.(2ul) >>. 13ul) ^. (f.(3ul) <<. 38ul) in 
    let f3 = (f.(3ul) >>. 13ul) in
    u64s.(0ul) <- f0;
    u64s.(1ul) <- f1;
    u64s.(2ul) <- f2;
    u64s.(3ul) <- f3
...
val set_zero: f:felem -> Stack unit
                   (requires (fun h -> live h f))
		   (ensures (fun h0 _ h1 -> modifies (loc f) h0 h1))
let set_zero f = memset f (u64 0) 5ul
...
//inline_for_extraction
[@ CInline]
val mul_felem: out:felem_wide -> f1:felem -> r:felem -> r38:felem  -> Stack unit
                   (requires (fun h -> live h out /\ live h f1 /\ live h r /\ live h r38))
		   (ensures (fun h0 _ h1 -> modifies (loc out) h0 h1))
[@ CInline]
let mul_felem out f1 r r38 = 
  push_frame();
  let tmp = create 5ul (u64 0) in
  smul_felem out f1.(size 0) r;
  tmp.(size 0) <- r38.(size 4);
  tmp.(size 1) <- r.(size 0);
  tmp.(size 2) <- r.(size 1);
  tmp.(size 3) <- r.(size 2);
  tmp.(size 4) <- r.(size 3);
...
let carry_top_felem f =
  ...
  let f3 = f.(size 3) in
  let f4 = f.(size 4) in
  let tmp4,carry = carry51 f4 (u64 0) in
  let tmp0,carry = carry51 f0 (carry *. u64 19) in
...
//  tmp.(0ul) <- f2.(0ul) *. u64 38;
  tmp.(1ul) <- f2.(1ul) *. u64 38;
  tmp.(2ul) <- f2.(2ul) *. u64 38;
  tmp.(3ul) <- f2.(3ul) *. u64 38;
  tmp.(4ul) <- f2.(4ul) *. u64 38;
