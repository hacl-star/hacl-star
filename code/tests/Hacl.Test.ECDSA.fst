...
let check_bound b =
  ...
  [@inline_let]
  let q1 = normalize_term (p256_order % pow2 64) in 
  [@inline_let]
  let q2 = normalize_term ((p256_order / pow2 64) % pow2 64) in
  [@inline_let]
  let q3 = normalize_term ((p256_order / pow2 128) % pow2 64) in
  [@inline_let]
  let q4 = normalize_term (((p256_order / pow2 128) / pow2 64) % pow2 64) in
  assert_norm (pow2 128 * pow2 64 == pow2 192);
  assert (p256_order == q1 + pow2 64 * q2 + pow2 128 * q3 + pow2 192 * q4); 
