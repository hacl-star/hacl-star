...
let store_state #w b st =
    ...
    assert (Lib.Sequence.equal (as_seq h1 b)
...
inline_for_extraction
val transpose8: st:state 8 -> ST unit
		  (requires (fun h -> live h st))
...
inline_for_extraction
val transpose: #w:lanes -> st:state w -> ST unit
		  (requires (fun h -> live h st))
