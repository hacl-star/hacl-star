...
inline_for_extraction
val salsa20_init: ctx:state -> k:lbuffer uint8 32ul -> n:lbuffer uint8 8ul -> ctr0:size_t -> Stack unit
...
inline_for_extraction
val salsa20_encrypt_block: ctx:state ->
...
inline_for_extraction
val salsa20_encrypt_last: ctx:state ->
			   len:size_t{v len < 64} ->
...
inline_for_extraction
val salsa20_update: ctx:state -> len:size_t -> out:lbuffer uint8 len -> text:lbuffer uint8 len -> Stack unit
		  (requires (fun h ->
			    live h ctx /\
			    live h text /\
			    live h out /\
