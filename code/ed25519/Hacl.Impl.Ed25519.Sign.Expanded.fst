...
let sign_ signature ks len msg tmp_bytes tmp_ints =
  ...
  let tmp_xsecret = sub tmp_bytes 96ul 64ul in
  (**) let h0 = ST.get() in
  ...
  (**) assert (as_seq h1 (gsub tmp_bytes 96ul 32ul) `Seq.equal` as_seq h0 (gsub ks 32ul 32ul));
  (**) assert (Seq.slice (as_seq h0 (gsub ks 32ul 64ul)) 32 64 `Seq.equal` as_seq h0 (gsub ks 64ul 32ul));
  (**) assert (as_seq h1 (gsub tmp_bytes 128ul 32ul) `Seq.equal` as_seq h0 (gsub ks 64ul 32ul));
...
let sign_expanded signature ks msg len =
  push_frame();
  let tmp_bytes = create 160ul (u8 0) in
  let tmp_ints  = create 25ul (u64 0) in
