...
let sign_ signature ks len msg tmp_bytes tmp_ints =
  ...
  let tmp_public = sub tmp_bytes 0ul 32ul in
  let tmp_xsecret = sub tmp_bytes 128ul 64ul in
  (**) let h0 = get() in
  ...
  (**) let h1 = get() in
  (**) assert (as_seq h1 (gsub tmp_bytes 0ul 32ul) `Seq.equal` as_seq h0 (gsub ks 0ul 32ul));
  (**) assert (Seq.slice (as_seq h0 (gsub ks 32ul 64ul)) 0 32 `Seq.equal` as_seq h0 (gsub ks 32ul 32ul));
  (**) assert (as_seq h1 (gsub tmp_bytes 128ul 32ul) `Seq.equal` as_seq h0 (gsub ks 32ul 32ul));
  (**) assert (Seq.slice (as_seq h0 (gsub ks 32ul 64ul)) 32 64 `Seq.equal` as_seq h0 (gsub ks 64ul 32ul));
  (**) assert (as_seq h1 (gsub tmp_bytes 160ul 32ul) `Seq.equal` as_seq h0 (gsub ks 64ul 32ul));
  ...
  (**) let h5 = ST.get() in
  (**) BSeq.lemma_nat_from_to_bytes_le_preserves_value (as_seq h5 s') 32;
...
let sign_expanded signature ks msg len =
  push_frame();
  let tmp_bytes = create 192ul (u8 0) in
  let tmp_ints  = create 16ul (u64 0) in
