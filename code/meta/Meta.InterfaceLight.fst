module Meta.InterfaceLight

open FStar.List.Tot
open FStar.Reflection
open FStar.Tactics

/// This is a lightweight variant of the rewriting tactic in Meta.Interface, which does not
/// parameterize over an additional predicate `p` and does not add this p as requires clauses
/// all over the call-graph when this is not needed.
/// This brings significant verification performance improvements when reverifying after
/// splicing, since the code remains closer to the original generic code, especially
/// when many abbreviations exist
...
let lambda_over_index (st: state) (f_name: name) (f_typ: term) : Tac term =
  ...
  | Tv_Arrow bv c ->
      let { binder_bv = bv; binder_sort = sort; binder_qual = qual } = inspect_binder bv in
      if not (is_implicit qual) then
        fail ("The first parameter in the type of " ^ string_of_name f_name ^ " is not implicit");
      print (st.indent ^ "  Found index of type " ^ term_to_string sort);
      let f_typ =
        match inspect_comp c with
        | C_Total t ->
            print (st.indent ^ term_to_string t ^ "\n");
            pack (Tv_Abs (mk_implicit_binder bv sort) t)
        | _ ->
...
let binder_is_legit f_name t_i binder: Tac bool =
  let { binder_bv = bv; binder_sort = sort; binder_qual = qual } = inspect_binder binder in
  let { bv_ppname = name } = inspect_bv bv in
  let implicit = is_implicit qual in
  let right_type = term_eq t_i sort in
      ...
      | Sg_Let r lbs ->
        ...
        let st, new_body, new_args, new_sigelts =
          let index_bv_tm_opt : option term =
            ...
            | _ -> None
          in
          visit_body t_i index_bv_tm_opt st [] f_body
        in
        ...
        let f_typ, f_typ_typ, has_index =
          ...
          | Some (index_bv, index_bv_sort) ->
              ...
              mk_tot_arr [ mk_implicit_binder index_bv index_bv_sort ] (`Type0),
              true
          | _ ->
              f_typ,
              (`Type0),
