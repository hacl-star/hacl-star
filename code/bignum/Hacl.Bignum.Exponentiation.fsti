...
inline_for_extraction noextract
let bn_mod_exp_mont_ladder_st (modBits:size_t{v modBits > 0})
  (nLen:size_t{0 < v nLen /\ v nLen = v (blocks modBits 64ul) /\ 128 * v nLen <= max_size_t})
=
    n:lbignum nLen
  -> a:lbignum nLen
  ...
  (requires fun h ->
    ...
    disjoint res a /\ disjoint res b /\ disjoint res n /\ disjoint n a)
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_seq h1 res == S.bn_mod_exp_mont_ladder (v modBits) (v nLen) (as_seq h0 n) (as_seq h0 a) (v bBits) (as_seq h0 b))

// This version is fully run-time.
val bn_mod_exp_mont_ladder: modBits:_ -> nLen:_ -> bn_mod_exp_mont_ladder_st modBits nLen
