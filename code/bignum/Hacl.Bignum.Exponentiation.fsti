...
let bn_mod_exp_st (nLen:size_t{0 < v nLen /\ 128 * v nLen <= max_size_t})
 ...
 =
  ...
  (requires fun h ->
    ...
    disjoint res a /\ disjoint res b /\ disjoint res n /\ disjoint n a)
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_seq h1 res == S.bn_mod_exp (v nLen) (v nBits) (as_seq h0 n) (as_seq h0 a) (v bBits) (as_seq h0 b))

// This version is fully run-time.
val bn_mod_exp: nLen:_ -> nBits:_ -> bn_mod_exp_st nLen nBits
...
let bn_mod_exp_mont_ladder_st (nLen:size_t{0 < v nLen /\ 128 * v nLen <= max_size_t})
 ...
 =
  ...
  (requires fun h ->
    ...
    disjoint res a /\ disjoint res b /\ disjoint res n /\ disjoint n a)
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_seq h1 res == S.bn_mod_exp_mont_ladder (v nLen) (v nBits) (as_seq h0 n) (as_seq h0 a) (v bBits) (as_seq h0 b))

// This version is fully run-time.
val bn_mod_exp_mont_ladder: nLen:_ -> nBits:_ -> bn_mod_exp_mont_ladder_st nLen nBits
