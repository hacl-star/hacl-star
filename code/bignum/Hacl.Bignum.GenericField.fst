...
let bn_field_get_len k _ =
  k.len


let bn_field_init km r nBits n =
  [@inline_let]
  let len = km.BM.bn.BN.len in
  let h0 = ST.get () in
  let r2 : buffer (limb U64) = LowStar.Monotonic.Buffer.mmalloc r (uint #U64 #SEC 0) len in
  let n1 : buffer (limb U64) = LowStar.Monotonic.Buffer.mmalloc r (uint #U64 #SEC 0) len in
  let h1 = ST.get () in
  B.(modifies_only_not_unused_in loc_none h0 h1);
  assert (B.length r2 == FStar.UInt32.v len);
  assert (B.length n1 == FStar.UInt32.v len);
  let r2 : lbignum U64 len = r2 in
  let n1 : lbignum U64 len = n1 in
  copy n1 n;

  km.BM.precomp (nBits -! 1ul) n r2;
  let mu = BM.mod_inv_limb n.(0ul) in
  let res : bn_mont_ctx = { nBits = nBits; len = len; n = n1; mu = mu; r2 = r2 } in
  let h2 = ST.get () in
...
let bn_field_mul km k aM bM cM =
  let h0 = ST.get () in
...
let bn_field_one km k oneM =
  let h0 = ST.get () in
  BM.bn_mont_one km k.n k.mu k.r2 oneM;
  let h1 = ST.get () in
  assert (as_seq h1 oneM == S.bn_field_one (as_ctx h0 k));
  assert (bn_v h1 oneM < bn_v h0 k.n)


let bn_field_inv km k aM aInvM =
  [@inline_let]
  let len = k.len in
  push_frame ();
  let n2 = create len (uint #U64 #SEC 0) in
  let c = BN.bn_sub1 len k.n (uint #U64 #SEC 2) n2 in
  let h0 = ST.get () in
  S.bn_field_inv_lemma (as_ctx h0 k) (as_seq h0 aM);
  Hacl.Spec.Bignum.bn_sub1_lemma (as_seq h0 k.n) (uint #U64 #SEC 2);
  Hacl.Spec.Bignum.Definitions.bn_eval_bound (as_seq h0 n2) (v len);
  Hacl.Spec.Bignum.Definitions.bn_eval_bound (as_seq h0 k.n) (v len);
  BE.bn_mont_one km k.n k.mu k.r2 aInvM;
  IE.lexp_fw_vartime len len (BE.mk_lexp U64 km ((as_ctx h0 k).S.n) k.mu) k.n aM len k.nBits n2 aInvM 4ul;
  let h1 = ST.get () in
  assert (bn_v h0 n2 == bn_v h0 k.n - 2);
  assert (as_seq h1 aInvM ==
    ...
    (as_seq h0 aM) (v k.nBits) (bn_v h0 k.n - 2) 4);
  ...
  assert (bn_v h1 aInvM < bn_v h0 k.n);
  pop_frame ()
