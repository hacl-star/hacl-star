...
let bn_mont_one_st (t:limb_t) (len:BN.meta_len t) =
  ...
  (ensures  fun h0 _ h1 -> modifies (loc oneM) h0 h1 /\
    bn_v h1 oneM == E.mont_one_ll (bits t) (v len) (bn_v h0 n) (v mu))
...
val bn_mont_mul:
  ...
  -> mu:limb t{SM.bn_mont_pre n mu} ->
  BE.lmul_st t k.BM.bn.BN.len k.BM.bn.BN.len (mk_to_nat_mont_ll_comm_monoid t k.BM.bn.BN.len n mu)
...
val bn_mont_sqr:
  ...
  -> n:Ghost.erased (BD.lbignum t (v k.BM.bn.BN.len))
  -> mu:limb t{SM.bn_mont_pre n mu} ->
  BE.lsqr_st t k.BM.bn.BN.len k.BM.bn.BN.len (mk_to_nat_mont_ll_comm_monoid t k.BM.bn.BN.len n mu)
