...
friend Hacl.Spec.Exponentiation
...
val lprecomp_table_f:
  ...
  (requires fun h ->
    ...
    (forall (j:nat{j <= v i + 1}).
      precomp_table_inv (v len) (v ctx_len) k.to (as_seq h a) (v table_len) (as_seq h table) j))
  (ensures  fun h0 _ h1 -> modifies (loc table) h0 h1 /\
    (forall (j:nat{j <= v i + 2}).
      precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) j))
...
let lprecomp_table_f #a_t len ctx_len k ctx a table_len i table =
  ...
  let h0 = ST.get () in
  assert (precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h0 table) (v i + 1));
  ...
  assert (precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) (v i + 2));
  LSeq.eq_intro
    (LSeq.sub (as_seq h0 table) 0 ((v i + 2) * v len))
    (LSeq.sub (as_seq h1 table) 0 ((v i + 2) * v len));

  assert (forall (j:nat{j <= v i + 1}).
    precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h0 table) j);
  precomp_table_inv_lemma (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h0 table) (as_seq h1 table) (v i + 2);
  assert (forall (j:nat{j <= v i + 1}).
    precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) j)
#pop-options
...
val lprecomp_table:
  ...
  (ensures  fun h0 _ h1 -> modifies (loc table) h0 h1 /\
    (forall (j:nat{j < v table_len}).
      {:pattern precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) j}
      precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) j))
...
let lprecomp_table #a_t len ctx_len k ctx a table_len table =
  ...
  S.lemma_pow1 k.to.comm_monoid (k.to.refl (as_seq h0 a));
  assert (precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) 0);
  assert (precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h1 table) 1);

  [@ inline_let]
  let inv h (i:nat{i <= v table_len - 2}) =
    modifies (loc table) h1 h /\
    (forall (j:nat{j < i + 2}).
      precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h table) j) in
...
val ltable_select_consttime_f:
  ...
  (ensures  fun h0 _ h1 -> modifies (loc acc) h0 h1 /\
    as_seq h1 acc ==
    table_select_consttime_f (v len) (v table_len) (as_seq h0 table) i (v j) (as_seq h0 acc))

let ltable_select_consttime_f #a_t len table_len table i j acc =
...
let ltable_select_consttime #a_t len table_len table i res =
  let h0 = ST.get () in
  ...
  let inv h (j:nat{j <= v table_len - 1}) =
    ...
    as_seq h res ==
    Loops.repeati j
      (table_select_consttime_f (v len) (v table_len) (as_seq h0 table) i) (as_seq h1 res) in

  Loops.eq_repeati0 (v table_len - 1)
    (table_select_consttime_f (v len) (v table_len) (as_seq h0 table) i) (as_seq h1 res);
  Lib.Loops.for 0ul (table_len -! 1ul) inv
  (fun j ->
    Loops.unfold_repeati (v j + 1)
      (table_select_consttime_f (v len) (v table_len) (as_seq h0 table) i) (as_seq h1 res) (v j);
    ltable_select_consttime_f len table_len table i j res);
  let h2 = ST.get () in
  table_select_consttime_lemma (v len) (v table_len) (as_seq h0 table) i
...
let bn_get_bits_l #b_t bLen bBits b l i =
  ...
  let h0 = ST.get () in
  SN.bn_get_bits_lemma (as_seq h0 b) (v bk - v l * v i - v l) (v l);
  BN.bn_get_bits bLen b (bk -! l *! i -! l) l
...
let bn_get_bits_c #b_t bLen bBits b l =
  ...
  SN.bn_get_bits_lemma (as_seq h0 b) (v bBits / v l * v l) (v l);
  BN.bn_get_bits bLen b (bBits /. l *! l) l
#pop-options
...
let lprecomp_get_vartime #a_t len ctx_len k a table_len table bits_l tmp =
  ...
  let h1 = ST.get () in
  assert (precomp_table_inv (v len) (v ctx_len) k.to (as_seq h0 a) (v table_len) (as_seq h0 table) (v bits_l));
...
let lprecomp_get_consttime #a_t len ctx_len k a table_len table bits_l tmp =
  Math.Lemmas.lemma_mult_le_right (v len) (v bits_l + 1) (v table_len);
  let h0 = ST.get () in
  ltable_select_consttime len table_len table bits_l tmp;
  let h1 = ST.get () in
