...
val bn_mod_precomp_table_mont_loop_lemma:
  ...
  (ensures
   (let t3 : lbignum t (table_len * nLen) = Loops.repeati j (bn_mod_precomp_table_mont_f n mu aM table_len) t01 in
    let t4 : lseq nat table_len = Loops.repeati j
      (E.mod_precomp_table_mont_f (bits t) nLen (bn_v n) (v mu) (bn_v aM) table_len) t02 in
    (forall (i:nat{i < j + 2}).
     (Math.Lemmas.lemma_mult_le_right nLen (i + 1) (table_len);
      assert (i * nLen + nLen <= table_len * nLen);
      let bi = sub t3 (i * nLen) nLen in bn_v bi == index t4 i /\ bn_v bi < bn_v n))))

let bn_mod_precomp_table_mont_loop_lemma #t #nLen n mu table_len aM t01 t02 j =
  admit()
...
let bn_mod_precomp_table_mont_lemma #t #nLen n mu table_len aM oneM i =
  ...
  bn_mod_precomp_table_mont_loop_lemma #t #nLen n mu table_len aM t01 t02 (table_len - 2)
...
let bn_mod_exp_fw_mont_f_lemma #t #nLen n mu bBits bLen b l table_len oneM aM i accM =
  let t1 = bn_mod_precomp_table_mont n mu table_len aM oneM in
  let t2 = E.mod_precomp_table_mont (bits t) nLen (bn_v n) (v mu) table_len (bn_v aM) in
