...
All the arithmetic operations are performed in the Montgomery domen.
...
"]
val field_init: len:BN.meta_len Lib.IntTypes.U64 -> GF.bn_field_init_st len
...
val field_get_len: k:GF.bn_mont_ctx -> GF.bn_field_get_len_st k
...
[@@ Comment "Convert a bignum to its Montgomery representation.
  ...
  • a < n
"]
val to_field: k:GF.bn_mont_ctx -> GF.bn_to_field_st k
...
[@@ Comment "Convert the result back from the Montgomery representation to the regular representation.
  ...
  • aM < n
"]
val from_field: k:GF.bn_mont_ctx -> GF.bn_from_field_st k
...
[@@ Comment "Write `aM + bM mod n` in `cM`.
  ...
  • aM < n
  • bM < n "]
val add: k:GF.bn_mont_ctx -> GF.bn_field_add_st k
...
[@@ Comment "Write `aM - bM mod n` to `cM`.
  ...
  • aM < n
  • bM < n "]
val sub: k:GF.bn_mont_ctx -> GF.bn_field_sub_st k
...
[@@ Comment "Write `aM * bM mod n` in `cM`.
  ...
  • aM < n
  • bM < n "]
val mul: k:GF.bn_mont_ctx -> GF.bn_field_mul_st k
...
[@@ Comment "Write `aM * aM mod n` in `cM`.
  ...
  from Hacl.Spec.Bignum.GenericField.fsti, which amounts to:
  • aM < n "]
val sqr: k:GF.bn_mont_ctx -> GF.bn_field_sqr_st k
...
[@@ Comment "Convert a bignum `one` to its Montgomery representation.
  ...
  The argument k is a montgomery context obtained through Hacl_GenericField64_field_init."]
val one: k:GF.bn_mont_ctx -> GF.bn_field_one_st k
...
[@@ Comment "Write `aM ^ (-1) mod n` in `aInvM`.
  ...
  • aM < n
  • 0 < aM "]
val inverse: k:GF.bn_mont_ctx -> GF.bn_field_inv_st k
