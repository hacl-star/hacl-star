...
val lib_ntt: 
  ...
  -> psi:field m
  -> p:lib_poly n m
  -> Tot (p':lib_poly n m{forall (k:nat{k<n}). p'.[k] = modular_sum_n (mapi (fun j g -> (psi^%j) *% g *% (omega ^% (k*j))) p)})
...
val lib_nttinv: 
  ...
  -> psiinv:field m
  -> (p:lib_poly n m) 
  -> Tot (p':lib_poly n m{forall (k:nat{k<n}). p'.[k] = ninv *% (psiinv^%k) *% modular_sum_n (mapi (fun j g -> g *% (omegainv ^% (k*j))) p)})
