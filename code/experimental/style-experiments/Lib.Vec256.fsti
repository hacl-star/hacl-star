...
noextract
val vec256_load_le: b:lbytes 32 -> ST vec256
			   (requires (fun h -> live h b))
			   (ensures (fun h0 _ h1 ->  h1 == h0))

noextract
val vec256_load_be: b:lbytes 32 -> ST vec256
			   (requires (fun h -> live h b))
			   (ensures (fun h0 _ h1 ->  h1 == h0))
			   
noextract
val vec256_store_le: b:lbytes 32 -> vec256 -> ST unit
			   (requires (fun h -> live h b))
			   (ensures (fun h0 _ h1 ->  live h1 b /\ modifies (loc_buffer b) h0 h1))

noextract
val vec256_store_be: b:lbytes 32 -> vec256 -> ST unit
			   (requires (fun h -> live h b))
			   (ensures (fun h0 _ h1 ->  live h1 b /\ modifies (loc_buffer b) h0 h1))
