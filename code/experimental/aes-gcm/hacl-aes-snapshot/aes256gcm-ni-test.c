...
int main() {
  ...
  memset(comp_tv0, 0x0, (size_tv0 + 16) * sizeof(uint8_t));
  ...
  printf("AESGCM-AES256-NI Test 0:\n");
  ok = true;
  for (int i = 0; i < 76; i++) {
    ok = ok & (exp[i] == comp[i]);
    if (!ok) {
  ...
  bool res_tv0 = Hacl_AES_256_GCM_NI_aes256_gcm_decrypt(ctx,size_tv0,comp1_tv0,comp_tv0,20,aad); 
   ...
   if (!res_tv0)
    ...
    if (ok) printf("Decrypt Success!\n");
      ...
      printf("Decrypt FAILURE!\n");
      printf("AES256-GCM-NI Decrypt computed:");
      for (int i = 0; i < size_tv0; i++)
	printf("%02x",comp1_tv0[i]);
      printf("\n");
      printf("AES256-GCM-NI Decrypt expected:");
      for (int i = 0; i < size_tv0; i++)
