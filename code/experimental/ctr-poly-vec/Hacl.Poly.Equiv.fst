...
//TODO: change the definition of the repeat_blocks_c function in Lib.Sequence
val repeat_blocks_split:
  ...
  Lemma
   (let len = length inp in
    ...
    repeat_blocks_c blocksize inp f l acc0 ==
    repeat_blocks_c blocksize (Seq.slice inp len0 len) f l
...
val repeat_blocks_last:
  ...
  (let pre = create w (pow_w w r) in
   repeat_blocks_c #uint8 #(felem_v w) #felem (w * blocksize) text (poly_update_nblocks #w pre) (poly_update_last_v #w r) acc_v ==
...
let poly_update_nblocks_lemma_lp #w text r i acc =
  ...
  repeat_right_step #felem blocksize (w * i) w repeat_bf_s1 repeat_bf_s acc w
...
let repeat_blocks_multi_v_equiv_aux #w text r i acc_v =
  ...
  calc (==) {
    normalize_v r (repeat_bf_vec i acc_v);
    (==) { }
    normalize_v r (poly_update_nblocks #w pre block acc_v);
    (==) { poly_update_nblocks_lemma #w r block acc_v }
    repeat_blocks_multi blocksize block (poly_update1 r) (normalize_v r acc_v);
    (==) { lemma_repeat_blocks_multi blocksize block (poly_update1 r) (normalize_v r acc_v) }
    Loops.repeati w repeat_bf_s1 (normalize_v r acc_v);
    (==) { Loops.repeati_def w repeat_bf_s1 (normalize_v r acc_v) }
    Loops.repeat_right 0 w (Loops.fixed_a felem) repeat_bf_s1 (normalize_v r acc_v);
    (==) { poly_update_nblocks_lemma_lp #w text r i (normalize_v r acc_v) }
    Loops.repeat_right (w * i) (w * i + w) (Loops.fixed_a felem) repeat_bf_s (normalize_v r acc_v);
    }
...
val repeat_blocks_v_equiv:
  ...
  -> r:felem -> Lemma
  (let pre = create w (pow_w w r) in
   repeat_blocks_c #uint8 #(felem_v w) #felem (w * blocksize) text (poly_update_nblocks #w pre) (poly_update_last_v #w r) acc_v0 ==
   repeat_blocks_c #uint8 #felem #felem blocksize text (poly_update1 r) (poly_update_last r) (normalize_v r acc_v0))
...
let repeat_blocks_v_equiv #w text acc_v0 r =
  ...
  let pre = create w (pow_w w r) in
  let acc = repeat_blocks_c #uint8 #(felem_v w) #felem blocksize_v text (poly_update_nblocks #w pre) (poly_update_last_v #w r) acc_v0 in
  ...
  assert (acc == repeat_blocks_c #uint8 #felem blocksize (Seq.slice text len0 len) (poly_update1 r) (poly_update_last r) (normalize_v r acc_v1));
...
let poly_equivalence #w text acc0 r =
  ...
  let res_v = repeat_blocks_c blocksize_v text1 (poly_update_nblocks #w pre) (poly_update_last_v #w r) acc_v in
  repeat_blocks_v_equiv #w text1 acc_v r;
  assert (res_v == repeat_blocks_c #uint8 #felem #felem blocksize text1 (poly_update1 r) (poly_update_last r) (normalize_v r acc_v));
