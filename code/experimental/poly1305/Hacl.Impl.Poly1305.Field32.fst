...
val create_felem: unit -> StackInline felem
		       (requires (fun h -> True))
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 (u32 0))))
inline_for_extraction
let create_felem () = create (size 5) (u32 0) 

inline_for_extraction
val create_wide: unit -> StackInline felem_wide
		       (requires (fun h -> True))
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 (u64 0))))
inline_for_extraction
let create_wide () = create (size 5) (u64 0) 
...
let load_felem f lo hi = 
    ...
    f.(size 1) <- (to_u32 (lo >>. size 26)) &. mask26;
    f.(size 2) <- (to_u32 (lo >>. size 52)) ^. (((to_u32 hi &. u32 0x3fff) <<. size 12));
    f.(size 3) <- (to_u32 (hi >>. size 14)) &. mask26;
    f.(size 4) <- to_u32 (hi >>. size 40)
...
let store_felem f = 
    let f0 = to_u64 f.(size 0) |. ((to_u64 f.(size 1)) <<. size 26) |. ((to_u64 f.(size 2)) <<. size 52) in
    let f1 = ((to_u64 f.(size 2)) >>. size 12) |. ((to_u64 f.(size 3)) <<. size 14) |. ((to_u64 f.(size 4)) <<. size 40) in
    (f0,f1)
...
val set_bit: f:felem -> i:size_t{size_v i < 130} -> Stack unit
		   ...
		   (ensures (fun h0 _ h1 -> modifies (loc f) h0 h1))
let set_bit f i = 
  f.(i /. size 26) <- f.(i /. size 26) |. (u32 1 <<. (i %. size 26))
...
val set_zero: f:felem -> Stack unit
                   (requires (fun h -> live h f))
		   (ensures (fun h0 _ h1 -> modifies (loc f) h0 h1 /\ as_nat h1 f == 0))
let set_zero f = 
    f.(size 0) <- u32 0;
    f.(size 1) <- u32 0;
    f.(size 2) <- u32 0;
...
val copy_felem: f1:felem -> f2:felem -> Stack unit
		   ...
		   (ensures (fun h0 _ h1 -> modifies (loc f1) h0 h1 /\ as_nat h1 f1 == as_nat h0 f2))
...
val carry_top_felem: f:felem -> Stack unit
		   ...
		   (ensures (fun h0 _ h1 -> modifies (loc f) h0 h1 /\ felem_fits h1 f (1,2,1,1,1)))
[@ CInline]
