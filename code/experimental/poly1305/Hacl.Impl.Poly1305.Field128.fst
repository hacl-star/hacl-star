...
let felem = lbuffer vec128 5ul
let felem_wide = lbuffer vec128 5ul
let precomp_r = lbuffer vec128 20ul
...
val create_felem: unit -> StackInline felem
		       (requires (fun h -> True))
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 vec128_zero)))
inline_for_extraction
let create_felem () = create (size 5) vec128_zero 
...
val create_wide: unit -> StackInline felem_wide
		       ...
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 vec128_zero)))
inline_for_extraction
let create_wide () = create (size 5) vec128_zero 
