...
let felem = lbuffer vec256 5ul
let felem_wide = lbuffer vec256 5ul
let precomp_r = lbuffer vec256 20ul
...
val create_felem: unit -> StackInline felem
		       (requires (fun h -> True))
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 vec256_zero)))
inline_for_extraction
let create_felem () = create (size 5) vec256_zero 

inline_for_extraction
val create_wide: unit -> StackInline felem
		       (requires (fun h -> True))
		       (ensures (fun h0 b h1 -> live h1 b /\ stack_allocated b h0 h1 (Lib.Sequence.create 5 vec256_zero)))
inline_for_extraction
let create_wide () = create (size 5) vec256_zero
...
inline_for_extraction
val load_felems_le: f:felem -> b:lbuffer uint8 64ul -> Stack unit
...
let store_felem_le b f = 
    ...
    copy (sub b 0ul 16ul) (sub tmp 0ul 16ul);
    pop_frame()
    
inline_for_extraction
val store_felems_le: b:lbuffer uint8 64ul -> f:felem -> Stack unit
                   (requires (fun h -> live h f /\ live h b))
		   (ensures (fun h0 _ h1 -> modifies (loc b |+| loc f) h0 h1))
