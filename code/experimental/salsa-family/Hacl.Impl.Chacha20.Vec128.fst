...
val line:
  ...
  Stack unit
    ...
    (ensures (fun h0 _ h1 -> live h1 st /\ modifies_1 st h0 h1 /\ live h0 st /\
      as_seq h1 st == Spec.line (U32.v a) (U32.v b) (U32.v d) s (as_seq h0 st)))
[@ "c_inline"]
let line st a b d s =
  let h0 = ST.get() in
  let sa = st.(a) in 
  ...
  st.(d) <- sd;
  let h1 = ST.get() in
  Seq.lemma_eq_intro (as_seq h1 st) (Spec.line (U32.v a) (U32.v b) (U32.v d) s (as_seq h0 st))
...
val round:
  ...
  Stack unit
    ...
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ modifies_1 st h0 h1 /\
      as_seq h1 st == Spec.round (as_seq h0 st)))
[@ "c_inline"]
let round st =
  let h0 = ST.get() in
  ...
  let h1 = ST.get() in
  Seq.lemma_eq_intro (as_seq h1 st) (Spec.round (as_seq h0 st))
...
val column_round:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ modifies_1 st h0 h1 /\
      as_seq h1 st == Spec.column_round (as_seq h0 st)))
[@ "substitute"]
...
val shuffle_rows_0123:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ modifies_1 st h0 h1 /\
      as_seq h1 st == Spec.shuffle_rows_0123 (as_seq h0 st)))
[@ "substitute"]
let shuffle_rows_0123 st = 
  ...
  let h1 = ST.get() in
  Seq.lemma_eq_intro (as_seq h1 st) (Spec.shuffle_rows_0123 (as_seq h0 st))
...
val shuffle_rows_0321:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ modifies_1 st h0 h1 /\
      as_seq h1 st == Spec.shuffle_rows_0321 (as_seq h0 st)))
[@ "substitute"]
let shuffle_rows_0321 st = 
  ...
  let h1 = ST.get() in
  Seq.lemma_eq_intro (as_seq h1 st) (Spec.shuffle_rows_0321 (as_seq h0 st))
...
val diagonal_round:
  ...
  Stack unit
    (requires (fun h -> live h st))
    (ensures (fun h0 _ h1 -> live h0 st /\ live h1 st /\ modifies_1 st h0 h1 /\
      as_seq h1 st == Spec.diagonal_round (as_seq h0 st)))
[@ "substitute"]
...
[@ "substitute"]
let rounds st =
  repeat #vec 4ul (Spec.double_round) st 10ul double_round
