...
genWeirdPrime :: Integer -> Int -> IO (Integer,Int)
genWeirdPrime p i = do
    x <- randomRIO (0,1000000)
    if isPrimeMR 40 $ 2 * p * x + 1 then pure (x,i) else genWeirdPrime p (i+1)
...
genDataDGK :: [(Integer,Integer)] -> Int -> IO (Integer,Integer,Integer,Integer,Integer,Integer)
genDataDGK uFacts bits = do
    let u = fromFacts uFacts
    let vbits = 160
    let ubits = fromIntegral $ log2 u
    let rbits = bits `div` 2 - ubits - vbits
    when (rbits < 0) $ error "genDataDGK: incorrect params"
    --putTextLn $ "Rbits: " <> show rbits

    v <- genPrime vbits
    let genRp i = do
          r_p <- genPrime rbits
          let p = 2 * u * v * r_p + 1
          if isPrimeMR 40 p then pure (r_p,p) else genRp (i+1)

    let genRq i = do
          r_q <- genPrime rbits
          let q = 2 * u * v * r_q + 1
          if isPrimeMR 40 q then pure (r_q,q) else genRq (i+1)
    ...
    let possibleOrders = factToOrders phinFacts

    a <- randomRIO (0,n-1)
    unless (exp n a ((p-1)*(q-1)) == 1) $ error "er1"
    unless (fromFacts phinFacts == (p-1)*(q-1)) $ error "er2"
    unless (((p-1)*(q-1)) `elem` possibleOrders) $ error "er3"

    --putTextLn $  "Factors: " <> show phinFacts
    --putTextLn $  "Length of possible orders: " <> show (length possibleOrders)

    let findWithOrd reqO = do
            g <- randomRIO (0, n-1)
            let o = orderFact possibleOrders n g
            if o `mod` reqO == 0 then do
                let cand = exp n g (o `div` reqO)
                let o1 = orderFact possibleOrders p (cand `mod` p)
                let o2 = orderFact possibleOrders q (cand `mod` q)
