...
val get_acc: ctx: gcm_ctx ->
  ...
  (ensures (fun h0 f h1 -> h0 == h1 /\ live h1 f /\ f == gsub ctx 0ul 2ul))
...
val get_r: ctx:gcm_ctx ->
  Stack felem
  (requires (fun h -> live h ctx))
  (ensures (fun h0 f h1 -> h0 == h1 /\ live h1 f /\ f == gsub ctx 8ul 2ul))
...
val get_precomp: ctx:gcm_ctx ->
  Stack precomp
  (requires (fun h -> live h ctx))
  (ensures (fun h0 f h1 -> h0 == h1 /\ live h1 f /\ f == gsub ctx 2ul 264ul))
...
let load_precompute_r pre key =
  ...
  fmul r_4 r_2;
  let h2 = ST.get () in
  assert(modifies1 pre h1 h2);
  prepare table r_4;
  let h3 = ST.get () in
  assert(modifies1 table h2 h3)
