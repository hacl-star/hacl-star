...
let mont_reduction_f #nBits rBits nInv_u64 n c i res =
  ...
  let (c, res1) = bn_lshift_mul_add #nBits #(nBits + rBits) n (64*i) qi res in
  assume (bn_v res1 < pow2 (nBits + rBits));
  let res1 = bn_cast_le #(nBits + rBits + 1) (nBits + rBits) res1 in
  res1
...
let mul_mod_mont #nBits rBits nInv_u64 n a b =
  ...
  let c = bn_cast_gt #(nBits + nBits) (nBits + rBits) c in
...
let from_mont #nBits rBits nInv_u64 n a_r =
  ...
  let a_r = bn_cast_gt #nBits (nBits + rBits) a_r in
...
let mod_exp #nBits n a bBits b =
  ...
  let r2 = bn_pow2_r_mod #nBits n (rBits * 2) in
  let n0 = bn_get_bits n 0 64 in
  let nInv_u64 = bn_from_u64 (mod_inv_u64 (bn_to_u64 n0)) in
  let a_r = to_mont rBits nInv_u64 n r2 a in
  let acc_r = to_mont rBits nInv_u64 n r2 (bn_const_1 #nBits) in
  let res_r = mod_exp_ rBits nInv_u64 n a_r bBits b acc_r in
  let res = from_mont rBits nInv_u64 n res_r in
  res
