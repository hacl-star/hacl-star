...
val ctest:
    pow2_i:size_t ->
    modBits:size_t -> n:lbytes (v (blocks modBits (size 8))) ->
    pkeyBits:size_t -> e:lbytes (v (blocks pkeyBits (size 8))) ->
    skeyBits:size_t -> d:lbytes (v (blocks skeyBits (size 8))) ->
    ...
    sgnt_expected:lbytes (v (blocks modBits (size 8))) -> Stack bool
    (requires (fun h -> True))
    (ensures  (fun h0 r h1 -> True))
let ctest pow2_i modBits n pkeyBits e skeyBits d pTLen p qTLen q rBlindTLen rBlind msgLen msg saltLen salt sgnt_expected =
...
let test1() =
    ...
    let res = ctest (size 16) modBits n pkeyBits e skeyBits d pLen p qLen q rBlindLen rBlind msgLen msg saltLen salt sgnt_expected in
    res
...
let test2() =
    ...
    let sgnt_expected : lbytes (v nLen) = createL [
        ...
        u8 0x2d ] in
    let res = ctest (size 32) modBits n pkeyBits e skeyBits d pLen p qLen q rBlindLen rBlind msgLen msg saltLen salt sgnt_expected in
    res
