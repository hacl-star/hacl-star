...
let rec karatsuba_ pow2_i aLen a b tmp res =
  ...
  if (aLen <. size 32) then
    bn_mul aLen a aLen b res
  else begin
    let a0 = Buffer.sub #uint64 #(v aLen) #(v pow2_i0) a (size 0) pow2_i0 in
    let b0 = Buffer.sub #uint64 #(v aLen) #(v pow2_i0) b (size 0) pow2_i0 in
    ...
    let a1 = Buffer.sub #uint64 #(v aLen) #(v pow2_i0) a pow2_i0 pow2_i0 in
    let b1 = Buffer.sub #uint64 #(v aLen) #(v pow2_i0) b pow2_i0 pow2_i0 in
    let tmp0 = Buffer.sub #uint64 #(v tmpLen) #(4 * v pow2_i0) tmp (size 0) (mul #SIZE (size 4) pow2_i0) in
