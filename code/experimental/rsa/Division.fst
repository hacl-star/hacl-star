...
type rem_state (rLen:bnlen) (modLen:bnlen) = lbignum U32.(rLen +^ rLen +^ modLen +^ modLen)

let get_r_i #r #m (st:rem_state r m) : lbignum r = Buffer.sub st 0ul r
let get_tmp #r #m (st:rem_state r m) : lbignum r = Buffer.sub st r r
let get_mod #r #m (st:rem_state r m) : lbignum m = Buffer.sub st U32.(r +^ r) m
let get_mod_1 #r #m (st:rem_state r m) :lbignum m = Buffer.sub st U32.(r +^ r +^ m) m
...
val remainder:
    ...
    a:lbignum aLen -> mod:lbignum modLen{disjoint a mod} ->
    res:lbignum resLen{disjoint a res /\ disjoint mod res} -> Stack unit
