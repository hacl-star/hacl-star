...
let rec mult_by_limb_addj_ aLen a l i j resLen res =
    if i =^ 0ul then 0uL
    else begin
        let i1 = i -^ 1ul in
        let c1 = mult_by_limb_addj_ aLen a l i1 j resLen res in
        let (carry', res_ij) = mult_by_limb_addj_f a.(i1) l c1 res.(i1 +^ j) in
        res.(i1 +^ j) <- res_ij;
        carry'
    end
...
let mult_by_limb_addj aLen a l j resLen res =
    mult_by_limb_addj_ aLen a l aLen j resLen res
...
let rec mult_ aLen a bLen b j resLen res =
    if j =^ 0ul then ()
    else begin
        let j1 = j -^ 1ul in
        mult_ aLen a bLen b j1 resLen res;
        let c' = mult_by_limb_addj aLen a b.(j1) j1 resLen res in
        res.(aLen +^ j1) <- c'
    end
...
let mult aLen a bLen b res =
    mult_ aLen a bLen b bLen (aLen +^ bLen) res
