...
let rec isMore_loop aLen a bLen b count =
    if U32.(count >^ 0ul) then
        ...
        else isMore_loop aLen a bLen b count)
    else false

(* if a > b then true else false *)
val isMore:
    aLen:bnlen -> a:lbignum aLen ->
    bLen:bnlen -> b:lbignum bLen{disjoint a b} -> Stack bool
    (requires (fun h -> live h a /\ live h b))
    (ensures (fun h0 _ h1 -> live h0 a /\ live h0 b
        /\ live h1 a /\ live h1 b /\ h0 == h1))

let isMore aLen a bLen b =
    if U32.(aLen >^ bLen) then true
    else if U32.(aLen <^ bLen) then false
         else isMore_loop aLen a bLen b aLen