...
val mont_reduction_:
  ...
  c:lbignum (rLen + rLen) -> n:lbignum rLen -> nInv_u64:uint64 ->
  i:size_t -> carry':uint64 -> Stack unit
    (requires (fun h -> live h c /\ live h n /\ disjoint c n))
    (ensures (fun h0 _ h1 -> preserves_live h0 h1 /\ modifies1 c h0 h1))
  [@"c_inline"]
let rec mont_reduction_ #rLen rrLen c n nInv_u64 i carry' =
  if (i <. rrLen) then begin
    let ci = c.(i) in
    let qi = mul_mod #U64 nInv_u64 ci in
    let carry' = bn_mult_by_limb_addj_carry rrLen n qi carry' (size 0) i (add #SIZE rrLen rrLen) c in
    mont_reduction_ rrLen c n nInv_u64 (size_incr i) carry'
  end
...
let mont_reduction #rLen rrLen n nInv_u64 c tmp res =
  ...
  copy rLen2 c tmp;
  mont_reduction_ rrLen tmp n nInv_u64 (size 0) (u64 0);
...
val from_mont:
  ...
  Stack unit
    (requires (fun h -> live h n /\ live h a /\ live h aM /\ live h st /\ 
                      disjoint st n))
    (ensures (fun h0 _ h1 -> preserves_live h0 h1 /\ modifies2 a st h0 h1))
  [@"c_inline"]
