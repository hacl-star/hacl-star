...
type state = lbuffer (vec_t U128 1) 4ul
type key1 =  lbuffer (vec_t U128 1) 1ul
type nonce =  lbuffer (vec_t U128 1) 1ul

inline_for_extraction
val create_state: unit -> StackInline state
                   (requires (fun h -> True))
		   (ensures (fun h0 f h1 -> live h1 f /\ stack_allocated f h0 h1 (Seq.create 4 (vec_zero U128 1))))
let create_state () = create (size 4) (vec_zero U128 1)
...
let load_state st nonce counter = 
    ...
    let n0 = nonce.(0ul) in
    let v0 = vec_set (cast U32 4 n0) (size 3) counter0 in
    let v1 = vec_set (cast U32 4 n0) (size 3) counter1 in
    let v2 = vec_set (cast U32 4 n0) (size 3) counter2 in
    let v3 = vec_set (cast U32 4 n0) (size 3) counter3 in
    st.(size 0) <- cast U128 1 v0;
    st.(size 1) <- cast U128 1 v1;
    st.(size 2) <- cast U128 1 v2;
    st.(size 3) <- cast U128 1 v3
...
val aes_enc: st:state -> key:key1 -> ST unit
			     ...
			     (ensures (fun h0 _ h1 -> live h1 st /\ live h1 key /\ modifies (loc st) h0 h1))
let aes_enc st key = 
    st.(size 0) <- vec_aes_enc st.(size 0) key.(size 0);
    st.(size 1) <- vec_aes_enc st.(size 1) key.(size 0);
    st.(size 2) <- vec_aes_enc st.(size 2) key.(size 0);
    st.(size 3) <- vec_aes_enc st.(size 3) key.(size 0)
...
let aes_enc_last st key = 
    st.(size 0) <- vec_aes_enc_last st.(size 0) key.(size 0);
    st.(size 1) <- vec_aes_enc_last st.(size 1) key.(size 0);
    st.(size 2) <- vec_aes_enc_last st.(size 2) key.(size 0);
    st.(size 3) <- vec_aes_enc_last st.(size 3) key.(size 0)
...
let aes_keygen_assist ok ik rcon = 
    ok.(size 0) <- vec_aes_keygen_assist ik.(size 0) rcon		      
...
let key_expansion_step next prev = 
    ...
    next.(size 0) <- cast U128 1 (vec_permute4 (cast U32 4 n0) (size 3) (size 3) (size 3) (size 3));
...
let key_expansion_step2 next prev = 
    let n0 = next.(size 0) in
    next.(size 0) <- cast U128 1 (vec_permute4 (cast U32 4 n0) (size 2) (size 2) (size 2) (size 2));
    let key = prev.(size 0) in
    let key = vec_xor key (vec_shift_left key (size 32)) in
    let key = vec_xor key (vec_shift_left key (size 32)) in
    let key = vec_xor key (vec_shift_left key (size 32)) in
