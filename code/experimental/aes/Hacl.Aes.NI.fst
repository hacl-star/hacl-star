...
val aes_update4: out:lbuffer uint8 64ul -> inp:lbuffer uint8 64ul -> ctx:aes_ctx -> counter:size_t -> rounds:size_t -> ST unit
			     (requires (fun h -> live h out /\ live h inp /\ live h ctx))
			     (ensures (fun h0 b h1 -> modifies (loc out) h0 h1))
let aes_update4 out inp ctx ctr rounds = aes_update4 #MAES out inp ctx ctr rounds

[@ CInline ]
val aes_ctr: len:size_t -> out:lbuffer uint8 len -> inp:lbuffer uint8 len -> ctx:aes_ctx -> counter:size_t -> rounds:size_t -> ST unit
			     (requires (fun h -> live h out /\ live h inp /\ live h ctx))
			     (ensures (fun h0 _ h1 -> modifies (loc out) h0 h1))
[@ CInline ]
let aes_ctr len out inp ctx counter rounds = aes_ctr #MAES len out inp ctx counter rounds
