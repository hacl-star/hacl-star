...
val load_block0: out:state -> inp:block -> ST unit 
...
val load_nonce: out:nonce -> nonce:lbuffer uint8 12ul -> ST unit 
...
let rcon : b:ilbuffer uint8 11ul =  
  [@ inline_let]
  let rcon_l = [u8(0x8d); u8(0x01); u8(0x02); u8(0x04); u8(0x08); u8(0x10); u8(0x20); u8(0x40); u8(0x80); u8(0x1b); u8(0x36)] in
  assert_norm (List.Tot.length rcon_l == 11);
  createL_global rcon_l

inline_for_extraction 
val aes_keygen_assisti: rcon:uint8 -> i:shiftval U8 -> u:uint64 -> uint64
