...
val bignum_from_bytes_be_:
  ...
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_seq h1 res == S.bignum_from_bytes_be_ (v len) (as_seq h0 b))

let bignum_from_bytes_be_ len b res =
  let h0 = ST.get () in
  [@inline_let]
  let spec h = S.bignum_from_bytes_be_f (v len) (as_seq h b) in
  fill h0 len res spec
  (fun j -> uint_from_bytes_be (sub b ((len -! j -! 1ul) *! 8ul) 8ul))


val bignum_from_bytes_be:
...
[@CInline]
let bignum_from_bytes_be len b res =
  let h0 = ST.get () in
  push_frame ();
  ...
  bignum_from_bytes_be_ bnLen tmp res;
  S.bignum_from_bytes_be_lemma (v len) (as_seq h0 b);
  pop_frame ()


inline_for_extraction noextract
val bignum_to_bytes_be_:
    len:size_t{8 * v len <= max_size_t}
  -> b:lbignum len
  ...
  (requires fun h -> live h b /\ live h res /\ disjoint res b)
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_seq h1 res == S.bignum_to_bytes_be_ (v len) (as_seq h0 b))

let bignum_to_bytes_be_ len b res =
  let h0 = ST.get () in
  [@ inline_let]
  let a_spec (i:nat{i <= v len}) = unit in
  [@ inline_let]
  let spec (h:mem) = S.bignum_to_bytes_be_f (v len) (as_seq h b) in
  ...
  assert_norm (S.bignum_to_bytes_be_ (v len) (as_seq h0 b) == norm [delta] S.bignum_to_bytes_be_ (v len) (as_seq h0 b))


val bignum_to_bytes_be:
    len:size_t{0 < v len /\ 8 * v (blocks len 8ul) <= max_size_t}
...
[@CInline]
let bignum_to_bytes_be len b res =
  let h0 = ST.get () in
  push_frame ();
  let bnLen = blocks len 8ul in
  let tmpLen = 8ul *! bnLen in
  let tmp = create tmpLen (u8 0) in
  bignum_to_bytes_be_ bnLen b tmp;
  copy res (sub tmp (tmpLen -! len) len);
  S.bignum_to_bytes_be_lemma (v len) (as_seq h0 b);
  pop_frame ()
