...
open Hacl.Impl.Ed25519.Ladder.BigLoop
...
private inline_for_extraction let cmult_ result point_buf n q =
  ...
  let nq    = Buffer.sub point_buf 0ul  20ul in
  let nqpq  = Buffer.sub point_buf 20ul 20ul in
  let nq2   = Buffer.sub point_buf 40ul 20ul in
  ...
  (gety nq).(0ul) <- limb_one;
  (getz nq).(0ul) <- limb_one;
  let h = ST.get() in
