...
let live_coords h x y z t =
  let _ = () in
  live h x /\ live h z /\ live h y /\ live h t

unfold val live: mem -> point -> GTot Type0
let live h p =
  let _ = () in
  live_coords h (getx p) (gety p) (getz p) (gett p)

(* val make_pre: x:felem -> y:felem -> z:felem -> t:felem -> GTot Type0 *)
(* let make_pre x y z = *)
(*   let _ = () in *)
...
let swap_conditional a b iswap =
  ...
  swap_conditional_ (gety a) (gety b) swap clen;
  swap_conditional_ (getz a) (getz b) swap clen;
  swap_conditional_ (gett a) (gett b) swap clen
