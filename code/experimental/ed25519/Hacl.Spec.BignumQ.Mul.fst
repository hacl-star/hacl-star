...
val carry56_wide: x:uint128 ->
  Pure (uint128 & uint64)
  (requires True) // v x < pow2 117 ==> v c < pow2 61
  (ensures fun (c, t) -> v t < pow56 /\
    v c * pow56 + v t == v x)
...
let mul_5 (x0, x1, x2, x3, x4) (y0, y1, y2, y3, y4) =
  ...
  let z1 = xy01 +. xy10 in // < pow2 113
  let z2 = xy02 +. xy11 +. xy20 in // < pow2 114
  let z3 = xy03 +. xy12 +. xy21 +. xy30 in // < pow2 115
  let z4 = xy04 +. xy13 +. xy22 +. xy31 +. xy40 in // < pow2 116
  ...
  let (c0, x0) = carry56_wide z0 in
  let (c1, x1) = carry56_wide (z1 +. c0) in
  let (c2, x2) = carry56_wide (z2 +. c1) in
  let (c3, x3) = carry56_wide (z3 +. c2) in
  let (c4, x4) = carry56_wide (z4 +. c3) in
  let (c5, x5) = carry56_wide (z5 +. c4) in
  let (c6, x6) = carry56_wide (z6 +. c5) in
  let (c7, x7) = carry56_wide (z7 +. c6) in
  let (c8, x8) = carry56_wide (z8 +. c7) in
  let x9 = to_u64 c8 in admit();
...
let low_mul_5 (x0, x1, x2, x3, x4) (y0, y1, y2, y3, y4) =
  ...
  let (c1, t1) = carry56_wide (xy01 +. xy10 +. c0) in
  let (c2, t2) = carry56_wide (xy02 +. xy11 +. xy20 +. c1) in
  let (c3, t3) = carry56_wide (xy03 +. xy12 +. xy21 +. xy30 +. c2) in
  let t4 = to_u64 (xy04 +. xy13 +. xy22 +. xy31 +. xy40 +. c3) &. mask40 in admit();
  (t0, t1, t2, t3, t4)
