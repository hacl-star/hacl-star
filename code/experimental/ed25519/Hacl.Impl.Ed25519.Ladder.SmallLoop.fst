module Hacl.Impl.Ed25519.Ladder.SmallLoop
...
private inline_for_extraction val cmult_small_loop_step_1:
  ...
  Stack unit
    (requires (fun h -> (* fmonty_pre h nq2 nqpq2 nq nqpq q *)
      live h nq /\ live h nqpq /\ live h nq2 /\ live h nqpq2 ))
...
private inline_for_extraction let cmult_small_loop_step_1 nq nqpq nq2 nqpq2 byt =
  let bit = last_bit byt in
  let h0 = ST.get() in
  Hacl.Impl.Ed25519.SwapConditional.swap_conditional nq nqpq bit;
  let h1 = ST.get() in
...
private inline_for_extraction val cmult_small_loop_step_2:
  ...
  byte:H8.t ->
  Stack unit
    (requires (fun h -> (* fmonty_pre h nq2 nqpq2 nq nqpq q *) True))
    (ensures (fun h0 _ h1 -> (* fmonty_pre h0 nq2 nqpq2 nq nqpq q *)
      (* /\  *)HyperStack.modifies_one (frameOf nq) h0 h1
...
#reset-options "--initial_fuel 0 --max_fuel 0 --z3rlimit 100"
private inline_for_extraction let cmult_small_loop_step_2 nq nqpq nq2 nqpq2 byt =
...
private inline_for_extraction let cmult_small_loop_double_step nq nqpq nq2 nqpq2 byt (* i *) =
  cmult_small_loop_step nq nqpq nq2 nqpq2 byt (* i *);
  let byt = H8.(byt <<^ 1ul) in
  cmult_small_loop_step nq2 nqpq2 nq nqpq byt (* U32.(i-^1ul) *)
...
let rec cmult_small_loop nq nqpq nq2 nqpq2 byt i =
  if (U32.(i =^ 0ul)) then (
    (* let h = ST.get() in *)
    ...
    cmult_small_loop_double_step nq nqpq nq2 nqpq2 byt;
    let byt' = H8.(byt <<^ 2ul) in
    cmult_small_loop nq nqpq nq2 nqpq2 byt' i'
  )
