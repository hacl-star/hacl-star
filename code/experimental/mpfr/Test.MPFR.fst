...
let test_add1sp1 s bp bx cp cx rnd_mode p rp rx rt =
    C.String.print (C.String.of_literal "Testing add1sp1: \n");
    ...
    pop_frame ();
    C.String.print (C.String.of_literal "Result: \n");
    // print_i32 asign;
    // print_u64 ap;
    // print_i64 ax;
    // print_i32 at;
    if ((U64.(ap =^ rp) && I64.(ax =^ rx)) || (I64.(ax =^ mpfr_EXP_INF) && I64.(ax =^ rx))) &&
        ...
        C.String.print (C.String.of_literal "YES!!!\n");
	true
    end else begin
        C.String.print (C.String.of_literal "no...\n");
	false
    end


let test_sub1sp1 s bp bx cp cx rnd_mode p rp rx rt =
    C.String.print (C.String.of_literal "Testing sub1sp1: \n");
    //print_i32 s; print_u64 bp; print_i64 bx; print_u64 cp; print_i64 cx; print_rnd rnd_mode;
    //print_i64 p; print_u64 rp; print_i64 rx; print_i32 rt;
    //cprint "\n";
    push_frame();
    ...
    let cs = mk_mpfr_struct p s cx cm in
    let c = createL [cs] in
    let at = mpfr_sub1sp1 c b c rnd_mode p in
    let asign = mpfr_SIGN c in
    let am = mpfr_MANT c in
    let ap = am.(0ul) in
    let ax = mpfr_EXP c in
...
let tests1 () =
    test_sub1sp1 1l 12105675798371893248uL 6L 9799832789158199296uL 5L MPFR_RNDN 63L 14411518807585587200uL 5L 0l
