...
val reduction_prime_2prime_with_carry_impl:
    cin:uint64
  -> x:felem
  -> result:felem ->
  Stack unit
...
val reduction_prime_2prime_8_with_carry_impl: x:widefelem -> res:felem -> Stack unit
  (requires fun h ->
    ...
    wide_as_nat h x < 2 * prime)
  (ensures fun h0 _ h1 -> modifies (loc res) h0 h1 /\
    as_nat h1 res == wide_as_nat h0 x % prime)
...
let fsub x y out =
  ...
  let t2 = u64 0 in
  let t3 = t -. (t <<. (size 32)) in
    modulo_addition_lemma  (as_nat h0 x - as_nat h0 y) prime 1;
  let c = bn_add4_variables out (u64 0)  t0 t1 t2 t3 out in
    let h2 = ST.get() in
      assert(
      if as_nat h0 x - as_nat h0 y >= 0 then
	begin
	  modulo_lemma (as_nat h0 x - as_nat h0 y) prime;
	  as_nat h2 out == (as_nat h0 x - as_nat h0 y) % prime
	end
      else
          begin
...
val add8_without_carry1: t:widefelem -> t1:widefelem -> result:widefelem -> Stack unit
  (requires fun h ->
    ...
    wide_as_nat h t1 < pow2 320 /\ wide_as_nat h t < prime * prime)
  (ensures fun h0 _ h1 -> modifies (loc result) h0 h1 /\
...
val montgomery_multiplication_round: t:widefelem -> round:widefelem -> Stack unit
  ...
  (ensures fun h0 _ h1 -> modifies (loc round) h0 h1 /\
    wide_as_nat h1 round = (wide_as_nat h0 t + prime * (wide_as_nat h0 t % pow2 64)) / pow2 64)
...
val montgomery_multiplication_one_round_proof:
  t: nat {t < prime * prime} ->
  result: nat {result = (t + (t % pow2 64) * prime) / pow2 64} ->
  co: nat {co % prime == t % prime} -> Lemma (
    result % prime == co * modp_inv2 (pow2 64) % prime /\
    result < prime * prime)

let montgomery_multiplication_one_round_proof t result co =
  mult_one_round t co;
  mul_lemma_1 (t % pow2 64) (pow2 64) prime;
  assert_norm (prime * prime + pow2 64 * prime < pow2 575);
  lemma_div_lt (t + (t % pow2 64) * prime) 575 64;
  assert_norm (prime * prime > pow2 (575 - 64))


inline_for_extraction noextract
val montgomery_multiplication_round_twice: t: widefelem -> result: widefelem -> Stack unit
  (requires fun h -> live h t /\ live h result  /\ wide_as_nat h t < prime * prime)
  (ensures fun h0 _ h1 ->
    modifies (loc result) h0 h1 /\
    (
      let round = (wide_as_nat h0 t + prime * (wide_as_nat h0 t % pow2 64)) / pow2 64 in
      wide_as_nat h1 result < prime * prime /\
      wide_as_nat h1 result % prime == (wide_as_nat h0 t * modp_inv2_prime (pow2 128) prime) % prime /\
      wide_as_nat h1 result == (round + prime * (round % pow2 64)) / pow2 64
    )
 )
...
let montgomery_multiplication_round_twice t result =
  ...
  push_frame();
    ...
    montgomery_multiplication_round t tempRound;
      let h1 = ST.get() in
      Math.Lemmas.swap_mul (wide_as_nat h0 t % pow2 64) prime;
    ...
    montgomery_multiplication_round tempRound result;
      let h2 = ST.get() in
      Math.Lemmas.swap_mul (wide_as_nat h1 tempRound % pow2 64) prime;
      montgomery_multiplication_one_round_proof (wide_as_nat h1 tempRound) (wide_as_nat h2 result) (wide_as_nat h0 t * modp_inv2_prime (pow2 64) prime);
...
let montgomery_multiplication_buffer_by_one a result =
  ...
  push_frame();
    ...
    montgomery_multiplication_round_twice round2 round4;
      ...
      lemma_mod_mul_distr_l (wide_as_nat h1 t * modp_inv2_prime (pow2 128) prime) (modp_inv2_prime (pow2 128) prime) prime;
	mul_lemma_2 (wide_as_nat h1 t % pow2 64) (pow2 64 - 1) prime;
	mul_lemma_2 (let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in round % pow2 64) (pow2 64 - 1) prime;
	mul_lemma_2 (
	  let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in
	  let round2 = (round + prime * (round % pow2 64)) / pow2 64 in
	  round2 % pow2 64) (pow2 64 - 1) prime;
	mul_lemma_2 (
	  let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in
	  let round2 = (round + prime * (round % pow2 64)) / pow2 64 in
	  let round3 = (round2 + prime * (round2 % pow2 64)) / pow2 64 in
	  round3 % pow2 64) (pow2 64 - 1) prime;
...
let fmul a b result =
  ...
  montgomery_multiplication_round_twice round2 round4;
	...
	lemma_mod_mul_distr_l (wide_as_nat h1 t * modp_inv2_prime (pow2 128) prime) (modp_inv2_prime (pow2 128) prime) prime;
      ...
      mul_lemma_1 (
         let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in
	 round % pow2 64) (pow2 64) prime;
      mul_lemma_1 (
	let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in
	let round2 = (round + prime * (round % pow2 64)) / pow2 64 in
	round2 % pow2 64) (pow2 64) prime;
      mul_lemma_1 (
	let round = (wide_as_nat h1 t + prime * (wide_as_nat h1 t % pow2 64)) / pow2 64 in
	let round2 = (round + prime * (round % pow2 64)) / pow2 64 in
	let round3 = (round2 + prime * (round2 % pow2 64)) / pow2 64 in
	round3 % pow2 64) (pow2 64) prime;
      assert_norm((prime * pow2 64 + (prime * pow2 64 + (prime * pow2 64 + ((pow2 64 - 1) * prime + prime * prime) / pow2 64) / pow2 64)/ pow2 64) / pow2 64 < 2 * prime);
  reduction_prime_2prime_8_with_carry_impl round4 result;
...
let fsqr a result =
  ...
  push_frame();
    ...
    bn_sqr4 a t;
      let h1 = ST.get() in
      mul_lemma_ (as_nat h0 a) (as_nat h0 a) prime;
  montgomery_multiplication_round_twice t round2;
      let h2 = ST.get() in
  montgomery_multiplication_round_twice round2 round4;
      let h3 = ST.get() in
