...
val lemma_pointAddToSpecification:
  ...
  (
    let x3D, y3D, z3D = fromDomain_ x3, fromDomain_ y3, fromDomain_ z3 in
    let (xN, yN, zN) = point_add (pxD, pyD, pzD) (qxD, qyD, qzD) in
...
let lemma_pointAddToSpecification x1D y1D z1D x2D y2D z2D x3 y3 z3  u1 u2 s1 s2 h r = admit()
  ...
  //     u1 == toDomain_ (z2D * z2D * pxD % prime) /\
  //     u2 == toDomain_ (pzD * pzD * qxD % prime) /\
  //     s1 == toDomain_ (qzD * qzD * qzD * pyD % prime) /\
  //     s2 == toDomain_ (pzD * pzD * pzD * qyD % prime) /\
  ...
  //   let (xN, yN, zN) = point_add (x1D, y1D, z1D) (x2D, y2D, z2D) in
