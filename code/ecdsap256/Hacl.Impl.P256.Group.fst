...
unfold
let refl (p:LSeq.lseq uint64 12{point_inv_seq p}) : GTot S.aff_point_c =
  S.to_aff_point (from_mont_point_c (as_point_nat_seq p))


inline_for_extraction noextract
let mk_to_p256_comm_monoid : BE.to_comm_monoid U64 12ul 0ul = {
  BE.a_spec = S.aff_point_c;
  BE.comm_monoid = Spec.ECC.mk_ec_comm_monoid S.p256;
...
let point_add ctx x y xy =
  let h0 = ST.get () in
  Spec.EC.Projective.Lemmas.to_aff_point_add_lemma_a3 S.p256
    (from_mont_point_c (as_point_nat h0 x)) (from_mont_point_c (as_point_nat h0 y));
...
let point_double ctx x xx =
  let h0 = ST.get () in
  Spec.EC.Projective.Lemmas.to_aff_point_double_lemma_a3 S.p256
    (from_mont_point_c (as_point_nat h0 x));
...
let point_zero ctx one =
  let h0 = ST.get () in
  Spec.EC.Projective.Lemmas.to_aff_point_at_infinity_lemma S.p256;
