module Hacl.Impl.ECDSA.P256SHA256.Signature.Hashless
...
let ecdsa_signature_step12 m result = 
  let h0 = ST.get() in 
...
val ecdsa_signature_core: r: felem -> s: felem -> m: lbuffer uint8 (size 32) ->  
  ...
  (ensures fun h0 flag h1 -> 
    ...
    (
      let z = nat_from_bytes_be (as_seq h0 m) % prime_p256_order in 
      let (rxN, ryN, rzN), _ = montgomery_ladder_spec (as_seq h0 k) ((0,0,0), basePoint) in 
      let (xN, _, _) = _norm (rxN, ryN, rzN) in 
...
let ecdsa_signature_core r s m privKeyAsFelem k = 
  push_frame();
  let h0 = ST.get() in 
...
let ecdsa_signature_without_hash result m privKey k = 
  push_frame();
  let h0 = ST.get() in 
  ...
  let flag = ecdsa_signature_core r s m privKeyAsFelem k in 
