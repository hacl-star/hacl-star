...
let super_hack () =
  ...
  let optional_state = [ "Hacl"; "Streaming"; "Blake2"; "Types"; "optional_block_state_blake2b_256"] in
...
val malloc_with_params_and_key:
  ...
  let k: Common.params_and_key Spec.Blake2B (G.reveal i) = p, k in
  r: HS.rid ->
...
let malloc_with_key k kk r =
  ...
  (if not (B.g_is_null s) then
    F.frame_invariant blake2b_256 i (B.loc_region_only false (HS.get_tip h1)) s h1 hf);
  s
...
val reset_with_key_and_params: (i: G.erased (Common.index Spec.Blake2B)) -> (
  ...
  p:P.params Spec.Blake2B i ->
  k:LowStar.Buffer.buffer Lib.IntTypes.uint8 { LowStar.Buffer.length k == UInt8.v ((G.reveal i).key_length) } -> (
  let key: Common.params_and_key Spec.Blake2B (G.reveal i) = (p, k) in
  unit ->
  Stack unit
  (requires (fun h0 ->
...
//    blake2b_256.key.invariant #i h0 key /\
  ...
  (ensures (fun h0 _ h1 ->
    invariant c i h1 state /\
    seen c i h1 state == S.empty /\
    reveal_key blake2b_256 i h1 state ==
...
let reset_with_key (i: G.erased (Common.index Spec.Blake2B)) s k () =
  ...
  F.frame_invariant blake2b_256 i (P.footprint h1 p) s hi h1;
  ...
  assert (F.reveal_key blake2b_256 i h2 s ==
  ...
  assert (F.footprint blake2b_256 i hi s == F.footprint blake2b_256 i h2 s);
  assert (F.preserves_freeable blake2b_256 i s hi h2);

  pop_frame();
  let hf = ST.get() in
  // The modifies below is obtained from B.popped_modifies
  F.frame_invariant blake2b_256 i (B.loc_region_only false (HS.get_tip h2)) s h2 hf;

  B.modifies_fresh_frame_popped hi h0 (F.footprint blake2b_256 (G.reveal i) hi s) h2 hf;
  let h0 = hi in
  let h1 = ST.get () in
  let c = blake2b_256 in
  let i = G.reveal i in
...
let digest (i: G.erased (Common.index Spec.Blake2B)) s dst l =
  F.digest_erased blake2b_256 i (Common.s Spec.Blake2B i Core.M256) (Common.blake_key Spec.Blake2B i) s dst l;
  (F.index_of_state blake2b_256 i (Common.s Spec.Blake2B i Core.M256) (Common.blake_key Spec.Blake2B i) s).digest_length

let info (i: G.erased (Common.index Spec.Blake2B)): Tot _ =
  F.index_of_state blake2b_256 i (Common.s Spec.Blake2B i Core.M256) (Common.blake_key Spec.Blake2B i)
...
let hash_with_key_and_params : Impl.blake2_with_params_st Spec.Blake2B Core.M256 =
