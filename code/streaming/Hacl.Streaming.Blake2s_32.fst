...
val malloc_with_params_and_key:
  ...
  ST (state c i t t')
  (requires (fun h0 ->
    blake2s_32.key.invariant #i h0 k /\
    HyperStack.ST.is_eternal_region r))
  (ensures (fun h0 s h1 ->
    invariant c i h1 s /\
    freeable c i h1 s /\
    seen c i h1 s == S.empty /\
    reveal_key blake2s_32 i h1 s == blake2s_32.key.v i h0 k /\
...
val malloc_with_key:
  ...
  let open F in
  let c = blake2s_32 in
  let i: Common.index Spec.Blake2S = kk, UInt8.uint_to_t (Spec.max_output Spec.Blake2S) in
  let t: Type0 = c.state.s i in
  let t': Type0 = I.optional_key (G.reveal i) c.km c.key in
  r: HS.rid ->
  ST (state c i t t')
...
let malloc_with_key k kk r =
  ...
  let i: Common.index Spec.Blake2S = kk, nn in
  let h00 = ST.get () in
  ...
  let h1 = ST.get () in
  assert F.(freeable blake2s_32 i h1 s);
  ...
  (==) { _ by (FStar.Tactics.trefl ()) }
    P.v #Spec.Blake2S h0 p, (if fst i = 0uy then S.empty #Lib.IntTypes.uint8 else B.as_seq h0 (k <: B.buffer Lib.IntTypes.uint8));
  (==) { }
    { Spec.blake2_default_params Spec.Blake2S with Spec.key_length = kk; Spec.digest_length = nn }, (if fst i = 0uy then S.empty #Lib.IntTypes.uint8 else B.as_seq h0 (k <: B.buffer Lib.IntTypes.uint8));
  (==) { }
    { Spec.blake2_default_params Spec.Blake2S with Spec.key_length = kk }, (if fst i = 0uy then S.empty #Lib.IntTypes.uint8 else B.as_seq h0 (k <: B.buffer Lib.IntTypes.uint8));
  };
  assert (P.invariant h1 p);
  P.invariant_loc_in_footprint h1 p;
  assert B.(loc_disjoint (P.footprint h1 p) (F.footprint blake2s_32 i h1 s));
  P.free p;
  let h2 = ST.get () in
  F.frame_invariant blake2s_32 i (P.footprint h1 p) s h1 h2;
  F.frame_seen blake2s_32 i (P.footprint h1 p) s h1 h2;
  assert F.(freeable blake2s_32 i h2 s);
  s
...
//    blake2s_32.key.invariant #i h0 key /\
  ...
  (ensures (fun h0 _ h1 ->
    invariant c i h1 state /\
    seen c i h1 state == S.empty /\
    reveal_key blake2s_32 i h1 state ==
      ({ Spec.blake2_default_params Spec.Blake2S with Spec.key_length = (fst i); Spec.digest_length = (snd i) },
      (if fst i = 0uy then S.empty #uint8 else B.as_seq h0 (k <: B.buffer uint8))) /\
    footprint c i h0 state == footprint c i h1 state /\
    B.(modifies (footprint c i h0 state) h0 h1) /\
    preserves_freeable c i state h0 h1))))
...
let reset_with_key (i: G.erased (Common.index Spec.Blake2S)) s k () =
  ...
  F.frame_invariant blake2s_32 i (P.footprint h1 p) s hi h1;
  ...
  (==) { _ by (FStar.Tactics.trefl())  }
    P.v #Spec.Blake2S h1 p, (if fst idx = 0uy then S.empty #Lib.IntTypes.uint8 else B.as_seq h1 (k <: B.buffer Lib.IntTypes.uint8));
  (==) { }
    { Spec.blake2_default_params Spec.Blake2S with Spec.key_length = (fst idx); Spec.digest_length = (snd idx) }, (if fst idx = 0uy then S.empty #Lib.IntTypes.uint8 else B.as_seq h1 (k <: B.buffer Lib.IntTypes.uint8));
  };

  assert (G.reveal i == idx);
  assert (F.reveal_key blake2s_32 i h2 s ==
   ({ Spec.blake2_default_params Spec.Blake2S with Spec.key_length = (fst i); Spec.digest_length = (snd i) },
  ...
  assert (F.footprint blake2s_32 i hi s == F.footprint blake2s_32 i h2 s);
  assert (F.preserves_freeable blake2s_32 i s hi h2);

  pop_frame();
  let hf = ST.get() in
  // The modifies below is obtained from B.popped_modifies
  F.frame_invariant blake2s_32 i (B.loc_region_only false (HS.get_tip h2)) s h2 hf;

  B.modifies_fresh_frame_popped hi h0 (F.footprint blake2s_32 (G.reveal i) hi s) h2 hf;
  let h0 = hi in
  let h1 = ST.get () in
  let c = blake2s_32 in
  let i = G.reveal i in
...
let hash_with_key_and_paramas : Impl.blake2_with_params_st Spec.Blake2S Core.M32 =
