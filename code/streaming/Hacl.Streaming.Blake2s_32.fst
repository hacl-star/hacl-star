...
let malloc_with_key k kk r =
  ...
  pop_frame ();
  let hf = ST.get () in
  F.frame_invariant blake2s_32 i (B.loc_region_only false (HS.get_tip h1)) s h1 hf;
  s
...
let digest (i: G.erased (Common.index Spec.Blake2S)) s dst l =
  F.digest_erased blake2s_32 i (Common.s Spec.Blake2S i Core.M32) (Common.blake_key Spec.Blake2S i) s dst l;
  (F.index_of_state blake2s_32 i (Common.s Spec.Blake2S i Core.M32) (Common.blake_key Spec.Blake2S i) s).digest_length

let info (i: G.erased (Common.index Spec.Blake2S)): Tot _ =
  F.index_of_state blake2s_32 i (Common.s Spec.Blake2S i Core.M32) (Common.blake_key Spec.Blake2S i)
