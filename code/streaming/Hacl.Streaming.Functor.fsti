...
inline_for_extraction noextract
val finish: #c:index -> finish_st c
...
/// The "higher-order" functions generated by splice.
/// =================================================

inline_for_extraction noextract
val mk_create_in (#c: Block.index) :
    init: Block.init_st c ->
    key_copy: Stateful.copy_st c.key ->
    key_create_in: Stateful.create_in_st c.key ->
    state_create_in: Stateful.create_in_st c.state ->
  create_in_st c
...
val mk_copy (#c: index) :
  key_copy: Stateful.copy_st c.key ->
  key_create_in: Stateful.create_in_st c.key ->
  state_copy: Stateful.copy_st c.state ->
  state_create_in: Stateful.create_in_st c.state ->
  copy_st c

inline_for_extraction noextract
val mk_alloca (#c: Block.index) :
  init: Block.init_st c ->
  key_copy: Stateful.copy_st c.key ->
  key_alloca: Stateful.alloca_st c.key ->
  state_alloca: Stateful.alloca_st c.state ->
  alloca_st c

inline_for_extraction noextract
val mk_init (#c: Block.index) :
  key_copy: Stateful.copy_st c.key ->
  init: Block.init_st c ->
  init_st c

inline_for_extraction noextract
val mk_update (#c: Block.index) :
  update_multi: Block.update_multi_st c ->
  update_st c

inline_for_extraction noextract
val mk_finish (#c: Block.index) :
  finish: Block.finish_st c ->
  update_last: Block.update_last_st c ->
  update_multi: Block.update_multi_st c ->
  state_copy: Stateful.copy_st c.state ->
  state_alloca: Stateful.alloca_st c.state ->
  finish_st c
...
inline_for_extraction noextract
val mk_free (#c: Hacl.Streaming.Block.index) :
  state_free: Stateful.free_st c.state ->
  key_free: Stateful.free_st c.key ->
  free_st c
