...
#push-options "--ifuel 1 --z3cliopt smt.arith.nl=false"
inline_for_extraction noextract
let blake2_fun_spec (a : alg) (m : m_spec) (key_size : key_size_ty a) =
  I.Block
    I.Erased (* key management *)
...
inline_for_extraction noextract
let blake2_spec_props1 (a : alg) (m : m_spec) (key_size : key_size_ty a) =
  blake2_fun_spec a m key_size
    (fun () prevlen acc -> update_multi_zero key_size () prevlen acc) (* update_multi_zero *)
    (* update_multi_associative *)
    (fun () acc prevlen1 prevlen2 input1 input2 ->
      update_multi_associative key_size () acc prevlen1 prevlen2 input1 input2)

/// TODO: this one sometimes loops
#push-options "--ifuel 0"
inline_for_extraction noextract
let blake2_spec_props2 (a : alg) (m : m_spec) (key_size : key_size_ty a) =
  blake2_spec_props1 a m key_size
    (fun () key input -> spec_is_incremental a #key_size () key input)
      ...
      let hash0 = init_s #a #key_size i key in
      let hash1 = update_multi_s #a key_size i hash0 0 bs in
      let hash2 = update_last_s #a key_size i hash1 (S.length bs) l in
      let hash3 = finish_s #a #key_size i key hash2 in
      assume(hash3 `S.equal` spec_s a #key_size i key input))
...
spec_is_incremental a #key_size () k input) (* spec_is_incremental *)

inline_for_extraction noextract
let blake2_spec_props3 (a : alg) (m : m_spec) (key_size : key_size_ty a) =
  blake2_spec_props2 a m key_size
    (fun _ acc -> ()) (* index_of_state *)

inline_for_extraction noextract
let blake2 (a : alg) (m : m_spec) (key_size : key_size_ty a) =
  blake2_spec_props3 a m key_size
    (* init *)
    (fun _ key acc ->
      [@inline_let] let wv = get_wv acc in
      [@inline_let] let h = get_state_p acc in
