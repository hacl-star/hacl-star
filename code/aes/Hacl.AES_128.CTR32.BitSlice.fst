...
let aes_ctx = aes_ctx M32 Spec.AES128
...
val create_ctx: unit ->
  ...
  (ensures  (fun h0 f h1 -> live h1 f /\
    stack_allocated f h0 h1 (Seq.create (size_v (ctxlen M32 Spec.AES128))
    (elem_zero M32))))

[@ CInline ]
let create_ctx () = create_ctx M32 Spec.AES128
...
val aes128_key_block:
  ...
  (ensures  (fun h0 _ h1 -> modifies1 kb h0 h1 /\
    ...
    as_seq h1 kb == u8_16_to_le (Spec.aes_encrypt_block Spec.AES128
      (keyx_to_bytes M32 Spec.AES128 k) (Spec.aes_ctr32_set_counter_LE
      (nonce_to_bytes M32 n) counter)))))

[@ CInline ]
let aes128_key_block kb ctx counter = aes_key_block #M32 #Spec.AES128 kb ctx counter
...
let aes128_ctr len out inp ctx c = aes_ctr #M32 #Spec.AES128 len out inp ctx c
...
let aes128_ctr_encrypt len out inp k n c = aes_ctr_encrypt #M32 #Spec.AES128 len out inp k n c
...
val aes128_ctr_decrypt:
  ...
  (ensures (fun h0 _ h1 -> modifies (loc out) h0 h1 /\
    ...
    (as_seq h0 k) (as_seq h0 n) counter (as_seq h0 inp)))
let aes128_ctr_decrypt len out inp k n c = aes_ctr_decrypt #M32 #Spec.AES128 len out inp k n c
