...
let poly1305_do_core_padded #w aadlen aad mlen m ctx =
  let h_pre = ST.get() in
  push_frame();
  let h0 = ST.get() in
  ...
  Stack unit
    ...
    (ensures fun h0 _ h1 -> modifies (loc out |+| loc ctx) h0 h1 /\
      (let r = Poly.as_get_r h0 ctx in
       ...
       as_seq h1 out == tag))
let poly1305_do_core_finish #w k out ctx block =
...
let poly1305_do_core_ #w k aadlen aad mlen m out ctx block =
...
inline_for_extraction noextract
val poly1305_do_: #w:field_spec -> poly1305_do_core_st w
let poly1305_do_ #w =
  match w with
  | M32 -> poly1305_do_32
...
let poly1305_do #w k n aadlen aad mlen m out =
  push_frame ();
  ...
  poly1305_do_ #w key aadlen aad mlen m out;
  pop_frame()
