...
let point_add_and_double0 #i nq_p1 ab dc tmp2 =
  [@inline_let]
  let s, _ = i in
...
[@ Meta.Attribute.inline_ ]
let point_add_and_double1 #i nq nq_p1 tmp1 tmp2 =
  [@inline_let]
  let s, _ = i in
  ...
  copy_felem a c;                           // a = bb
  fsub #i c d c;   // c = e = aa - bb
  assert_norm (121665 < pow2 17);
  fmul1 #i b c (u64 121665); // b = e * 121665
  fadd #i b b d;   // b = (e * 121665) + aa
...
let point_add_and_double #i q p01_tmp1 tmp2 =
  [@inline_let]
  let s, _ = i in
  let h0 = ST.get () in
  ...
  fadd #i a x2 z2; // a = x2 + z2
  fsub #i b x2 z2; // b = x2 - z2

  point_add_and_double0 #i nq_p1 ab dc tmp2;
  point_add_and_double1 #i nq nq_p1 tmp1 tmp2;
...
[@ Meta.Attribute.specialize ]
let point_double #i nq tmp1 tmp2 =
  [@inline_let]
  let s, _ = i in
  let x2 = sub nq 0ul (nlimb s) in
  let z2 = sub nq (nlimb s) (nlimb s) in
