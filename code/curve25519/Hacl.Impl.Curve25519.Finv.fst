...
[@ Meta.Attribute.inline_ ]
let fmul_s #i out f1 f2 tmp =
  C.fmul #i out f1 f2 tmp
...
let fsquare_times #id o inp tmp n =
  let h0 = ST.get() in
  fsqr_s #id o inp tmp;
  let h1 = ST.get() in
  ...
  let inv (h:mem) (i:nat{i < v n}) =
    modifies (loc o |+| loc tmp) h0 h /\ fsquare_times_inv #(fst id) h o /\
    feval h o == S.pow (feval #(fst id) h0 inp) (pow2 (i + 1)) in
  ...
  (fun i ->
    ...
    S.lemma_pow_mul (feval #(fst id) h0 inp) (pow2 (v i + 1)) (pow2 1);
...
let finv0 #id i t1 tmp =
  ...
  (* 2 *) fsquare_times #id a i tmp1 1ul;
  (* 8 *) fsquare_times #id t0 a tmp1 2ul;
  (* 9 *) fmul_s #id b t0 i tmp;
  (* 11 *) fmul_s #id a b a tmp;
  (* 22 *) fsquare_times #id t0 a tmp1 1ul;
  (* 2^5 - 2^0 = 31 *) fmul_s #id b t0 b tmp;
  (* 2^10 - 2^5 *) fsquare_times #id t0 b tmp1 5ul;
  (* 2^10 - 2^0 *) fmul_s #id b t0 b tmp;
  (* 2^20 - 2^10 *) fsquare_times #id t0 b tmp1 10ul;
  (* 2^20 - 2^0 *) fmul_s #id c t0 b tmp;
  (* 2^40 - 2^20 *) fsquare_times #id t0 c tmp1 20ul;
  (* 2^40 - 2^0 *) fmul_s #id t0 t0 c tmp;
  (* 2^50 - 2^10 *) fsquare_times #id t0 t0 tmp1 10ul;
  (* 2^50 - 2^0 *) fmul_s #id b t0 b tmp;
  (* 2^100 - 2^50 *) fsquare_times #id t0 b tmp1 50ul;
  (* 2^100 - 2^0 *) fmul_s #id c t0 b tmp;
  (* 2^200 - 2^100 *) fsquare_times #id t0 c tmp1 100ul;
  (* 2^200 - 2^0 *) fmul_s #id t0 t0 c tmp;
  (* 2^250 - 2^50 *) fsquare_times #id t0 t0 tmp1 50ul;
  let h0' = ST.get () in
  (* 2^250 - 2^0 *) fmul_s #id t0 t0 b tmp;
  let h1' = ST.get () in
  assert (modifies (loc t1 |+| loc tmp) h0' h1');
  assert (modifies (loc t1 |+| loc tmp) h0 h1');
  (* 2^255 - 2^5 *) fsquare_times #id t0 t0 tmp1 5ul;
  let h1 = ST.get () in
