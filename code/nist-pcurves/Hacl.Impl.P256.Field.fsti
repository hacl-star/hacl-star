...
val make_fzero: {| S.curve_params |} -> n:felem -> Stack unit
...
val make_fone: {| S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n < S.prime /\
    fmont_as_nat h1 n == 1)
...
val bn_is_lt_prime_mask: {| S.curve_params |} -> f:felem -> Stack uint64
...
val fadd: {| S.curve_params |} -> res:felem -> x:felem -> y:felem -> Stack unit
  (requires fun h ->
...
val fdouble: {| S.curve_params |} -> res:felem -> x:felem -> Stack unit
  (requires fun h ->
...
val fsub: {| S.curve_params |} -> res:felem -> x:felem -> y:felem -> Stack unit
  (requires fun h ->
...
val fnegate_conditional_vartime: {| S.curve_params |} -> f:felem -> is_negate:bool -> Stack unit
...
val fmul: {| S.curve_params |} -> res:felem -> x:felem -> y:felem -> Stack unit
  (requires fun h ->
    live h x /\ live h y /\ live h res /\
    eq_or_disjoint x y /\ eq_or_disjoint x res /\ eq_or_disjoint y res /\
    as_nat h x < S.prime /\ as_nat h y < S.prime)
...
val fsqr: {| S.curve_params |} -> res:felem -> x:felem -> Stack unit
  (requires fun h ->
    live h x /\ live h res /\ eq_or_disjoint x res /\
    as_nat h x < S.prime)
  (ensures  fun h0 _ h1 -> modifies (loc res) h0 h1 /\
...
val from_mont: {| S.curve_params |} -> res:felem -> x:felem -> Stack unit
  (requires fun h ->
...
val to_mont: {| S.curve_params |} -> res:felem -> f:felem -> Stack unit
  (requires fun h ->
...
val fmul_by_b_coeff: {| S.curve_params |} -> res:felem -> x:felem -> Stack unit
  (requires fun h ->
    live h x /\ live h res /\ eq_or_disjoint x res /\
    as_nat h x < S.prime)
  (ensures fun h0 _ h1 -> modifies (loc res) h0 h1 /\
...
val fcube: {| S.curve_params |} -> res:felem -> x:felem -> Stack unit
  (requires fun h ->
