...
val make_prime: {| cp:S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n == S.prime)

[@CInline]
let make_prime {| cp:S.curve_params |} n =
  if cp.bn_limbs = 4ul then (
    assume (cp.prime == Spec.P256.p256_prime);
...
val make_order: {| cp:S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n == S.order)

[@CInline]
let make_order {| cp:S.curve_params |} n =
  if cp.bn_limbs = 4ul then (
    assume (cp.order = Spec.P256.p256_order);
...
val make_a_coeff: {| cp:S.curve_params |} -> a:felem -> Stack unit
...
[@CInline]
let make_a_coeff {| cp:S.curve_params |} a =
  if cp.bn_limbs = 4ul then (
    admit();
...
val make_b_coeff: {| cp:S.curve_params |} -> b:felem -> Stack unit
...
[@CInline]
let make_b_coeff {| cp:S.curve_params |} b =
  if cp.bn_limbs = 4ul then (
    admit();
...
val make_g_x: {| cp:S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n < S.prime /\
...
[@CInline]
let make_g_x {| cp:S.curve_params |} n =
  admit();
  if cp.bn_limbs = 4ul then
    ...
    assert_norm (v n0 + v n1 * pow2 64 + v n2 * pow2 128 + v n3 * pow2 192 < S.prime);
    assert_norm (SM.to_mont S.basepoint_x == v n0 + v n1 * pow2 64 + v n2 * pow2 128 + v n3 * pow2 192);
    SM.lemma_to_from_mont_id S.basepoint_x;
    bn_make_u64_4 n n0 n1 n2 n3
  else admit()

val make_g_y: {| cp:S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n < S.prime /\
...
[@CInline]
let make_g_y {| cp:S.curve_params |} n =
  admit();
  if cp.bn_limbs = 4ul then
    ...
    assert_norm (v n0 + v n1 * pow2 64 + v n2 * pow2 128 + v n3 * pow2 192 < S.prime);
    assert_norm (SM.to_mont S.basepoint_y == v n0 + v n1 * pow2 64 + v n2 * pow2 128 + v n3 * pow2 192);
    SM.lemma_to_from_mont_id S.basepoint_y;
    bn_make_u64_4 n n0 n1 n2 n3
  else admit()

val make_fmont_R2: {| cp:S.curve_params |} -> n:felem -> Stack unit
  (requires fun h -> live h n)
  (ensures  fun h0 _ h1 -> modifies (loc n) h0 h1 /\
    as_nat h1 n == SM.fmont_R * SM.fmont_R % S.prime)

[@CInline]
let make_fmont_R2 {| cp:S.curve_params |}n =
  admit();
  if cp.bn_limbs = 4ul then 
