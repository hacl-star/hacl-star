...
include Hacl.PCurves.PrecompTable


#set-options "--z3rlimit 30 --fuel 0 --ifuel 0"

val point_mul: {| cp:S.curve_params |} -> {| pt:precomp_tables |} -> res:point -> scalar:felem -> p:point -> Stack unit
  (requires fun h ->
...
val point_mul_g: {| cp:S.curve_params |} -> {| pt:precomp_tables |} -> res:point -> scalar:felem -> Stack unit
  (requires fun h ->
...
val point_mul_double_g: {| cp:S.curve_params |} -> {| pt:precomp_tables |} -> res:point -> scalar1:felem -> scalar2:felem -> p:point -> Stack unit
  (requires fun h ->
