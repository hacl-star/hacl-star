...
open Hacl.Impl.PCurves.PointMul.P521
...
let msg_as_felem alg msg_len msg res =
  ...
  begin
  match alg with
    | S.NoHash -> copy mHash (sub msg 0ul 64ul)
  ...
  let mHash64 = sub mHash 0ul 66ul in
  BN.bn_from_bytes_be res mHash;
  qmod_short res res;
  pop_frame ()

[@ CInline ]
val ecdsa_sign_msg_as_qelem:
    signature:lbuffer uint8 (2ul *. 66ul)
  -> m_q:BN.felem
...
val ecdsa_verify_msg_as_qelem:
    m_q:BN.felem
  -> public_key:lbuffer uint8 (2ul *. 66ul)
...
inline_for_extraction noextract
val ecdsa_signature: alg:S.hash_alg_ecdsa -> ecdsa_sign_p521_st alg
let ecdsa_signature alg signature msg_len msg private_key nonce =
  push_frame ();
  let m_q = BN.create_felem #p521_params in
...
inline_for_extraction noextract
val ecdsa_verification: alg:S.hash_alg_ecdsa -> ecdsa_verify_p521_st alg
...
let ecdsa_sign_p521_sha2 signature msg_len msg private_key nonce =
  ecdsa_signature (S.Hash SHA2_256) signature msg_len msg private_key nonce

let ecdsa_sign_p521_sha384 signature msg_len msg private_key nonce =
  ecdsa_signature (S.Hash SHA2_384) signature msg_len msg private_key nonce

let ecdsa_sign_p521_sha512 signature msg_len msg private_key nonce =
  ecdsa_signature (S.Hash SHA2_512) signature msg_len msg private_key nonce

let ecdsa_sign_p521_without_hash signature msg_len msg private_key nonce =
  ecdsa_signature S.NoHash signature msg_len msg private_key nonce


let ecdsa_verif_p521_sha2 msg_len msg public_key signature_r signature_s =
  ecdsa_verification (S.Hash SHA2_256) msg_len msg public_key signature_r signature_s

let ecdsa_verif_p521_sha384 msg_len msg public_key signature_r signature_s =
  ecdsa_verification (S.Hash SHA2_384) msg_len msg public_key signature_r signature_s

let ecdsa_verif_p521_sha512 msg_len msg public_key signature_r signature_s =
...
let validate_public_key public_key =
  push_frame ();
  let point_jac = P.create_point #p521_params in
...
let validate_private_key private_key =
  push_frame ();
  let bn_sk = BN.create_felem #p521_params in
