...
let ecdsa_sign_p384_st (alg:S.hash_alg_ecdsa) =
    signature:lbuffer uint8 96ul
...
inline_for_extraction noextract
let ecdsa_verify_p384_st (alg:S.hash_alg_ecdsa) =
    msg_len:size_t{S.min_input_length alg (v msg_len)}
  -> msg:lbuffer uint8 msg_len
  -> public_key:lbuffer uint8 96ul
...
val ecdsa_sign_p384_sha2: ecdsa_sign_p384_st (S.Hash SHA2_256)
...
[@@ Comment "Create an ECDSA signature using SHA2-384.
  ...
  The function also checks whether `private_key` and `nonce` are valid:
    • 0 < `private_key` < the order of the curve
    • 0 < `nonce` < the order of the curve"]
val ecdsa_sign_p384_sha384: ecdsa_sign_p384_st (S.Hash SHA2_384)
...
[@@ Comment "Create an ECDSA signature using SHA2-512.
  ...
  The function also checks whether `private_key` and `nonce` are valid:
    • 0 < `private_key` < the order of the curve
    • 0 < `nonce` < the order of the curve"]
val ecdsa_sign_p384_sha512: ecdsa_sign_p384_st (S.Hash SHA2_512)
...
[@@ Comment "Create an ECDSA signature WITHOUT hashing first.
  ...
  The function also checks whether `private_key` and `nonce` are valid values:
    • 0 < `private_key` < the order of the curve
    • 0 < `nonce` < the order of the curve"]
val ecdsa_sign_p384_without_hash: ecdsa_sign_p384_st (S.NoHash)
...
val ecdsa_verif_p384_sha2: ecdsa_verify_p384_st (S.Hash SHA2_256)
...
[@@ Comment "Verify an ECDSA signature using SHA2-384.
  ...
  The function also checks whether `public_key` is valid"]
val ecdsa_verif_p384_sha384: ecdsa_verify_p384_st (S.Hash SHA2_384)
...
[@@ Comment "Verify an ECDSA signature using SHA2-512.
  ...
  The function also checks whether `public_key` is valid"]
val ecdsa_verif_p384_sha512: ecdsa_verify_p384_st (S.Hash SHA2_512)
...
[@@ Comment "Verify an ECDSA signature WITHOUT hashing first.
  ...
  The function also checks whether `public_key` is valid"]
val ecdsa_verif_without_hash: ecdsa_verify_p384_st S.NoHash
...
val validate_public_key: public_key:lbuffer uint8 96ul -> Stack bool
...
[@@ Comment "Private key validation.
  ...
  The private key is valid:
    • 0 < `private_key` < the order of the curve"]
val validate_private_key: private_key:lbuffer uint8 48ul -> Stack bool
...
val uncompressed_to_raw: pk:lbuffer uint8 97ul -> pk_raw:lbuffer uint8 96ul -> Stack bool
...
val compressed_to_raw: pk:lbuffer uint8 49ul -> pk_raw:lbuffer uint8 96ul -> Stack bool
  (requires fun h -> live h pk /\ live h pk_raw /\ disjoint pk pk_raw)
  (ensures  fun h0 b h1 -> modifies (loc pk_raw) h0 h1 /\
...
[@@ Comment "Convert a public key from raw to its uncompressed form.
  ...
  The function DOESN'T check whether (x, y) is a valid point."]
val raw_to_uncompressed: pk_raw:lbuffer uint8 96ul -> pk:lbuffer uint8 97ul -> Stack unit
  (requires fun h -> live h pk /\ live h pk_raw /\ disjoint pk pk_raw)
...
[@@ Comment "Convert a public key from raw to its compressed form.
  ...
  The function DOESN'T check whether (x, y) is a valid point."]
val raw_to_compressed: pk_raw:lbuffer uint8 96ul -> pk:lbuffer uint8 49ul -> Stack unit
  (requires fun h -> live h pk /\ live h pk_raw /\ disjoint pk pk_raw)
  (ensures  fun h0 _ h1 -> modifies (loc pk) h0 h1 /\
...
val dh_responder:
    shared_secret:lbuffer uint8 96ul
  -> their_pubkey:lbuffer uint8 96ul
  -> private_key:lbuffer uint8 48ul ->
  Stack bool
  (requires fun h ->
