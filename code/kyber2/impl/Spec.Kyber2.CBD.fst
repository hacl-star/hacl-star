...
let to_bits #l #len bytes =
  let (_,s) = generate_blocks 8 len len a_spec to_bits_inner bytes in s
...
#reset-options "--z3rlimit 100 --max_fuel 3 --max_ifuel 3 --using_facts_from '* -FStar.Seq'"
val cbd_eta: (bytes:lbytes_l SEC (64*params_eta)) -> lseq (Group.t) params_n

let cbd_eta_inner_ (bits_a:lseq Group.t params_eta) (bits_b: lseq Group.t params_eta) =
  let m = monoid_plus_t in
  let r = ring_t in
  let a = sum_n bits_a in
  let b = sum_n bits_b in
  minus a b

let cbd_eta_inner (bits:lseq Group.t (2*params_n*params_eta)) (i:size_nat{i<params_n}) =
  cbd_eta_inner_ (Seq.sub bits (2*i*params_eta) params_eta) (Seq.sub bits (2*i*params_eta + params_eta) params_eta)

let cbd_eta bytes =
  createi params_n (cbd_eta_inner (Seq.map int16_to_t (Seq.map to_i16 (to_bits bytes))))

let cbd_eta_lemma (bytes:lbytes_l SEC (64*params_eta)) (i:size_nat{i < params_n}) : Lemma (Seq.index (cbd_eta bytes) i == cbd_eta_inner (Seq.map Group.int16_to_t (Seq.map to_i16 (to_bits bytes))) i) = ()
