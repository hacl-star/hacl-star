...
let modular_add_swap_lemma #q x y = ()
...
let modular_mul_swap_lemma #q x y = ()
...
let modular_mul_add_distrib_left_lemma #q x y z =
  calc (==) {
    x*%(y+%z);
      = {lemma_mod_mul_distr_r x (y+z) q}
    (x*(y+z)) % q;
    ...
    (x*y + x*z) % q;
      = {lemma_mod_add_distr (x*y) (x*z) q; lemma_mod_add_distr (x*%z) (x*y) q}
    ((x*y) %q) +% ((x*z)%q);
    }
...
let rec gcd_lemma2 x y =
  ...
  if (y=0) then ()
    ...
    gcd_lemma2 y (x%y);
    extended_gcd_simple_lemma x y;
    euclidean_division_definition x y;
