...
let lib_ntt #a [| ring a |] #n zeta p = 
  let (peven, podd) = split_seq p in
  join_seq (lib_ntt (exp #a zeta 2) zeta peven) (lib_ntt (exp #a zeta 2) zeta podd)
  
//let lib_ntt_lemma #n #m omega psi p p' = ()

let lib_nttinv #a [| ring a |] #n halfninv zetainv p = 
  let (peven, podd) = split_seq p in
  join_seq (lib_nttinv halfninv (exp #a zetainv 2) zetainv peven) (lib_nttinv halfninv (exp #a zetainv 2) zetainv podd)
...
let lib_ntt_inversion_lemma1 #a [| ring a |] #n halfninv zeta zetainv p =
  ...
  let peven' = Lib.Poly.NTT.lib_nttinv halfninv zetainv2 zetainv pntt1 in
  let podd' = Lib.Poly.NTT.lib_nttinv halfninv zetainv2 zetainv pntt2 in
...
let lib_ntt_inversion_lemma2 #a [| ring a |] #n halfninv zeta zetainv p =
  let zeta2 = exp #a zeta 2 in
  let zetainv2 = exp #a zetainv 2 in
  let (peven, podd) = split_seq p in
  let p1 = Lib.Poly.NTT.lib_nttinv halfninv zetainv2 zetainv peven in
  let p2 = Lib.Poly.NTT.lib_nttinv halfninv zetainv2 zetainv podd in
  let pnttinv = lib_nttinv halfninv zetainv p in
  let p' = lib_ntt zeta pnttinv in
  ...
  lib_ntt_inversion_lemma2 halfninv zeta2 zetainv2 zeta zetainv peven;
  lib_ntt_inversion_lemma2 halfninv zeta2 zetainv2 zeta zetainv podd; 
  assert(peven == peven'); 
  assert(podd == podd');
...
let lib_ntt_mul #a [| ring a |] #n zeta p1 p2 =
  let plus = plus #a in
  let mul = mul #a in
  let p1even,p1odd = split_seq p1 in
  let p2even,p2odd = split_seq p2 in
  let f (i:nat{i<n/2}) = plus (mul p1even.[i] p2even.[i]) (mul (mul p1odd.[i] p2odd.[i]) (exp zeta (2*i+1))) in
  let g (i:nat{i<n/2}) = plus (mul p1odd.[i] p2even.[i]) (mul p1even.[i] p2odd.[i]) in
...
val lib_ntt_lemma_mul_assoc:
  ...
  -> p2:lib_poly a n
  -> p3:lib_poly a n
  -> Lemma (lib_ntt_mul zeta (lib_ntt_mul zeta p1 p2) p3 == lib_ntt_mul zeta p1 (lib_ntt_mul zeta p2 p3))

let lib_ntt_lemma_mul_assoc #a [| ring a |] #n zeta p1 p2 p3 = 
  let rec commut_zeta (x:a) (i:nat) : Lemma (ensures mul #a x (exp zeta i) == mul #a (exp zeta i) x) (decreases i) =
    if (i=0) then
      (lemma_one1 x; lemma_one2 x)
    else begin
      lemma_exp_succ1 #a zeta (i-1);
      lemma_mul_assoc x zeta (exp zeta (i-1));
      lemma_mul_assoc #a zeta x (exp zeta (i-1));
      commut_zeta x (i-1);
      lemma_mul_assoc zeta (exp zeta (i-1)) x end
  in
  let p12 = lib_ntt_mul zeta p1 p2 in
  let p12_3 = lib_ntt_mul zeta p12 p3 in
  let p23 = lib_ntt_mul zeta p2 p3 in
  let p1_23 = lib_ntt_mul zeta p1 p23 in
...
val lib_ntt_lemma_one1:
  ...
  -> zeta:a
  -> p:lib_poly a n
  -> Lemma (lib_ntt_mul zeta lib_ntt_one p == p)


let lib_ntt_lemma_one1 #a [| ring a |] #n zeta p =
  let p' = lib_ntt_mul zeta lib_ntt_one p in
  let customprop (k:nat{k<n}) : Type0 = (p'.[k] == p.[k]) in
  let customlemma (k:nat{k<n}) : Lemma (customprop k) =
    if(k%2=0) then begin
      lemma_one1 #a p.[k];
...
val lib_ntt_lemma_one2:
  ...
  -> zeta:a
  -> p:lib_poly a n
  -> Lemma (lib_ntt_mul zeta p lib_ntt_one == p)


let lib_ntt_lemma_one2 #a [| ring a |] #n zeta p =
  let p' = lib_ntt_mul zeta p lib_ntt_one in
  let customprop (k:nat{k<n}) : Type0 = (p'.[k] == p.[k]) in
  let customlemma (k:nat{k<n}) : Lemma (customprop k) =
    if(k%2=0) then begin
      lemma_one2 #a p.[k];
...
instance lib_ntt_mul_monoid : (#a:Type0) -> (#[tcresolve ()] r:ring a) -> (#n:size_nat{n%2=0}) -> zeta:a{forall (x:a). mul x zeta == mul zeta x} -> monoid (lib_poly a n) =
  fun #a #r #n zeta ->
    {
      id = lib_ntt_one;
      op = lib_ntt_mul #a zeta;
      lemma_assoc = lib_ntt_lemma_mul_assoc #a zeta;
      lemma_id1 = lib_ntt_lemma_one1 #a zeta;
      lemma_id2 = lib_ntt_lemma_one2 #a zeta;
    }
...
val lib_ntt_lemma_distr_left:
  ...
  -> p2:lib_poly a n
  -> p3:lib_poly a n
  -> Lemma (lib_ntt_mul zeta p1 (lib_ntt_plus p2 p3) == lib_ntt_plus (lib_ntt_mul zeta p1 p2) (lib_ntt_mul zeta p1 p3))

let lib_ntt_lemma_distr_left #a [| ring a |] #n zeta p1 p2 p3 =
  let p1_23 = lib_ntt_mul zeta p1 (lib_ntt_plus p2 p3) in
  let p12_13 = lib_ntt_plus (lib_ntt_mul zeta p1 p2) (lib_ntt_mul zeta p1 p3) in
...
val lib_ntt_lemma_distr_right:
  ...
  -> p2:lib_poly a n
  -> p3:lib_poly a n
  -> Lemma (lib_ntt_mul zeta (lib_ntt_plus p2 p3) p1 == lib_ntt_plus (lib_ntt_mul zeta p2 p1) (lib_ntt_mul zeta p3 p1))

let lib_ntt_lemma_distr_right #a [| ring a |] #n zeta p1 p2 p3 =
  let p23_1 = lib_ntt_mul zeta (lib_ntt_plus p2 p3) p1 in
  let p21_31 = lib_ntt_plus (lib_ntt_mul zeta p2 p1) (lib_ntt_mul zeta p3 p1) in
...
let lib_ntt_ring #a [| ring a |] #n zeta = (*: (#a:Type0) -> (#[tcresolve ()] r:ring a) -> (#n:size_nat{n%2=0}) -> (zeta:a{forall (x:a). mul x zeta == mul zeta x}) -> ring (lib_poly a n) =
  fun #a #r #n zeta -> *)
    {
      add_ag = lib_ntt_plus_abelian_group;
      mul_m = lib_ntt_mul_monoid #a zeta;
      lemma_distr_left = lib_ntt_lemma_distr_left #a zeta;
      lemma_distr_right = lib_ntt_lemma_distr_right #a zeta;
    }
...
let lib_nttinv_mul #a [| ring a |] #n halfninv zeta zetainv p1 p2 =
  lib_nttinv halfninv zetainv (lib_ntt_mul zeta (lib_ntt zeta p1) (lib_ntt zeta p2))
