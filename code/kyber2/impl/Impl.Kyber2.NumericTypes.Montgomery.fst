module Impl.Kyber2.NumericTypes.Montgomery
...
let to_spec_vec (h:mem) (v:vec) : Ghost (Spec.Kyber2.NumericTypes.vec) (requires True) (ensures fun v' -> forall (i:size_nat{i<params_k}). Spec.Kyber2.NumericTypes.index_vec v' i == Seq.map MGroup.to_t h.[|gsub v (size i *! size params_n) (size params_n)|]) =
  let vec_aux (j:size_nat{j<=params_k}) : Type0 = v':Spec.Kyber2.NumericTypes.vec{forall (k:size_nat{k < j}). Spec.Kyber2.NumericTypes.index_vec v' k == Seq.map MGroup.to_t h.[|gsub v (size k *! size params_n) (size params_n)|]} in
  let rec aux (j:size_nat{j<=params_k}) (acc:vec_aux j) : GTot (vec_aux params_k) (decreases (params_k - j))=
    ...
    let p = Seq.map MGroup.to_t h.[|gsub v (size j *! size params_n) (size params_n)|] in
...
let new_poly () : StackInline (poly) (requires fun h0 -> True) (ensures fun h0 p h1 -> live h1 p /\ stack_allocated p h0 h1 (Seq.create params_n MGroup.zero_m) /\ Seq.map MGroup.to_t h1.[|p|] == Spec.Kyber2.NumericTypes.new_poly ()) =
  assert_norm (MGroup.to_t MGroup.zero_m == Group.zero_t);
  eq_intro (Seq.map MGroup.to_t (Seq.create params_n MGroup.zero_m)) (Spec.Kyber2.NumericTypes.new_poly ());
...
let new_vec () : StackInline (vec) (requires fun h0 -> True) (ensures fun h0 v h1 -> live h1 v /\ to_spec_vec h1 v == Spec.Kyber2.NumericTypes.new_vec ()) =
  ...
  let customlemma (x:size_nat{x < params_k}) : Lemma (customprop x) =
    assert(Spec.Kyber2.NumericTypes.index_vec (to_spec_vec h1 v) x == Seq.map MGroup.to_t h1.[|gsub v (size x *! size params_n) (size params_n)|]);
    as_seq_gsub h1 v (size x *! size params_n) (size params_n);
    Seq.eq_intro (Seq.map MGroup.to_t (h1.[|gsub v (size x *! size params_n) (size params_n)|])) (Spec.Kyber2.NumericTypes.new_poly ())
...
let new_matrix () : StackInline (matrix) (requires fun h0 -> True) (ensures fun h0 m h1 -> live h1 m /\ (forall (x:size_t{v x <params_k}) (y:size_t{v y < params_k}). to_spec_matrix h1 m == (Spec.Kyber2.NumericTypes.new_matrix ()))) =
  ...
  let customlemma (x:size_nat{x < params_k}) (y:b x) : Lemma (customprop x y) =
    ...
    Seq.eq_intro (Seq.map MGroup.to_t h1.[|gsub m (((size x *!size params_k) +. size y) *! size params_n) (size params_n)|]) (Spec.Kyber2.NumericTypes.new_poly ())
  in FStar.Classical.forall_intro_2 customlemma;
...
val get_index_vec:
  ...
  -> Stack (poly)
    ...
    (ensures fun h0 p h1 -> h0 == h1 /\(* Buf.disjoint p buf /\*) p == gsub v (i*!(size params_n)) (size params_n) /\ Seq.map MGroup.to_t h1.[|p|] == Spec.Kyber2.NumericTypes.index_vec (to_spec_vec h0 v) (Lib.IntTypes.v i))

let get_index_vec v i = // #len v buf i =
  Buf.sub #_ #MGroup.montgomery_t #(size params_k *! size params_n) v (i*!(size params_n)) (size params_n)
...
val get_index_vec_lemma:
  ...
  -> Lemma (requires live h0 v /\ modifies1 (gsub v (i*!(size params_n)) (size params_n)) h0 h1 /\ Seq.map MGroup.to_t h1.[|gsub v (i*!(size params_n)) (size params_n)|] == specp)
...
-> Stack unit
  ...
  (ensures fun h0 _ h1 -> modifies1 output h0 h1 /\ Seq.map MGroup.to_t h1.[|output|] == Spec.Kyber2.NumericTypes.index_vec (to_spec_vec h0 v) (Lib.IntTypes.v i))
...
let get_line m i =
  Buf.sub #_ #MGroup.montgomery_t #(size params_k *! size params_k *! size params_n) m (i *! size params_k *! size params_n) (size params_k *! size params_n)
...
val get_element:
  ...
  -> Stack poly
    (requires fun h0 -> live h0 m)
    (ensures fun h0 p h1 -> h0 == h1 /\ p == gsub m (((i*!size params_k) +! j) *! (size params_n)) (size params_n) /\ Seq.map MGroup.to_t h1.[|p|] == Spec.Kyber2.NumericTypes.get_element (to_spec_matrix h0 m) (v i) (v j))
...
let get_element m i j =
  ...
  Buf.sub #_ #MGroup.montgomery_t #(size params_k *! size params_k *! size params_n) m (((i*!size params_k) +! j) *! (size params_n)) (size params_n)
...
val get_element_lemma:
  ...
  -> j:size_t{v j < params_k}
  -> specp:Spec.Kyber2.NumericTypes.poly
  -> Lemma (requires live h0 m /\ modifies1 (gsub m (((i*!size params_k) +! j) *! (size params_n)) (size params_n)) h0 h1 /\ Seq.map MGroup.to_t h1.[|gsub m (((i*!size params_k) +! j) *! (size params_n)) (size params_n)|] == specp) (ensures to_spec_matrix h1 m == Spec.Kyber2.NumericTypes.upd_matrix (to_spec_matrix h0 m) (v i) (v j) specp)
...
let get_element_lemma h0 h1 m i j specp =
  ...
  let customlemma (k:size_nat{k<params_k}) (l: b k) : Lemma (customprop k l) =
    ...
    let p0 = Seq.map MGroup.to_t h0.[|gsub m (((size k *! size params_k) +! size l) *! size params_n) (size params_n)|] in
    let p1 = Seq.map MGroup.to_t h1.[|gsub m (((size k *! size params_k) +! size l) *! size params_n) (size params_n)|] in
    as_seq_gsub h1 m (size k *! size params_k *! size params_n) (size params_k *! size params_n);
...
val copy_element:
  ...
  -> Stack unit
    (requires fun h0 -> live h0 m /\ live h0 output /\ Buf.disjoint m output)
    (ensures fun h0 p h1 -> modifies1 output h0 h1 /\ Seq.map MGroup.to_t h1.[|output|] == Spec.Kyber2.NumericTypes.get_element (to_spec_matrix h0 m) (v i) (v j))
...
val upd_vec:
  ...
  -> Stack unit
    ...
    (ensures fun h0 _ h1 -> modifies1 v h0 h1 /\ to_spec_vec h1 v == Spec.Kyber2.NumericTypes.upd_vec (to_spec_vec h0 v) (Lib.IntTypes.v i) (Seq.map MGroup.to_t h0.[|p|]))

let upd_vec v i p =
  let h0 = ST.get () in
  Buf.copy (get_index_vec v i) p;
  let h1 = ST.get () in
  get_index_vec_lemma h0 h1 v i (Seq.map MGroup.to_t h0.[|p|])
...
val upd_matrix:
  ...
  -> Stack unit
    ...
    (ensures fun h0 _ h1 -> modifies1 m h0 h1 /\ to_spec_matrix h1 m == Spec.Kyber2.NumericTypes.upd_matrix (to_spec_matrix h0 m) (v i) (v j) (Seq.map MGroup.to_t h0.[|p|]))
...
let upd_matrix m i j p =
  ...
  as_seq_gsub h1 m (((i *! size params_k) +! j) *! size params_n) (size params_n);
  eq_intro h1.[|gsub (gsub m (i *! size params_k *! size params_n) (size params_k *! size params_n)) (j *! size params_n) (size params_n)|] h1.[|gsub m (((i *! size params_k) +! j) *! size params_n) (size params_n)|];
  get_element_lemma h0 h1 m i j (Seq.map MGroup.to_t h0.[|p|])
...
val gen_matrix_inner:
  ...
  -> (impl_f: ((k:size_t{v k < params_k}) -> (l:size_t{v l < params_k}) -> (out:poly)
             -> Stack bool (requires fun h -> live h out)
                          ...
                          |None -> b == false | Some p -> (b == true /\ Seq.map MGroup.to_t h1.[|out|] == p)))))
