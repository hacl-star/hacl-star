...
let new_poly () : StackInline (poly) (requires fun h0 -> True) (ensures fun h0 p h1 -> live h1 p /\ stack_allocated p h0 h1 (Seq.create params_n MGroup.zero_m) /\ Seq.map MGroup.int16_to_t h1.[|p|] == Spec.Kyber2.NumericTypes.new_poly ()) =
  ...
  eq_intro (Seq.map MGroup.int16_to_t (Seq.create params_n MGroup.zero_m)) (Spec.Kyber2.NumericTypes.new_poly ());
...
val get_index_vec:
  ...
  -> Stack (poly)
    ...
    (ensures fun h0 p h1 -> h0 == h1 /\(* Buf.disjoint p buf /\*) p == gsub v (i*!(size params_n)) (size params_n) /\ Seq.map MGroup.int16_to_t h1.[|p|] == Spec.Kyber2.NumericTypes.index_vec (to_spec_vec h0 v) (Lib.IntTypes.v i))

let get_index_vec v i = // #len v buf i =
  Buf.sub #_ #int16 #(size params_k *! size params_n) v (i*!(size params_n)) (size params_n)
...
let get_line m i =
  Buf.sub #_ #int16 #(size params_k *! size params_k *! size params_n) m (i *! size params_k *! size params_n) (size params_k *! size params_n)
...
let get_element m i j =
  ...
  Buf.sub #_ #int16 #(size params_k *! size params_k *! size params_n) m (((i*!size params_k) +! j) *! (size params_n)) (size params_n)
