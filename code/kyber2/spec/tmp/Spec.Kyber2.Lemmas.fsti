...
val modulo_plus_minus: a:int -> n:pos -> res:int{a % n = res % n /\ -n/2 <= res /\ res <= n/2 /\ (n%2=0==> -n/2 < res)}


let (%+-) a n = modulo_plus_minus a n

val lemma_mod_plus_minus_injective: n:pos -> a:int -> p:int{p % n = a % n /\ -n/2 <= p /\ p <= n/2 /\ (n%2=0 ==> -n/2 < p)} -> Lemma (p = a %+- n)

val lemma_mod_plus_minus_opposite: n:pos -> a:int -> Lemma((-a)%+-n == -(a%+-n))
...
val modulo_pow2_pub_u32:
  a:pub_uint32 -> b:size_nat{b < 32} -> Lemma
  (uint_v a % pow2 b == uint_v (a &. ((uint #U32 #PUB 1 <<. size b) -. uint #U32 #PUB 1)))
