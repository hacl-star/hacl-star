...
let sum_n_simple_lemma1 #a [|monoid a|] #n l = ()
...
let rec sum_n_simple_lemma2 #a [|monoid a|] #n l =
  if n = 1 then (lemma_id1 #a l.[0]; lemma_id2 #a l.[0])
    ...
    (*calc (==) {
      sum_n l;
	== {sum_n_simple_lemma1 l}
      op l.[0] (sum_n (sub l 1 (n-1)));
	== {sum_n_simple_lemma2 (sub l 1 (n-1)); ()}
      op l.[0] (op (sum_n (sub (sub l 1 (n-1)) 0 (n-2))) (sub l 1 (n-1)).[n-2]);
	== {simpl_seq_sub_sub_lemma l 1 (n-1) 0 (n-2); assert((sub l 1 (n-1)).[n-2] == l.[n-1]); ()}
      op l.[0] (op (sum_n (sub l 1 (n-2))) l.[n-1]);
	== {lemma_assoc l.[0] (sum_n (sub l 1 (n-2))) l.[n-1]; ()}
      op (op l.[0] (sum_n (sub l 1 (n-2)))) l.[n-1];
	== {simpl_seq_sub_sub_lemma l 0 (n-1) 1 (n-2); ()}
      op (op (sub l 0 (n-1)).[0] (sum_n (sub (sub l 0 (n-1)) 1 (n-2)))) l.[n-1]; 
	== {}
      op (sum_n (sub l 0 (n-1))) (l.[n-1]);
    }*)
...
val sum_n_fubini_sublemma: 
  ...
  -> l1: lseq a n1 
  -> l2: lseq b n2 
  -> Lemma(ensures (let m = bm in sum_n (Seq.map (fun x -> sum_n (Seq.map (fun y -> f x y) l2)) l1) == op (sum_n (Seq.map (fun x -> f x l2.[0]) l1)) (sum_n (Seq.map (fun x -> sum_n (Seq.map (fun y -> f x y) (sub l2 1 (n2-1)))) l1)))) (decreases n1)
...
val sum_n_fubini_sublemma_ring: 
  ...
  -> l1: lseq a n1 
  -> l2: lseq b n2 
  -> Lemma(ensures (let m = add_ag.g.m in sum_n (Seq.map (fun x -> sum_n (Seq.map (fun y -> f x y) l2)) l1) == op (sum_n (Seq.map (fun x -> f x l2.[0]) l1)) (sum_n (Seq.map (fun x -> sum_n (Seq.map (fun y -> f x y) (sub l2 1 (n2-1)))) l1)))) (decreases n1)
