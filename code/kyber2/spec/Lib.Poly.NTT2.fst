...
val br: i:pos -> x:nat{x<pow2 i} -> y:nat{y<pow2 i}
let br i x =
  let a (j:nat{j<=i}) = acc:nat{acc<pow2 j} in
  let g (j:nat{j<i}) (acc:a j) : a (j+1) =
    assert(acc+1<=pow2 j);
    lemma_mult_le_left 2 (acc+1) (pow2 j);
    distributivity_add_right 2 acc 1;
    pow2_double_sum j;
    2*acc + ((x / pow2 j)%2)
  in Lib.LoopCombinators.repeat_gen i a g 0

val reorg: #a:Type0 -> #n:size_nat -> i:pos{n = pow2 i} -> p:lib_poly a n -> lib_poly a n
let reorg #a #n i p =
...
instance lib_ntt_mul_monoid : (#a:Type0) -> (#[tcresolve ()] r:ring a) -> (#n:size_nat{n%2=0}) -> (i:pos{pow2 i = n/2}) -> zeta:a{forall (x:a). mul x zeta == mul zeta x} -> monoid (lib_poly a n) =
  fun #a #r #n i zeta ->
    {
      id = lib_ntt_one;
