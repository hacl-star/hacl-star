...
// TODO: Add to FStar.Int.Cast, with Kremlin and OCaml support
...
val little_endian_null: len:nat{len < 16} -> Lemma (little_endian (create len 0uy) == 0)
let rec little_endian_null len =
  if len <> 0 then
    begin
      lemma_eq_intro (slice (create len 0uy) 1 len) (create (len - 1) 0uy);
      assert (little_endian (create len 0uy) == 0 + pow2 8 * little_endian (slice (create len 0uy) 1 len));
      little_endian_null (len - 1)
    end

val little_endian_singleton: n:UInt8.t -> Lemma (little_endian (create 1 n) == UInt8.v n)
...
val little_endian_append: w1:bytes -> w2:bytes -> Lemma
  (ensures
    little_endian (w1 @| w2) == little_endian w1 + pow2 (8 * length w1) * little_endian w2)
  (decreases (length w1))
...
val lemma_big_endian_is_bounded: b:bytes -> Lemma
  (requires True)
  (ensures big_endian b < pow2 (8 * length b))
  (decreases (Seq.length b))
  [SMTPat (big_endian b)]
let rec lemma_big_endian_is_bounded b =
  if Seq.length b <> 0 then
    begin
      let s = slice b 0 (length b - 1) in
      assert(length s = length b - 1);
      lemma_big_endian_is_bounded s;
      assert(UInt8.v (last b) < pow2 8);
      assert(big_endian s < pow2 (8 * length s));
      assert(big_endian b < pow2 8 + pow2 8 * pow2 (8 * (length b - 1)));
      lemma_euclidean_division (UInt8.v (last b)) (big_endian s) (pow2 8);
      assert(big_endian b <= pow2 8 * (big_endian s + 1));
      assert(big_endian b <= pow2 8 * pow2 (8 * (length b - 1)));
      Math.Lemmas.pow2_plus 8 (8 * (length b - 1));
      lemma_factorise 8 (length b - 1)
    end
...
type word = b:seq UInt8.t {length b <= 16}
open FStar.Math.Lib
...
private val lemma_little_endian_is_injective_3: b:word -> b':word -> len:pos{len <= length b /\ len <= length b'} -> Lemma
  (requires 
    ...
    index b (length b - len) = index b' (length b' - len))
...
let lemma_little_endian_inj b b' =
  let len = length b in 
  lemma_eq_intro b (slice b 0  len);
  lemma_eq_intro b' (slice b' 0  len);
