...
let montgomery_ladder_step1 #c p q tempBuffer = 
  ...
  assume ( ~ (pointEqual #c (fromDomainPoint #c #DH (point_as_nat c h2 p)) (fromDomainPoint #c #DH (point_as_nat c h2 q))))
...
val montgomery_ladder: #c: curve -> #buf_type: buftype -> p: point c -> q: point c ->
  ...
  (requires fun h -> live h p /\ live h q /\ live h scalar /\  live h tempBuffer /\
    LowStar.Monotonic.Buffer.all_disjoint [loc p; loc q; loc tempBuffer; loc scalar] /\
    point_eval c h p /\ point_eval c h q /\
    pointEqual #c (fromDomainPoint #c #DH (point_as_nat c h p) (point_mult #c 0 (fromDomainPoint #c #DH (point_as_nat c h q)) /\
    ~ (pointEqual #c (fromDomainPoint #c #DH (point_as_nat c h p)) (fromDomainPoint #c #DH (point_as_nat c h q))))
  (ensures fun h0 _ h1 -> modifies (loc p |+| loc q |+| loc tempBuffer) h0 h1 /\
    point_eval c h1 p /\ point_eval c h1 q /\ (admit();
    let p1 = fromDomainPoint #c #DH (point_as_nat c h1 p) in 
    let q1 = fromDomainPoint #c #DH (point_as_nat c h1 q) in 
    let rN, qN = montgomery_ladder_spec_left #c (as_seq h0 scalar) (
    fromDomainPoint #c #DH (point_as_nat c h0 p),  
    fromDomainPoint #c #DH (point_as_nat c h0 q)) in 
    rN == p1 /\ qN == q1))
