...
val montgomery_multiplication_round_w_k0: #c: curve -> t: widefelem c -> t2: widefelem c -> 
  Stack unit
  (requires fun h -> live h t /\ live h t2)
...
let montgomery_multiplication_round_ #c m t t2 = 
  match m with 
  |DSA ->
    let k0 = Hacl.Bignum.ModInvLimb.mod_inv_limb  (getLastWordOrder #c) in 
  ...
  |DH -> 
    ...
    match r with   
    |true ->
      assert (supportsReducedMultiplication #c);
      assert (v (Hacl.Spec.Bignum.ModInvLimb.mod_inv_limb (getLastWordPrime #c)) == 1);
      Hacl.Spec.Bignum.ModInvLimb.mod_inv_limb_lemma (getLastWordPrime #c);
      if (1 + getPrime c % pow2 64) < pow2 64 then
	small_mod (1 + getPrime c % pow2 64) (pow2 64)
      else
	begin
	  assert(getPrime c % pow2 64 = pow2 64 - 1);
	  assert (v (Hacl.Spec.Bignum.ModInvLimb.mod_inv_limb  (getLastWordPrime #c)) == 1)
	end;
      montgomery_multiplication_round_w_k0 t t2
    |false -> 
      let h0 = ST.get() in 
      let k0 = getK0 c in 
...
val montgomery_multiplication_round: #c: curve -> m: mode -> t: widefelem c 
  ...
  Stack unit 
  (requires fun h -> live h t /\ live h round /\ live h t2 /\ 
    disjoint t t2 /\ disjoint t2 round /\ eq_or_disjoint t round)
...
inline_for_extraction noextract
val montgomery_multiplication_reduction_: #c: curve
  -> m: mode
  -> t: widefelem c 
  -> t2: widefelem c -> 
  Stack unit 
  (requires (fun h -> live h t /\ live h t2 /\ disjoint t t2 /\ (let prime = getModePrime m c in  
    wide_as_nat c h t < prime * pow2 (getPower c))))
  (ensures (fun h0 _ h1 -> modifies (loc t |+| loc t2) h0 h1 /\
    (let prime = getModePrime m c in  
    wide_as_nat c h0 t < prime * pow2 (getPower c) /\ 
    wide_as_nat c h1 t <= wide_as_nat c h0 t / (pow2 (64 * v (getCoordinateLenU64 c))) + prime /\ 
    wide_as_nat c h1 t % prime = wide_as_nat c h0 t * modp_inv2_prime (pow2 (v (getCoordinateLenU64 c) * 64)) prime % prime)))

let montgomery_multiplication_reduction_ #c m t t2 = 
    let h0 = ST.get() in 
    let len = getCoordinateLenU64 c in 
    
    let inv h (i: nat { i <= uint_v (getCoordinateLenU64 c)}) = 
      let prime = getModePrime m c in  
      live h t /\ live h t2 /\ modifies (loc t |+| loc t2) h0 h /\
      wide_as_nat c h0 t < prime * pow2 (getPower c) /\ 
...
let montgomery_multiplication_reduction #c m t result = 
  ...
  push_frame();
    ...
    montgomery_multiplication_reduction_ #c m t t2;
  pop_frame(); 
    let h2 = ST.get() in 
...
let fsquarePowN_dh #c n a = 
  let h0 = ST.get() in 
    lemmaFromDomainToDomain #c #DH (as_nat c h0 a);  
  assert_norm (pow2 0 == 1); 
  let inv (h0: HyperStack.mem) (h1: HyperStack.mem) (i: nat) : Type0 = 
    let k = fromDomain_ #c #DH (as_nat c h0 a) in 
    as_nat c h1 a = toDomain_ #c #DH (pow k (pow2 i)) /\
    as_nat c h1 a < getModePrime DH c /\ live h1 a /\ modifies (loc a) h0 h1 in 

  Hacl.EC.Lemmas.power_one_2 (fromDomain_ #c #DH (as_nat c h0 a)); 
  
  for (size 0) n (inv h0) (fun x -> 
    let h0_ = ST.get() in 
     montgomery_square_buffer_dh a a;
     let k = fromDomain_ #c #DH (as_nat c h0 a) in  
     inDomain_mod_is_not_mod #c #DH (fromDomain_ #c #DH (as_nat c h0_ a) * fromDomain_ #c #DH (as_nat c h0_ a)); 
     lemmaFromDomainToDomainModuloPrime #c #DH (let k = fromDomain_ #c #DH (as_nat c h0 a) in pow k (pow2 (v x)));

  Spec.ECDSA.Lemmas.modulo_distributivity_mult (pow k (pow2 (v x))) (pow k (pow2 (v x))) (getModePrime DH c); 
  pow_plus k (pow2 (v x)) (pow2 (v x )); 
  pow2_double_sum (v x); 
  inDomain_mod_is_not_mod #c #DH (pow k (pow2 (v x + 1)))); 
  
  inDomain_mod_is_not_mod #c #DH (let k = fromDomain_ #c #DH (as_nat c h0 a) in pow k (pow2 (v n)))
