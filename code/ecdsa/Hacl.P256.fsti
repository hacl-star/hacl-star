...
val ecdsa_sign_p256_sha2: result: lbuffer uint8 (size 64) 
  ...
  (requires fun h -> 
    ...
    nat_from_bytes_be (as_seq h k) < getOrder #P256 /\ nat_from_bytes_be (as_seq h k) > 0)
  (ensures fun h0 flag h1 -> 
    modifies (loc result) h0 h1 /\ (assert_norm (pow2 32 < pow2 61);
...
val ecdsa_sign_p256_sha384: result: lbuffer uint8 (size 64) -> mLen: size_t -> m: lbuffer uint8 mLen ->
  ...
  (requires fun h -> 
    live h result /\ live h m /\ live h privKey /\ live h k /\
    disjoint result m /\ disjoint result privKey /\ disjoint result k /\
    nat_from_bytes_be (as_seq h privKey) < getOrder #P256 /\
    nat_from_bytes_be (as_seq h k) < getOrder #P256 /\ nat_from_bytes_be (as_seq h k) > 0)
  (ensures fun h0 flag h1 -> modifies (loc result) h0 h1 /\ (assert_norm (pow2 32 < pow2 61);
      let resultR = gsub result (size 0) (size 32) in 
      let resultS = gsub result (size 32) (size 32) in 
...
val ecdsa_sign_p256_without_hash: result: lbuffer uint8 (size 64) 
  -> mLen: size_t {uint_v mLen >= Spec.ECDSA.min_input_length #P256 Spec.ECDSA.NoHash /\ v mLen < pow2 32 - 256}
  -> m: lbuffer uint8 mLen
  -> privKey: lbuffer uint8 (size 32) 
  -> k: lbuffer uint8 (size 32) -> 
  Stack bool
...
val ecdsa_verif_p256_sha2:
  ...
  -> s: lbuffer uint8 (size 32) ->
   Stack bool
    ...
    nat_from_bytes_be (as_seq h r) < getOrder #P256 /\ nat_from_bytes_be (as_seq h r) > 0 /\
    nat_from_bytes_be (as_seq h s) < getOrder #P256 /\ nat_from_bytes_be (as_seq h s) > 0 /\ (
    let pubKeyX = gsub pubKey (size 0) 32ul in
    let pubKeyY = gsub pubKey 32ul 32ul in 
    nat_from_bytes_be (as_seq h pubKeyX) < getPrime P256 /\ 
    nat_from_bytes_be (as_seq h pubKeyY) < getPrime P256 /\
...
val verify_q_public: 
  ...
  (requires fun h -> live h pubKey /\ (
    let pointScalarXSeq = nat_from_bytes_be (as_seq h (getXAff8 #P256 pubKey)) in
    let pointScalarYSeq = nat_from_bytes_be (as_seq h (getYAff8 #P256 pubKey)) in 
...
val ecp256dh_i_ml:
  ...
  -> Stack bool
  (requires fun h ->
    live h result /\ live h scalar /\ disjoint result scalar /\ scalar_as_nat #P256 (as_seq h scalar) < getOrder #P256)
  (ensures fun h0 success h1 -> modifies (loc result) h0 h1 /\  (
    let rX = nat_from_bytes_be (as_seq h1 (getXAff8 #P256 result)) in 
    let rY = nat_from_bytes_be (as_seq h1 (getYAff8 #P256 result)) in
    let p, flag = ecp256_dh_i #P256 (as_seq h0 scalar) in 
...
val ecp384dh_i_ml:
  ...
  -> Stack bool
  (requires fun h ->
    live h result /\ live h scalar /\ disjoint result scalar /\ scalar_as_nat #P384 (as_seq h scalar) < getOrder #P384)
  (ensures fun h0 success h1 -> modifies (loc result) h0 h1 /\  (
    let rX = nat_from_bytes_be (as_seq h1 (getXAff8 #P384 result)) in 
    let rY = nat_from_bytes_be (as_seq h1 (getYAff8 #P384 result)) in
    let p, flag = ecp256_dh_i #P384 (as_seq h0 scalar) in 
...
val ecp256dh_r_public_ml:
  ...
  (requires fun h -> live h result /\ live h pubKey /\ live h scalar /\ disjoint result pubKey /\ disjoint result scalar /\
    scalar_as_nat #P256 (as_seq h scalar) < getOrder #P256  /\ (
    let pointScalarXSeq = nat_from_bytes_be (as_seq h (getXAff8 #P256 pubKey)) in
    let pointScalarYSeq = nat_from_bytes_be (as_seq h (getYAff8 #P256 pubKey)) in 
      pointScalarXSeq <> 0 /\  pointScalarYSeq <> 0))
  (ensures fun h0 success h1 -> modifies (loc result) h0 h1 /\  (
    let pk = nat_from_bytes_be (as_seq h0 (getXAff8 #P256 pubKey)), nat_from_bytes_be (as_seq h0 (getYAff8 #P256 pubKey)) in 
    let coordinateX = nat_from_bytes_be (as_seq h1 (getXAff8 #P256 result)) in 
    let coordinateY = nat_from_bytes_be (as_seq h1 (getYAff8 #P256 result)) in
    coordinateX < getPrime P256 /\ coordinateY < getPrime P256 /\ (
    let p, flag = ecp256_dh_r #P256 pk (as_seq h0 scalar) in 
    flag == success /\ success ==> (coordinateX, coordinateY) == p)))
...
val ecp384dh_r_public_ml:
  ...
  -> Stack bool
  (requires fun h -> live h result /\ live h pubKey /\ live h scalar /\ disjoint result pubKey /\ disjoint result scalar /\
    scalar_as_nat #P384 (as_seq h scalar) < getOrder #P384 /\ (
    let pointScalarXSeq = nat_from_bytes_be (as_seq h (getXAff8 #P384 pubKey)) in
    let pointScalarYSeq = nat_from_bytes_be (as_seq h (getYAff8 #P384 pubKey)) in 
      pointScalarXSeq <> 0 /\  pointScalarYSeq <> 0))
  (ensures fun h0 success h1 -> modifies (loc result) h0 h1 /\  (
    let pk = nat_from_bytes_be (as_seq h0 (getXAff8 #P384 pubKey)), nat_from_bytes_be (as_seq h0 (getYAff8 #P384 pubKey)) in 
    let coordinateX = nat_from_bytes_be (as_seq h1 (getXAff8 #P384 result)) in 
    let coordinateY = nat_from_bytes_be (as_seq h1 (getYAff8 #P384 result)) in
    coordinateX < getPrime P384 /\ coordinateY < getPrime P384 /\ (
    let p, flag = ecp256_dh_r #P384 pk (as_seq h0 scalar) in 
    flag == success /\ success ==> (coordinateX, coordinateY) == p)))
