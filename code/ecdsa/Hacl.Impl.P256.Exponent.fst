...
let fromDomain_ = fromDomain_ #P256 #DH
let toDomain_ = toDomain_ #P256 #DH

let as_nat h1 result = as_nat P256 h1 result
...
let exponent_0 t t0 t1 t2 t6 t7 = 
  ...
  calc (==) {
      as_nat h3 t0;
      (==) {}
      toDomain_ (fromDomain_ (as_nat h2 t2) * fromDomain_ (as_nat h2 t2) % prime256);
      (==) {}
      toDomain_ ((tD * tD % prime256 * tD % prime256) * (tD * tD % prime256 * tD % prime256) % prime256); 
      (==) {lemma_6_powers tD} 
      toDomain_ (pow tD 6 % prime256);};

  calc (==) {
    as_nat h5 t6;
    (==) {}
    toDomain_ (((pow tD 6 % prime256) * (pow tD 6 % prime256) % prime256) * (tD * tD % prime256 * tD % prime256) % prime256); 
    (==) {lemma_15_powers tD} 
    toDomain_ (pow tD 15 % prime256); };

  calc (==) {
    as_nat h6 t0;
    (==) {}
    toDomain_ (pow tD 15 % prime256 * (pow tD 15 % prime256) % prime256);};

    calc (==) {
      as_nat h7 t0; 
      (==) {}
      toDomain_ (pow (fromDomain_ (as_nat h6 t0)) 8  % prime256); 
      (==) {}
      toDomain_ (pow ((pow tD 15 % prime256 * (pow tD 15 % prime256) % prime256)) 8 % prime256); 
      (==) {lemma_pow_sum tD 15 15} 
      toDomain_ (pow ((pow tD 30 % prime256)) 8 % prime256);  
      (==) {power_distributivity (pow tD 30) 8 prime256}
      toDomain_ (pow (pow tD 30) 8 % prime256);  
      (==) {power_mult tD 30 8}
      toDomain_ (pow tD 240 % prime256);};
...
let exponent_p256 t result tempBuffer = 
  ...
  let tD = fromDomain #P256 (as_nat h0 t) in 
