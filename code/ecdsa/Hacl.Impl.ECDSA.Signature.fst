...
val lemma_norm_of_equal_points: #c: curve -> p: Spec.ECC.point #c
  -> a: pos {a < getOrder #c} -> q: Spec.ECC.point #c -> Lemma
  (requires (pointEqual p q /\ pointEqual q (point_mult a (basePoint #c))))
  (ensures (_norm p == _norm q))

let lemma_norm_of_equal_points #c p a q = 
  let pAffineX, pAffineY, pAffineZ = _norm p in 
  let qAffineX, qAffineY, qAffineZ = _norm q in 

  if (isPointAtInfinity #Jacobian (pAffineX, pAffineY, pAffineZ) && isPointAtInfinity #Jacobian (qAffineX, qAffineY, qAffineZ))
    then 
      begin
...
val ecdsa_signature_step45: #c: curve -> #l: ladder 
  ...
  (requires fun h -> live h x /\ live h k /\ live h tempBuffer /\
    scalar_as_nat (as_seq h k) < getOrder #c /\ scalar_as_nat (as_seq h k) > 0 /\
  ...
  (ensures fun h0 r h1 -> modifies (loc x |+| loc tempBuffer) h0 h1 /\ 
    ...
    let (rxN, ryN, rzN) = secret_to_public (as_seq h0 k) in 
...
let ecdsa_signature_step45 #c #l x k tempBuffer = 
  push_frame();
    ...
    normX #c result x tempForNorm;
      ...
      lemma_norm_of_equal_points #c (fromDomainPoint #c #DH (point_as_nat c h1 result)) (scalar_as_nat (as_seq h0 k)) (point_mult #c (scalar_as_nat (as_seq h0 k)) (basePoint #c));
...
val lemma_scalar_is_nat: #c: curve -> k: scalar_bytes #c -> i: nat {i > 0 /\ i <= v (getScalarLenBytes c)} -> 
  Lemma (let open Lib.Sequence in scalar_as_nat_ k (i * 8) == pow2 8 * scalar_as_nat_ k (8 * i - 8) + v k.[i - 1])

let lemma_scalar_is_nat #c k i = 
  Hacl.Impl.EC.Masking.ScalarAccess.Lemmas.lemma_index_scalar_as_nat_ k (i - 1);
  Hacl.Impl.EC.Masking.ScalarAccess.Lemmas.lemma_index_as_ith k (i - 1)
    

val lemma_scalar_is_nat_is_big_endian_: #c: curve -> k: scalar_bytes #c -> i: nat {i <= v (getScalarLenBytes c)} -> Lemma (
  nat_from_intseq_be (Seq.slice k 0 i) == scalar_as_nat_ k (i * 8))

let rec lemma_scalar_is_nat_is_big_endian_ #c k i = 
    let len = v (getScalarLenBytes c) in 
  let open Lib.Sequence in 
  match i with 
  | 0 -> 
    scalar_as_nat_zero k
  | 1 -> 
    nat_from_intseq_be_lemma0 (slice k 0 1);
    assert(nat_from_intseq_be (Seq.slice k 0 i) == v k.[0]);
    Hacl.Impl.EC.Masking.ScalarAccess.Lemmas.lemma_index_as_ith k 0;
    scalar_as_nat_def k (i * 8);
    Hacl.Impl.EC.Masking.ScalarAccess.Lemmas.lemma_index_scalar_as_nat_ k (i - 1);

    scalar_as_nat_zero k;
    assert(scalar_as_nat_ k (8 * i) ==  v k.[0])
    
  | _ -> 
    lemma_scalar_is_nat_is_big_endian_ #c k (i - 1);
    scalar_as_nat_def k (v (getScalarLen c));
    lemma_scalar_is_nat k i;
    assert(scalar_as_nat_ k (i * 8) == pow2 8 * scalar_as_nat_ k (8 * i - 8) + v k.[i - 1]);

    let b = (Seq.slice k 0 i) in 
    let i0 = i - 1 in 
    nat_from_intseq_be_slice_lemma #_ #_ #i b i0;
    nat_from_intseq_be_lemma0 (Seq.slice b (i - 1) i)


val lemma_scalar_is_nat_is_big_endian: #c: curve -> k: scalar_bytes #c-> Lemma (
  nat_from_intseq_be  k == scalar_as_nat k)

let lemma_scalar_is_nat_is_big_endian #c k = lemma_scalar_is_nat_is_big_endian_ k (v (getScalarLenBytes c))
...
val ecdsa_signature_core: #c: curve -> #l: ladder -> alg: hash_alg_ecdsa
  ...
  (ensures fun h0 flag h1 -> modifies (loc r |+| loc s) h0 h1 /\ (
    ...
    let baseMultK = scalar_multiplication (as_seq h0 k) (basePoint #c) in 
...
let ecdsa_signature_core #c #l alg r s mLen m privKeyAsFelem k = 
  ...
  let step5Flag = ecdsa_signature_step45 #c #l r k tempBuffer in 
    let h1 = ST.get() in 
    Hacl.Impl.EC.Intro.lemma_lseq_nat_from_bytes (as_seq h1 kAsFelem);
    lemma_scalar_is_nat_is_big_endian (as_seq h0 k);
