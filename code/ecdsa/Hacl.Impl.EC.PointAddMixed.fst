...
val pointAffineIsNotZero: #c: curve -> p: pointAffine c -> Stack uint64
  ...
  (ensures fun h0 r h1 -> modifies0 h0 h1 /\ (v r == 0 \/ v r == pow2 64 - 1) /\ (
    v r == pow2 64 - 1 <==> isPointAtInfinity (toJacobianCoordinates (point_affine_as_nat c h0 p))))
...
val copy_point_conditional1_: #c: curve -> x3_out: felem c -> y3_out: felem c -> z3_out: felem c -> p: point c 
  ...
  (ensures fun h0 _ h1 -> modifies (loc x3_out |+| loc y3_out |+| loc z3_out) h0 h1 /\
    felem_eval c h1 x3_out /\ felem_eval c h1 y3_out /\ felem_eval c h1 z3_out /\ (
    let len = getCoordinateLenU64 c in 
    if isPointAtInfinity #Jacobian (toJacobianCoordinates (point_affine_as_nat c h0 q)) then
      as_nat c h1 x3_out == point_x_as_nat c h0 p /\
      as_nat c h1 y3_out == point_y_as_nat c h0 p /\ 
      as_nat c h1 z3_out == point_z_as_nat c h0 p
    else 
      as_nat c h1 x3_out == as_nat c h0 x3_out /\ 
      as_nat c h1 y3_out == as_nat c h0 y3_out /\ 
      as_nat c h1 z3_out == as_nat c h0 z3_out))
...
val copy_result_point_add: #c: curve 
  ...
  (ensures fun h0 _ h1 -> 
    ...
    if isPointAtInfinity (toJacobianCoordinates (point_affine_as_nat c h0 q)) then
    ...
    else if isPointAtInfinity (point_as_nat c h0 p) then 
...
val point_affine_not_infinity_domain: #c: curve -> p: point_affine_nat_prime #c -> Lemma (
  ~ (isPointAtInfinity #Affine p) == ~ (isPointAtInfinity (fromDomainPointAffine p)))

let point_affine_not_infinity_domain #c p = 
    let x, y = p in 
    let xD, yD = fromDomainPointAffine p in 

    assert(xD == fromDomain #c x);
    assert(yD == fromDomain #c y);
    
    lemmaFromDomain #c #DH x;
    Hacl.Impl.EC.Math.lemma_multiplication_not_mod_prime #c x;

    lemmaFromDomain #c #DH y;
    Hacl.Impl.EC.Math.lemma_multiplication_not_mod_prime #c y
...
val computeXYZ: #c: curve -> result: point c 
  ...
  (ensures fun h0 _ h1 -> modifies (loc t5 |+| loc result) h0 h1 /\ point_eval c h1 result /\ (
    ...
    let qX, qY, qZ = toJacobianCoordinates (point_affine_as_nat c h0 q) in

    let pxD, pyD, pzD = fromDomainPoint #c #DH (point_as_nat c h0 p) in 
    let qxD, qyD, qzD = fromDomainPoint #c #DH (toJacobianCoordinates (point_affine_as_nat c h0 q)) in 
    ...
    let u1D = fromDomain #c (as_nat c h0 u1) in  
     ...
     if (isPointAtInfinity #Affine (point_affine_as_nat c h0 q)) then
      x3D == pxD /\ y3D == pyD /\ z3D == pzD
    else if isPointAtInfinity (fromDomainPoint #c #DH (point_as_nat c h0 p)) then 
...
let computeXYZ #c result p q u1 u2 s1 s2 r h uh hCube t5 = 
  ...
  point_affine_not_infinity_domain (point_affine_as_nat c h0 q);

  assert(
     let prime = getPrime c in 

    let x3, y3, z3 = point_x_as_nat c h3 result, point_y_as_nat c h3 result, point_z_as_nat c h3 result in
    let pX, pY, pZ = point_as_nat c h0 p in 
    let qX, qY, qZ = toJacobianCoordinates (point_affine_as_nat c h0 q) in
...
val lemma_point_infinity_affine_jacobian: #c: curve -> p: point_affine_nat_prime #c -> Lemma (
  let q = toJacobianCoordinates p in 
  let qD = fromDomainPoint #c #DH q in 
  isPointAtInfinity #Affine p == isPointAtInfinity #Jacobian qD)

let lemma_point_infinity_affine_jacobian #c p = 
    let (x, y, z) = (toJacobianCoordinates p) in 
    lemma_pointAtInfInDomain #c x y z


inline_for_extraction noextract
val _point_add_if_second_branch_impl_: #c: curve -> result: point c 
  -> p: point c -> q: pointAffine c 
  ...
  (ensures fun h0 _ h1 -> modifies (loc t5 |+| loc result) h0 h1 /\ point_eval c h1 result /\ (
    ...
    let qxD, qyD, qzD = fromDomainPoint #c #DH (toJacobianCoordinates (point_affine_as_nat c h0 q)) in 
    let x3D, y3D, z3D = fromDomainPoint #c #DH (x3, y3, z3) in 
    ...
    if isPointAtInfinity #Affine (point_affine_as_nat c h0 q) then
      x3D == pxD /\ y3D == pyD /\ z3D == pzD
    else if isPointAtInfinity (fromDomainPoint #c #DH p) then 
      x3D == qxD /\  y3D == qyD /\ z3D == qzD
    else 
      x3 == toDomain #c ((rD * rD - hD * hD * hD - 2 * hD * hD * u1D) % prime) /\ 
      y3 == toDomain #c (((hD * hD * u1D - fromDomain  #c x3) * rD - s1D * hD * hD * hD) % prime) /\
...
let _point_add_if_second_branch_impl_ #c result p q0 x3hCube t5 = 
  [@inline_let]
  let len = getCoordinateLenU64 c in 
  let h0 = ST.get() in 
  ...
  let h1 = ST.get() in 
  assert(
    eq_or_disjoint p q0 /\ eq_or_disjoint s1 hCube /\ 
    eq_or_disjoint p result /\ disjoint result t5 /\ disjoint p q0 /\ disjoint hCube t5 /\ disjoint uh t5 /\ disjoint r t5 /\ 
    disjoint t5 s1 /\ disjoint t5 h /\ disjoint t5 p /\ disjoint t5 q0);
    
  computeXYZ result p q0 u1 u2 s1 s2 r h uh hCube t5
...
val _point_add_if_second_branch_impl: #c: curve -> result: point c 
  ...
  (ensures fun h0 _ h1 -> modifies (loc t5 |+| loc result) h0 h1 /\ point_eval c h1 result /\ (
    ...
    if isPointAtInfinity (fromDomainPoint #c #DH (toJacobianCoordinates (point_affine_as_nat c h0 q))) then
      x3D == pxD /\ y3D == pyD /\ z3D == pzD
    else if isPointAtInfinity (fromDomainPoint #c #DH p) then 
      x3D == qxD /\  y3D == qyD /\ z3D == qzD
    else 
...
let _point_add_if_second_branch_impl #c result p q x3hCube t5 = 
  let h0 = ST.get() in 
  _point_add_if_second_branch_impl_ result p q x3hCube t5;
  lemma_point_infinity_affine_jacobian #c (point_affine_as_nat c h0 q)
...
val point_add_mixed_lemmas: pxD: nat -> pyD: nat -> pzD: nat -> qxD: nat -> qyD: nat -> qzD : nat -> prime : pos -> 
  Lemma (
    qzD * qzD * pxD % prime == pxD * (qzD * qzD) % prime /\
    pzD * pzD * qxD % prime == qxD * (pzD * pzD) % prime /\
    qzD * qzD * qzD * pyD % prime == pyD * qzD * (qzD * qzD) % prime /\
    pzD * pzD * pzD * qyD % prime == qyD * pzD * (pzD * pzD) % prime)

let point_add_mixed_lemmas pxD pyD pzD qxD qyD qzD prime = 
  calc (==)
  {
    qzD * qzD * pxD % prime;
    (==) {_ by (canon())}
...
val affine_to_jacobian_z_is_1: #c: curve -> q: pointAffine c -> h0: mem {point_aff_eval c h0 q} ->
  Lemma (
    let q = toJacobianCoordinates (point_affine_as_nat c h0 q) in 
    let qD = fromDomainPoint #c #DH q in 
    let _, _, qZ = q in isPointAtInfinity #Jacobian qD = false ==> qZ == 1)

let affine_to_jacobian_z_is_1 #c q h0 = 
  point_affine_not_infinity_domain #c (point_affine_as_nat c h0 q)


val point_add_mixed_to_spec: #c: curve -> p: point c -> q: pointAffine c -> result: point c -> h0: mem -> h1: mem -> h2: mem
  -> t12: lbuffer uint64 (size 12 *! getCoordinateLenU64 c) -> Lemma
  (requires (
    let u1, u2, s1, s2, h, r, uh, hCube = getU1HCube t12 in  
    let prime = getPrime c in
    u1Invariant #c h0 h1 u1 p /\ u2Invariant #c h0 h1 u2 p q /\ 
    ...
    let rD = fromDomain #c (as_nat c h1 r) in  
    let hD = fromDomain #c (as_nat c h1 h) in 
    let s1D = fromDomain #c (as_nat c h1 s1) in 
    let u1D = fromDomain #c (as_nat c h1 u1) in  
    
    if isPointAtInfinity (fromDomainPoint #c #DH q)  then
      x3D == pxD /\ y3D == pyD /\ z3D == pzD
    else if isPointAtInfinity (fromDomainPoint #c #DH p) then 
      x3D == qxD /\  y3D == qyD /\ z3D == qzD
    else 
  ...
  (ensures (
    ...
    fromDomainPoint #c #DH (point_as_nat c h2 result) == _point_add #c  pD qD))

let point_add_mixed_to_spec #c p q0 result h0 h1 h2 t12 = 
  let prime = getPrime c in
  let u1, u2, s1, s2, h, r, uh, hCube = getU1HCube t12 in  

  let p = point_as_nat c h0 p in 
  let q = toJacobianCoordinates (point_affine_as_nat c h0 q0) in 
  
  let pD = fromDomainPoint #c #DH p in 
  let qD = fromDomainPoint #c #DH q in 
  ...
  if (isPointAtInfinity #Jacobian qD = false) && (isPointAtInfinity #Jacobian pD = false) then 
    begin
      affine_to_jacobian_z_is_1 q0 h0
    end
...
let point_add_mixed_ #c p q result tempBuffer = 
  ...
  let h2 = ST.get() in 
  point_add_mixed_to_spec #c p q result h0 h1 h2 t12
