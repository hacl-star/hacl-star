...
let scalar_multiplication_t_0 #c #t #l p result scalar tempBuffer = 
  ...
  |WNAF -> 
    (* here we do not support calculation of the scalar multiplication by WNAF *)
    (* we fall on the Radix case *)
    pointToDomain #c p result;
    Hacl.Impl.EC.ScalarMult.Radix.scalar_mult_radix #c result scalar tempBuffer
...
let norm_twice_lemma #c p q =
  ...
  assert(pointEqual p (_norm #c p));
  assert(pointEqual #c p q ==> pointEqual (_norm #c p) q)
...
let scalarMultiplicationWithoutNorm #c #l = 
  ...
  |P256 -> begin
    ...
    (* we do not support WNAF for scalar multiplication, so we fall on the Radix algorithm *)
    |WNAF -> scalarMultiplicationWithoutNorm_p256_radix
  end
  |P384 -> 
    begin
      match l with 
      ...
      (* we do not support WNAF for scalar multiplication, so we fall on the Radix algorithm *)
      |WNAF -> scalarMultiplicationWithoutNorm_p384_radix
    end
...
let secretToPublic #c #l result scalar tempBuffer = 
  ...
  |WNAF -> 
    Hacl.Impl.EC.ScalarMultiplication.WNAF.scalar_multiplication_cmb #c q scalar buff;
      let h1 = ST.get() in 
    norm q result buff;
    norm_twice_lemma #c (fromDomainPoint #c #DH (point_as_nat c h1 q)) (point_mult #c (scalar_as_nat #c (as_seq h0 scalar)) (basePoint #c))
...
let secretToPublicWithoutNorm_ #c #l result scalar tempBuffer = 
  ...
  |WNAF -> 
    Hacl.Impl.EC.ScalarMultiplication.WNAF.scalar_multiplication_cmb #c result scalar buff
...
[@CInline]
let secretToPublicWithoutNorm_p256_wnaf = secretToPublicWithoutNorm_ #P256 #WNAF
[@CInline]
let secretToPublicWithoutNorm_p384_wnaf = secretToPublicWithoutNorm_ #P384 #WNAF
...
let secretToPublicWithoutNorm #c #l = 
  ...
  |P256 -> begin
    ...
    |WNAF -> secretToPublicWithoutNorm_p256_wnaf
  end
  |P384 -> 
    begin
      match l with 
      ...
      |WNAF -> secretToPublicWithoutNorm_p384_wnaf
    end
