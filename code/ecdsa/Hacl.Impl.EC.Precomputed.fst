...
let getPointPrecomputedMixed_ #c scalar i pointToAdd = 
  ...
  let invK h (k: nat) = live h pointToAdd /\ modifies (loc pointToAdd) h0 h /\ (
      ...
      if k > v bits then
	begin
	  as_nat c h pointToAddX < getPrime c /\ as_nat c h pointToAddY < getPrime c /\ (
...
inline_for_extraction noextract
val getPointPrecomputedMixed: #c: curve -> #buf_type: buftype
  -> scalar: lbuffer_t buf_type (uint_t U8 SEC) (getScalarLenBytes c)
  -> i:size_t {v i < v (getScalarLenBytes c) * 2} -> pointToAdd: pointAffine c ->
  Stack unit 
  (requires fun h -> live h scalar /\ live h pointToAdd)
  (ensures fun h0 _ h1 -> modifies (loc pointToAdd) h0 h1 /\ point_aff_eval c h1 pointToAdd /\ (
...
let getPointPrecomputedMixed #c #buf_type  scalar i pointToAdd = getPointPrecomputedMixed_ #c #buf_type scalar i pointToAdd
