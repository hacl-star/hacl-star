...
(* First bit is 1 *)
inline_for_extraction let mpfr_d_valn_cond (m:mp_limb_t): GTot Type0 =
    v m >= pow2 63

(* Ending bits are 0 *)
inline_for_extraction let mpfr_d_val0_cond (m:mp_limb_t) (p:mpfr_prec_t{U32.v p > 0}): GTot Type0 =
    v m % pow2 (prec_to_len (U32.v p) - U32.v p) = 0

inline_for_extraction let normal_cond_ h (s:mpfr_struct): GTot Type0 =
...
inline_for_extraction let normal_cond h (a:mpfr_ptr): GTot Type0 = normal_cond_ h (Seq.index (as_seq h a) 0)

inline_for_extraction let mpfr_reg_cond_ h (s:mpfr_struct): GTot Type0 =
    normal_cond_ h s /\
    mpfr_PREC_COND (U32.v s.mpfr_prec) /\ mpfr_EXP_COND (I32.v s.mpfr_exp)

inline_for_extraction let mpfr_reg_cond h (a:mpfr_ptr): GTot Type0 = mpfr_reg_cond_ h (Seq.index (as_seq h a) 0)

inline_for_extraction let mpfr_valid_cond_ h (s:mpfr_struct): GTot Type0 =
...
(* set special value *)
inline_for_extraction val mpfr_setmax_rec: x:mpfr_ptr -> i:u32 -> Stack unit
    (requires (fun h -> 
