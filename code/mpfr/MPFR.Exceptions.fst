...
val mpfr_overflow: x:mpfr_ptr -> rnd_mode:mpfr_rnd_t -> sign:mpfr_sign_t ->
    Stack i32
    (requires (fun h -> let p = (as_struct h x).mpfr_prec in
	...
	I32.v f = mpfr_overflow_ternary_spec exact (U32.v p) rnd_mode)))

let mpfr_overflow x rnd_mode sign =
    mpfr_SET_SIGN x sign;
    if mpfr_IS_LIKE_RNDZ rnd_mode (sign = mpfr_SIGN_NEG) then begin
	mpfr_setmax x;
	mpfr_NEG_SIGN sign
    end else begin
        mpfr_SET_INF x;
