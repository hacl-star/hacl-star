...
let (rnd_mode : MPFR_Lib.mpfr_rnd_t) = MPFR_Lib.MPFR_RNDN
...
fun str ->
  ...
  while true do
    ...
    if (String.equal (List.nth tokens 5) "MPFR_RNDN") then begin
    ...
    let af = MPFR_Add1sp1.mpfr_add1sp1 b b c rnd_mode p  in
    let asign = MPFR_Lib.mpfr_SIGN b  in
    let am = MPFR_Lib.mpfr_MANT b  in
    let ap =
      FStar_Buffer.op_Array_Access am
        (FStar_UInt32.uint_to_t (Prims.parse_int "0"))
       in
    let ax = MPFR_Lib.mpfr_GET_EXP b  in
