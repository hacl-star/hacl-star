...
let (print_i32 : FStar_Int32.t -> unit) =
  fun x  ->
    FStar_HyperStack_IO.print_string
      (Prims.string_of_int (cmod (FStar_Int32.v x)));
    FStar_HyperStack_IO.print_string " "
...
fun str ->
  ...
  while true do
    ...
    let bsgn = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 0)) in
    ...
    let csgn = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 3)) in
    let cp   = FStar_UInt64.uint_to_t (Prims.parse_int (List.nth tokens 4)) in
    let cx   = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 5)) in
    let p    = FStar_UInt32.uint_to_t (Prims.parse_int (List.nth tokens 7)) in
    let rsgn = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 8)) in
    let rp   = FStar_UInt64.uint_to_t (Prims.parse_int (List.nth tokens 9)) in
    let rx   = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 10)) in
    let flag = FStar_Int32.int_to_t   (Prims.parse_int (List.nth tokens 11)) in
    let bm = FStar_Buffer.createL [bp]  in
    let bs = MPFR_Lib.mk_mpfr_struct p bsgn bx bm  in
    let b = FStar_Buffer.createL [bs]  in
    let cm = FStar_Buffer.createL [cp]  in
    let cs = MPFR_Lib.mk_mpfr_struct p csgn cx cm  in
    let c = FStar_Buffer.createL [cs]  in
    let af = MPFR_Mul_1.mpfr_mul_1 c b c rnd_mode p  in
    let asgn = MPFR_Lib.mpfr_SIGN c  in
    ...
    print_i32 ax;
    print_i32 af;
    if (((FStar_UInt64.eq ap rp) && (FStar_Int32.eq ax rx) || (FStar_Int32.eq ax rx) && (FStar_Int32.eq ax MPFR_Lib.mpfr_EXP_INF) || (FStar_Int32.eq ax rx) && (FStar_Int32.eq ax MPFR_Lib.mpfr_EXP_ZERO)) && (FStar_Int32.eq asgn rsgn) && (FStar_Int32.eq af flag)) then
        print_string "YES!!!\n"
    else begin
        raise Exit;
        print_string "no...\n"
