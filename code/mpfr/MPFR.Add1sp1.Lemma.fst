...
val mpfr_round_spec_lemma:
    high1:normal_fp ->
    high2:normal_fp{high1.prec = high2.prec /\ eval high1 =. eval high2} ->
    rb:bool -> sb:bool -> rnd_mode:mpfr_rnd_t -> Lemma
    (eval (mpfr_add1sp1_round_spec high1 rb sb rnd_mode) =.
     eval (mpfr_add1sp1_round_spec high2 rb sb rnd_mode))

let mpfr_round_spec_lemma high1 high2 rb sb rnd_mode =
    eval_eq_reveal_lemma high1 high2;
    eval_eq_intro_lemma (add_one_ulp high1) (add_one_ulp high2)

val mpfr_round_rb_sb_lemma_: a:normal_fp -> p:pos{p < a.prec} ->
    high:normal_fp{high == high_mant a p} ->
    rb:bool{rb = rb_def a p} -> sb:bool{sb = sb_def a p} -> rnd_mode:mpfr_rnd_t -> Lemma
    (mpfr_add1sp1_round_spec high rb sb rnd_mode == round_def a p rnd_mode)
    
let mpfr_round_rb_sb_lemma_ a p high rb sb rnd_mode = rb_sb_lemma a p
...
let mpfr_round_rb_sb_lemma a p high rb sb rnd_mode =
    mpfr_round_rb_sb_lemma_ a p (high_mant a p) rb sb rnd_mode;
    mpfr_round_spec_lemma high (high_mant a p) rb sb rnd_mode

val mpfr_round_cond_lemma_: a:normal_fp -> p:pos{p < a.prec /\ mpfr_PREC_COND p} ->
    high:normal_fp{high.prec = p /\ eval high =. eval (high_mant a p)} ->
    rb:bool{rb = rb_def a p} -> sb:bool{sb = sb_def a p} -> rnd_mode:mpfr_rnd_t ->
    r:mpfr_fp{r.prec = p} -> Lemma
    (requires (normal_to_mpfr_cond (mpfr_add1sp1_round_spec high rb sb rnd_mode) r))
    (ensures  (normal_to_mpfr_cond (round_def a p rnd_mode) r))

let mpfr_round_cond_lemma_ a p high rb sb rnd_mode r =
...
let mpfr_round_cond_lemma a p high rb sb rnd_mode r =
    ...
    mpfr_round_cond_lemma_ a p high rb sb rnd_mode r

let mpfr_add1sp1_ternary_spec (high:normal_fp) (rb:bool) (sb:bool) (rnd_mode:mpfr_rnd_t):
    Tot int =
...
val mpfr_truncate_ternary_cond_lemma: a:normal_fp{mpfr_EXP_COND a.exp} ->
    ...
    (requires (normal_to_mpfr_cond high r /\
...
let mpfr_truncate_ternary_cond_lemma a p high rb sb rnd_mode f r =
    ...
    mpfr_overflow_bound_lemma high;
    mpfr_underflow_bound_lemma high;
    normal_to_mpfr_cond_in_range_lemma high r;
    rb_sb_lemma a p;
...
val mpfr_add_one_ulp_ternary_cond_lemma: a:normal_fp{mpfr_EXP_COND a.exp} ->
    p:pos{p < a.prec /\ mpfr_PREC_COND p} ->
    high:normal_fp{high.prec = p /\ eval high =. eval (high_mant a p)} -> rnd_mode:mpfr_rnd_t ->
    f:int -> r:mpfr_fp{r.prec = p} -> Lemma
    (requires (normal_to_mpfr_cond (add_one_ulp high) r /\
               f = a.sign))
    (ensures  (mpfr_ternary_cond f a r))
...
let mpfr_add_one_ulp_ternary_cond_lemma a p high rnd_mode f r =
	...
	normal_to_mpfr_cond_in_range_lemma (add_one_ulp high) r;
...
val mpfr_ternary_cond_lemma_: a:normal_fp{mpfr_EXP_COND a.exp} ->
    p:pos{p < a.prec /\ mpfr_PREC_COND p} ->
    high:normal_fp{high.prec = p /\ eval high =. eval (high_mant a p)} ->
    rb:bool{rb = rb_def a p} -> sb:bool{sb = sb_def a p} -> rnd_mode:mpfr_rnd_t ->
    f:int -> r:mpfr_fp{r.prec = p} -> Lemma
...
let mpfr_ternary_cond_lemma_ a p high rb sb rnd_mode f r =
    if rb = false && sb = false then mpfr_truncate_ternary_cond_lemma a p high rb sb rnd_mode f r
    else if MPFR_RNDN? rnd_mode then begin
	if rb = false || (sb = false && is_even high)
	then mpfr_truncate_ternary_cond_lemma a p high rb sb rnd_mode f r
	else mpfr_add_one_ulp_ternary_cond_lemma a p high rnd_mode f r
...
let mpfr_ternary_cond_lemma a p high rb sb rnd_mode f r =
    assert(eval_abs high =. eval_abs (high_mant a p));
    mpfr_ternary_cond_lemma_ a p high rb sb rnd_mode f r

val mpfr_add1sp1_post_cond_lemma: a:normal_fp{mpfr_EXP_COND a.exp} ->
    p:pos{p < a.prec /\ mpfr_PREC_COND p} ->
    high:normal_fp{high.prec = p /\ high.sign = (high_mant a p).sign /\
                   high.exp = (high_mant a p).exp /\ high.len <= (high_mant a p).len /\
		   high.limb * pow2 (a.len - high.len) = (high_mant a p).limb} ->
...
let mpfr_add1sp1_post_cond_lemma a p high rb sb rnd_mode f r =
