...
val add: a:mpfr_structP{valid_mpfr_struct a} ->
         b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
    Tot (r:mpfr_structP{r.sign = a.sign /\ r.exp >= mpfr_EMIN_spec /\ mpfr_mp_cond r /\
          ((a.mant * pow2 (a.exp - mpfr_EMIN_spec)) +
	   (b.mant * pow2 (b.exp - mpfr_EMIN_spec)) =
	   (r.mant * pow2 (r.exp - mpfr_EMIN_spec)))})
...
val add_mant: a:mpfr_structP{valid_mpfr_struct a} ->
              b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
	  Tot nat
let add_mant a b =
...
val add_exp : a:mpfr_structP{valid_mpfr_struct a} ->
              b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
	 Tot (r:int{r >= mpfr_EMIN_spec})
let add_exp a b = min a.exp b.exp

val add_prec: a:mpfr_structP{valid_mpfr_struct a} ->
              b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
	  Tot nat
let add_prec a b =
...
(* Useful lemmas *)
val add_mp_cond_lemma: a:mpfr_structP{valid_mpfr_struct a} ->
                       b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
...
val add_value_lemma: a:mpfr_structP{valid_mpfr_struct a} ->
                     b:mpfr_structP{valid_mpfr_struct b /\ a.sign = b.sign} ->
    Lemma (add_exp a b >= mpfr_EMIN_spec /\
           (a.mant * pow2 (a.exp - mpfr_EMIN_spec)) +
	   (b.mant * pow2 (b.exp - mpfr_EMIN_spec)) =
	   (add_mant a b * pow2 (add_exp a b - mpfr_EMIN_spec)))
let add_value_lemma a b = 
    let sx = add_exp a b in
    let sm = add_mant a b in
    if a.exp >= b.exp then begin
        distributivity_add_left (a.mant * pow2 (a.exp - b.exp)) b.mant (pow2 (sx - mpfr_EMIN_spec));
	paren_mul_right a.mant (pow2 (a.exp - b.exp)) (pow2 (sx - mpfr_EMIN_spec));
	pow2_plus (a.exp - b.exp) (sx - mpfr_EMIN_spec)
    end else begin
        distributivity_add_left (b.mant * pow2 (b.exp - a.exp)) a.mant (pow2 (sx - mpfr_EMIN_spec));
	paren_mul_right b.mant (pow2 (b.exp - a.exp)) (pow2 (sx - mpfr_EMIN_spec));
	pow2_plus (b.exp - a.exp) (sx - mpfr_EMIN_spec)
    end
