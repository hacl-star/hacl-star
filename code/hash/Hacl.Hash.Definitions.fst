...
let blocks_t (a: hash_alg) =
  b:B.buffer U8.t { B.length b % block_length a = 0 }

let hash_t (a: hash_alg) = b:B.buffer U8.t { B.length b = hash_length a }
...
let update_st (a: hash_alg) =
  s:state a ->
  block:B.buffer U8.t { B.length block = block_length a } ->
  ST.Stack unit
    (requires (fun h ->
...
let update_multi_st (a: hash_alg) =
  s:state a ->
  blocks:blocks_t a ->
  n:U32.t { B.length blocks = block_length a * U32.v n } ->
  ST.Stack unit
    (requires (fun h ->
...
let update_last_st (a: hash_alg) =
  ...
  input:B.buffer U8.t { B.length input + len_v a prev_len < max_input_length a } ->
