...
inline_for_extraction noextract
val concat_lemma (#a:Type0) (#len0:flen) (#len1:flen)
		 (s0:ntuple a len0) (s1:ntuple a len1) (i:nat):
		Lemma (requires (len0 + len1 <= max_size_t /\ i < len0 + len1))
...
val index_update_sub_lemma (#a:Type) (#len:flen) (s:ntuple a len) (start:nat) (n:flen{start + n <= len}) (x:ntuple a n) (i:nat{i < n}) :
  Lemma (index (update_sub #a #len s start n x) i ==
         (if i >= start && i < start + n then
           index x (i - start)
          else index s i))
...
#set-options "--max_ifuel 16 --max_fuel 16 --z3rlimit 40"
...
val tup4_lemma (#a:Type0) (#l:flen{l = 4}) (t:ntuple a l) :
  Lemma (let (x0,(x1,(x2,x3))) = tup4 t in
         x0 == t.(|0|) /\
         x1 == t.(|1|) /\
         x2 == t.(|2|) /\
         x3 == t.(|3|))
