...
noextract val lemma_cast_to_u64: x:uint32 -> Lemma
  (requires (True))
...
noextract val lemma_modifies0_is_modifies1: #a0:Type -> #len0:size_nat -> b0:lbuffer a0 len0 -> h0:mem -> h1:mem -> Lemma
  (requires (True))
  (ensures  (h0 == h1 ==> modifies1 b0 h0 h1))
let lemma_modifies0_is_modifies1 #a0 #len0 b0 h0 h1 = admit()


noextract val lemma_modifies0_is_modifies2: #a0:Type -> #a1:Type -> #len0:size_nat -> #len1:size_nat -> b0:lbuffer a0 len0 -> b1:lbuffer a1 len1 -> h0:mem -> h1:mem -> Lemma
  (requires (True))
  (ensures  (h0 == h1 ==> modifies2 b0 b1 h0 h1))
let lemma_modifies0_is_modifies2 #a0 #a1 #len0 #len1 b0 b1 h0 h1 = admit()


noextract val lemma_modifies1_is_modifies2: #a0:Type -> #a1:Type -> #len0:size_nat -> #len1:size_nat -> b0:lbuffer a0 len0 -> b1:lbuffer a1 len1 -> h0:mem -> h1:mem -> Lemma
  (requires (True))
  (ensures  (modifies1 b0 h0 h1 ==> modifies2 b0 b1 h0 h1))
let lemma_modifies1_is_modifies2 #a0 #a1 #len0 #len1 b0 b1 h0 h1 = admit()


noextract val lemma_repeati: #a:Type -> n:size_nat -> f:(i:size_nat{i < n}  -> a -> Tot a) -> init:a -> i:size_nat{i < n} -> Lemma
  (requires True)
...
noextract val lemma_repeati_zero: #a:Type -> n:size_nat -> f:(i:size_nat{i < n}  -> a -> Tot a) -> init:a -> Lemma
  (requires True)
...
(* noextract val lemma_size_to_uint32_equal_u32_of_v_of_size_t: x:size_t -> Lemma *)
(*   (requires True) *)
...
noextract val lemma_value_mixed_size_addition: x:size_t -> y:size_nat -> Lemma
  (requires True)
  (ensures (v (x +. (size y)) == v x + y))
  [SMTPat (v (x +. (size y)) == v x + y)]
let lemma_value_mixed_size_addition x y = admit()

(* noextract val lemma_modifies2_of_subs_is_modifies1: #h0:mem -> #h1:mem -> #a:Type0 -> #len:size_nat -> #pos0:size_nat -> #len0:size_nat{pos0 + len0 < len} -> #pos1:size_nat -> #len1:size_nat{pos1 + len1 <= len} -> buf:lbuffer a len -> sub0:lbuffer a len0 -> sub1:lbuffer a len1 -> *)
(*   Lemma *)
(*     (requires (True)) *)
...
noextract val lemma_modifies2_trans: #h0:mem -> #h1:mem -> #h2:mem -> #a0:Type0 -> #a1:Type0 -> #len0:size_nat -> #len1:size_nat -> buf0:lbuffer a0 len0 -> buf1:lbuffer a1 len1 ->
  Lemma
...
let blake2s #vll #vkk #vnn output d ll kk k nn =
  ...
  if  ll =. (size 0) && kk =. (size 0) then begin
    let h0 = ST.get () in
    alloc1 #h0 #_ #_ #_ #Spec.size_block #vnn (size Spec.size_block) (u8 0) output
    (fun h -> (fun _ r -> True))
    (fun data ->
      let h0 = ST.get () in
