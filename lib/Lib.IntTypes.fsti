...
inline_for_extraction
let mod_mask (#t:inttype{~(S8? t) /\ ~(S16? t) /\ ~(S32? t) /\ ~(S64? t)}) (#l:secrecy_level) (m:shiftval t) : int_t t l =
  (nat_to_int 1 `shift_left` m) `sub_mod` nat_to_int 1

val mod_mask_lemma: #t:inttype{~(S8? t) /\ ~(S16? t) /\ ~(S32? t) /\ ~(S64? t)} -> #l:secrecy_level -> a:int_t t l -> m:shiftval t ->
  Lemma
    (requires True)
