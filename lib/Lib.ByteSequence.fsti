...
val nat_from_intseq_be: #t:inttype -> b:seq (uint_t t) -> Tot (n:nat{n < pow2 (length b `op_Multiply` bits t)})

val nat_from_intseq_le: #t:inttype -> b:seq (uint_t t) -> Tot (n:nat{n < pow2 (length b `op_Multiply` bits t)})
...
val uints_to_bytes_le: #t:inttype -> #len:size_nat -> s:lseq (uint_t t) len -> b:lbytes (len `op_Multiply` numbytes t)
val uints_to_bytes_be: #t:inttype -> #len:size_nat -> s:lseq (uint_t t) len -> b:lbytes (len `op_Multiply` numbytes t)

val uints_from_bytes_le: #t:inttype -> #len:nat -> b:bytes{length b == len `op_Multiply` numbytes t} -> s:lseq (uint_t t) len 
val uints_from_bytes_be: #t:inttype -> #len:nat -> b:bytes{length b == len `op_Multiply` numbytes t} -> s:lseq (uint_t t) len
