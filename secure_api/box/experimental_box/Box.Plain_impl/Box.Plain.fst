...
val coerce: (#pp:plain_package) -> #key_length:(n:nat{n<=32}) -> (#id:eqtype) -> (#i:id) -> (#key_type:(id -> Type0)) -> (kp:KEY.key_package #id key_length key_type) -> (key:key_type i{KEY.hon kp i key == false \/ ~pp.flag}) -> p:plain pp -> p:protected_plain pp i
let coerce #pp #key_length #id #i #kt kp key p =
  p

val repr: (#pp:plain_package) -> #key_length:(n:nat{n<=32}) -> (#id:eqtype) -> (#i:id) -> (#key_type:(id -> Type0)) -> (kp:KEY.key_package #id key_length key_type) -> (key:key_type i{KEY.hon kp i key == false \/ ~pp.flag}) -> p:protected_plain pp i -> p:plain pp
