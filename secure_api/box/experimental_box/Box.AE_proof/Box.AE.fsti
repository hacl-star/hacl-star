...
noeq abstract type ae_package (#id:eqtype) (#key_length:(n:nat{n<=32})) (kp:KEY.key_package #id key_length (key key_length #id)) (aparams:ae_parameters{aparams.keylength = key_length}) (pp:plain_package{valid_ae_plain_package aparams pp}) =
  | AE:
    rgn:erid ->
    log:message_log #pp #id rgn aparams ->
    ae_package #id #key_length kp aparams pp

val get_ap_rgn: (#id:eqtype) -> (#key_length:(n:nat{n<=32})) -> (#kp:KEY.key_package #id key_length (key key_length #id)) -> (#aparams:ae_parameters{aparams.keylength = key_length}) -> (#pp:plain_package{valid_ae_plain_package aparams pp}) -> (ap:ae_package #id #key_length kp aparams pp) -> rgn:erid{rgn = ap.rgn}

val recall_log: (#id:eqtype) -> (#key_length:(n:nat{n<=32})) -> (#kp:KEY.key_package #id key_length (key key_length #id)) -> (#aparams:ae_parameters{aparams.keylength = key_length}) -> (#pp:plain_package{valid_ae_plain_package aparams pp}) -> (ap:ae_package #id kp aparams pp) -> ST unit
...
val get_ap_log: (#id:eqtype) -> (#key_length:(n:nat{n<=32})) -> (#kp:KEY.key_package #id key_length (key key_length #id)) -> (#aparams:ae_parameters{aparams.keylength = key_length}) -> (#pp:plain_package{valid_ae_plain_package aparams pp}) -> (ap:ae_package #id #key_length kp aparams pp) -> GTot (message_log #pp #id ap.rgn aparams)

let nonce_is_unique (#id:eqtype) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) (i:id) (n:nonce aparams) (h0:mem) =
  forall c . MM.fresh ap.log (LOG_KEY i n c) h0

val create_ae_package:(rgn:erid) -> (#id:eqtype) -> (#key_length:(n:nat{n<=32})) -> (kp:KEY.key_package #id key_length (key key_length #id)) -> (aparams:ae_parameters{aparams.keylength = key_length}) -> (pp:plain_package{valid_ae_plain_package aparams pp}) -> ST (ae_package #id #key_length kp aparams pp)
  (requires (fun h0 -> True))
  (ensures (fun h0 ap h1 ->
    ...
    /\ extends ap.rgn rgn
    /\ contains h1 ap.log
    /\ sel h1 ap.log == empty_message_log aparams pp
  ))

val zero_bytes: (valid_length:(n:nat -> bool)) -> (n:nat{valid_length n}) -> p:lbytes n{valid_length (Seq.length p)}

let encrypt_modified_regions (#id:eqtype) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) : (Set.set rid) = Set.singleton ap.rgn

let encrypt_log_change (#id:eqtype) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) (i:id) (n:nonce aparams) (c:ciphertext aparams) (p:protected_plain pp i) (h0:mem) (h1:mem) =
    let a = 1 in
    witnessed (MM.contains ap.log (LOG_KEY i n c) p)
    /\ contains h0 ap.log
    /\ sel h1 ap.log == MM.upd (sel h0 ap.log) (LOG_KEY i n c) p

let encrypt_functional_spec (#id:eqtype) (#i:id) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) (k:key key_length i) (n:nonce aparams) (c:ciphertext aparams) (p:protected_plain pp i) =
  let a = 1 in
  ((KEY.(hon kp i k) /\ get_ae_flagGT aparams) ==>
...
let decrypt_modified_regions (#id:eqtype) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) = Set.empty

let decrypt_log_change (#id:eqtype) (#key_length:(n:nat{n<=32})) (#kp:KEY.key_package #id key_length (key key_length #id)) (#aparams:ae_parameters{aparams.keylength = key_length}) (#pp:plain_package{valid_ae_plain_package aparams pp}) (ap:ae_package #id #key_length kp aparams pp) (h0:mem) (h1:mem) =
...
let decrypt_functional_spec (#id:eqtype)
                            ...
                            (#pp:plain_package{valid_ae_plain_package aparams pp})
                            (ap:ae_package #id #key_length kp aparams pp)
                            (k:key key_length i)
                            (n:nonce aparams)
