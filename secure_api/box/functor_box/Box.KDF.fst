...
type out_index_package (im:index_module) = kim:index_module{id kim == key_id im}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The following functions belong to the formal KEY package in the model.
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
abstract noeq type kdf_package (im:index_module) =
  | DM :
    key_log_region: log_region im ->
    key_log: (key_log_t im key_log_region) ->
    out_im:out_index_package im ->
    out_km:key_module out_im{get_keylen out_im out_km = keylen} ->
    kdf_package im


val recall_logs: #im:index_module -> dm:kdf_package im -> ST unit
...
val instantiate: im:index_module -> rgn:log_region im -> out_im:out_index_package im ->out_km:key_module out_im{get_keylen out_im out_km = keylen} -> ST (dm:kdf_package im)
(requires (fun h0 ->
  True
))
...
let instantiate im rgn out_im out_km =
...
*)
private val gen: #im:index_module  -> dm:kdf_package im -> i:id im -> ST (k:key im{get_index k=i})
  (requires (fun h0 ->
...
val instantiate_km: #im:index_module -> dm:kdf_package im -> km:key_module im{
...
let instantiate_km #im dm =
  ...
  km
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
// The following functions belong to the formal KDF package in the model.
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
 

  val derive: im:index_module -> dm:kdf_package im -> k:key im -> ST (k':Key.get_keytype dm.out_im dm.out_km{Key.get_index dm.out_im dm.out_km k' = Derived (get_index k) } )
  (requires (fun h0 ->
