...
let coerce_dh_sh #rgn ip oparam raw_sh =
  register ip (PK_id raw_sh) false;
  SH raw_sh

let coerce_dh_exp #rgn ip oparam raw_exp =
  let raw_sh = oparam.exponentiate raw_exp oparam.generator in
  register ip (PK_id raw_sh) false;
  let sh = SH raw_sh in
  EXP raw_exp sh

let get_op_rgn #rgn #ip #key_length #key_type #kp #oparam op = op.kp_log_rgn

let get_op_log #rgn #ip #key_length #key_type #kp #oparam op = op.kp_log

let recall_op_log #rgn #ip #key_length #key_type #kp #oparam op =
  recall op.kp_log

let create_odh_package #rgn #ip #key_length #key_type kp oparam =
...
let dh_op #rgn #ip #key_length #key_type #kp #oparam op sh exp =
