...
val enc_star: n:(n:nat{AE_Spec.valid_plain_length n}) -> b:lbytes (n+16){AE_Spec.valid_cipher_length (n+16)}
let enc_star n = Seq.append (random_bytes n) (random_bytes 16)
...
let secretbox_scheme =
  AE.ES #(AE_Spec.keylen) #(AE_Spec.noncelen) (AE_Spec.valid_plain_length) (AE_Spec.valid_cipher_length) (fun () -> random_bytes 32) AE_Spec.secretbox_easy (enc_star) AE_Spec.secretbox_open_easy

let ae_params = AE.GP AE_Spec.keylen AE_Spec.noncelen secretbox_scheme

let pp = PP (Flags.pkae) (fun n -> n / Spec.Salsa20.blocklen < pow2 32)
...
noeq abstract type pkae_package (rgn:erid) =
  | PKAE_P :
    pkae_rgn:erid{extends pkae_rgn rgn} ->
    ip: index_package pkae_rgn ->
    kp: Key.key_package ip AE_Spec.keylen (AE.key AE_Spec.keylen) ->
    op: ODH.odh_package #pkae_rgn #ip #AE_Spec.keylen #(AE.key AE_Spec.keylen) kp odh_params ->
    pkae_package rgn

let ip = create_index_package
