...
val create_key_package:
  ...
  (key_length:(n:nat{n<=32})) ->
  (key_type:(id ip -> Type0)) ->
  (param_getGT: (#i:id ip -> k:key_type i -> GTot (lbytes key_length))) ->
  (param_leak: (#i:id ip{corrupt i \/ ~flag} -> k:key_type i -> (raw:lbytes key_length{raw = param_getGT k}))) ->
  (param_gen: (i:id ip -> k:key_type i)) ->
  (param_coerce: (i:id ip{corrupt i \/ ~flag} -> raw:lbytes key_length -> (k:key_type i{raw = param_getGT k}))) ->
  (kp:key_package ip)
