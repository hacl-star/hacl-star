...
type tag (#hsz:pos) = b:Spec.Hash.Definitions.bytes { Seq.length b = hsz }

type tag_fun_t (#hsz:pos) = tag #hsz -> tag #hsz -> GTot (tag #hsz)

val sha256_compress: tag_fun_t #32
...
/// requires extending the definitions of tags and tages. Our
/// extended definition of tag justifies skipping any concrete
/// computation on dummies.
noeq
type padded_tag #hsz =
| HRaw: hr:tag #hsz -> padded_tag #hsz
| HPad // right padding to make the size of a Merkle tree a power of 2

val padded_tag_fun: (#hsz:pos) -> (f:tag_fun_t #hsz) -> (lh:padded_tag #hsz) -> (rh:padded_tag #hsz) -> GTot (padded_tag #hsz)
let padded_tag_fun #hsz f lh rh =
  allow_inversion (padded_tag #hsz);
  match lh, rh with
  | HPad    , _        -> HPad
  | _       , HPad     -> lh
...
noextract
val tags (#hsz:pos): Type0
let tags #hsz = S.seq (padded_tag #hsz)

type merkle_tree (#hsz:pos) n = hs:tags #hsz {S.length hs = pow2 n}

val mt_get: #hsz:pos -> #n:nat -> mt:merkle_tree #hsz n -> idx:nat{idx < pow2 n} -> GTot (padded_tag #hsz)
let mt_get #_ #_ mt idx = S.index mt idx

unfold let op_String_Access (#hsz:pos) = S.index #(padded_tag #hsz)
...
val hs_next_lv: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat -> hs:tags #hsz {S.length hs = 2 * n} -> GTot (nhs:tags #hsz {S.length nhs = n})
let rec hs_next_lv #hsz #f #n hs =
  if n = 0 then S.empty
  else S.cons
    (padded_tag_fun #hsz f hs.[0] hs.[1])
    (hs_next_lv #hsz #f #(n-1) (S.slice hs 2 (S.length hs)))

val hs_next_lv_index: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat  -> hs:tags{S.length hs = 2 * n} -> i:nat{i < n} ->
  Lemma ((hs_next_lv #hsz #f #n hs).[i] == padded_tag_fun #hsz f hs.[2 * i] hs.[2 * i + 1])
let rec hs_next_lv_index #hsz #f #n hs i =
  if n = 0 || i = 0 then ()
  else hs_next_lv_index #hsz #f #(n - 1) (S.slice hs 2 (S.length hs)) (i - 1)

val hs_next_lv_slice:
  #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat  ->
  hs:tags{S.length hs = 2 * n} -> i:nat -> j:nat{i <= j && j <= n} ->
  Lemma (requires True)
...
let rec hs_next_lv_slice #hsz #f #n hs i j =
  if i = j then ()
    ...
    assert (S.equal (hs_next_lv #hsz #f #(j - i) x)
                    (S.cons (padded_tag_fun #hsz f x.[0] x.[1])
                            (hs_next_lv #hsz #f #(j - i - 1) (S.slice x 2 (S.length x)))));
    hs_next_lv_slice #hsz #f #n hs (i + 1) j;
    hs_next_lv_index #hsz #f #n hs i
  end

val mt_next_lv: #hsz:pos -> #f:tag_fun_t #hsz -> #n:pos -> mt:merkle_tree #hsz n -> GTot (merkle_tree #hsz (n-1))
let mt_next_lv #_ #f #n mt = hs_next_lv #_ #f #(pow2 (n-1)) mt

val mt_next_lv_mt_left: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat{1 < n} -> mt:merkle_tree #hsz n ->
...
val mt_next_lv_mt_right: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat{1 < n} -> mt:merkle_tree #hsz n ->
...
val hs_next_lv_equiv:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  j:nat -> n:pos{j <= 2 * n} ->
  hs1:tags{S.length hs1 = 2 * n} ->
  hs2:tags{S.length hs2 = 2 * n} ->
...
let hs_next_lv_equiv #hsz #f j n hs1 hs2 =
  ...
  assert (forall (i:nat{i < j / 2}). hs1'.[i] == padded_tag_fun #hsz f hs1.[2 * i] hs1.[2 * i + 1]);
  assert (forall (i:nat{i < j / 2}). hs2'.[i] == padded_tag_fun #hsz f hs2.[2 * i] hs2.[2 * i + 1]);
...
val hs_next_rel:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  n:nat ->
  hs:tags #hsz {S.length hs = 2 * n} ->
  nhs:tags #hsz {S.length nhs = n} ->
  GTot Type0
let hs_next_rel #hsz #f n hs nhs =
  forall (i:nat{i < n}).
    S.index nhs i ==
    padded_tag_fun #hsz f (S.index hs (2 * i)) (S.index hs (2 * i + 1))
...
val hs_next_rel_next_lv:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  n:nat ->
  hs:tags{S.length hs = 2 * n} ->
  nhs:tags{S.length nhs = n} ->
...
val mt_next_rel_upd_even:
  ...
  Lemma (requires mt_next_rel #_ #f n mt nmt)
        (ensures  mt_next_rel #_ #f n
                   (S.upd mt (2 * i) v)
                   (S.upd nmt i (padded_tag_fun #hsz f v (S.index mt (2 * i + 1)))))
...
val mt_next_rel_upd_odd:
  ...
  Lemma (requires mt_next_rel #_ #f n mt nmt)
        (ensures  mt_next_rel #_ #f n
                   (S.upd mt (2 * i + 1) v)
                   (S.upd nmt i (padded_tag_fun #_ f (S.index mt (2 * i)) v)))
let mt_next_rel_upd_odd #hsz #f n mt nmt i v = ()

// fournet: just [root]?
val mt_get_root: 
  #hsz:pos -> #f:tag_fun_t #hsz ->
  #n:nat -> mt:merkle_tree #hsz n -> GTot (padded_tag #hsz)
...
val mt_get_root_step: #hsz:pos -> #f:tag_fun_t #hsz -> #n:pos -> mt:merkle_tree #hsz n ->
  Lemma (mt_get_root #_ #f mt ==
         padded_tag_fun #_ f (mt_get_root #_ #f (mt_left mt)) (mt_get_root #_ #f (mt_right mt)))
...
type merkle_path #hsz n = S.lseq (padded_tag #hsz) n

/// We first specify full paths, including padding.

val mt_get_path: 
  #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat -> 
  mt:merkle_tree #hsz n -> i:nat{i < pow2 n} -> GTot (merkle_path #hsz n)
...
val mt_verify_: 
  #hsz:pos -> #f:tag_fun_t #hsz ->#n:nat -> 
  p:merkle_path #hsz n -> idx:nat{idx < pow2 n} -> padded_tag #hsz -> GTot (padded_tag #hsz)
let rec mt_verify_ #hsz #f #n p idx h =
  if n = 0 then h
  else mt_verify_ #_ #f #(n-1) (S.tail p) (idx / 2)
                  (if idx % 2 = 0
                   then padded_tag_fun #_ f h (S.head p)
                   else padded_tag_fun #_ f (S.head p) h)

val mt_verify: 
  #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat -> 
  p:(merkle_path #hsz n) -> idx:nat{idx < pow2 n} -> padded_tag #hsz -> padded_tag #hsz -> GTot prop
...
val hs_next_lv_get: 
  #hsz:pos -> #f:tag_fun_t #hsz -> #n:pos -> 
  hs:tags{S.length hs = 2 * n} -> idx:nat{idx < 2 * n} ->
  Lemma ((hs_next_lv #_ #f #n hs).[idx / 2] ==
         (if idx % 2 = 0
          then padded_tag_fun #_ f hs.[idx] hs.[idx + 1]
          else padded_tag_fun #_ f hs.[idx - 1] hs.[idx]))
...
val mt_next_lv_get: 
  ...
  Lemma (
     ...
     (if idx % 2 = 0
      then padded_tag_fun #_ f mt.[idx] mt.[idx + 1]
      else padded_tag_fun #_ f mt.[idx - 1] mt.[idx]))
...
/// Security: we reduce tree collisions to collisions on the tag
...
/// All tages in a sequence are raw tages, not padding
val raw_tages: 
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags #hsz -> Tot Type0 (decreases (S.length hs))
let rec raw_tages #hsz #f hs =
  if S.length hs = 0 then True
  else (HRaw? (S.head hs) /\ raw_tages #_ #f (S.tail hs))

val raw_tages_raws: 
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags{raw_tages #hsz #f hs} ->
  Tot (S.seq (tag #hsz)) (decreases (S.length hs))
let rec raw_tages_raws #hsz #f hs =
  if S.length hs = 0 then S.empty
  else S.cons (HRaw?.hr (S.head hs)) (raw_tages_raws #_ #f (S.tail hs))

val raw_tages_index:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags -> i:nat{i < S.length hs} ->
  Lemma (requires raw_tages #_ #f hs)
        (ensures HRaw? #hsz hs.[i])
        (decreases i)
let rec raw_tages_index #hsz #f hs i =
  if i = 0 then ()
  else raw_tages_index #_ #f (S.tail hs) (i - 1)

val raw_tages_slice:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags -> i:nat -> j:nat{i <= j && j <= S.length hs} ->
  Lemma (requires raw_tages #_ #f hs)
        (ensures  raw_tages #_ #f (S.slice hs i j))
        (decreases (j - i))
let rec raw_tages_slice #hsz #f hs i j =
  if i = j then ()
  else (
    raw_tages_index #_ #f hs i;
    raw_tages_slice #_ #f hs (i + 1) j)

/// All tages in a sequence are just padding
val pad_tages: 
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags #hsz -> Type0
let pad_tages #hsz #f hs =
  S.equal hs (S.create (S.length hs) HPad)

val pad_tages_slice:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  hs:tags -> i:nat -> j:nat{i <= j && j <= S.length hs} ->
  Lemma (requires pad_tages #_ #f hs)
        (ensures  pad_tages #_ #f (S.slice hs i j))
        (decreases (j - i))
let rec pad_tages_slice #hsz #f hs i j =
  if i = j then ()
  else pad_tages_slice #_ #f hs (i + 1) j

/// Right-padded Merkle tree, a tree refinement

let rpmt (#hsz:pos) (#f:tag_fun_t) (n:nat) (i:nat{i <= pow2 n}) =
  mt:merkle_tree #hsz n {
    raw_tages #_ #f (S.slice mt 0 i) /\
    pad_tages #_ #f (S.slice mt i (S.length mt)) }

val rpmt_raws: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat -> #i:nat{i <= pow2 n} -> mt:rpmt #hsz #f n i -> S.seq (tag #hsz)
let rpmt_raws #hsz #f #n #i mt = raw_tages_raws #_ #f (S.slice mt 0 i)

val rpmt_i_0: #hsz:pos -> #f:tag_fun_t #hsz -> #n:nat -> mt:rpmt #hsz #f n 0 ->
  Lemma (S.equal mt (S.create (pow2 n) (HPad #hsz)))
let rpmt_i_0 #hsz #f #n mt = ()

val rpmt_left: #hsz:pos -> #f:tag_fun_t #hsz -> #n:pos -> #i:nat{i <= pow2 n} -> rpmt #hsz #f n i ->
  rpmt #hsz #f (n-1) (if i <= pow2 (n-1) then i else pow2 (n-1))
let rpmt_left #hsz #f #n #i mt =
  if i <= pow2 (n-1)
  then pad_tages_slice #_ #f (S.slice mt i (S.length mt)) 0 (pow2 (n-1) - i)
  else raw_tages_slice #_ #f (S.slice mt 0 i) 0 (pow2 (n-1));
  mt_left mt

#push-options "--z3rlimit 40"

val rpmt_right: #hsz:pos -> #f:tag_fun_t #hsz -> #n:pos -> #i:nat{i <= pow2 n} -> rpmt #hsz #f n i ->
  rpmt #_ #f (n-1) (if i <= pow2 (n-1) then 0 else i - pow2 (n-1))
let rpmt_right #hsz #f #n #i mt =
  if i <= pow2 (n-1)
  then pad_tages_slice #_ #f (S.slice mt i (S.length mt)) (pow2 (n-1) - i) (pow2 n - i)
  else raw_tages_slice #_ #f (S.slice mt 0 i) (pow2 (n-1)) i;
  mt_right mt

/// Two right-padded Merkle trees collide when
/// 1) they have the same height (`n`) and number of raw tages (`i`),
/// 2) their contents differ, and
/// 3) their roots are same.

// fournet: we may want to work towards removing 1) using a tag prefix
noeq
type mt_collide (#hsz:pos) (#f:tag_fun_t #hsz) (n:nat) (i:nat{i <= pow2 n}) = | Collision:
  mt1:rpmt #_ #f n i -> mt2:rpmt #_ #f n i {
    mt1 =!= mt2 /\
    mt_get_root #_ #f #_ mt1 == mt_get_root #_ #f #_ mt2 } -> mt_collide #_ #f n i

noeq
type tag2_raw_collide = | Collision2:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  lh1:tag -> rh1:tag ->
  lh2:tag -> rh2:tag {
    (lh1 =!= lh2 \/ rh1 =!= rh2) /\
    f lh1 rh1 == f lh2 rh2 } -> tag2_raw_collide

/// Auxiliary lemmas for the proof

val rpmt_pad_tages_0:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  #n:nat -> #i:nat{i <= pow2 n} -> mt:rpmt #_ #f n i ->
  Lemma (i = 0 <==> pad_tages #_ #f mt )
let rpmt_pad_tages_0 #_ #_ #n #i mt = ()

val rpmt_pad_tages_index_0:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  #n:nat -> #i:nat{i <= pow2 n} ->
  mt:rpmt #_ #f n i ->
  Lemma (pad_tages #_ #f mt <==> HPad? mt.[0])
let rpmt_pad_tages_index_0 #_ #_ #n #i mt = ()
...
val rpmt_get_root_pad_tages:
  #hsz:pos -> #f:tag_fun_t #hsz ->
  #n:nat -> #i:nat{i <= pow2 n} -> mt:rpmt #_ #f n i ->
  Lemma (pad_tages #_ #f mt <==> HPad? (mt_get_root #_ #f mt))
let rpmt_get_root_pad_tages #_ #f #n #i mt =
  rpmt_pad_tages_index_0 #_ #f mt;
...
let rpmt_get_root_pad #_ #f #n #i mt =
  rpmt_get_root_pad_tages #_ #f mt;
  rpmt_pad_tages_0 #_ #f mt
...
val extract: 
  #hsz:pos -> #f:tag_fun_t #hsz ->
  #n:nat -> #i:nat{i <= pow2 n} -> mt_collide #_ #f n i -> GTot tag2_raw_collide
