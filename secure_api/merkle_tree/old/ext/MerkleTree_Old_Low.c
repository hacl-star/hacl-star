...
#include "MerkleTree_Old_Low.h"

uint32_t MerkleTree_Old_Low_hash_size = (uint32_t)32U;
...
LowStar_Vector_create_reserve__uint8_t_(uint32_t len, uint8_t *ia)
{
  KRML_CHECK_SIZE(sizeof(ia), len);
  uint8_t **buf = KRML_HOST_MALLOC(sizeof(ia) * len);
...
static LowStar_Vector_vector_str__uint8_t_
LowStar_BufVector_create_reserve__uint8_t(uint32_t blen, uint32_t len)
{
  return LowStar_Vector_create_reserve__uint8_t_(len, NULL);
}
...
MerkleTree_Old_Low_merkle_tree *MerkleTree_Old_Low_create_merkle_tree()
{
  ...
  LowStar_BufVector_new_region_();
  LowStar_Vector_vector_str__uint8_t_
  values = LowStar_BufVector_create_reserve__uint8_t(MerkleTree_Old_Low_hash_size, (uint32_t)1U);
  LowStar_BufVector_new_region_();
  LowStar_Vector_vector_str__uint8_t_
  iroots =
    LowStar_BufVector_create_rid__uint8_t((uint8_t)0U,
      MerkleTree_Old_Low_hash_size,
      (uint32_t)32U);
  KRML_CHECK_SIZE(sizeof (MerkleTree_Old_Low_merkle_tree), (uint32_t)1U);
  MerkleTree_Old_Low_merkle_tree
  *mptr = KRML_HOST_MALLOC(sizeof (MerkleTree_Old_Low_merkle_tree));
  mptr[0U] = ((MerkleTree_Old_Low_merkle_tree){ .values = values, .iroots = iroots });
  return mptr;
}
...
static LowStar_Vector_vector_str__uint8_t_
LowStar_BufVector_insert_ptr__uint8_t(
  uint8_t ia,
  uint32_t blen,
  LowStar_Vector_vector_str__uint8_t_ bv,
  uint8_t *v1
...
{
  LowStar_Vector_vector_str__uint8_t_ ibv = LowStar_Vector_insert__uint8_t_(bv, v1);
  return ibv;
}
...
)
{
  ...
  return LowStar_BufVector_insert_ptr__uint8_t(ia, blen, bv, nv);
}
...
)
{
  if (irps % (uint32_t)2U == (uint32_t)0U)
    LowStar_BufVector_assign_copy__uint8_t(MerkleTree_Old_Low_hash_size, irs, cpos, acc);
  else
  {
    MerkleTree_Old_Low_hash_from_hashes(LowStar_Vector_index__uint8_t_(irs, cpos), acc, acc);
    MerkleTree_Old_Low_insert_iroots(irs, cpos + (uint32_t)1U, irps / (uint32_t)2U, acc);
  }
}

void MerkleTree_Old_Low_insert(MerkleTree_Old_Low_merkle_tree *mt, uint8_t *nv)
{
  MerkleTree_Old_Low_merkle_tree mtv = mt[0U];
  ...
  uint32_t nvalues = sz;
  LowStar_Vector_vector_str__uint8_t_ ivalues = MerkleTree_Old_Low_insert_value(values0, nv);
  LowStar_Vector_vector_str__uint8_t_ iroots = mtv.iroots;
  LowStar_Vector_vector_str__uint8_t_ iroots0 = iroots;
  MerkleTree_Old_Low_insert_iroots(iroots0, (uint32_t)0U, nvalues, nv);
  mt[0U] = ((MerkleTree_Old_Low_merkle_tree){ .values = ivalues, .iroots = iroots0 });
}

void MerkleTree_Old_Low_compress_or_init(bool actd, uint8_t *acc, uint8_t *nh)
{
  if (actd)
    MerkleTree_Old_Low_hash_from_hashes(nh, acc, acc);
  else
    memcpy(acc, nh, MerkleTree_Old_Low_hash_size * sizeof nh[0U]);
}
...
)
{
  if (cpos == (uint32_t)31U)
    MerkleTree_Old_Low_compress_or_init(actd, acc, LowStar_Vector_index__uint8_t_(irs, cpos));
  else if (irps % (uint32_t)2U == (uint32_t)0U)
    MerkleTree_Old_Low_merkle_root_of_iroots(irs,
      cpos + (uint32_t)1U,
      irps / (uint32_t)2U,
      acc,
      actd);
...
void MerkleTree_Old_Low_get_root(MerkleTree_Old_Low_merkle_tree *mt, uint8_t *rt)
{
  ...
  LowStar_Vector_vector_str__uint8_t_ iroots = mtv.iroots;
  LowStar_Vector_vector_str__uint8_t_ iroots0 = iroots;
  MerkleTree_Old_Low_merkle_root_of_iroots(iroots0, (uint32_t)0U, nvalues, rt, false);
}
...
void MerkleTree_Old_Low_free_merkle_tree(MerkleTree_Old_Low_merkle_tree *mt)
{
  MerkleTree_Old_Low_merkle_tree mtv = mt[0U];
  LowStar_Vector_vector_str__uint8_t_ values = mtv.values;
  LowStar_BufVector_free__uint8_t(MerkleTree_Old_Low_hash_size, values);
  LowStar_Vector_vector_str__uint8_t_ iroots = mtv.iroots;
  LowStar_BufVector_free__uint8_t(MerkleTree_Old_Low_hash_size, iroots);
  KRML_HOST_FREE(mt);
}
