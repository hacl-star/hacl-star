...
MerkleTree_Low_merkle_tree *MerkleTree_Low_create_merkle_tree()
{
  ...
  LowStar_BufVector_new_region_();
  LowStar_Vector_vector_str__uint8_t_
  values = LowStar_BufVector_create_reserve__uint8_t(MerkleTree_Low_hash_size, (uint32_t)1U);
  LowStar_BufVector_new_region_();
  LowStar_Vector_vector_str__uint8_t_
  iroots =
  ...
  MerkleTree_Low_merkle_tree *mptr = KRML_HOST_MALLOC(sizeof (MerkleTree_Low_merkle_tree));
  mptr[0U] = ((MerkleTree_Low_merkle_tree){ .values = values, .iroots = iroots });
  return mptr;
}
