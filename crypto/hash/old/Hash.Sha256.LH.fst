...
let size_lblock = 16ul  // 512 bits = 64 words of 32 bits (blocksize/4)
let size_lblocklen = 1ul // 32 bits (UInt32)
let size_count  = 1ul   // 32 bits (UInt32)
let size_state = size_k @+ size_ws @+ size_whash @+ size_wblock @+ size_lblock @+ size_lblocklen @+ size_count
...
let pos_lblocklen = pos_lblock @+ size_lblock
let pos_count     = pos_lblocklen @+ 1ul
...
let init state =
  ...
  upd state pos_lblocklen (uint32_to_sint32 blocksize)
...
let update_step state maskflag =
  ...
  let output_state0 = S32.logor (S32.logand maskflag (add_mod (index whash 0ul) input_state0)) 
                                (S32.logand (S32.lognot maskflag) input_state0) in
  let output_state1 = S32.logor (S32.logand maskflag (add_mod (index whash 1ul) input_state1)) 
                                (S32.logand (S32.lognot maskflag) input_state1) in
  let output_state2 = S32.logor (S32.logand maskflag (add_mod (index whash 2ul) input_state2)) 
                                (S32.logand (S32.lognot maskflag) input_state2) in
  let output_state3 = S32.logor (S32.logand maskflag (add_mod (index whash 3ul) input_state3)) 
                                (S32.logand (S32.lognot maskflag) input_state3) in
  let output_state4 = S32.logor (S32.logand maskflag (add_mod (index whash 4ul) input_state4)) 
                                (S32.logand (S32.lognot maskflag) input_state4) in
  let output_state5 = S32.logor (S32.logand maskflag (add_mod (index whash 5ul) input_state5)) 
                                (S32.logand (S32.lognot maskflag) input_state5) in
  let output_state6 = S32.logor (S32.logand maskflag (add_mod (index whash 6ul) input_state6)) 
                                (S32.logand (S32.lognot maskflag) input_state6) in
  let output_state7 = S32.logor (S32.logand maskflag (add_mod (index whash 7ul) input_state7)) 
...
val update' : (memb  :bytes{length memb >= U32.v blocksize}) ->
              ...
              (len   :s32) ->
              (mask :bytes{(length mask) % (U32.v blocksize) = 0 /\ length mask >= length data
                        /\ disjoint mask memb /\ disjoint mask state /\ disjoint mask data}) ->
...
let rec update' memb state data len mask rounds rounds_mask i =
  ...
  (* BB.TODO: Discuss the possible compiler optimizations for this *)
  let cblocklen = index state pos_lblocklen in
  let mask_lblocklen = S32.eq_mask (uint32_to_sint32 i) (uint32_to_sint32 rounds) in
  let lblocklen = S32.logor (S32.logand mask_lblocklen pblen)
                    (S32.logand (S32.lognot mask_lblocklen) cblocklen) in    

  (* Update the current length of the partial block after processing *)
  upd state pos_lblocklen lblocklen;

  if U32.lt i rounds_mask then begin

    (* Signal update_step to update the working hash or not *)
    let maskflag = S32.lt_mask (uint32_to_sint32 i) (uint32_to_sint32 rounds) in

    (* Process the block *)
    update_step state maskflag;
    update' memb state data len mask rounds rounds_mask (U32.add i 1ul) end
  else ()
...
val update : (state :uint32s{length state = U32.v size_state}) ->
             ...
             (mask :bytes{(length mask) % (U32.v blocksize) = 0 /\ disjoint mask state /\ disjoint mask data}) ->
             (rounds_mask:u32{U32.v rounds_mask * U32.v blocksize = length data})
              -> STL unit
                    (requires (fun h -> live h state /\ live h data))
                    (ensures  (fun h0 r h1 -> live h1 state /\ modifies_1 state h0 h1))

let update state data len mask rounds =
  ...
  let rounds_data = nblocks len @- 1ul in
  let rounds_mask = rounds @- 1ul in

  (* Perform updates for all blocks except the last *)
  update' memb state data (uint32_to_sint32 len) mask rounds_data rounds_mask 0ul;
...
let finish_1' memb state =
  ...
  let lblock = sub state pos_lblock size_lblock in
  ...
  let partiallen = index state pos_lblocklen in
...
val sha256': (memb :uint32s{ length memb >= U32.v size_state}) ->
             ...
             (len  :u32{U32.v len + 8 < pow2 32 /\ U32.v len <= length data}) ->
             (mask :bytes{(length mask) % (U32.v blocksize) = 0 /\ length mask >= length data
                        /\ disjoint mask hash /\ disjoint mask data}) ->
             (masklen:u32{U32.v masklen <= length mask /\ U32.v masklen >= U32.v len})
             -> STL unit
...
let sha256' memb hash data len mask masklen =
  let state = sub memb 0ul size_state in
  init state;
  update state data len mask masklen;
  finish hash state
...
let sha256 hash data len mask masklen =
  ...
  sha256' memb hash data len mask masklen;
