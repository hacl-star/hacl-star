...
open Hacl.Bignum.Lemmas

module U32 = FStar.UInt32

val bitweight: n:nat{n < len} -> Tot nat
let rec bitweight n = if n = 0 then 0 else template n + bitweight (n- 1)
...
let rec eval_wide_ h b i =
  if i = 0 then 0
  else pow2 (bitweight (i - 1)) * Hacl.Bignum.Wide.v (get h b (i-1)) + eval_wide_ h b (i-1)
