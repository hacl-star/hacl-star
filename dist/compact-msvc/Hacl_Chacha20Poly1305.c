...
)
{
  ...
  Hacl_Impl_Poly1305_poly1305_update_32(ctx, n1 * (uint32_t)16U, blocks);
  memcpy(tmp, rem1, r * sizeof rem1[0U]);
  if (r > (uint32_t)0U)
  {
    Hacl_Impl_Poly1305_poly1305_update_32(ctx, (uint32_t)16U, tmp);
  }
}
...
{
  Hacl_Impl_Poly1305_poly1305_finish_32(out, k, ctx);
}
...
)
{
  ...
  uint64_t *ctx = alloca(((uint32_t)5U + (uint32_t)20U) * sizeof (uint64_t));
