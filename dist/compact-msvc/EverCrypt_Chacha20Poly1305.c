...
/*
  ...
  val aead_encrypt :
    ...
    mlen:
      size_t{v mlen + 16 <= max_size_t /\ v aadlen + v mlen / 64 <= max_size_t} ->
  ...
  -> Stack unit
      (requires
        (fun h ->
            disjoint k cipher /\ disjoint n cipher /\ disjoint k tag /\
            disjoint n tag /\ disjoint cipher tag /\ eq_or_disjoint m cipher /\
            disjoint aad cipher /\ live h k /\ live h n /\ live h aad /\
            live h m /\ live h cipher /\ live h tag))
      (ensures
        (fun h0 _ h1 ->
            modifies (loc cipher |+| loc tag) h0 h1 /\
            Seq.equal (Seq.concat (as_seq h1 cipher) (as_seq h1 tag))
              (Spec.aead_encrypt (as_seq h0 k)
                  (as_seq h0 n)
                  (as_seq h0 m)
                  (as_seq h0 aad))))
*/
...
{
  Hacl_Impl_Chacha20Poly1305_aead_encrypt_chacha_poly(k, n1, aadlen, aad, mlen, m, cipher, tag);
}
...
/*
  ...
  -> Stack UInt32.t
      (requires
        (fun h ->
            eq_or_disjoint m c /\ live h k /\ live h n /\ live h aad /\ live h m /\
            live h c /\ live h mac))
      (ensures
        (fun h0 z h1 ->
            modifies (loc m) h0 h1 /\
            (let plain =
                Spec.aead_decrypt (as_seq h0 k)
                  (as_seq h0 n)
                  (as_seq h0 c)
                  (as_seq h0 mac)
                  (as_seq h0 aad)
              in
              match z with
              | 0ul -> Some? plain /\ as_seq h1 m == Some?.v plain
              | 1ul -> None? plain
              | _ -> false)))
*/
uint32_t
...
{
  return
    Hacl_Impl_Chacha20Poly1305_aead_decrypt_chacha_poly(k,
      n1,
      aadlen,
      aad,
      mlen,
