...
)
{
  ...
  {
    EverCrypt_Hash_hash(Spec_Hash_Definitions_SHA1, nkey, key, keylen);
  }
  uint8_t ipad[64U];
  for (uint32_t _i = 0U; _i < (uint32_t)64U; ++_i)
  ...
  uint32_t ll = datalen % (uint32_t)64U;
  uint32_t lb = datalen - ll;
  uint8_t *blocks = data;
  uint8_t *last1 = data + lb;
  EverCrypt_Hash_init(&acc);
  EverCrypt_Hash_update(&acc, ipad);
  EverCrypt_Hash_update_multi(&acc, blocks, lb);
  EverCrypt_Hash_update_last(&acc, last1, (uint64_t)((uint32_t)64U + datalen));
  uint8_t *tag = ipad;
  EverCrypt_Hash_finish(&acc, tag);
  uint8_t *inner = ipad;
  uint32_t totLen = (uint32_t)84U;
  EverCrypt_Hash_init(&acc);
  EverCrypt_Hash_update(&acc, opad);
  EverCrypt_Hash_update_last(&acc, inner, (uint64_t)totLen);
  EverCrypt_Hash_finish(&acc, mac);
}
...
)
{
  ...
  {
    EverCrypt_Hash_hash(Spec_Hash_Definitions_SHA2_256, nkey, key, keylen);
  }
  uint8_t ipad[64U];
  for (uint32_t _i = 0U; _i < (uint32_t)64U; ++_i)
    ipad[_i] = (uint8_t)0x36U;
...
)
{
  ...
  {
    EverCrypt_Hash_hash(Spec_Hash_Definitions_SHA2_384, nkey, key, keylen);
  }
  uint8_t ipad[128U];
  for (uint32_t _i = 0U; _i < (uint32_t)128U; ++_i)
    ipad[_i] = (uint8_t)0x36U;
  ...
  uint32_t ll = datalen % (uint32_t)128U;
  uint32_t lb = datalen - ll;
  uint8_t *blocks = data;
  uint8_t *last1 = data + lb;
  EverCrypt_Hash_init(&acc);
  EverCrypt_Hash_update(&acc, ipad);
  EverCrypt_Hash_update_multi(&acc, blocks, lb);
  EverCrypt_Hash_update_last(&acc, last1, (uint64_t)((uint32_t)128U + datalen));
  uint8_t *tag = ipad;
  EverCrypt_Hash_finish(&acc, tag);
  uint8_t *inner = ipad;
  uint32_t totLen = (uint32_t)176U;
  EverCrypt_Hash_init(&acc);
  EverCrypt_Hash_update(&acc, opad);
  EverCrypt_Hash_update_last(&acc, inner, (uint64_t)totLen);
  EverCrypt_Hash_finish(&acc, mac);
...
)
{
  ...
  {
    EverCrypt_Hash_hash(Spec_Hash_Definitions_SHA2_512, nkey, key, keylen);
  }
  uint8_t ipad[128U];
  for (uint32_t _i = 0U; _i < (uint32_t)128U; ++_i)
    ipad[_i] = (uint8_t)0x36U;
