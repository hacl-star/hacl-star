...
pub type pbn_mont_ctx_u32 <'a> = &'a mut [bignum::bignum::bn_mont_ctx_u32];
...
*/
pub fn
field_modulus_check(len: u32, n: &mut [u32]) ->
    bool
{
    ...
    Box<[bignum::bignum::bn_mont_ctx_u32]>
{
    ...
    let mut res: bignum::bignum::bn_mont_ctx_u32 =
        bignum::bignum::bn_mont_ctx_u32 { len, n: (*n11).into(), mu, r2: (*r21).into() };
    let mut buf: Box<[bignum::bignum::bn_mont_ctx_u32]> = vec![res].into_boxed_slice();
    buf
}
...
*/
pub fn
field_get_len(k: &mut [bignum::bignum::bn_mont_ctx_u32]) ->
    u32
{ (k[0usize]).len }
...
*/
pub fn
to_field(k: &mut [bignum::bignum::bn_mont_ctx_u32], a: &mut [u32], aM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    bignum::bignum::bn_to_mont_u32(len1, &mut uu____0.n, uu____0.mu, &mut uu____0.r2, a, aM)
}
...
*/
pub fn
from_field(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], a: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    bignum::bignum::bn_from_mont_u32(len1, &mut uu____0.n, uu____0.mu, aM, a)
}
...
*/
pub fn
add(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], bM: &mut [u32], cM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    ...
    bignum::bignum::bn_add_mod_n_u32(len1, &mut uu____0.n, &mut a_copy, &mut b_copy, cM)
}
...
*/
pub fn
sub(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], bM: &mut [u32], cM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    bignum::bignum::bn_sub_mod_n_u32(len1, &mut (k[0usize]).n, aM, bM, cM)
}
...
*/
pub fn
mul(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], bM: &mut [u32], cM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    bignum::bignum::bn_mont_mul_u32(len1, &mut uu____0.n, uu____0.mu, aM, bM, cM)
}
...
*/
pub fn
sqr(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], cM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    bignum::bignum::bn_mont_sqr_u32(len1, &mut uu____0.n, uu____0.mu, aM, cM)
}
...
*/
pub fn
one(k: &mut [bignum::bignum::bn_mont_ctx_u32], oneM: &mut [u32])
{
    let len1: u32 = crate::genericfield32::field_get_len(k);
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    bignum::bignum::bn_from_mont_u32(len1, &mut uu____0.n, uu____0.mu, &mut uu____0.r2, oneM)
}
...
pub fn
exp_consttime(
    k: &mut [bignum::bignum::bn_mont_ctx_u32],
    aM: &mut [u32],
    bBits: u32,
    b: &mut [u32],
    resM: &mut [u32]
)
{
    ...
    if bBits < 200u32
    {
        ...
        for i in 0u32..bBits
        {
            ...
            for i0 in 0u32..len1
            {
                ...
                (&mut aMc)[i0 as usize] ^= dummy
            };
            let ctx_n0: (&mut [u32], &mut [u32]) = ctx_r2.1.split_at_mut(0usize - len1 as usize);
            bignum::bignum::bn_mont_mul_u32(len1, ctx_n0.1, uu____0.mu, &mut aMc, resM, &mut aMc);
    ...
    else
    {
        ...
        krml::unroll_for!(
            ...
            1u32,
            {
                let t11: (&mut [u32], &mut [u32]) =
                    table.split_at_mut(i.wrapping_add(1u32).wrapping_mul(len1) as usize);
            ...
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    let res_j: (&mut [u32], &mut [u32]) =
                        table.split_at_mut(i0.wrapping_add(1u32).wrapping_mul(len1) as usize);
                    for i1 in 0u32..len1
                    {
        ...
        for i in 0u32..bBits.wrapping_div(4u32)
        {
            ...
            let ctx_n0: (&mut [u32], &mut [u32]) = ctx_r2.1.split_at_mut(0usize - len1 as usize);
            bignum::bignum::bn_mont_mul_u32(len1, ctx_n0.1, uu____0.mu, resM, &mut tmp0, resM)
        }
    }
}
    ...
    if bBits < 200u32
    {
        ...
        for i in 0u32..bBits
        {
            ...
            if bit != 0u32
            {
                let ctx_n0: (&mut [u32], &mut [u32]) =
                    ctx_r2.1.split_at_mut(0usize - len1 as usize);
                bignum::bignum::bn_mont_mul_u32(len1, ctx_n0.1, uu____0.mu, resM, &mut aMc, resM)
            };
            let ctx_n0: (&mut [u32], &mut [u32]) = ctx_r2.1.split_at_mut(0usize - len1 as usize);
            bignum::bignum::bn_mont_sqr_u32(len1, ctx_n0.1, uu____0.mu, &mut aMc, &mut aMc)
        }
    }
    else
    {
        ...
        if bBits.wrapping_rem(4u32) != 0u32
        {
            ...
            let a_bits_l: (&mut [u32], &mut [u32]) =
                table.split_at_mut(bits_l32.wrapping_mul(len1) as usize);
...
*/
pub fn
inverse(k: &mut [bignum::bignum::bn_mont_ctx_u32], aM: &mut [u32], aInvM: &mut [u32])
{
    let mut uu____0: &mut bignum::bignum::bn_mont_ctx_u32 = &mut k[0usize];
    ...
    crate::genericfield32::exp_vartime(k, aM, uu____0.len.wrapping_mul(32u32), &mut n2, aInvM)
}
