    ...
    u32
{
    ...
    if res1 == 0u32
    {
        ...
        if res2 == 0u32
        {
            ...
            crate::hkdf::extract_sha2_256(&mut o_secret, &mut shared, 32u32, &mut tmp1, len1);
            let mut label_exp: [u8; 3] = [0x65u8, 0x78u8, 0x70u8];
            let len2: u32 = 87u32;
            let mut tmp2: Box<[u8]> = vec![0u8; len2 as usize].into_boxed_slice();
            lowstar::endianness::store16_be(&mut (&mut tmp2)[0usize..], 32u32 as u16);
    ...
    u32
{
    ...
    let res: u32 =
        crate::hpke_curve51_cp256_sha256::setupBaseS(o_enc, o_ctx, skE, pkR, infolen, info);
    if res == 0u32
    {
    ...
    u32
{
    ...
    let res: u32 = crate::hpke_curve51_cp256_sha256::setupBaseR(o_ctx, pkE, skR, infolen, info);
    if res == 0u32
    {
        let mut nonce: [u8; 12] = [0u8; 12usize];
        let s: u64 = o_ctx.ctx_seq[0usize];
