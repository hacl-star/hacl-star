...
pub(crate) fn poly1305_init(ctx: &mut [u64], key: &mut [u8])
{
...
pub(crate) fn poly1305_finish(tag: &mut [u8], key: &mut [u8], ctx: &mut [u64])
{
    let acc: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
...
pub fn malloc(key: &mut [u8]) -> Box<[crate::mac_poly1305::state_t]>
{
    let mut buf: Box<[u8]> = vec![0u8; 16usize].into_boxed_slice();
    ...
    let mut s: crate::mac_poly1305::state_t =
        crate::mac_poly1305::state_t
        { block_state: (*block_state).into(), buf, total_len: 0u32 as u64, p_key: (*k·0).into() };
    let mut p: Box<[crate::mac_poly1305::state_t]> = vec![s].into_boxed_slice();
    p
}

pub fn reset(state: &mut [crate::mac_poly1305::state_t], key: &mut [u8])
{
...
*/
pub fn
update(state: &mut [crate::mac_poly1305::state_t], chunk: &mut [u8], chunk_len: u32) ->
...
pub fn digest(state: &mut [crate::mac_poly1305::state_t], output: &mut [u8])
{
    let block_state: &mut [u64] = &mut (state[0usize]).block_state;
