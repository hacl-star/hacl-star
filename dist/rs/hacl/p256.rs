...
#[inline] fn bn_is_zero_mask4(f: &[u64]) -> u64
{
...
#[inline] fn bn_is_zero_vartime4(f: &[u64]) -> bool
{
    let m: u64 = bn_is_zero_mask4(f);
    m == 0xFFFFFFFFFFFFFFFFu64
}

#[inline] fn bn_is_eq_mask4(a: &[u64], b: &[u64]) -> u64
{
    let mut mask: [u64; 1] = [0xFFFFFFFFFFFFFFFFu64; 1usize];
    krml::unroll_for!(
        4,
...
#[inline] fn bn_is_eq_vartime4(a: &[u64], b: &[u64]) -> bool
{
    let m: u64 = bn_is_eq_mask4(a, b);
    m == 0xFFFFFFFFFFFFFFFFu64
}

#[inline] fn bn_cmovznz4(res: &mut [u64], cin: u64, x: &[u64], y: &[u64])
{
...
#[inline] fn bn_add_mod4(res: &mut [u64], n: &[u64], x: &[u64], y: &[u64])
{
...
#[inline] fn bn_sub4(res: &mut [u64], x: &[u64], y: &[u64]) -> u64
{
    let mut c: [u64; 1] = [0u64; 1usize];
    {
        let t1: u64 = x[4u32.wrapping_mul(0u32) as usize];
...
#[inline] fn bn_sub_mod4(res: &mut [u64], n: &[u64], x: &[u64], y: &[u64])
{
    let mut c: [u64; 1] = [0u64; 1usize];
    {
        let t1: u64 = x[4u32.wrapping_mul(0u32) as usize];
...
#[inline] fn bn_mul4(res: &mut [u64], x: &[u64], y: &[u64])
{
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            ...
            let mut c: [u64; 1] = [0u64; 1usize];
            {
                ...
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i, bj, (&c)[0usize], res_i.1);
...
#[inline] fn bn_sqr4(res: &mut [u64], x: &[u64])
{
    (res[0usize..8usize]).copy_from_slice(&[0u64; 8usize]);
    krml::unroll_for!(
        4,
    ...
    let r: u64 = crate::bignum_base::bn_add_eq_len_u64(8u32, &a_copy, &b_copy, res);
    let c0: u64 = r;
    ...
    let r0: u64 = crate::bignum_base::bn_add_eq_len_u64(8u32, &a_copy0, &b_copy0, res);
    let c1: u64 = r0;
    lowstar::ignore::ignore::<u64>(c1)
}

#[inline] fn bn_to_bytes_be4(res: &mut [u8], f: &[u64])
{
...
#[inline] fn bn_from_bytes_be4(res: &mut [u64], b: &[u8])
{
    krml::unroll_for!(
        4,
        "i",
...
#[inline] fn bn2_to_bytes_be4(res: &mut [u8], x: &[u64], y: &[u64])
{
...
#[inline] fn bn_is_lt_prime_mask4(f: &[u64]) -> u64
{
    let mut tmp: [u64; 4] = [0u64; 4usize];
    make_prime(&mut tmp);
...
#[inline] fn feq_mask(a: &[u64], b: &[u64]) -> u64
{
    let r: u64 = bn_is_eq_mask4(a, b);
    r
}

#[inline] fn fadd(res: &mut [u64], x: &[u64], y: &[u64])
{
...
#[inline] fn fsub(res: &mut [u64], x: &[u64], y: &[u64])
{
    let mut n: [u64; 4] = [0u64; 4usize];
    make_prime(&mut n);
        ...
        1u32,
        {
            ...
            let mut c: [u64; 1] = [0u64; 1usize];
            {
                ...
                let res_i: (&mut [u64], &mut [u64]) =
                    res_j.1.split_at_mut(4u32.wrapping_mul(0u32) as usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i, qj, (&c)[0usize], res_i.1);
                let a_i0: u64 = (&n)[4u32.wrapping_mul(0u32).wrapping_add(1u32) as usize];
                let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i0, qj, (&c)[0usize], res_i0.1);
                let a_i1: u64 = (&n)[4u32.wrapping_mul(0u32).wrapping_add(2u32) as usize];
                let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i1, qj, (&c)[0usize], res_i1.1);
                let a_i2: u64 = (&n)[4u32.wrapping_mul(0u32).wrapping_add(3u32) as usize];
                let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i2, qj, (&c)[0usize], res_i2.1)
            };
            let r: u64 = (&c)[0usize];
            let c1: u64 = r;
...
#[inline] fn fmul(res: &mut [u64], x: &[u64], y: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
    bn_mul4(&mut tmp, x, y);
...
#[inline] fn fsqr(res: &mut [u64], x: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
    bn_sqr4(&mut tmp, x);
    mont_reduction(res, &mut tmp)
...
#[inline] fn from_mont(res: &mut [u64], a: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
...
#[inline] fn to_mont(res: &mut [u64], a: &[u64])
{
...
#[inline] fn fmul_by_b_coeff(res: &mut [u64], x: &[u64])
{
...
#[inline] fn fcube(res: &mut [u64], x: &[u64])
{
    fsqr(res, x);
...
#[inline] fn finv(res: &mut [u64], a: &[u64])
{
...
#[inline] fn fsqrt(res: &mut [u64], a: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
...
#[inline] fn is_point_at_inf_vartime(p: &[u64]) -> bool
{
    let pz: (&[u64], &[u64]) = p.split_at(8usize);
    bn_is_zero_vartime4(pz.1)
}

#[inline] fn to_aff_point(res: &mut [u64], p: &[u64])
{
...
#[inline] fn to_aff_point_x(res: &mut [u64], p: &[u64])
{
    let mut zinv: [u64; 4] = [0u64; 4usize];
    let px: (&[u64], &[u64]) = p.split_at(0usize);
...
#[inline] fn to_proj_point(res: &mut [u64], p: &[u64])
{
    let px: (&[u64], &[u64]) = p.split_at(0usize);
    let py: (&[u64], &[u64]) = px.1.split_at(4usize);
...
#[inline] fn aff_point_store(res: &mut [u8], p: &[u64])
{
    let px: (&[u64], &[u64]) = p.split_at(0usize);
    let py: (&[u64], &[u64]) = px.1.split_at(4usize);
...
#[inline] fn point_store(res: &mut [u8], p: &[u64])
{
...
#[inline] fn aff_point_load_vartime(p: &mut [u64], b: &[u8]) -> bool
{
...
#[inline] fn point_double(res: &mut [u64], p: &[u64])
{
    ...
    let mut x_copy1: [u64; 4] = [0u64; 4usize];
    ((&mut x_copy1)[0usize..4usize]).copy_from_slice(&(&x_copy)[0usize..4usize]);
    fadd(t4.0, &x_copy1, &x_copy);
    fmul(t4.1, z.0, z.1);
    ...
    ((&mut x_copy110)[0usize..4usize]).copy_from_slice(&(&x_copy19)[0usize..4usize]);
    fadd(z3.1, &x_copy110, &x_copy19);
    ...
    ((&mut x_copy111)[0usize..4usize]).copy_from_slice(&(&x_copy20)[0usize..4usize]);
    fadd(z3.1, &x_copy111, &x_copy20)
}

#[inline] fn point_add(res: &mut [u64], p: &[u64], q: &[u64])
{
...
#[inline] fn point_mul(res: &mut [u64], scalar: &[u64], p: &[u64])
{
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            ...
            usize
            +
            12usize]).copy_from_slice(&(&tmp)[0usize..12usize]);
            let t2: (&[u64], &[u64]) =
    ...
    for i in 0u32..64u32
    {
        ...
        let bits_l: u64 = crate::bignum_base::bn_get_bits_u64(4u32, scalar, k, 4u32);
        lowstar::ignore::ignore::<&[u64]>(&table);
        ((&mut tmp0)[0usize..12usize]).copy_from_slice(
            &(&(&table)[0usize..] as &[u64])[0usize..12usize]
        );
        krml::unroll_for!(
            15,
            "i0",
...
#[inline] fn point_mul_g(res: &mut [u64], scalar: &[u64])
{
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            ...
            let bits_l: u64 = crate::bignum_base::bn_get_bits_u64(1u32, r4.1, k, 4u32);
            ...
            let bits_l0: u64 = crate::bignum_base::bn_get_bits_u64(1u32, r4.0, k0, 4u32);
            lowstar::ignore::ignore::<&[u64]>(
            ...
            let bits_l1: u64 = crate::bignum_base::bn_get_bits_u64(1u32, r3.0, k1, 4u32);
            lowstar::ignore::ignore::<&[u64]>(
            ...
            let bits_l2: u64 = crate::bignum_base::bn_get_bits_u64(1u32, r2.0, k2, 4u32);
            lowstar::ignore::ignore::<&[u64]>(
...
#[inline] fn point_mul_double_g(res: &mut [u64], scalar1: &[u64], scalar2: &[u64], q2: &[u64])
{
    let mut q1: [u64; 12] = [0u64; 12usize];
    make_base_point(&mut q1);
    ...
    let mut tmp0: [u64; 12] = [0u64; 12usize];
    let i: u32 = 255u32;
    let bits_c: u64 = crate::bignum_base::bn_get_bits_u64(4u32, scalar1, i, 5u32);
    let bits_l32: u32 = bits_c as u32;
    let a_bits_l: &[u64] =
        &(&crate::p256_precomptable::precomp_basepoint_table_w5)[bits_l32.wrapping_mul(12u32)
        as
        usize..];
    (res[0usize..12usize]).copy_from_slice(&a_bits_l[0usize..12usize]);
    let i0: u32 = 255u32;
    let bits_c0: u64 = crate::bignum_base::bn_get_bits_u64(4u32, scalar2, i0, 5u32);
    ...
    for i1 in 0u32..51u32
    {
        ...
        let bits_l: u64 = crate::bignum_base::bn_get_bits_u64(4u32, scalar2, k, 5u32);
        lowstar::ignore::ignore::<&[u64]>(&table2);
        ...
        let bits_l0: u64 = crate::bignum_base::bn_get_bits_u64(4u32, scalar1, k0, 5u32);
        lowstar::ignore::ignore::<&[u64]>(
            &crate::p256_precomptable::precomp_basepoint_table_w5
        );
        let bits_l322: u32 = bits_l0 as u32;
        let a_bits_l2: &[u64] =
            &(&crate::p256_precomptable::precomp_basepoint_table_w5)[bits_l322.wrapping_mul(
                12u32
            )
            as
            usize..];
...
#[inline] fn bn_is_lt_order_mask4(f: &[u64]) -> u64
{
    let mut tmp: [u64; 4] = [0u64; 4usize];
    make_order(&mut tmp);
    let mut y_copy: [u64; 4] = [0u64; 4usize];
...
#[inline] fn bn_is_lt_order_and_gt_zero_mask4(f: &[u64]) -> u64
{
...
#[inline] fn qmod_short(res: &mut [u64], x: &[u64])
{
    let mut tmp: [u64; 4] = [0u64; 4usize];
    make_order(&mut tmp);
    let mut y_copy: [u64; 4] = [0u64; 4usize];
...
#[inline] fn qadd(res: &mut [u64], x: &[u64], y: &[u64])
{
    let mut n: [u64; 4] = [0u64; 4usize];
    make_order(&mut n);
    bn_add_mod4(res, &n, x, y)
...
#[inline] fn from_qmont(res: &mut [u64], x: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
...
#[inline] fn qmul(res: &mut [u64], x: &[u64], y: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
    bn_mul4(&mut tmp, x, y);
    qmont_reduction(res, &mut tmp)
...
#[inline] fn qsqr(res: &mut [u64], x: &[u64])
{
    let mut tmp: [u64; 8] = [0u64; 8usize];
    bn_sqr4(&mut tmp, x);
    qmont_reduction(res, &mut tmp)
...
#[inline] fn qinv(res: &mut [u64], r: &[u64])
{
...
#[inline] fn qmul_mont(sinv: &[u64], b: &[u64], res: &mut [u64])
{
    let mut tmp: [u64; 4] = [0u64; 4usize];
