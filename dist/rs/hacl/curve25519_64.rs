...
#[inline] fn fmul(out: &mut [u64], f1: &[u64], f2: &mut [u64], tmp: &mut [u64])
{
    ...
    else
    {
        lowstar::ignore::ignore::<u64>(
            vale::stdcalls_x64_fmul::fmul_e(&mut tmp[0usize..], f1, out, f2)
        )
    }
}
...
#[inline] fn fsqr2(out: &mut [u64], f: &[u64], tmp: &mut [u64])
{
    ...
    { lowstar::ignore::ignore::<u64>(vale::stdcalls_x64_fsqr::fsqr2_e(tmp, f, out)) }
}

#[inline] fn cswap2(bit: u64, p1: &mut [u64], p2: &mut [u64])
{
    if evercrypt::targetconfig::hacl_can_compile_inline_asm
    { vale::inline_x64_fswap_inline::cswap2(bit, p1, p2) }
    else
    { lowstar::ignore::ignore::<u64>(vale::stdcalls_x64_fswap::cswap2_e(bit, p1, p2)) }
}
...
fn point_add_and_double(q: &mut [u64], p01_tmp1: &mut [u64], tmp2: &mut [u64])
{
    ...
    crate::curve25519_64::fadd(b.0, z2.0, z2.1);
    crate::curve25519_64::fsub(b.1, z2.0, z2.1);
    ...
    crate::curve25519_64::fadd(c.1, z31.0, z31.1);
...
fn point_double(nq: &mut [u64], tmp1: &mut [u64], tmp2: &mut [u64])
{
    ...
    crate::curve25519_64::fadd(b.0, z2.0, z2.1);
    crate::curve25519_64::fsub(b.1, z2.0, z2.1);
    crate::curve25519_64::fsqr2(dc.1, dc.0, tmp2);
    let d: (&mut [u64], &mut [u64]) = (dc.1).split_at_mut(0usize);
    let c: (&mut [u64], &mut [u64]) = (d.1).split_at_mut(4usize);
...
fn montgomery_ladder(out: &mut [u64], key: &[u8], init: &mut [u64])
{
    ...
    let nq_p1: (&mut [u64], &mut [u64]) = (nq.1).split_at_mut(8usize);
    crate::curve25519_64::cswap2(1u64, nq_p1.0, nq_p1.1);
    let p01_tmp11: (&mut [u64], &mut [u64]) = (p01_tmp1.1).split_at_mut(0usize);
    crate::curve25519_64::point_add_and_double(init, p01_tmp11.1, &mut tmp2);
    swap.1[0usize] = 1u64;
    for i in 0u32..251u32
    {
        ...
        crate::curve25519_64::cswap2(sw, nq_p11.0, nq_p11.1);
        crate::curve25519_64::point_add_and_double(init, p01_tmp12.1, &mut tmp2);
        swap1.1[0usize] = bit
    };
...
fn finv(o: &mut [u64], i: &mut [u64], tmp: &mut [u64])
{
    ...
    crate::curve25519_64::fsquare_times(b1.0, i, tmp1.1, 1u32);
    crate::curve25519_64::fsquare_times(t01.1, b1.0, tmp1.1, 2u32);
    crate::curve25519_64::fmul(t01.0, t01.1, i, tmp);
    let mut f2_copy: [u64; 4] = [0u64; 4usize];
    ((&mut f2_copy)[0usize..4usize]).copy_from_slice(&b1.0[0usize..4usize]);
    crate::curve25519_64::fmul(b1.0, t01.0, &mut f2_copy, tmp);
    let tmp11: (&mut [u64], &mut [u64]) = tmp.split_at_mut(0usize);
    crate::curve25519_64::fsquare_times(t01.1, b1.0, tmp11.1, 1u32);
    let mut f2_copy0: [u64; 4] = [0u64; 4usize];
    ((&mut f2_copy0)[0usize..4usize]).copy_from_slice(&t01.0[0usize..4usize]);
    crate::curve25519_64::fmul(t01.0, t01.1, &mut f2_copy0, tmp);
    let tmp12: (&mut [u64], &mut [u64]) = tmp.split_at_mut(0usize);
    crate::curve25519_64::fsquare_times(t01.1, t01.0, tmp12.1, 5u32);
    let mut f2_copy1: [u64; 4] = [0u64; 4usize];
    ((&mut f2_copy1)[0usize..4usize]).copy_from_slice(&t01.0[0usize..4usize]);
    crate::curve25519_64::fmul(t01.0, t01.1, &mut f2_copy1, tmp);
    ...
    crate::curve25519_64::fsquare_times(t010.1, c1.0, tmp10.1, 10u32);
    crate::curve25519_64::fmul(c1.1, t010.1, c1.0, tmp);
    ...
    crate::curve25519_64::fsquare_times(t010.1, c1.0, tmp13.1, 50u32);
    crate::curve25519_64::fmul(c1.1, t010.1, c1.0, tmp);
...
fn store_felem(b: &mut [u64], f: &mut [u64])
{
    ...
    crate::curve25519_64::add_scalar(f, f, 19u64.wrapping_mul(top_bit));
    let f30: u64 = f[3usize];
    let top_bit0: u64 = f30.wrapping_shr(63u32);
    f[3usize] = f30 & 0x7fffffffffffffffu64;
    crate::curve25519_64::add_scalar(f, f, 19u64.wrapping_mul(top_bit0));
    let f0: u64 = f[0usize];
    let f1: u64 = f[1usize];
    let f2: u64 = f[2usize];
