...
*/
pub fn
add(a: &mut [u64], b: &mut [u64], res: &mut [u64]) ->
    u64
{
...
*/
pub fn
sub(a: &mut [u64], b: &mut [u64], res: &mut [u64]) ->
    u64
{
    let mut c: [u64; 1] = [0u64; 1usize];
    {
...
*/
pub fn
add_mod(n: &mut [u64], a: &mut [u64], b: &mut [u64], res: &mut [u64])
{
    let mut c: [u64; 1] = [0u64; 1usize];
    {
        let t1: u64 = a[4u32.wrapping_mul(0u32) as usize];
...
*/
pub fn
sub_mod(n: &mut [u64], a: &mut [u64], b: &mut [u64], res: &mut [u64])
{
    let mut c: [u64; 1] = [0u64; 1usize];
    {
        let t1: u64 = a[4u32.wrapping_mul(0u32) as usize];
...
*/
pub fn
mul(a: &mut [u64], b: &mut [u64], res: &mut [u64])
{
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            ...
            let mut c: [u64; 1] = [0u64; 1usize];
            {
                ...
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i, bj, (&mut c)[0usize], res_i.1);
                let a_i0: u64 = a[4u32.wrapping_mul(0u32).wrapping_add(1u32) as usize];
                let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i0, bj, (&mut c)[0usize], res_i0.1);
                let a_i1: u64 = a[4u32.wrapping_mul(0u32).wrapping_add(2u32) as usize];
                let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i1, bj, (&mut c)[0usize], res_i1.1);
                let a_i2: u64 = a[4u32.wrapping_mul(0u32).wrapping_add(3u32) as usize];
                let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i2, bj, (&mut c)[0usize], res_i2.1)
            };
...
sqr(a: &mut [u64], res: &mut [u64])
{
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            ...
            for i0 in 0u32..i.wrapping_div(4u32)
            {
                ...
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i, a_j, (&mut c)[0usize], res_i.1);
                let a_i0: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(1u32) as usize];
                let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i0, a_j, (&mut c)[0usize], res_i0.1);
                let a_i1: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(2u32) as usize];
                let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i1, a_j, (&mut c)[0usize], res_i1.1);
                let a_i2: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(3u32) as usize];
                let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
                (&mut c)[0usize] =
                    crate::bignum_base::mul_wide_add2_u64(a_i2, a_j, (&mut c)[0usize], res_i2.1)
            };
    ...
    let r: u64 = crate::bignum_base::bn_add_eq_len_u64(8u32, &mut a_copy, &mut b_copy, res);
    let c0: u64 = r;
    ...
    let r0: u64 = crate::bignum_base::bn_add_eq_len_u64(8u32, &mut a_copy0, &mut b_copy0, res);
    let c1: u64 = r0;
...
#[inline] fn precompr2(nBits: u32, n: &mut [u64], res: &mut [u64])
{
    ...
    for _i in 0u32..512u32.wrapping_sub(nBits)
    {
        ...
        crate::bignum256::add_mod(n, &mut a_copy, &mut b_copy, res)
    }
}
...
#[inline] fn to(n: &mut [u64], nInv: u64, r2: &mut [u64], a: &mut [u64], aM: &mut [u64])
{
    ...
    crate::bignum256::mul(a, r2, &mut c);
    crate::bignum256::reduction(n, nInv, &mut c, aM)
}
...
#[inline] fn from(n: &mut [u64], nInv_u64: u64, aM: &mut [u64], a: &mut [u64])
{
    ...
    crate::bignum256::reduction(n, nInv_u64, &mut tmp, a)
}
...
#[inline] fn areduction(n: &mut [u64], nInv: u64, c: &mut [u64], res: &mut [u64])
{
    ...
    let c00: u64 = (&mut c0)[0usize];
    let mut tmp: [u64; 4] = [0u64; 4usize];
    let c1: u64 = crate::bignum256::sub(res, n, &mut tmp);
    lowstar::ignore::ignore::<u64>(c1);
...
{
    let mut c: [u64; 8] = [0u64; 8usize];
    crate::bignum256::mul(aM, bM, &mut c);
    crate::bignum256::areduction(n, nInv_u64, &mut c, resM)
}

#[inline] fn amont_sqr(n: &mut [u64], nInv_u64: u64, aM: &mut [u64], resM: &mut [u64])
{
    let mut c: [u64; 8] = [0u64; 8usize];
    crate::bignum256::sqr(aM, &mut c);
    crate::bignum256::areduction(n, nInv_u64, &mut c, resM)
}
...
)
{
    ...
    crate::bignum256::areduction(n, mu, &mut a1, &mut a_mod);
    crate::bignum256::to(n, mu, r2, &mut a_mod, res)
}
...
*/
pub fn
r#mod(n: &mut [u64], a: &mut [u64], res: &mut [u64]) ->
    bool
{
    ...
    let nBits: u32 = 64u32.wrapping_mul(crate::bignum_base::bn_get_top_index_u64(4u32, n) as u32);
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64
    {
        let mut r2: [u64; 4] = [0u64; 4usize];
        crate::bignum256::precompr2(nBits, n, &mut r2);
        let mu: u64 = crate::bignum::mod_inv_uint64(n[0usize]);
        crate::bignum256::bn_slow_precomp(n, mu, &mut r2, a, res)
    }
    else
    { (res[0usize..4usize]).copy_from_slice(&[0u64; 4usize]) };
...
)
{
    if bBits < 200u32
    {
        ...
        crate::bignum256::to(n, mu, r2, a, &mut aM);
        ...
        crate::bignum256::from(ctx_r2.0, mu, ctx_r2.1, &mut resM);
        ...
        for i in 0u32..bBits
        {
            ...
            if bit != 0u64
            {
                ...
                crate::bignum256::amont_mul(ctx_n0.1, mu, &mut aM_copy, &mut aM, &mut resM);
                lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
            };
            let mut aM_copy: [u64; 4] = [0u64; 4usize];
            ((&mut aM_copy)[0usize..4usize]).copy_from_slice(&(&mut aM)[0usize..4usize]);
            let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum256::amont_sqr(ctx_n0.1, mu, &mut aM_copy, &mut aM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
        };
        crate::bignum256::from(n, mu, &mut resM, res)
    }
    else
    {
        let mut aM: [u64; 4] = [0u64; 4usize];
        ...
        let ctx_n: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
        let ctx_r2: (&mut [u64], &mut [u64]) = ctx_n.1.split_at_mut(4usize);
        crate::bignum256::from(ctx_r2.0, mu, ctx_r2.1, t1.0);
        lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
        ...
        krml::unroll_for!(
            ...
            1u32,
            {
                let t11: (&mut [u64], &mut [u64]) =
                    table.split_at_mut(i.wrapping_add(1u32).wrapping_mul(4u32) as usize);
                let mut aM_copy: [u64; 4] = [0u64; 4usize];
                ((&mut aM_copy)[0usize..4usize]).copy_from_slice(&t11.1[0usize..4usize]);
                let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                crate::bignum256::amont_sqr(ctx_n0.1, mu, &mut aM_copy, &mut tmp);
                lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
                ...
                crate::bignum256::amont_mul(ctx_n1.1, mu, &mut aM_copy0, t2.1, &mut tmp);
                lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
        ...
        if bBits.wrapping_rem(4u32) != 0u32
        {
            ...
            let a_bits_l: (&mut [u64], &mut [u64]) =
                table.split_at_mut(bits_l32.wrapping_mul(4u32) as usize);
            ((&mut resM)[0usize..4usize]).copy_from_slice(&a_bits_l.1[0usize..4usize])
        }
        else
        {
            let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            let ctx_r20: (&mut [u64], &mut [u64]) = ctx_n0.1.split_at_mut(4usize);
            crate::bignum256::from(ctx_r20.0, mu, ctx_r20.1, &mut resM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
        };
        ...
        for i in 0u32..bBits.wrapping_div(4u32)
        {
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    ((&mut aM_copy)[0usize..4usize]).copy_from_slice(&(&mut resM)[0usize..4usize]);
                    let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                    crate::bignum256::amont_sqr(ctx_n0.1, mu, &mut aM_copy, &mut resM);
                    lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
                }
            );
            let k: u32 =
            ...
            ((&mut aM_copy)[0usize..4usize]).copy_from_slice(&(&mut resM)[0usize..4usize]);
            let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum256::amont_mul(ctx_n0.1, mu, &mut aM_copy, &mut tmp0, &mut resM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
        };
        crate::bignum256::from(n, mu, &mut resM, res)
    }
...
)
{
    if bBits < 200u32
    {
        ...
        for i in 0u32..bBits
        {
            ...
            ((&mut aM_copy)[0usize..4usize]).copy_from_slice(&(&mut aM)[0usize..4usize]);
            let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum256::amont_mul(ctx_n0.1, mu, &mut aM_copy, &mut resM, &mut aM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
            let mut aM_copy0: [u64; 4] = [0u64; 4usize];
            ((&mut aM_copy0)[0usize..4usize]).copy_from_slice(&(&mut resM)[0usize..4usize]);
            let ctx_n1: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum256::amont_sqr(ctx_n1.1, mu, &mut aM_copy0, &mut resM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
            (&mut sw)[0usize] = bit
        };
            ...
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    let res_j: (&mut [u64], &mut [u64]) =
                        table.split_at_mut(i0.wrapping_add(1u32).wrapping_mul(4u32) as usize);
                    krml::unroll_for!(
                        4,
                        "i1",
                        0u32,
...
)
{
    ...
    crate::bignum256::precompr2(nBits, n, &mut r2);
    let mu: u64 = crate::bignum::mod_inv_uint64(n[0usize]);
    crate::bignum256::exp_vartime_precomp(n, mu, &mut r2, a, bBits, b, res)
}
...
)
{
    ...
    crate::bignum256::precompr2(nBits, n, &mut r2);
    let mu: u64 = crate::bignum::mod_inv_uint64(n[0usize]);
    crate::bignum256::exp_consttime_precomp(n, mu, &mut r2, a, bBits, b, res)
}
...
*/
pub fn
mod_exp_vartime(n: &mut [u64], a: &mut [u64], bBits: u32, b: &mut [u64], res: &mut [u64]) ->
    bool
{
    let is_valid_m: u64 = crate::bignum256::exp_check(n, a, bBits, b);
    let nBits: u32 = 64u32.wrapping_mul(crate::bignum_base::bn_get_top_index_u64(4u32, n) as u32);
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64
    { crate::bignum256::exp_vartime(nBits, n, a, bBits, b, res) }
    else
    { (res[0usize..4usize]).copy_from_slice(&[0u64; 4usize]) };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
}
...
*/
pub fn
mod_exp_consttime(n: &mut [u64], a: &mut [u64], bBits: u32, b: &mut [u64], res: &mut [u64]) ->
    bool
{
    let is_valid_m: u64 = crate::bignum256::exp_check(n, a, bBits, b);
    let nBits: u32 = 64u32.wrapping_mul(crate::bignum_base::bn_get_top_index_u64(4u32, n) as u32);
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64
    { crate::bignum256::exp_consttime(nBits, n, a, bBits, b, res) }
    else
    { (res[0usize..4usize]).copy_from_slice(&[0u64; 4usize]) };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
}
...
*/
pub fn
mod_inv_prime_vartime(n: &mut [u64], a: &mut [u64], res: &mut [u64]) ->
    bool
{
    let mut one: [u64; 4] = [0u64; 4usize];
    ((&mut one)[0usize..4usize]).copy_from_slice(&[0u64; 4usize]);
    ...
    krml::unroll_for!(
        ...
        1u32,
        {
            let uu____0: u64 = fstar::uint64::eq_mask(a[i as usize], (&mut bn_zero)[i as usize]);
    ...
    if is_valid_m == 0xFFFFFFFFFFFFFFFFu64
    {
        ...
        crate::bignum256::exp_vartime(nBits, n, a, 256u32, &mut n2, res)
    }
    else
    { (res[0usize..4usize]).copy_from_slice(&[0u64; 4usize]) };
    is_valid_m == 0xFFFFFFFFFFFFFFFFu64
    ...
    Box<[crate::bignum::bn_mont_ctx_u64]>
{
    ...
    let nBits: u32 = 64u32.wrapping_mul(crate::bignum_base::bn_get_top_index_u64(4u32, n) as u32);
    crate::bignum256::precompr2(nBits, n, r21);
    let mu: u64 = crate::bignum::mod_inv_uint64(n[0usize]);
    let mut res: crate::bignum::bn_mont_ctx_u64 =
        crate::bignum::bn_mont_ctx_u64 { len: 4u32, n: (*n11).into(), mu, r2: (*r21).into() };
    let mut buf: Box<[crate::bignum::bn_mont_ctx_u64]> = vec![res].into_boxed_slice();
    buf
}
...
*/
pub fn
mod_precomp(k: &mut [crate::bignum::bn_mont_ctx_u64], a: &mut [u64], res: &mut [u64])
{
    let n: &mut [u64] = &mut (k[0usize]).n;
    let mu: u64 = (k[0usize]).mu;
    let r2: &mut [u64] = &mut (k[0usize]).r2;
    crate::bignum256::bn_slow_precomp(n, mu, r2, a, res)
}
...
mod_exp_vartime_precomp(
    k: &mut [crate::bignum::bn_mont_ctx_u64],
    a: &mut [u64],
    bBits: u32,
    b: &mut [u64],
    res: &mut [u64]
)
{
    ...
    let mu: u64 = (k[0usize]).mu;
    let r2: &mut [u64] = &mut (k[0usize]).r2;
    crate::bignum256::exp_vartime_precomp(n, mu, r2, a, bBits, b, res)
}
...
)
{
    ...
    let mu: u64 = (k[0usize]).mu;
    let r2: &mut [u64] = &mut (k[0usize]).r2;
    crate::bignum256::exp_consttime_precomp(n, mu, r2, a, bBits, b, res)
}
...
)
{
    ...
    let c2: u64 = c1;
    lowstar::ignore::ignore::<u64>(c2);
    crate::bignum256::exp_vartime_precomp(n, mu, r2, a, 256u32, &mut n2, res)
}
    ...
    Box<[u64]>
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                let bj: (&mut [u8], &mut [u8]) = tmp.split_at_mut(i.wrapping_mul(8u32) as usize);
...
*/
pub fn
bn_to_bytes_be(b: &mut [u64], res: &mut [u8])
{
...
*/
pub fn
bn_to_bytes_le(b: &mut [u64], res: &mut [u8])
{
    let mut tmp: [u8; 32] = [0u8; 32usize];
    lowstar::ignore::ignore::<&mut [u8]>(&mut tmp);
    krml::unroll_for!(
