...
)
{
    ...
    { crate::bignum_base::bn_mul_u32(aLen, a, aLen, b, res) }
    else
    {
        ...
        let c0: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, a1.0, a1.1, tmp·.1);
        let c1: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, a1.1, a1.0, t1.0);
        for i in 0u32..len2
        {
            let x: u32 =
        ...
        let c00: u32 = c0;
        let c01: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, b1.0, b1.1, tmp·.1);
        let c10: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, b1.1, b1.0, tmp·.0);
        for i in 0u32..len2
        {
            let x: u32 =
        ...
        crate::bignum::bn_karatsuba_mul_uint32(len2, t1.0, tmp·.0, tmp1.1, tmp1.0);
        let r01: (&mut [u32], &mut [u32]) = res.split_at_mut(0usize);
        let r23: (&mut [u32], &mut [u32]) = r01.1.split_at_mut(aLen as usize);
        crate::bignum::bn_karatsuba_mul_uint32(len2, a1.0, b1.0, tmp1.1, r23.0);
        crate::bignum::bn_karatsuba_mul_uint32(len2, a1.1, b1.1, tmp1.1, r23.1);
        lowstar::ignore::ignore::<&mut [u32]>(res);
        lowstar::ignore::ignore::<&mut [u32]>(tmp);
        ...
        let c2: u32 = crate::bignum_base::bn_add_eq_len_u32(aLen, r231.0, r231.1, t231.0);
        let c_sign: u32 = c00 ^ c11;
        let c3: u32 = crate::bignum_base::bn_sub_eq_len_u32(aLen, t231.0, t45.0, t67.1);
        let c31: u32 = c2.wrapping_sub(c3);
        let c4: u32 = crate::bignum_base::bn_add_eq_len_u32(aLen, t231.0, t45.0, t67.0);
        ...
        let r1: u32 = crate::bignum_base::bn_add_eq_len_u32(aLen, &mut a_copy, &mut b_copy, r.1);
        let r10: u32 = r1;
        let c: u32 = r10;
        let c6: u32 = c;
        ...
        let r11: u32 =
            if 1u32 < aLen.wrapping_add(aLen).wrapping_sub(aLen.wrapping_add(aLen2))
            {
                ...
                ).wrapping_div(4u32)
                {
                    ...
                    (&mut c8)[0usize] =
                        lib::inttypes_intrinsics::add_carry_u32(
                            (&mut c8)[0usize],
                            t11,
                            0u32,
                            res_i.1
...
{
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32
    { crate::bignum_base::bn_mul_u64(aLen, a, aLen, b, res) }
    else
    {
        let len2: u32 = aLen.wrapping_div(2u32);
        ...
        let c0: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, a1.0, a1.1, tmp·.1);
        let c1: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, a1.1, a1.0, t1.0);
        for i in 0u32..len2
        {
            let x: u64 =
        ...
        let c00: u64 = c0;
        let c01: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, b1.0, b1.1, tmp·.1);
        let c10: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, b1.1, b1.0, tmp·.0);
        for i in 0u32..len2
        {
            let x: u64 =
        ...
        let tmp1: (&mut [u64], &mut [u64]) =
            t23.1.split_at_mut(aLen.wrapping_add(aLen) as usize - aLen as usize);
        crate::bignum::bn_karatsuba_mul_uint64(len2, t1.0, tmp·.0, tmp1.1, tmp1.0);
        let r01: (&mut [u64], &mut [u64]) = res.split_at_mut(0usize);
        let r23: (&mut [u64], &mut [u64]) = r01.1.split_at_mut(aLen as usize);
        crate::bignum::bn_karatsuba_mul_uint64(len2, a1.0, b1.0, tmp1.1, r23.0);
        crate::bignum::bn_karatsuba_mul_uint64(len2, a1.1, b1.1, tmp1.1, r23.1);
        lowstar::ignore::ignore::<&mut [u64]>(res);
        lowstar::ignore::ignore::<&mut [u64]>(tmp);
        ...
        let t67: (&mut [u64], &mut [u64]) =
            t45.1.split_at_mut(3u32.wrapping_mul(aLen) as usize - 2u32.wrapping_mul(aLen) as usize);
        let c2: u64 = crate::bignum_base::bn_add_eq_len_u64(aLen, r231.0, r231.1, t231.0);
        let c_sign: u64 = c00 ^ c11;
        let c3: u64 = crate::bignum_base::bn_sub_eq_len_u64(aLen, t231.0, t45.0, t67.1);
        let c31: u64 = c2.wrapping_sub(c3);
        let c4: u64 = crate::bignum_base::bn_add_eq_len_u64(aLen, t231.0, t45.0, t67.0);
        ...
        ((&mut a_copy)[0usize..aLen as usize]).copy_from_slice(&r.1[0usize..aLen as usize]);
        ((&mut b_copy)[0usize..aLen as usize]).copy_from_slice(&t67.0[0usize..aLen as usize]);
        let r1: u64 = crate::bignum_base::bn_add_eq_len_u64(aLen, &mut a_copy, &mut b_copy, r.1);
        let r10: u64 = r1;
        let c: u64 = r10;
        let c6: u64 = c;
...
pub(crate) fn bn_karatsuba_sqr_uint32(
    aLen: u32,
    a: &mut [u32],
    tmp: &mut [u32],
    res: &mut [u32]
...
{
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32
    { crate::bignum_base::bn_sqr_u32(aLen, a, res) }
    else
    {
        let len2: u32 = aLen.wrapping_div(2u32);
        let a0: (&mut [u32], &mut [u32]) = a.split_at_mut(0usize);
        ...
        let c0: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, a1.0, a1.1, tmp·.1);
        let c1: u32 = crate::bignum_base::bn_sub_eq_len_u32(len2, a1.1, a1.0, tmp·.0);
        for i in 0u32..len2
        {
            let x: u32 =
        ...
        let tmp1: (&mut [u32], &mut [u32]) =
            t23.1.split_at_mut(aLen.wrapping_add(aLen) as usize - aLen as usize);
        crate::bignum::bn_karatsuba_sqr_uint32(len2, tmp·.0, tmp1.1, tmp1.0);
        let r01: (&mut [u32], &mut [u32]) = res.split_at_mut(0usize);
        let r23: (&mut [u32], &mut [u32]) = r01.1.split_at_mut(aLen as usize);
        crate::bignum::bn_karatsuba_sqr_uint32(len2, a1.0, tmp1.1, r23.0);
        crate::bignum::bn_karatsuba_sqr_uint32(len2, a1.1, tmp1.1, r23.1);
        lowstar::ignore::ignore::<&mut [u32]>(res);
        lowstar::ignore::ignore::<&mut [u32]>(tmp);
        let r011: (&mut [u32], &mut [u32]) = res.split_at_mut(0usize);
        let r231: (&mut [u32], &mut [u32]) = r011.1.split_at_mut(aLen as usize);
        ...
        let t45: (&mut [u32], &mut [u32]) =
            t231.1.split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let c2: u32 = crate::bignum_base::bn_add_eq_len_u32(aLen, r231.0, r231.1, t231.0);
        let c3: u32 = crate::bignum_base::bn_sub_eq_len_u32(aLen, t231.0, t45.0, t45.1);
...
pub(crate) fn bn_karatsuba_sqr_uint64(
    aLen: u32,
    a: &mut [u64],
    tmp: &mut [u64],
    res: &mut [u64]
...
{
    if aLen < 32u32 || aLen.wrapping_rem(2u32) == 1u32
    { crate::bignum_base::bn_sqr_u64(aLen, a, res) }
    else
    {
        let len2: u32 = aLen.wrapping_div(2u32);
        let a0: (&mut [u64], &mut [u64]) = a.split_at_mut(0usize);
        ...
        let c0: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, a1.0, a1.1, tmp·.1);
        let c1: u64 = crate::bignum_base::bn_sub_eq_len_u64(len2, a1.1, a1.0, tmp·.0);
        for i in 0u32..len2
        {
            let x: u64 =
        ...
        let tmp1: (&mut [u64], &mut [u64]) =
            t23.1.split_at_mut(aLen.wrapping_add(aLen) as usize - aLen as usize);
        crate::bignum::bn_karatsuba_sqr_uint64(len2, tmp·.0, tmp1.1, tmp1.0);
        let r01: (&mut [u64], &mut [u64]) = res.split_at_mut(0usize);
        let r23: (&mut [u64], &mut [u64]) = r01.1.split_at_mut(aLen as usize);
        crate::bignum::bn_karatsuba_sqr_uint64(len2, a1.0, tmp1.1, r23.0);
        crate::bignum::bn_karatsuba_sqr_uint64(len2, a1.1, tmp1.1, r23.1);
        lowstar::ignore::ignore::<&mut [u64]>(res);
        lowstar::ignore::ignore::<&mut [u64]>(tmp);
        let r011: (&mut [u64], &mut [u64]) = res.split_at_mut(0usize);
        let r231: (&mut [u64], &mut [u64]) = r011.1.split_at_mut(aLen as usize);
        ...
        let t45: (&mut [u64], &mut [u64]) =
            t231.1.split_at_mut(2u32.wrapping_mul(aLen) as usize - aLen as usize);
        let c2: u64 = crate::bignum_base::bn_add_eq_len_u64(aLen, r231.0, r231.1, t231.0);
        let c3: u64 = crate::bignum_base::bn_sub_eq_len_u64(aLen, t231.0, t45.0, t45.1);
...
*/
pub fn
bn_add_mod_n_u32(len1: u32, n: &mut [u32], a: &mut [u32], b: &mut [u32], res: &mut [u32])
{
    ...
    for i in 0u32..len1.wrapping_div(4u32)
    {
        ...
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c)[0usize], t1, t2, res_i.1);
        ...
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c)[0usize], t10, t20, res_i0.1);
        ...
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c)[0usize], t11, t21, res_i1.1);
        ...
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c)[0usize], t12, t22, res_i2.1)
    };
    ...
    for i in 0u32..len1.wrapping_div(4u32)
    {
        ...
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c1)[0usize], t1, t2, res_i.1);
        ...
        let res_i0: (&mut [u32], &mut [u32]) = res_i.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c1)[0usize], t10, t20, res_i0.1);
        ...
        let res_i1: (&mut [u32], &mut [u32]) = res_i0.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c1)[0usize], t11, t21, res_i1.1);
        ...
        let res_i2: (&mut [u32], &mut [u32]) = res_i1.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c1)[0usize], t12, t22, res_i2.1)
    };
    for i in len1.wrapping_div(4u32).wrapping_mul(4u32)..len1
    {
...
*/
pub fn
bn_add_mod_n_u64(len1: u32, n: &mut [u64], a: &mut [u64], b: &mut [u64], res: &mut [u64])
{
...
*/
pub fn
bn_sub_mod_n_u32(len1: u32, n: &mut [u32], a: &mut [u32], b: &mut [u32], res: &mut [u32])
{
    let mut c: [u32; 1] = [0u32; 1usize];
    for i in 0u32..len1.wrapping_div(4u32)
    {
        ...
        let res_i: (&mut [u32], &mut [u32]) = res.split_at_mut(4u32.wrapping_mul(i) as usize);
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c)[0usize], t1, t2, res_i.1);
        let t10: u32 = a[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
        let t20: u32 = b[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
        let res_i0: (&mut [u32], &mut [u32]) = res_i.1.split_at_mut(1usize);
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c)[0usize], t10, t20, res_i0.1);
        let t11: u32 = a[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
        let t21: u32 = b[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
        let res_i1: (&mut [u32], &mut [u32]) = res_i0.1.split_at_mut(1usize);
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c)[0usize], t11, t21, res_i1.1);
        let t12: u32 = a[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
        let t22: u32 = b[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
        let res_i2: (&mut [u32], &mut [u32]) = res_i1.1.split_at_mut(1usize);
        (&mut c)[0usize] =
            lib::inttypes_intrinsics::sub_borrow_u32((&mut c)[0usize], t12, t22, res_i2.1)
    };
    for i in len1.wrapping_div(4u32).wrapping_mul(4u32)..len1
    {
        let t1: u32 = a[i as usize];
    ...
    for i in 0u32..len1.wrapping_div(4u32)
    {
        ...
        let res_i: (&mut [u32], &mut [u32]) = tmp.split_at_mut(4u32.wrapping_mul(i) as usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c1)[0usize], t1, t2, res_i.1);
        let t10: u32 = res[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
        let t20: u32 = n[4u32.wrapping_mul(i).wrapping_add(1u32) as usize];
        let res_i0: (&mut [u32], &mut [u32]) = res_i.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c1)[0usize], t10, t20, res_i0.1);
        let t11: u32 = res[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
        let t21: u32 = n[4u32.wrapping_mul(i).wrapping_add(2u32) as usize];
        let res_i1: (&mut [u32], &mut [u32]) = res_i0.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c1)[0usize], t11, t21, res_i1.1);
        let t12: u32 = res[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
        let t22: u32 = n[4u32.wrapping_mul(i).wrapping_add(3u32) as usize];
        let res_i2: (&mut [u32], &mut [u32]) = res_i1.1.split_at_mut(1usize);
        (&mut c1)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c1)[0usize], t12, t22, res_i2.1)
    };
    for i in len1.wrapping_div(4u32).wrapping_mul(4u32)..len1
    {
        let t1: u32 = res[i as usize];
...
*/
pub fn
bn_sub_mod_n_u64(len1: u32, n: &mut [u64], a: &mut [u64], b: &mut [u64], res: &mut [u64])
{
    let mut c: [u64; 1] = [0u64; 1usize];
    for i in 0u32..len1.wrapping_div(4u32)
    {
...
*/
pub fn
bn_check_modulus_u32(len: u32, n: &mut [u32]) ->
    u32
{
    ...
    for i in 0u32..len
    {
        let beq: u32 = fstar::uint32::eq_mask((&mut one)[i as usize], n[i as usize]);
        let blt: u32 = ! fstar::uint32::gte_mask((&mut one)[i as usize], n[i as usize]);
...
*/
pub fn
bn_precomp_r2_mod_n_u32(len: u32, nBits: u32, n: &mut [u32], res: &mut [u32])
{
    ...
    for _i in 0u32..64u32.wrapping_mul(len).wrapping_sub(nBits)
    {
        ...
        crate::bignum::bn_add_mod_n_u32(len, n, &mut a_copy, &mut b_copy, res)
    }
}
...
fn bn_mont_reduction_u32(len: u32, n: &mut [u32], nInv: u32, c: &mut [u32], res: &mut [u32])
{
    ...
    for i in 0u32..len
    {
        ...
        for i0 in 0u32..len.wrapping_div(4u32)
        {
            ...
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u32(a_i, qj, (&mut c1)[0usize], res_i.1);
            let a_i0: u32 = n[4u32.wrapping_mul(i0).wrapping_add(1u32) as usize];
            let res_i0: (&mut [u32], &mut [u32]) = res_i.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u32(a_i0, qj, (&mut c1)[0usize], res_i0.1);
            let a_i1: u32 = n[4u32.wrapping_mul(i0).wrapping_add(2u32) as usize];
            let res_i1: (&mut [u32], &mut [u32]) = res_i0.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u32(a_i1, qj, (&mut c1)[0usize], res_i1.1);
            let a_i2: u32 = n[4u32.wrapping_mul(i0).wrapping_add(3u32) as usize];
            let res_i2: (&mut [u32], &mut [u32]) = res_i1.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u32(a_i2, qj, (&mut c1)[0usize], res_i2.1)
        };
        ...
        (&mut c0)[0usize] =
            lib::inttypes_intrinsics::add_carry_u32((&mut c0)[0usize], c10, res_j0, resb.1)
    };
...
)
{
    ...
    crate::bignum::bn_karatsuba_mul_uint32(len, a, r2, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u32(len, n, nInv, &mut c, aM)
}
...
*/
pub fn
bn_from_mont_u32(len: u32, n: &mut [u32], nInv_u64: u32, aM: &mut [u32], a: &mut [u32])
{
    ...
    crate::bignum::bn_mont_reduction_u32(len, n, nInv_u64, &mut tmp, a)
}
...
)
{
    let mut c: Box<[u32]> = vec![0u32; len.wrapping_add(len) as usize].into_boxed_slice();
    let mut tmp: Box<[u32]> = vec![0u32; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_mul_uint32(len, aM, bM, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u32(len, n, nInv_u64, &mut c, resM)
}
...
*/
pub fn
bn_mont_sqr_u32(len: u32, n: &mut [u32], nInv_u64: u32, aM: &mut [u32], resM: &mut [u32])
{
    let mut c: Box<[u32]> = vec![0u32; len.wrapping_add(len) as usize].into_boxed_slice();
    let mut tmp: Box<[u32]> = vec![0u32; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_sqr_uint32(len, aM, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u32(len, n, nInv_u64, &mut c, resM)
}
...
*/
pub fn
bn_check_modulus_u64(len: u32, n: &mut [u64]) ->
    u64
{
    ...
    for i in 0u32..len
    {
        let beq: u64 = fstar::uint64::eq_mask((&mut one)[i as usize], n[i as usize]);
        let blt: u64 = ! fstar::uint64::gte_mask((&mut one)[i as usize], n[i as usize]);
        (&mut acc)[0usize] = beq & (&mut acc)[0usize] | ! beq & blt
    };
...
*/
pub fn
bn_precomp_r2_mod_n_u64(len: u32, nBits: u32, n: &mut [u64], res: &mut [u64])
{
    ...
    for _i in 0u32..128u32.wrapping_mul(len).wrapping_sub(nBits)
    {
        ...
        ((&mut a_copy)[0usize..len as usize]).copy_from_slice(&res[0usize..len as usize]);
        ((&mut b_copy)[0usize..len as usize]).copy_from_slice(&res[0usize..len as usize]);
        crate::bignum::bn_add_mod_n_u64(len, n, &mut a_copy, &mut b_copy, res)
    }
}
    ...
    for i in 0u32..len
    {
        ...
        for i0 in 0u32..len.wrapping_div(4u32)
        {
            ...
            let res_i: (&mut [u64], &mut [u64]) =
                res_j.1.split_at_mut(4u32.wrapping_mul(i0) as usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u64(a_i, qj, (&mut c1)[0usize], res_i.1);
            let a_i0: u64 = n[4u32.wrapping_mul(i0).wrapping_add(1u32) as usize];
            let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u64(a_i0, qj, (&mut c1)[0usize], res_i0.1);
            let a_i1: u64 = n[4u32.wrapping_mul(i0).wrapping_add(2u32) as usize];
            let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u64(a_i1, qj, (&mut c1)[0usize], res_i1.1);
            let a_i2: u64 = n[4u32.wrapping_mul(i0).wrapping_add(3u32) as usize];
            let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
            (&mut c1)[0usize] =
                crate::bignum_base::mul_wide_add2_u64(a_i2, qj, (&mut c1)[0usize], res_i2.1)
        };
        for i0 in len.wrapping_div(4u32).wrapping_mul(4u32)..len
        {
        ...
        (&mut c0)[0usize] =
            lib::inttypes_intrinsics::add_carry_u64((&mut c0)[0usize], c10, res_j0, resb.1)
    };
    (res[0usize..len.wrapping_add(len).wrapping_sub(len) as usize]).copy_from_slice(
        &(&mut c[len as usize..])[0usize..len.wrapping_add(len).wrapping_sub(len) as usize]
    );
...
)
{
    ...
    crate::bignum::bn_karatsuba_mul_uint64(len, a, r2, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u64(len, n, nInv, &mut c, aM)
}
...
*/
pub fn
bn_from_mont_u64(len: u32, n: &mut [u64], nInv_u64: u64, aM: &mut [u64], a: &mut [u64])
{
    let mut tmp: Box<[u64]> = vec![0u64; len.wrapping_add(len) as usize].into_boxed_slice();
    ((&mut tmp)[0usize..len as usize]).copy_from_slice(&aM[0usize..len as usize]);
    crate::bignum::bn_mont_reduction_u64(len, n, nInv_u64, &mut tmp, a)
}
...
)
{
    let mut c: Box<[u64]> = vec![0u64; len.wrapping_add(len) as usize].into_boxed_slice();
    let mut tmp: Box<[u64]> = vec![0u64; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_mul_uint64(len, aM, bM, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u64(len, n, nInv_u64, &mut c, resM)
}
...
*/
pub fn
bn_mont_sqr_u64(len: u32, n: &mut [u64], nInv_u64: u64, aM: &mut [u64], resM: &mut [u64])
{
    let mut c: Box<[u64]> = vec![0u64; len.wrapping_add(len) as usize].into_boxed_slice();
    let mut tmp: Box<[u64]> = vec![0u64; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_sqr_uint64(len, aM, &mut tmp, &mut c);
    crate::bignum::bn_mont_reduction_u64(len, n, nInv_u64, &mut c, resM)
}
...
)
{
    ...
    let c00: u32 = (&mut c0)[0usize];
    let mut tmp: Box<[u32]> = vec![0u32; len as usize].into_boxed_slice();
    let c1: u32 = crate::bignum_base::bn_sub_eq_len_u32(len, res, n, &mut tmp);
    lowstar::ignore::ignore::<u32>(c1);
...
)
{
    ...
    let mut tmp: Box<[u32]> = vec![0u32; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_mul_uint32(len, aM, bM, &mut tmp, &mut c);
    crate::bignum::bn_almost_mont_reduction_u32(len, n, nInv_u64, &mut c, resM)
}
...
)
{
    ...
    let c00: u64 = (&mut c0)[0usize];
    let mut tmp: Box<[u64]> = vec![0u64; len as usize].into_boxed_slice();
    let c1: u64 = crate::bignum_base::bn_sub_eq_len_u64(len, res, n, &mut tmp);
    lowstar::ignore::ignore::<u64>(c1);
...
)
{
    ...
    let mut tmp: Box<[u64]> = vec![0u64; 4u32.wrapping_mul(len) as usize].into_boxed_slice();
    crate::bignum::bn_karatsuba_mul_uint64(len, aM, bM, &mut tmp, &mut c);
    crate::bignum::bn_almost_mont_reduction_u64(len, n, nInv_u64, &mut c, resM)
}
...
pub(crate) fn bn_check_mod_exp_u32(
    len: u32,
    n: &mut [u32],
    a: &mut [u32],
    bBits: u32,
    ...
    u32
{
    ...
    let m10: u32 =
        if bBits < 32u32.wrapping_mul(bLen)
        {
            ...
            (&mut b2)[i as usize] |= 1u32.wrapping_shl(j);
            let mut acc0: [u32; 1] = [0u32; 1usize];
            for i0 in 0u32..bLen
            {
                let beq: u32 = fstar::uint32::eq_mask(b[i0 as usize], (&mut b2)[i0 as usize]);
                let blt: u32 = ! fstar::uint32::gte_mask(b[i0 as usize], (&mut b2)[i0 as usize]);
...
)
{
    if bBits < 200u32
    {
        ...
        crate::bignum::bn_to_mont_u32(len, n, mu, r2, a, &mut aM);
        ...
        let ctx_n: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
        let ctx_r2: (&mut [u32], &mut [u32]) = ctx_n.1.split_at_mut(len as usize);
        crate::bignum::bn_from_mont_u32(len, ctx_r2.0, mu, ctx_r2.1, &mut resM);
        lowstar::ignore::ignore::<&mut [u32]>(&mut ctx);
        for i in 0u32..bBits
        {
            ...
            if bit != 0u32
            {
                ...
                let ctx_n0: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_mul_u32(
                    len,
                    ctx_n0.1,
                    mu,
                    &mut aM_copy,
            ...
            let ctx_n0: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
            crate::bignum::bn_almost_mont_sqr_u32(len, ctx_n0.1, mu, &mut aM_copy, &mut aM);
            lowstar::ignore::ignore::<&mut [u32]>(&mut ctx)
        };
        crate::bignum::bn_from_mont_u32(len, n, mu, &mut resM, res)
    }
    else
    {
        let mut aM: Box<[u32]> = vec![0u32; len as usize].into_boxed_slice();
        ...
        let ctx_n: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
        let ctx_r2: (&mut [u32], &mut [u32]) = ctx_n.1.split_at_mut(len as usize);
        crate::bignum::bn_from_mont_u32(len, ctx_r2.0, mu, ctx_r2.1, t1.0);
        lowstar::ignore::ignore::<&mut [u32]>(&mut ctx);
        (t1.1[0usize..len as usize]).copy_from_slice(&(&mut aM)[0usize..len as usize]);
        lowstar::ignore::ignore::<&mut [u32]>(&mut table);
        krml::unroll_for!(
            7,
            "i",
            0u32,
            1u32,
            {
                let t11: (&mut [u32], &mut [u32]) =
                    table.split_at_mut(i.wrapping_add(1u32).wrapping_mul(len) as usize);
                let mut aM_copy: Box<[u32]> = vec![0u32; len as usize].into_boxed_slice();
                ((&mut aM_copy)[0usize..len as usize]).copy_from_slice(&t11.1[0usize..len as usize]);
                let ctx_n0: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_sqr_u32(len, ctx_n0.1, mu, &mut aM_copy, &mut tmp);
                lowstar::ignore::ignore::<&mut [u32]>(&mut ctx);
                ...
                let ctx_n1: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_mul_u32(
                    len,
                    ctx_n1.1,
                    mu,
        ...
        if bBits.wrapping_rem(4u32) != 0u32
        {
            ...
            let bits_c: u32 = crate::bignum_base::bn_get_bits_u32(bLen, b, i, 4u32);
            let bits_l32: u32 = bits_c;
            let a_bits_l: (&mut [u32], &mut [u32]) =
                table.split_at_mut(bits_l32.wrapping_mul(len) as usize);
        ...
        else
        {
            ...
            crate::bignum::bn_from_mont_u32(len, ctx_r20.0, mu, ctx_r20.1, &mut resM);
            lowstar::ignore::ignore::<&mut [u32]>(&mut ctx)
        };
        ...
        for i in 0u32..bBits.wrapping_div(4u32)
        {
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    let ctx_n0: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
                    crate::bignum::bn_almost_mont_sqr_u32(
                        len,
                        ctx_n0.1,
                        mu,
                        &mut aM_copy,
            ...
            let bits_l: u32 = crate::bignum_base::bn_get_bits_u32(bLen, b, k, 4u32);
            lowstar::ignore::ignore::<&mut [u32]>(&mut table);
    ...
    if bBits < 200u32
    {
        ...
        for i in 0u32..bBits
        {
            ...
            for i0 in 0u32..len
            {
                ...
                (&mut resM)[i0 as usize] ^= dummy;
                (&mut aM)[i0 as usize] ^= dummy
            };
            let mut aM_copy: Box<[u32]> = vec![0u32; len as usize].into_boxed_slice();
            ((&mut aM_copy)[0usize..len as usize]).copy_from_slice(&(&mut aM)[0usize..len as usize]);
            let ctx_n0: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
            ...
            let ctx_n1: (&mut [u32], &mut [u32]) = ctx.split_at_mut(0usize);
            crate::bignum::bn_almost_mont_sqr_u32(len, ctx_n1.1, mu, &mut aM_copy0, &mut resM);
            lowstar::ignore::ignore::<&mut [u32]>(&mut ctx);
            (&mut sw)[0usize] = bit
        };
        ...
        for i in 0u32..len
        {
            ...
            (&mut resM)[i as usize] ^= dummy;
            (&mut aM)[i as usize] ^= dummy
        };
        crate::bignum::bn_from_mont_u32(len, n, mu, &mut resM, res)
    }
    else
        ...
        if bBits.wrapping_rem(4u32) != 0u32
        {
            ...
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    let res_j: (&mut [u32], &mut [u32]) =
                        table.split_at_mut(i0.wrapping_add(1u32).wrapping_mul(len) as usize);
                    for i1 in 0u32..len
                    {
...
)
{
    ...
    crate::bignum::bn_mod_exp_vartime_precomp_u32(len, n, mu, &mut r2, a, bBits, b, res)
}
...
)
{
    ...
    crate::bignum::bn_precomp_r2_mod_n_u32(len, nBits, n, &mut r2);
    let mu: u32 = crate::bignum::mod_inv_uint32(n[0usize]);
    crate::bignum::bn_mod_exp_consttime_precomp_u32(len, n, mu, &mut r2, a, bBits, b, res)
}

pub(crate) fn bn_check_mod_exp_u64(
    len: u32,
    n: &mut [u64],
    a: &mut [u64],
    bBits: u32,
...
*/
pub fn
bn_mod_exp_vartime_precomp_u64(
    len: u32,
    n: &mut [u64],
    mu: u64,
    r2: &mut [u64],
...
)
{
    if bBits < 200u32
    {
        ...
        crate::bignum::bn_to_mont_u64(len, n, mu, r2, a, &mut aM);
        ...
        let ctx_n: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
        let ctx_r2: (&mut [u64], &mut [u64]) = ctx_n.1.split_at_mut(len as usize);
        crate::bignum::bn_from_mont_u64(len, ctx_r2.0, mu, ctx_r2.1, &mut resM);
        lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
        for i in 0u32..bBits
        {
            ...
            if bit != 0u64
            {
                ...
                let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_mul_u64(
                    len,
                    ctx_n0.1,
                    mu,
                    &mut aM_copy,
            ...
            ((&mut aM_copy)[0usize..len as usize]).copy_from_slice(&(&mut aM)[0usize..len as usize]);
            let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum::bn_almost_mont_sqr_u64(len, ctx_n0.1, mu, &mut aM_copy, &mut aM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
        };
        crate::bignum::bn_from_mont_u64(len, n, mu, &mut resM, res)
    }
    else
    {
        let mut aM: Box<[u64]> = vec![0u64; len as usize].into_boxed_slice();
        ...
        let ctx_n: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
        let ctx_r2: (&mut [u64], &mut [u64]) = ctx_n.1.split_at_mut(len as usize);
        crate::bignum::bn_from_mont_u64(len, ctx_r2.0, mu, ctx_r2.1, t1.0);
        lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
        (t1.1[0usize..len as usize]).copy_from_slice(&(&mut aM)[0usize..len as usize]);
        ...
        krml::unroll_for!(
            ...
            1u32,
            {
                ...
                ((&mut aM_copy)[0usize..len as usize]).copy_from_slice(&t11.1[0usize..len as usize]);
                let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_sqr_u64(len, ctx_n0.1, mu, &mut aM_copy, &mut tmp);
                lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
                ((&mut table)[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(len) as usize..2u32.wrapping_mul(
                    i
                ).wrapping_add(2u32).wrapping_mul(len)
                ...
                let ctx_n1: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                crate::bignum::bn_almost_mont_mul_u64(
                    len,
                    ctx_n1.1,
                    mu,
        ...
        else
        {
            ...
            crate::bignum::bn_from_mont_u64(len, ctx_r20.0, mu, ctx_r20.1, &mut resM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx)
        };
        ...
        for i in 0u32..bBits.wrapping_div(4u32)
        {
            krml::unroll_for!(
                ...
                1u32,
                {
                    ...
                    let ctx_n0: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
                    crate::bignum::bn_almost_mont_sqr_u64(
                        len,
                        ctx_n0.1,
                        mu,
                        &mut aM_copy,
...
*/
pub fn
bn_mod_exp_consttime_precomp_u64(
    len: u32,
    n: &mut [u64],
    mu: u64,
    r2: &mut [u64],
...
)
{
    if bBits < 200u32
    {
        ...
        for i in 0u32..bBits
        {
            ...
            let ctx_n1: (&mut [u64], &mut [u64]) = ctx.split_at_mut(0usize);
            crate::bignum::bn_almost_mont_sqr_u64(len, ctx_n1.1, mu, &mut aM_copy0, &mut resM);
            lowstar::ignore::ignore::<&mut [u64]>(&mut ctx);
            (&mut sw)[0usize] = bit
        };
...
)
{
    ...
    crate::bignum::bn_mod_exp_vartime_precomp_u64(len, n, mu, &mut r2, a, bBits, b, res)
}
...
)
{
    ...
    crate::bignum::bn_precomp_r2_mod_n_u64(len, nBits, n, &mut r2);
    let mu: u64 = crate::bignum::mod_inv_uint64(n[0usize]);
    crate::bignum::bn_mod_exp_consttime_precomp_u64(len, n, mu, &mut r2, a, bBits, b, res)
}
