fn quarter_round(st: &mut [u32], a: u32, b: u32, c: u32, d: u32) -> ()
{
  ...
  let std: u32 = st[a as usize];
  let sta1: u32 =
    sta ^ (stb.wrapping_add(std).wrapping_shl(9u32) | stb.wrapping_add(std).wrapping_shr(23u32));
  st[c as usize] = sta1;
  let sta: u32 = st[d as usize];
  let stb: u32 = st[c as usize];
  let std: u32 = st[b as usize];
  let sta1: u32 =
    sta ^ (stb.wrapping_add(std).wrapping_shl(13u32) | stb.wrapping_add(std).wrapping_shr(19u32));
  st[d as usize] = sta1;
  let sta: u32 = st[a as usize];
  let stb: u32 = st[d as usize];
  let std: u32 = st[c as usize];
  let sta1: u32 =
    sta ^ (stb.wrapping_add(std).wrapping_shl(18u32) | stb.wrapping_add(std).wrapping_shr(14u32));
  st[a as usize] = sta1
}
