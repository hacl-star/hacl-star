    ...
    else
    {
        ...
        if sz == 0u32
        {
            ...
            if ! (sz1 == 0u32)
            {
                ...
                let wv: &mut [u64] = block_state1.fst;
                let hash: &mut [u64] = block_state1.snd;
                let nb: u32 = 1u32;
                update_multi(
                    128u32,
                    wv,
        ...
        else
        {
            ...
            if ! (sz10 == 0u32)
            {
                ...
                let wv: &mut [u64] = block_state10.fst;
                let hash: &mut [u64] = block_state10.snd;
                let nb: u32 = 1u32;
                update_multi(
                    128u32,
                    wv,
...
pub fn digest(state: &mut [state_t], output: &mut [u8]) -> ()
{
    ...
    let src_b: &mut [u64] = block_state.snd;
    let dst_b: &mut [u64] = tmp_block_state.snd;
    ...
    let wv0: &mut [u64] = tmp_block_state.fst;
    let hash: &mut [u64] = tmp_block_state.snd;
    let nb: u32 = 0u32;
    update_multi(
        0u32,
        wv0,
    ...
    let wv1: &mut [u64] = tmp_block_state.fst;
    let hash0: &mut [u64] = tmp_block_state.snd;
    update_last(
        r,
        wv1,
        hash0,
