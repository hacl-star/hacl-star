...
pub fn mul(a: &mut [u64], b: &mut [u64], res: &mut [u64]) -> ()
{
    ...
    for i in 0u32..4u32
    {
        ...
        let r: u64 = c;
        res_j.0[4u32.wrapping_add(i) as usize] = r
    }
}
    ...
    ()
{
    ...
    ((&mut c)[0usize..8usize]).copy_from_slice(&[0u64; 8usize]);
    for i in 0u32..4u32
    {
        let bj: u64 = bM[i as usize];
        let res_j: (&mut [u64], &mut [u64]) = (&mut c).split_at_mut(i as usize);
        let mut c1: u64 = 0u64;
        for i0 in 0u32..1u32
        {
            let a_i: u64 = aM[4u32.wrapping_mul(i0) as usize];
            let res_i: (&mut [u64], &mut [u64]) =
                res_j.1.split_at_mut(4u32.wrapping_mul(i0) as usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i, bj, c1, res_i.1);
            let a_i0: u64 = aM[4u32.wrapping_mul(i0).wrapping_add(1u32) as usize];
            let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i0, bj, c1, res_i0.1);
            let a_i1: u64 = aM[4u32.wrapping_mul(i0).wrapping_add(2u32) as usize];
            let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i1, bj, c1, res_i1.1);
            let a_i2: u64 = aM[4u32.wrapping_mul(i0).wrapping_add(3u32) as usize];
            let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i2, bj, c1, res_i2.1)
        };
        for i0 in 4u32..4u32
        {
            let a_i: u64 = aM[i0 as usize];
            let res_i: (&mut [u64], &mut [u64]) = res_j.1.split_at_mut(i0 as usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i, bj, c1, res_i.1)
        };
        let r: u64 = c1;
...
#[inline] fn amont_sqr(n: &mut [u64], nInv_u64: u64, aM: &mut [u64], resM: &mut [u64]) -> ()
{
    ...
    for i in 0u32..4u32
    {
        let ab: (&mut [u64], &mut [u64]) = aM.split_at_mut(0usize);
        let a_j: u64 = ab.1[i as usize];
        let res_j: (&mut [u64], &mut [u64]) = (&mut c).split_at_mut(i as usize);
        let mut c1: u64 = 0u64;
        for i0 in 0u32..i.wrapping_div(4u32)
        {
            ...
            let res_i: (&mut [u64], &mut [u64]) =
                res_j.1.split_at_mut(4u32.wrapping_mul(i0) as usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i, a_j, c1, res_i.1);
            let a_i0: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(1u32) as usize];
            let res_i0: (&mut [u64], &mut [u64]) = res_i.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i0, a_j, c1, res_i0.1);
            let a_i1: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(2u32) as usize];
            let res_i1: (&mut [u64], &mut [u64]) = res_i0.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i1, a_j, c1, res_i1.1);
            let a_i2: u64 = ab.1[4u32.wrapping_mul(i0).wrapping_add(3u32) as usize];
            let res_i2: (&mut [u64], &mut [u64]) = res_i1.1.split_at_mut(1usize);
            c1 = crate::hacl::bignum_base::mul_wide_add2_u64(a_i2, a_j, c1, res_i2.1)
        };
        for i0 in i.wrapping_div(4u32).wrapping_mul(4u32)..i
        {
            let a_i: u64 = ab.1[i0 as usize];
    ...
    let mut a_copy: [u64; 8] = [0u64; 8usize];
    let mut b_copy: [u64; 8] = [0u64; 8usize];
    ((&mut a_copy)[0usize..8usize]).copy_from_slice(&(&mut c)[0usize..8usize]);
    ((&mut b_copy)[0usize..8usize]).copy_from_slice(&(&mut c)[0usize..8usize]);
    let r: u64 =
        crate::hacl::bignum_base::bn_add_eq_len_u64(8u32, &mut a_copy, &mut b_copy, &mut c);
    let c0: u64 = r;
    crate::lowstar::ignore::ignore::<u64>(c0);
    let mut tmp: [u64; 8] = [0u64; 8usize];
    for i in 0u32..4u32
    {
        let res: crate::fstar::uint128::uint128 =
            crate::fstar::uint128::mul_wide(aM[i as usize], aM[i as usize]);
        let hi: u64 =
            crate::fstar::uint128::uint128_to_uint64(crate::fstar::uint128::shift_right(res, 64u32));
        let lo: u64 = crate::fstar::uint128::uint128_to_uint64(res);
        (&mut tmp)[2u32.wrapping_mul(i) as usize] = lo;
        (&mut tmp)[2u32.wrapping_mul(i).wrapping_add(1u32) as usize] = hi
    };
    let mut a_copy0: [u64; 8] = [0u64; 8usize];
    let mut b_copy0: [u64; 8] = [0u64; 8usize];
    ((&mut a_copy0)[0usize..8usize]).copy_from_slice(&(&mut c)[0usize..8usize]);
    ((&mut b_copy0)[0usize..8usize]).copy_from_slice(&(&mut tmp)[0usize..8usize]);
    let r0: u64 =
        crate::hacl::bignum_base::bn_add_eq_len_u64(8u32, &mut a_copy0, &mut b_copy0, &mut c);
    let c1: u64 = r0;
    crate::lowstar::ignore::ignore::<u64>(c1);
    areduction(n, nInv_u64, &mut c, resM)
}
    ...
    ()
{
    ...
    ((&mut a_mod)[0usize..4usize]).copy_from_slice(&(&mut (&mut a1)[4usize..])[0usize..4usize]);
    let c00: u64 = c0;
    let mut tmp: [u64; 4] = [0u64; 4usize];
    ...
    ()
{
    if bBits < 200u32
    {
        ...
        let mut tmp: [u64; 8] = [0u64; 8usize];
        ((&mut tmp)[0usize..4usize]).copy_from_slice(&(&mut resM)[0usize..4usize]);
    ...
    else
    {
        ...
        for i in 0u32..7u32
        {
            ...
            (t11.0[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(4u32) as usize..2u32.wrapping_mul(
                i
            ...
            let t2: (&mut [u64], &mut [u64]) =
                t11.1.split_at_mut(
                    ...
                    -
                    i.wrapping_add(1u32).wrapping_mul(4u32) as usize
                );
            let ctx_n1: (&mut [u64], &mut [u64]) = ctx_n0.1.split_at_mut(0usize);
            amont_mul(ctx_n1.1, mu, &mut aM, t2.1, &mut tmp);
            (t11.0[2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(4u32) as usize..2u32.wrapping_mul(
                i
        ...
        let mut tmp1: [u64; 8] = [0u64; 8usize];
        ((&mut tmp1)[0usize..4usize]).copy_from_slice(&(&mut resM)[0usize..4usize]);
    ...
    else
    {
        ...
        if bBits.wrapping_rem(4u32) != 0u32
        {
            ...
            ((&mut resM)[0usize..4usize]).copy_from_slice(
                &(&mut t1.0[0usize..] as &mut [u64])[0usize..4usize]
            );
            for i0 in 0u32..15u32
            {
                let c0: u64 = crate::fstar::uint64::eq_mask(bits_c, i0.wrapping_add(1u32) as u64);
                let res_j: (&[u64], &[u64]) =
                    t1.0.split_at(i0.wrapping_add(1u32).wrapping_mul(4u32) as usize);
                for i1 in 0u32..4u32
                {
