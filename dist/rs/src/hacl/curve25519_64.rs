...
fn point_add_and_double(q: &mut [u64], p01_tmp1: &mut [u64], tmp2: &mut [u64]) -> ()
{
    ...
    ((&mut f1_copy)[0usize..8usize]).copy_from_slice(&c.0[0usize..8usize]);
    ...
    ((&mut f1_copy0)[0usize..8usize]).copy_from_slice(&x3.1[0usize..8usize]);
    ...
    ((&mut f1_copy2)[0usize..4usize]).copy_from_slice(&z31.0[0usize..4usize]);
...
fn point_double(nq: &mut [u64], tmp1: &mut [u64], tmp2: &mut [u64]) -> ()
{
    ...
    ((&mut f1_copy)[0usize..4usize]).copy_from_slice(&d.0[0usize..4usize]);
...
fn montgomery_ladder(out: &mut [u64], key: &mut [u8], init: &mut [u64]) -> ()
{
    ...
    point_double(tmp1.0, tmp1.1, &mut tmp2);
    point_double(tmp1.0, tmp1.1, &mut tmp2);
    (out[0usize..8usize]).copy_from_slice(&p01.0[0usize..8usize])
}
...
pub fn scalarmult(out: &mut [u8], r#priv: &mut [u8], r#pub: &mut [u8]) -> ()
{
    ...
    ((&mut init_copy)[0usize..8usize]).copy_from_slice(&z.0[0usize..8usize]);
