...
#include "GCMencryptOpt256_stdcalls.h"

void
GCMencryptOpt256_stdcalls_gcm256_encrypt_opt_stdcall(
  uint8_t *plain_b,
  uint64_t plain_len,
  uint8_t *auth_b,
  uint64_t auth_len,
...
)
{
  ...
  uint32_t plain_len_ = (uint32_t)plain_len / (uint32_t)16U * (uint32_t)16U;
  uint32_t auth_len_ = (uint32_t)auth_len / (uint32_t)16U * (uint32_t)16U;
  uint8_t *plain_b_ = plain_b;
  uint8_t *out_b_ = out_b;
  uint8_t *auth_b_ = auth_b;
  memcpy(inout_b,
    plain_b + plain_len_,
    (uint32_t)plain_len % (uint32_t)16U * sizeof plain_b[0U]);
  memcpy(abytes_b, auth_b + auth_len_, (uint32_t)auth_len % (uint32_t)16U * sizeof auth_b[0U]);
  uint64_t len128x6 = plain_len / (uint64_t)96U * (uint64_t)96U;
  if (len128x6 / (uint64_t)16U >= (uint64_t)18U)
  {
    uint64_t len128_num = plain_len / (uint64_t)16U * (uint64_t)16U - len128x6;
    ...
    uint64_t auth_num = auth_len / (uint64_t)16U;
  ...
  else
  {
    ...
    uint64_t len128_num = plain_len / (uint64_t)16U * (uint64_t)16U;
    uint8_t *in128x6_b = plain_b_;
    uint8_t *out128x6_b = out_b_;
  ...
  memcpy(out_b + (uint32_t)plain_len / (uint32_t)16U * (uint32_t)16U,
    inout_b,
    (uint32_t)plain_len % (uint32_t)16U * sizeof inout_b[0U]);
}
