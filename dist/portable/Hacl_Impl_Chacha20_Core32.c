...
#include "Hacl_Impl_Chacha20_Core32.h"

void Hacl_Impl_Chacha20_Core32_load_state(uint32_t *st, uint8_t *b)
{
...
void Hacl_Impl_Chacha20_Core32_store_state(uint8_t *st, uint32_t *b)
{
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)16U; i = i + (uint32_t)1U)
  {
    store32_le(st + i * (uint32_t)4U, b[i]);
  }
}

void Hacl_Impl_Chacha20_Core32_set_counter(uint32_t *st, uint32_t c)
{
  st[12U] = c;
}

void Hacl_Impl_Chacha20_Core32_incr_counter(uint32_t *st)
{
  uint32_t c = st[12U];
  st[12U] = c + (uint32_t)1U;
}

void Hacl_Impl_Chacha20_Core32_copy_state(uint32_t *st, uint32_t *ost)
{
  memcpy(st, ost, (uint32_t)16U * sizeof ost[0U]);
}

void Hacl_Impl_Chacha20_Core32_sum_state(uint32_t *st, uint32_t *ost)
{
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)16U; i = i + (uint32_t)1U)
  {
    uint32_t *os = st;
    uint32_t x = st[i] + ost[i];
    os[i] = x;
  }
}

void Hacl_Impl_Chacha20_Core32_xor_block(uint8_t *o, uint32_t *st, uint8_t *b)
{
  uint32_t bl[16U] = { 0U };
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)16U; i = i + (uint32_t)1U)
  {
  ...
  {
    uint32_t *os = bl;
    uint32_t x = bl[i] ^ st[i];
    os[i] = x;
  }
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)16U; i = i + (uint32_t)1U)
  {
    store32_le(o + i * (uint32_t)4U, bl[i]);
  }
}

void
Hacl_Impl_Chacha20_Core32_line(uint32_t *st, uint32_t a, uint32_t b, uint32_t d, uint32_t r)
{
  uint32_t sta = st[a];
  uint32_t stb = st[b];
  uint32_t std = st[d];
  uint32_t sta1 = sta + stb;
  uint32_t std1 = std ^ sta1;
  uint32_t std2 = std1 << r | std1 >> ((uint32_t)32U - r);
  st[a] = sta1;
  st[d] = std2;
}
...
inline void Hacl_Impl_Chacha20_Core32_double_round(uint32_t *st)
{
  uint32_t sta = st[0U];
  uint32_t stb0 = st[4U];
