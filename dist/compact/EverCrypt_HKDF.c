...
)
{
  ...
  if (i1 == (uint8_t)0U)
  {
    uint32_t len11 = infolen + (uint32_t)1U;
    EverCrypt_HMAC_compute(a, tag, prk, prklen, info_counter, len11);
  }
  else
  {
  ...
  if (len1 <= tlen)
  {
    memcpy(okm, tag, len1 * sizeof tag[0U]);
  }
  else
  {
    memcpy(okm, tag, tlen * sizeof tag[0U]);
    uint32_t len_ = len1 - tlen;
...
/*
  ...
  val hkdf_expand :
    ...
    len:
      uint8_l okm
        { disjoint okm prk /\ HMAC.keysized a (v prklen) /\
...
)
{
  ...
  EverCrypt_HKDF_hkdf_expand_loop(a, okm, prk, prklen, infolen, len1, text, (uint8_t)0U);
}
