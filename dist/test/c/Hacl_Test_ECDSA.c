...
)
{
  ...
  else
  {
    ...
    bool result_ = Hacl_Impl_ECDSA_ecdsa_p256_sha2_verify(msg_len, msg, qxy, r, s1);
    if (!(result_ == result))
    {
      LowStar_Printf_print_string("FAIL\n");
      exit((int32_t)1);
...
)
{
  ...
  else
  {
    ...
    if (flag == (uint64_t)0U)
    {
      ...
      if (okr && oks)
      {
        bool result = Hacl_Impl_ECDSA_ecdsa_p256_sha2_verify(msg_len, msg, qxy, r, s1);
        if (!result)
        {
