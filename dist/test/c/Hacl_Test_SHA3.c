...
static void state_permute(uint64_t *s)
{
  for (uint32_t i0 = (uint32_t)0U; i0 < (uint32_t)24U; i0++)
  {
    ...
    for (uint32_t i1 = (uint32_t)0U; i1 < (uint32_t)5U; i1++)
    {
      ...
      uint64_t _D = uu____0 ^ rotl(b0[(i1 + (uint32_t)1U) % (uint32_t)5U], (uint32_t)1U);
      for (uint32_t i = (uint32_t)0U; i < (uint32_t)5U; i++)
      {
...
)
{
  ...
  loadState(rateInBytes, b1, s);
  state_permute(s);
  memset(b1, 0U, rateInBytes * sizeof (b1[0U]));
  memset(b, 0U, rateInBytes * sizeof (b[0U]));
}

static void squeeze(uint64_t *s, uint32_t rateInBytes, uint32_t outputByteLen, uint8_t *output)
{
...
test_shake128(uint32_t msg_len, const uint8_t *msg, uint32_t out_len, const uint8_t *expected)
{
  ...
  uint8_t test1[out_len];
  memset(test1, 0U, out_len * sizeof (test1[0U]));
  shake128_hacl(msg_len, msg_, out_len, test1);
  if
  (
    !Lib_PrintBuffer_result_compare_display(out_len,
      to_const__uint8_t(MUT, (void *)test1),
      expected)
  )
  {
    exit((int32_t)255);
...
test_shake256(uint32_t msg_len, const uint8_t *msg, uint32_t out_len, const uint8_t *expected)
{
  ...
  uint8_t test1[out_len];
  memset(test1, 0U, out_len * sizeof (test1[0U]));
  shake256_hacl(msg_len, msg_, out_len, test1);
  if
  (
    !Lib_PrintBuffer_result_compare_display(out_len,
      to_const__uint8_t(MUT, (void *)test1),
