...
val gen_inv_tr:
  ...
  -> j:size_nat
  -> Type0
let gen_inv_tr h0 h1 h2 n1 n2 r res i j =
  B.live h2 res /\ B.live h2 r /\ disjoint r res /\
  modifies (loc_buffer res) h1 h2 /\ j <= v n2 /\
  (forall (i0:size_nat{i0 < v i}) (j:size_nat{j < v n2}). get h2 res i0 j == get h1 res i0 j) /\
  (forall (j0:size_nat{j0 < j}). get h2 res (v i) j0 == S.frodo_sample_matrix_tr_fc (v n1) (v n2) (as_seq h0 r) (v i) j0) /\
  (forall (j0:size_nat{j <= j0 /\ j0 < v n2}). get h2 res (v i) j0 == get h0 res (v i) j0) /\
  (forall (i0:size_nat{v i < i0 /\ i0 < v n1}) (j:size_nat{j < v n2}). get h2 res i0 j == get h0 res i0 j) /\
  as_seq h0 r == as_seq h2 r

inline_for_extraction noextract private
val frodo_sample_matrix_tr_fc:
    h0:HS.mem
  -> h1:HS.mem
  -> n1:size_t
  -> n2:size_t{0 < 2 * v n1 * v n2 /\ 2 * v n1 * v n2 < max_size_t}
  ...
  -> j:size_t{v j < v n2}
  -> Stack unit
    (requires fun h2 -> gen_inv_tr h0 h1 h2 n1 n2 r res0 i (v j))
    (ensures  fun _ _ h2 -> gen_inv_tr h0 h1 h2 n1 n2 r res0 i (v j + 1))
let frodo_sample_matrix_tr_fc h0 h1 n1 n2 r res0 i j =
  Lemmas.lemma_matrix_index_repeati2 (v n1) (v n2) (v i) (v j);
  let resij = uint_from_bytes_le (sub #_ #_ #2 r (size 2 *! (n1 *! j +! i)) (size 2)) in
  mset res0 i j (frodo_sample resij)

private
val sample_inner_inv_tr:
    h0:HS.mem
  -> h1:HS.mem
  -> n1:size_t
  -> n2:size_t{0 < 2 * v n1 * v n2 /\ 2 * v n1 * v n2 < max_size_t}
  ...
  -> i:size_nat{i <= v n1}
  -> Type0
let sample_inner_inv_tr h0 h1 n1 n2 r res i =
  B.live h1 r /\ B.live h1 res /\ disjoint r res /\
  modifies (loc_buffer res) h0 h1 /\ as_seq h0 r == as_seq h1 r /\
  (forall (i0:size_nat{i0 < i}) (j:size_nat{j < v n2}). get h1 res i0 j == S.frodo_sample_matrix_tr_fc (v n1) (v n2) (as_seq h0 r) i0 j) /\
  (forall (i0:size_nat{i <= i0 /\ i0 < v n1}) (j:size_nat{j < v n2}). get h1 res i0 j == get h0 res i0 j)
...
val frodo_sample_matrix_tr:
  ...
  -> Stack unit
    ...
    (ensures  fun h0 r h1 ->
      live h1 res /\ modifies (loc_buffer res) h0 h1 /\
      as_matrix h1 res == S.frodo_sample_matrix_tr (v n1) (v n2) (v seed_len) (as_seq h0 seed) ctr)
[@"c_inline"]
let frodo_sample_matrix_tr n1 n2 seed_len seed ctr res =
  ...
  let h0 = ST.get () in
  Lib.Loops.for (size 0) n1
    (fun h1 i -> sample_inner_inv_tr h0 h1 n1 n2 r res i)
    (fun i ->
      let h1 = ST.get () in
      Lib.Loops.for (size 0) n2
      (fun h2 j -> gen_inv_tr h0 h1 h2 n1 n2 r res i j)
      (fun j ->
        Lemmas.lemma_matrix_index_repeati2 (v n1) (v n2) (v i) (v j);
  ...
  pop_frame();
  let h1 = ST.get () in
  Spec.Matrix.extensionality (as_matrix h1 res) (S.frodo_sample_matrix_tr (v n1) (v n2) (v seed_len) (as_seq h0 seed) ctr)
