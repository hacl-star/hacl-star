...
#include <assert.h>
void aes128_load_schedule_ni(UNUSED const uint8_t *key, UNUSED void **_schedule) {
	assert(0);
}
void aes128_free_schedule_ni(UNUSED void *_schedule) {
	assert(0);
}
void aes128_enc_ni(UNUSED const uint8_t *plaintext, UNUSED const void *_schedule, UNUSED uint8_t *ciphertext) {
	assert(0);
}
void aes128_dec_ni(UNUSED const uint8_t *ciphertext, UNUSED const void *_schedule, UNUSED uint8_t *plaintext) {
	assert(0);
}
#else
...
#define key_exp2561(k, r, rcon) key_expand1(k, _mm_aeskeygenassist_si128(r, rcon))
#define key_exp256(k, r, rcon) key_expand(k, _mm_aeskeygenassist_si128(r, rcon))
#define key_exp2561(k, r, rcon) key_expand1(k, _mm_aeskeygenassist_si128(r, rcon))
void aes128_load_schedule_ni(const uint8_t *key, void **_schedule) {
...
void aes256_load_schedule_ni(const uint8_t *key, void **_schedule) {
	*_schedule = malloc(28 * 16);
	assert(*_schedule != NULL);
	__m128i *schedule = (__m128i *) *_schedule;
	schedule[0] = _mm_loadu_si128((const __m128i *) key);
	schedule[1] = _mm_loadu_si128((const __m128i *) (key+16));
	schedule[2] = key_exp256(schedule[0], schedule[1], 0x01);
	schedule[3] = key_exp2561(schedule[1], schedule[2], 0x00);
	schedule[4] = key_exp256(schedule[2], schedule[3], 0x02);
	schedule[5] = key_exp2561(schedule[3], schedule[4], 0x00);
	schedule[6] = key_exp256(schedule[4], schedule[5], 0x04);
	schedule[7] = key_exp2561(schedule[5], schedule[6], 0x00);
	schedule[8] = key_exp256(schedule[6], schedule[7], 0x08);
	schedule[9] = key_exp2561(schedule[7], schedule[8], 0x00);
	schedule[10] = key_exp256(schedule[8], schedule[9], 0x10);
	schedule[11] = key_exp2561(schedule[9], schedule[10], 0x00);
	schedule[12] = key_exp256(schedule[10], schedule[11], 0x20);
	schedule[13] = key_exp2561(schedule[11], schedule[12], 0x00);
...
void aes128_enc_ni(const uint8_t *plaintext, const void *_schedule, uint8_t *ciphertext) {
	__m128i *schedule = (__m128i *) _schedule;
...
void aes128_dec_ni(const uint8_t *ciphertext, const void *_schedule, uint8_t *plaintext) {
	__m128i *schedule = (__m128i *) _schedule;
...
void aes256_enc_ni(const uint8_t *plaintext, const void *_schedule, uint8_t *ciphertext) {
	__m128i *schedule = (__m128i *) _schedule;
	__m128i m = _mm_loadu_si128((__m128i *) plaintext);
...
void aes256_dec_ni(const uint8_t *ciphertext, const void *_schedule, uint8_t *plaintext) {
	__m128i *schedule = (__m128i *) _schedule;
	__m128i m = _mm_loadu_si128((__m128i *) ciphertext);
