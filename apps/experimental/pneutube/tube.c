...
int main (int argc, char *argv[])
{
	...
	while (1) {
		...
		char c = getopt_long (argc, argv, "k:p:h:", long_options,
				      &option_index);
		if (c == -1)
			break;
		...
		case 'h':
			...
			default:
				printf("Hiding size up to %u%c", n, u);
				print_usage();
				abort();
			}
			break;
	...
	if (argc - optind == 3 && strcmp(argv[optind], "send") == 0) {
		...
		Hacl_Tube_Send_file_send(strlen(file), (unsigned char*)file,
					 roundup, (unsigned char*)host, port,
					 sk, pk);
