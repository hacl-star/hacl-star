...
(* Testing the edge cases from https://github.com/novifinancial/ed25519-speccheck
 * in order to guard against silently introducing changes in the implementation
 * which, although consistent with RFC-8032, could result in changes w.r.t.
 * previous versions. *)

type ed25519_speccheck_case =
  { message: string; pub_key: string; signature: string; expected: bool }

(* Test  vectors from
 * https://github.com/novifinancial/ed25519-speccheck/blob/65519336fda78a3d016e947df6d82848aca0c9da/cases.json
 * The `expected` field refers to the current HACL* spec, not a general
 * expectation for all Ed25519 implementations. *)
let cases = [
  {
...
let ed25519_speccheck =
  let ed25519_speccheck_test () =
  List.iter (fun v ->
      let pk = bytes_of_hex v.pub_key in
      let msg = bytes_of_hex v.message in
      let signature = bytes_of_hex v.signature in
      Alcotest.(check bool "ed25519-speccheck" (Hacl.Ed25519.verify ~pk ~msg ~signature) v.expected)
    ) cases
  in
  [ ("ed25519-speccheck", `Quick, ed25519_speccheck_test); ]

let tests =
  [
    ("Ed25519", ed25519_speccheck)
  ]

let () =
  Alcotest.run "Ed25519" tests
