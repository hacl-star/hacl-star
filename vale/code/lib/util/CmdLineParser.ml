...
let parse_cmdline :
  (string * (Prims.bool ->
    ((Vale_X64_Decls.ins,Vale_X64_Decls.ocmp) Vale_X64_Machine_s.precode) Vale_Def_PossiblyMonad.possibly) * int * bool) list -> unit
  =
  fun l  ->
  ...
  then
    ...
    let l = List.map (fun (name, code, nbr_args, return_public) ->
        match code windows with
        | Vale_Def_PossiblyMonad.Ok c -> (name, (fun _ -> c), nbr_args, return_public)
