module Math.Poly2.Galois

module U = FStar.UInt
module PL = Math.Poly2.Lemmas
module D = Math.Poly2.Defs_s
//module PD = Math.Poly2.Defs
module GI = Math.Poly2.Galois.IntTypes
...
// TODO: move to Math.Poly2.Lemmas
let lemma_pointwise_equal (a b:poly) (pf:(i:int -> Lemma (a.[i] == b.[i]))) : Lemma (a == b) =
  FStar.Classical.forall_intro pf;
  lemma_equal a b

// TODO: move to Math.Poly2.Lemmas
let lemma_degree_high_bit (a:poly) : Lemma
  (ensures 0 <= degree a ==> a.[degree a])
  =
  reveal_defs ()

// TODO: move to Math.Poly2.Lemmas
let lemma_shift_define (p:poly) (n:int) : Lemma
  (forall (i:int).{:pattern (shift p n).[i]} (shift p n).[i] == (p.[i - n] && i >= 0))
  =
  reveal_defs ()

// TODO: move to Math.Poly2.Lemmas
let lemma_shift_define_all (_:unit) : Lemma
  (forall (p:poly) (n:int) (i:int).{:pattern (shift p n).[i]} (shift p n).[i] == (p.[i - n] && i >= 0))
  =
  reveal_defs ()

// TODO: move to Math.Poly2.Lemmas
let lemma_shift_define_forward (p:poly) (n:int) : Lemma
  (forall (i:int).{:pattern p.[i]} (shift p n).[i + n] == (p.[i] && i + n >= 0))
  =
  reveal_defs ()

// TODO: move to Math.Poly2.Lemmas
let lemma_shift_shift (a:poly) (m n:int) : Lemma
  (requires m >= 0 \/ n <= 0)
...
// TODO: move to Math.Poly2.Lemmas
let lemma_shift_mod (a b:poly) (n:nat) : Lemma
  (requires degree b >= 0)
  (ensures shift (a %. b) n %. b == shift a n %. b)
  =
  lemma_shift_is_mul a n;
  lemma_shift_is_mul (a %. b) n;
  PL.lemma_mod_mul_mod a b (monomial n);
  ()

// TODO: move to Math.Poly2.Lemmas
let lemma_mod_monomial (a:poly) (n:nat) : Lemma
  (forall (i:int).{:pattern (a %. monomial n).[i]} (a %. monomial n).[i] <==> i < n && a.[i])
  =
  PL.lemma_index_all ();
  PL.lemma_split_define a n;
  ()

// TODO: move to Math.Poly2.Lemmas
let lemma_degree_is (a:poly) (n:int) : Lemma
  (requires (n >= 0 ==> a.[n]) /\ (forall (i:int).{:pattern a.[i]} i > n ==> not a.[i]))
  (ensures degree a == max (-1) n)
  =
  PL.lemma_index_all ();
  if n >= 0 then PL.lemma_degree_is a n
  else lemma_equal a zero

// TODO: move to Math.Poly2.Lemmas
let lemma_degree_at_least (a:poly) : Lemma
  (requires True)
  (ensures degree a >= -1)
  [SMTPat (degree a)]
  =
  if degree a < 0 then PL.lemma_degree_negative a

// TODO: move to Math.Poly2.Lemmas
[@"opaque_to_smt"]
let bit_and a b =
  let x = of_fun (max 0 (max (degree a + 1) (degree b + 1))) (fun (i:nat) -> a.[i] && b.[i]) in
  PL.lemma_index_all ();
  lemma_degree_high_bit x;
  x
...
let bit_or a b =
  let x = of_fun (max 0 (max (degree a + 1) (degree b + 1))) (fun (i:nat) -> a.[i] || b.[i]) in
  PL.lemma_index_all ();
  lemma_degree_high_bit a;
  lemma_degree_high_bit b;
  lemma_degree_is x (max (degree a) (degree b));
  x
...
let lemma_irred_degree f =
  ...
  lemma_degree_is (irred_poly f) n;
  ()
...
let lemma_zero f =
  ...
  let eq_i (i:int) : Lemma (a.[i] == b.[i]) =
    ...
    ()
    in
  lemma_pointwise_equal a b eq_i
...
let lemma_shift_left f e n =
  ...
  PL.lemma_reverse_define_all ();
  lemma_shift_define_all ();
  lemma_mod_monomial (shift (to_poly e) (I.uint_v n)) nf;
...
//  s +. (if a.shift a (1 - k) = one then m else zero) // TODO: poly should be an eqtype
  s +. (if a.[k - 1] then irred else zero)

let fmul_t (f:G.field) = G.felem f & G.felem f & G.felem f

let fmul_iter (f:G.field) : i:nat -> fmul_t f -> fmul_t f =
  fun i (p, a, b) ->
...
let lemma_mul_pmul (a b:poly) : Lemma
  ...
  =
  lemma_pointwise_equal (mul_def a b) (pmul b a) (lemma_mul_pmul_k a b)
...
let lemma_mod_bit1 (a m:poly) : Lemma
  ...
  (
    lemma_degree_high_bit a;
    lemma_degree_high_bit m;
    lemma_degree_high_bit (a +. m);
...
let lemma_mod_shift1 (a irred:poly) (k:nat) : Lemma
  ...
  PL.lemma_one_define ();
  assert (m.[k]);
  lemma_mod_monomial (shift a 1) k;
  if a.[k - 1] then
  (
...
let rec lemma_mmul_smul_rec (a b m:poly) (n:nat) : Lemma
  ...
  (
    ...
    lemma_shift_shift b (-n1) (-1);
    lemma_shift_shift a n1 1;
    lemma_shift_mod (shift a n1) m 1;
...
let lemma_fmul_fmul (f:G.field) (a b:G.felem f) : Lemma
  ...
  let rec lem (n:nat) (f1:nat -> fmul_t f -> fmul_t f) (eqs:squash (forall (i:nat) (pab:fmul_t f). f1 i pab == fmul_iter f i pab)) : Lemma
    (repeati n (fmul_iter f) acc0 == repeati n f1 acc0)
    =
    if n = 0 then
    (
    ...
    in
  FStar.Classical.forall_impl_intro (lem (I.bits f.G.t - 1));
  ()
