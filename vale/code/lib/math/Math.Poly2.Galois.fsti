...
module P = Math.Poly2_s
open Math.Poly2_s
open Math.Poly2
open FStar.Seq

(*
Connect Spec.GaloisField to Math.Poly2*
*)
...
val bit_and (a b:poly) : Pure poly
  (requires True)
  (ensures fun x ->
    (degree x <= degree a) /\
    (degree x <= degree b) /\
    (forall (i:int).{:pattern (x.[i]) \/ (a.[i]) \/ (b.[i])} x.[i] <==> a.[i] && b.[i])
  )

// TODO: move to Math.Poly2.Lemmas
val bit_or (a b:poly) : Pure poly
  (requires True)
  (ensures fun x ->
    (degree x == max (degree a) (degree b)) /\
    (forall (i:int).{:pattern (x.[i]) \/ (a.[i]) \/ (b.[i])} x.[i] <==> a.[i] || b.[i])
  )
