...
open X64.Vale.StateEqSemantics
...
let eval_operand_eq_all o s1 s2 = ()

val eq_all_havoc (ins:BS.ins) (s1 s2:BS.state) : Lemma
  (requires state_eq_BS s1 s2)
  (ensures BS.havoc s1 ins == BS.havoc s2 ins)
  [SMTPat (BS.havoc s1 ins); SMTPat (state_eq_BS s1 s2)]

let eq_all_havoc ins s1 s2 = admit()
