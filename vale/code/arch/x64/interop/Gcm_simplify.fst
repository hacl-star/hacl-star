...
let be_quad32_to_bytes_sel q i =
  ...
  assert (Seq.index (Words.Seq_s.four_to_seq_BE q) 0 == q3);
  assert (Seq.index (Words.Seq_s.four_to_seq_BE q) 1 == q2);
  assert (Seq.index (Words.Seq_s.four_to_seq_BE q) 2 == q1);
  assert (Seq.index (Words.Seq_s.four_to_seq_BE q) 3 == q0);
                   ...
                   (Seq.index (Seq.init (Seq.length (Seq.init (Seq.length (four_to_seq_BE q))
                       ...
                       (fun () -> norm[primops; delta_only ["Types_s.le_quad32_to_bytes";
                          "Collections.Seqs_s.seq_map"; "Collections.Seqs_s.compose";
                            "Words.Seq_s.seq_four_to_seq_LE"]]);
...
let simplify_be_quad32 (q:quad32) : Lemma
 ...
 let aux (i:nat{i < 16}) : Lemma (Seq.index q_f i == Seq.index (le_quad32_to_bytes q) i) =
   GCM_helpers.le_quad32_to_bytes_sel q i;
...
let gcm_simplify4 b h =
  let b' = B.gsub b 0ul 16ul in
  assert (Seq.equal
    (B.as_seq h b')
    (Seq.slice (B.as_seq h b) 0 16));
  gcm_simplify2 (B.gsub b 0ul 16ul) h;
  DV.length_eq (get_downview b');
  assert (Seq.equal
    (seq_uint8_to_seq_nat8 (Seq.slice (B.as_seq h b) 0 16))
  ...
  calc (==) {
    ...
    (==) { UV.as_seq_sel h b_u 0; UV.get_sel h b_u 0; lemma_same_seq_dv h b;
           UV.as_seq_sel h b'_u 0; UV.get_sel h b'_u 0; lemma_same_seq_dv h b' }    
     low_buffer_read TUInt8 TUInt128 h b' 0;
  };
  
  le_bytes_to_quad32_to_bytes (low_buffer_read TUInt8 TUInt128 h b 0)
