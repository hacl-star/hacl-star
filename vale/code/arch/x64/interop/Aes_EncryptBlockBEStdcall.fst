...
open Vale_aes_EncryptBlockBEStdcall
...
val lemma_ghost_aes_EncryptBlockBEStdcall: is_win:bool -> output_b:s8 -> input_b:s8 -> key:Ghost.erased (aes_key_LE AES_128) -> keys_b:s8 ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 output_b input_b key keys_b /\ B.length stack_b == (if is_win then 256 else 96) /\ live h0 stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b]}) ->
  Ghost (TS.traceState * nat * HS.mem)
    ...
    (ensures (fun (s1, f1, h1) ->
      ...
      Some s1 == TS.taint_eval_code (va_code_aes_EncryptBlockBEStdcall is_win) f1 s0 /\
...
B.length stack_b == (if is_win then 256 else 96) /\ live h0 stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b] /\ (
  ...
  va_pre (va_code_aes_EncryptBlockBEStdcall is_win) s0 is_win stack_b output_b input_b (Ghost.reveal key) keys_b ))) =
  length_t_eq (TBase TUInt64) stack_b;
  length_t_eq (TBase TUInt128) output_b;
  length_t_eq (TBase TUInt128) input_b;
  length_t_eq (TBase TUInt128) keys_b;
...
let implies_post (is_win:bool) (va_s0:va_state) (va_sM:va_state) (va_fM:va_fuel) (output_b:s8) (input_b:s8) (key:Ghost.erased (aes_key_LE AES_128)) (keys_b:s8)  (stack_b:b8) : Lemma
  (requires pre_cond va_s0.mem.hs output_b input_b key keys_b /\ B.length stack_b == (if is_win then 256 else 96) /\ live va_s0.mem.hs stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b]/\
    va_post (va_code_aes_EncryptBlockBEStdcall is_win) va_s0 va_sM va_fM is_win stack_b output_b input_b (Ghost.reveal key) keys_b )
...
let lemma_ghost_aes_EncryptBlockBEStdcall is_win output_b input_b key keys_b stack_b h0 =
  length_t_eq (TBase TUInt64) stack_b;
  length_t_eq (TBase TUInt128) output_b;
  length_t_eq (TBase TUInt128) input_b;
  length_t_eq (TBase TUInt128) keys_b;
  ...
  let s_v, f_v = va_lemma_aes_EncryptBlockBEStdcall (va_code_aes_EncryptBlockBEStdcall is_win) s0' is_win stack_b output_b input_b (Ghost.reveal key) keys_b in
  implies_post is_win s0' s_v f_v output_b input_b key keys_b stack_b;
  let s1 = Some?.v (TS.taint_eval_code (va_code_aes_EncryptBlockBEStdcall is_win) f_v s0) in
...
irreducible
let aes_EncryptBlockBEStdcall_aux (output_b:s8) (input_b:s8) (key:Ghost.erased (aes_key_LE AES_128)) (keys_b:s8) (stack_b:b8) : Stack unit
  (fun h -> pre_cond h output_b input_b key keys_b  /\ B.length stack_b == (if win then 256 else 96) /\ live h stack_b /\ buf_disjoint_from stack_b [output_b;input_b;keys_b])
  (fun h0 _ h1 ->
    post_cond h0 h1 output_b input_b key keys_b  /\
    M.modifies (M.loc_union (M.loc_buffer stack_b) ( M.loc_buffer output_b)) h0 h1) =
  if win then
  st_put
    ...
    (fun h -> let _, _, h1 =
      lemma_ghost_aes_EncryptBlockBEStdcall true output_b input_b key keys_b stack_b h
    in (), h1)
  else st_put
    ...
    (fun h -> let _, _, h1 =
      lemma_ghost_aes_EncryptBlockBEStdcall false output_b input_b key keys_b stack_b h
    in (), h1)

let aes_EncryptBlockBEStdcall output_b input_b key keys_b  =
  push_frame();
  let (stack_b:b8) = B.alloca (UInt8.uint_to_t 0) (UInt32.uint_to_t (if win then 256 else 96)) in
  aes_EncryptBlockBEStdcall_aux output_b input_b key keys_b stack_b;
  pop_frame()
