...
open Vale_mk_quad32_lo0_be_1_stdcall
...
val lemma_ghost_mk_quad32_lo0_be_1_stdcall: is_win:bool -> b:s8 ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 b /\ B.length stack_b == 16 /\ live h0 stack_b /\ buf_disjoint_from stack_b [b]}) ->
  Ghost (TS.traceState * nat * HS.mem)
    (requires True)
    (ensures (fun (s1, f1, h1) ->
      (let s0 = create_initial_trusted_state is_win b stack_b h0 in
      Some s1 == TS.taint_eval_code (va_code_mk_quad32_lo0_be_1_stdcall is_win) f1 s0 /\
...
B.length stack_b == 16 /\ live h0 stack_b /\ buf_disjoint_from stack_b [b] /\ (
  ...
  va_pre (va_code_mk_quad32_lo0_be_1_stdcall is_win) s0 is_win stack_b b ))) =
  length_t_eq (TBase TUInt64) stack_b;
  length_t_eq (TBase TUInt128) b;
...
let implies_post (is_win:bool) (va_s0:va_state) (va_sM:va_state) (va_fM:va_fuel) (b:s8)  (stack_b:b8) : Lemma
  (requires pre_cond va_s0.mem.hs b /\ B.length stack_b == 16 /\ live va_s0.mem.hs stack_b /\ buf_disjoint_from stack_b [b]/\
    va_post (va_code_mk_quad32_lo0_be_1_stdcall is_win) va_s0 va_sM va_fM is_win stack_b b )
...
let lemma_ghost_mk_quad32_lo0_be_1_stdcall is_win b stack_b h0 =
  length_t_eq (TBase TUInt64) stack_b;
  length_t_eq (TBase TUInt128) b;
  ...
  let s_v, f_v = va_lemma_mk_quad32_lo0_be_1_stdcall (va_code_mk_quad32_lo0_be_1_stdcall is_win) s0' is_win stack_b b in
  implies_post is_win s0' s_v f_v b stack_b;
  let s1 = Some?.v (TS.taint_eval_code (va_code_mk_quad32_lo0_be_1_stdcall is_win) f_v s0) in
...
irreducible
let mk_quad32_lo0_be_1_stdcall_aux (b:s8) (stack_b:b8) : Stack unit
  ...
  st_put
    (fun h -> pre_cond h b /\ B.length stack_b == 16 /\ live h stack_b /\ buf_disjoint_from stack_b [b])
    (fun h -> let _, _, h1 =
      lemma_ghost_mk_quad32_lo0_be_1_stdcall true b stack_b h
    in (), h1)
  else st_put
    (fun h -> pre_cond h b /\ B.length stack_b == 16 /\ live h stack_b /\ buf_disjoint_from stack_b [b])
    (fun h -> let _, _, h1 =
      lemma_ghost_mk_quad32_lo0_be_1_stdcall false b stack_b h
    in (), h1)

let mk_quad32_lo0_be_1_stdcall b  =
  push_frame();
  let (stack_b:b8) = B.alloca (UInt8.uint_to_t 0) (UInt32.uint_to_t 16) in
  mk_quad32_lo0_be_1_stdcall_aux b stack_b;
  pop_frame()
