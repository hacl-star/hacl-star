...
let lemma_k_reqs_equiv k_b h =
  ...
  let aux (i:nat{i < size_k_w_256/4}) : Lemma
    ...
    UV.get_sel h ub i;
    //Proves
    //assert (k_seq.[i] == Views.get128 (Seq.slice down_s (i*16) (i*16+16)));
    ...
    // Gives us that Seq.slice down_s (i*16) (i*16+4) == Views.put32 (Seq.index s (4*i)) and same for other indices
...
let simplify_le_bytes_to_hash_uint32 b h =
  ...
  let aux (i:nat {i < Seq.length sf}) : Lemma (Seq.index sf i = Seq.index s_init i)
    = let i' = i/4 in
      ...
      assert (Views.get128 (Seq.slice (DV.as_seq h db) 
