...
let wrap code pre post v = fun (x0:M.buffer64) (x1:M.buffer64) ->
  ...
  //let h3 = HST.get () in
  //assert (LB.modifies (LB.loc_union (LB.loc_buffer (to_b8 stack_b)) (LB.loc_union (LB.loc_buffer (to_b8 x0)) (LB.loc_buffer (to_b8 x1)))) h0 h3);
  //assert (post IA.win x0 x1 va_s0 stack_b va_s1);
  HST.pop_frame ();
  let h4 = HST.get () in
  //TODO: assert (LB.modifies (LB.loc_union (LB.loc_buffer (to_b8 x0)) (LB.loc_buffer (to_b8 x1))) h0 h4);
