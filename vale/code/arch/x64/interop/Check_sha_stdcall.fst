...
val lemma_ghost_check_sha_stdcall: is_win:bool ->  stack_b:b8 -> (h0:HS.mem{pre_cond h0 /\ B.length stack_b == 8 /\ live h0 stack_b /\ buf_disjoint_from stack_b []}) ->
  Ghost (TS.traceState * nat * HS.mem * nat64)
    (requires True)
    (ensures (fun (s1, f1, h1, ret_val) ->
      (let s0 = create_initial_trusted_state is_win stack_b h0 in
      Some s1 == TS.taint_eval_code (va_code_check_sha_stdcall is_win) f1 s0 /\
...
#set-options "--max_fuel 1 --initial_fuel 1 --max_ifuel 0"
...
B.length stack_b == 8 /\ live h0 stack_b /\ buf_disjoint_from stack_b [] /\ (
  ...
  va_pre (va_code_check_sha_stdcall is_win) s0 is_win stack_b ))) =
  length_t_eq (TBase TUInt64) stack_b;
  ()
...
let lemma_ghost_check_sha_stdcall is_win stack_b h0 =
  length_t_eq (TBase TUInt64) stack_b;
  implies_pre is_win h0 stack_b;
  let s0 = create_initial_trusted_state is_win stack_b h0 in
  let s0' = create_initial_vale_state is_win stack_b h0 in
  create_lemma is_win stack_b h0;
  let s_v, f_v = va_lemma_check_sha_stdcall (va_code_check_sha_stdcall is_win) s0' is_win stack_b in
...
let check_sha_stdcall () =
  ...
  st_put
    ...
    (fun h -> let _, _, h1, x =
        lemma_ghost_check_sha_stdcall true stack_b h
    in x, h1)
  else st_put
    (fun h -> pre_cond h /\ B.length stack_b == 8 /\ live h stack_b /\ buf_disjoint_from stack_b [])
    (fun h -> let _, _, h1, x =
        lemma_ghost_check_sha_stdcall false stack_b h
    in x, h1))
  in pop_frame();
  UInt64.uint_to_t x
