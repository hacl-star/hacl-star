...
val same_mem_eq_slices128 (b:buffer128)
  ...
  (ensures (let open FStar.Mul in
    k * 16 + 16 <= B.length b /\
    Seq.slice (B.as_seq (IB.hs_of_mem h1) b) (k * 16) (k * 16 + 16) ==
    Seq.slice (B.as_seq (IB.hs_of_mem h2) b) (k * 16) (k * 16 + 16)))

let same_mem_eq_slices128 b i v k h1 h2 mem1 mem2 =
    let t = TUInt128 in
    BV.as_seq_sel (IB.hs_of_mem h1) (BV.mk_buffer_view b (uint_view t)) k;
    BV.as_seq_sel (IB.hs_of_mem h2) (BV.mk_buffer_view b (uint_view t)) k;
    BV.put_sel (IB.hs_of_mem h1) (BV.mk_buffer_view b (uint_view t)) k;
    BV.put_sel (IB.hs_of_mem h2) (BV.mk_buffer_view b (uint_view t)) k;
...
let same_mem_get_heap_val128 b j v k h1 h2 mem1 mem2 =
  ...
  let aux (i:nat{i < 16}) : Lemma (mem1.[addr+(16 `op_Multiply` k + i)] == mem2.[addr+(16 `op_Multiply` k +i)]) =
    BV.as_seq_sel (IB.hs_of_mem h1) (BV.mk_buffer_view b uint128_view) k;
    BV.as_seq_sel (IB.hs_of_mem h2) (BV.mk_buffer_view b uint128_view) k;
    same_mem_eq_slices128 b j v k h1 h2 mem1 mem2;
    let open FStar.Mul in
    let s1 = (Seq.slice (B.as_seq (IB.hs_of_mem h1) b) (k * 16) (k * 16 + 16)) in
    let s2 = (Seq.slice (B.as_seq (IB.hs_of_mem h2) b) (k * 16) (k * 16 + 16)) in
    assert (Seq.index s1 i == Seq.index (B.as_seq (IB.hs_of_mem h1) b) (k * 16 + i));
    length_up128 b h1 k i;
    assert (mem1.[addr+(16 * k + i)] == UInt8.v (Seq.index (B.as_seq (IB.hs_of_mem h1) b) (k * 16 + i)));
    assert (Seq.index s2 i == Seq.index (B.as_seq (IB.hs_of_mem h2) b) (k * 16 + i));
    length_up128 b h2 k i;
    assert (mem2.[addr+(16 * k + i)] == UInt8.v (Seq.index (B.as_seq (IB.hs_of_mem h2) b) (k * 16 + i)))
  in
...
let equiv_load_mem ptr h =
  ...
  let contents = B.as_seq h.IB.hs b in
...
let low_lemma_store_mem64_aux 
  ...
  : Lemma
    ...
    (ensures (let heap' = S.update_heap64 (buffer_addr b h + 8 `op_Multiply` i) v heap in
     ...
     h'.IB.hs == B.g_upd_seq b (I.get_seq_heap heap' h.IB.addrs b) h.IB.hs)) =
   ...
   store_buffer_write TUInt64 ptr v h;
   let b' = get_addr_ptr TUInt64 ptr h in
   assert (IB.disjoint_or_eq_b8 b b');
   length_t_eq TUInt64 b';
...
let low_lemma_store_mem128_aux 
  ...
  : Lemma
   ...
   length_t_eq TUInt128 b;
   store_buffer_write TUInt128 ptr v h;
   let b' = get_addr_ptr TUInt128 ptr h in
   assert (IB.disjoint_or_eq_b8 b b');
   length_t_eq TUInt128 b';
...
let rec written_buffer_down128_aux1 (b:buffer128) (i:nat{i < buffer_length b}) (v:quad32)
      (ps:list b8{IB.list_disjoint_or_eq ps /\ List.memP b ps})
      (h:mem{forall x. List.memP x ps ==> List.memP x (IB.ptrs_of_mem h)})
...
let written_buffer_down128 (b:buffer128) (i:nat{i < buffer_length b}) (v:quad32) (h:mem)
  : Lemma
      (requires List.memP b h.IB.ptrs)
      (ensures (
          let mem1 = I.down_mem h in
          let h1 = buffer_write b i v h in
          let mem2 = I.down_mem h1 in
...
let store_buffer_aux_down128_mem (ptr:int) (v:quad32) (h:mem{valid_mem128 ptr h})
  : Lemma
      (ensures (
        let mem1 = I.down_mem h in
        let h1 = store_mem TUInt128 ptr v h in
...
let store_buffer_aux_down128_mem2 (ptr:int) (v:quad32) (h:mem{valid_mem128 ptr h})
  : Lemma
      (ensures (
        let h1 = store_mem TUInt128 ptr v h in
        let mem2 = I.down_mem h1 in
