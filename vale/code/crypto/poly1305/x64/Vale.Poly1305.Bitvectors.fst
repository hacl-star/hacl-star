...
let lemma_and_mod_n x =
  assert (logand #64 x 3 == mod #64 x 4) by bv_tac ();
  assert (logand #64 x 15 == mod #64 x 16) by bv_tac ()
...
let lemma_and_constants x =
 assert (logand #64 x 0 == (0 <: uint_t 64)) by bv_tac ();
 assert (logand #64 x 0xffffffffffffffff == x) by bv_tac ()
...
let mul_bvshl (u:uint_t 64) :
  ...
  assert (int2bv #128 (mul_mod #128 0x10000000000000000 (uint_ext #64 #128 u)) ==
          bvmul #128 (int2bv #128 0x10000000000000000) (uint_ext #64 #128 u))
      by (mapply (`FStar.Tactics.BV.Lemmas.trans);
          arith_to_bv_tac ();
          trefl ()
      )
#pop-options
...
#push-options "--smtencoding.elim_box true --z3refresh --z3rlimit 100 --ifuel 1 --fuel 1"
