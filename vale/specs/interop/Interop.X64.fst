...
let wrap_variadic c num_b8_slots args #pre_rel #post_rel predict =
  let h0 = ST.get () in
  let h0' = h0 in
  ...
  let (fuel, final_mem) =
    IA.st_put
      ...
      (fun h0' ->
        let va_s0, mem_s0 =
          create_initial_trusted_state num_b8_slots args h0' stack_b in
...
let rec wrap_aux
  ...
  : as_lowstar_sig_t c num_b8_slots dom args pre_rel post_rel predict
  = match dom with
    | [] ->
      let f () :
        FStar.HyperStack.ST.Stack als_ret
           ...
           (ensures fun h0 ret h1 ->
             as_lowstar_sig_post c num_b8_slots args h0 #pre_rel #post_rel (elim_predict_t_nil predict) ret h1) =
        wrap_variadic c num_b8_slots args (elim_predict_t_nil predict)
      in
      f <: as_lowstar_sig_t c num_b8_slots [] args pre_rel post_rel predict

    | hd::tl ->
      fun (x:td_as_type hd) ->
      wrap_aux c num_b8_slots tl
        (x ++ args)
        (elim_rel_gen_t_cons hd tl pre_rel x)
        (elim_rel_gen_t_cons hd tl post_rel x)
        (elim_predict_t_cons hd tl predict x)

let wrap c num_b8_slots dom #pre_rel #post_rel predict =
  wrap_aux c num_b8_slots dom [] pre_rel post_rel predict
...
let rec wrap_aux_weak
    ...
    (post_rel:rel_gen_t c dom args (prediction_post_rel_t c num_b8_slots))
    (predict:prediction_t c num_b8_slots dom args pre_rel post_rel)
  : as_lowstar_sig_t_weak c num_b8_slots dom args pre_rel post_rel predict
  = match dom with
    | [] ->
      let f () :
        FStar.HyperStack.ST.Stack als_ret
           (requires fun h0 ->
             mem_roots_p h0 args /\ elim_rel_gen_t_nil pre_rel h0)
           (ensures fun h0 ret h1 ->
             as_lowstar_sig_post_weak c num_b8_slots args h0 #pre_rel #post_rel (elim_predict_t_nil predict) ret h1) =
         wrap_variadic c num_b8_slots args (elim_predict_t_nil predict)
      in
      f <: as_lowstar_sig_t_weak c num_b8_slots [] args pre_rel post_rel predict

    | hd::tl ->
      fun (x:td_as_type hd) ->
      wrap_aux_weak c num_b8_slots tl
        (x ++ args)
        (elim_rel_gen_t_cons hd tl pre_rel x)
        (elim_rel_gen_t_cons hd tl post_rel x)
        (elim_predict_t_cons hd tl predict x)

let wrap_weak c num_b8_slots dom #pre_rel #post_rel predict =
  wrap_aux_weak c num_b8_slots dom [] pre_rel post_rel predict
