...
let rec wrap_aux
    ...
    : as_lowstar_sig c dom args
    = match dom with
      | [] ->
        let f : as_lowstar_sig c [] args =
          let ff (h0:mem_roots args) (predict:prediction c args h0)
            : ST.Stack (as_lowstar_sig_ret args)
            ...
            let (fuel, final_mem) = IA.st_put (fun h0' -> h0' == h2) (fun h0' ->
              let va_s0, mem_s0 =
          ...
          in ff
        in
        f <: as_lowstar_sig c [] args

      | hd::tl ->
        let f (x:td_as_type hd)
           : as_lowstar_sig c tl ((| hd, x |) :: args)
           = wrap_aux c tl ((| hd, x |) :: args)
        in
        f
