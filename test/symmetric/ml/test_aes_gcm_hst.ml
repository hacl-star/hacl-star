...
let hex_string_to_u8s buffer str =
  for i = 0 to (String.length str / 2 - 1) do
    let block = String.concat "" ["0x"; String.sub str (i * 2) 2] in
    upd buffer i (Hacl_Cast.uint8_to_sint8 (int_of_string block))
  done
...
let test_crypt n k p iv a c t =
  ...
  hex_string_to_u8s key k;
  hex_string_to_u8s plaintext p;
  hex_string_to_u8s nonce iv;
  hex_string_to_u8s aad a;
