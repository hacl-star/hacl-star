...
let expected_x = "6B17D1F2E12C4247F8BCE6E563A440F277037D812DEB33A0F4A13945D898C296"
let expected_y = "B01CBD1C01E58065711814B583F061E9D431CCA994CEA1313449BF97C840AE0A"
...
let print_bytes b =
  for i = 0 to b.length - 1 do print_string (Hacl_UInt8.to_string_hex (index b i)) done;
...
let _ =
  ...
  let qx = import_from_string x in
  let qy = import_from_string y in
  let output = {content = Array.create 64 (Hacl_UInt8.of_string "0"); idx=0; length = 64} in
  Curve_P256.exp output qx qy scalar;
  let x = sub output 0 32 in
  let y = sub output 32 32 in
  print_string "Got:\n";
  print_bytes x;
  print_bytes y;
  print_string ("Expected:\n" ^ expected_x ^ "\n" ^ expected_y ^ "\n")(* ; *)
