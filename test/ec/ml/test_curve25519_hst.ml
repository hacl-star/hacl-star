...
let print_bigint_64 b =
  let mask = Hacl_UInt64.of_string "0x3ffffff" in
  for i = 0 to (norm_length)-1 do
    print_string (Hacl_UInt64.to_string (index b i));
    print_string " ";
  done;
  print_string "\n"
...
let print_bigint_donna_128 b =
  for i = 0 to Array.length b.content - 1 do
    print_string (Hacl_UInt128.to_string (index b i));
    print_string " ";
  done;
  print_string "\n"
...
let format_secret s =
  upd s 0 (Hacl_UInt8.logand (index s 0) (Hacl_UInt8.of_string "248"));
  upd s 31 (Hacl_UInt8.logand (index s 31) (Hacl_UInt8.of_string "127"));
  upd s 31 (Hacl_UInt8.logand (Hacl_UInt8.logor (index s 31) (Hacl_UInt8.of_string "64")) (Hacl_UInt8.of_string "255"))
...
let get_input input_string =
  ...
  let input51 = Array.make 5 Hacl_UInt64.zero in
  let mask = Uint64.of_string "0x7ffffffffffff" in
  input51.(0) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand input64.(0) mask));
  input51.(1) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.add (Uint64.logand (Uint64.shift_right input64.(0) 51) mask)
			    (Uint64.logand (Uint64.shift_left (input64.(1)) 13) mask)));
  input51.(2) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.add (Uint64.logand (Uint64.shift_right input64.(1) 38) mask)
			    (Uint64.logand (Uint64.shift_left (input64.(2)) 26) mask)));
  input51.(3) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.add (Uint64.logand (Uint64.shift_right input64.(2) 25) mask)
			    (Uint64.logand (Uint64.shift_left (input64.(3)) 39) mask)));
  input51.(4) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (Uint64.shift_right input64.(3) 12) mask));
...
let get_input2 input_string =
  ...
  let rec mk64 b idx i =
    ...
    | 0 -> Uint64.zero | _ -> Uint64.add (Uint64.shift_left (Uint64.of_int b.(idx+i-1)) (8*(i-1)))
				(mk64 b idx (i-1)) in
  let input51 = Array.make 5 Hacl_UInt64.zero in
  input51.(0) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (mk64 bytes 0 8) mask));
  input51.(1) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (Uint64.shift_right (mk64 bytes 6 8) 3) mask));
  input51.(2) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (Uint64.shift_right (mk64 bytes 12 8) 6) mask));
  input51.(3) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (Uint64.shift_right (mk64 bytes 19 8) 1) mask));
  input51.(4) <- Hacl_UInt64.of_string (Uint64.to_string (Uint64.logand (Uint64.shift_right (mk64 bytes 24 8) 12) mask));
  {content = input51; idx = 0; length = 5}
         
let get_output output =
  let input51 = output.content in
  let input64 = Array.make 5 Hacl_UInt64.zero in
  input64.(0) <- Hacl_UInt64.add input51.(0) (Hacl_UInt64.shift_left input51.(1) 51);
  input64.(1) <- Hacl_UInt64.add (Hacl_UInt64.shift_right input51.(1) 13) (Hacl_UInt64.shift_left input51.(2) 48);
  input64.(2) <- Hacl_UInt64.add (Hacl_UInt64.shift_right input51.(2) 26) (Hacl_UInt64.shift_left input51.(3) 25);
  input64.(3) <- Hacl_UInt64.add (Hacl_UInt64.shift_right input51.(3) 39) (Hacl_UInt64.shift_left input51.(4) 12);
  let s = ref "" in
  for i = 0 to 3 do
    for j = 0 to 7 do
      let s' = (Uint8.to_string_hex (Uint8.of_uint64 (Uint64.of_string (Hacl_UInt64.to_string (Hacl_UInt64.shift_right input64.(i) (j*8)))))) in
...
let test4 () =
  (* Output data *)
  let output = create zero norm_length in
  ...
  let qy = create zero norm_length in
  let qz = create zero norm_length in
  upd qz 0 one;
  print_string "Basepoint : \n";
  print_bigint_64 qx;
  print_bigint_64 qz;
  let basepoint = Point(qx, qy, qz) in

  (* Point to store the result *)
  let resx = create zero norm_length in
  let resy = create zero norm_length in
  let resz = create zero norm_length in
  let res = Point(resx, resy, resz) in

  (* Ladder *)
  time (fun () ->
      for i = 0 to 0 do
        Curve_Ladder.montgomery_ladder res scalar basepoint;
        let zrecip = create zero norm_length in
        (* Get the affine coordinates back *)
        Curve_Crecip.crecip' zrecip (get_z res);
