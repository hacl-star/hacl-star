...
static inline
void Hacl_EC_Curve25519_recursive_fsum_(uint64_t *a, const uint64_t *b, uint32_t ctr)
{
  if (ctr == (uint32_t )0)
    return;
  else
  {
    ...
    uint64_t bi = b[i];
    a[i] = ai + bi;
    Hacl_EC_Curve25519_recursive_fsum_(a, b, i);
    return;
  }
}

static inline
void Hacl_EC_Curve25519_recursive_fsum(uint64_t *a, const uint64_t *b)
{
  Hacl_EC_Curve25519_recursive_fsum_(a, b, (uint32_t )5);
  return;
}

static inline
void Hacl_EC_Curve25519_recursive_fdifference_(uint64_t *a, const uint64_t *b, uint32_t ctr)
{
  if (ctr == (uint32_t )0)
    return;
  else
  {
    ...
    uint64_t bi = b[i];
    a[i] = bi - ai;
    Hacl_EC_Curve25519_recursive_fdifference_(a, b, i);
    return;
  }
}
...
void Hacl_EC_Curve25519_recursive_fdifference(uint64_t *a, uint64_t *b)
{
  ...
  zero_prime[5] =
    {
      (uint64_t )0x3fffffffffff68,
      (uint64_t )0x3ffffffffffff8,
      (uint64_t )0x3ffffffffffff8,
      (uint64_t )0x3ffffffffffff8,
      (uint64_t )0x3ffffffffffff8
    };
  Hacl_EC_Curve25519_recursive_fsum(b, zero_prime);
  Hacl_EC_Curve25519_recursive_fdifference_(a, b, (uint32_t )5);
}
...
static inline
void Hacl_EC_Curve25519_recursive_fscalar_product(uint64_t *output, const uint64_t *a, uint64_t s)
{
  FStar_UInt128_t t[5] = {0};
...
static inline
void Hacl_EC_Curve25519_recursive_fmul(uint64_t *output, const uint64_t *input, const uint64_t *input2)
{
  uint64_t tmp[5] = { 0 };
  FStar_UInt128_t t[5] = {0};
  for (uint32_t i = 0; i < 5; i++) tmp[i] = input[i];
...
void Hacl_EC_Curve25519_recursive_fsquare_times_(uint64_t *tmp, uint32_t count)
{
  ...
  {
    Hacl_EC_Curve25519_recursive_fmul(tmp, tmp, tmp);
    Hacl_EC_Curve25519_recursive_fsquare_times_(tmp, count - (uint32_t )1);
    return;
  }
}
...
static inline
void
Hacl_EC_Curve25519_recursive_fsquare_times(uint64_t *output, const uint64_t *input, uint32_t count)
{
  uint64_t tmp[5] = { 0 };
  for (uint32_t i = 0; i < 5; i++) tmp[i] = input[i];
  //  memcpy(tmp, input, (uint32_t )5 * sizeof(uint64_t));
  Hacl_EC_Curve25519_recursive_fsquare_times_(tmp, count);
  for (uint32_t i = 0; i < 5; i++) output[i] = tmp[i];
  //  memcpy(output, tmp, (uint32_t )5 * sizeof(uint64_t));
}

static inline
uint64_t Hacl_EC_Curve25519_recursive_load64_le(const uint8_t *b)
{
...
static inline
void Hacl_EC_Curve25519_recursive_fexpand(uint64_t *output, const uint8_t *input)
{
...
static inline
void Hacl_EC_Curve25519_recursive_fcontract(uint8_t *output, const uint64_t *input)
{
  uint64_t mask_51 = (uint64_t )0x7ffffffffffff;
...
)
{
  ...
  for (uint32_t i = 0; i < 5; i++) origx[i] = x[i];
  //  memcpy(origx, x, (uint32_t )5 * sizeof(uint64_t));
  Hacl_EC_Curve25519_recursive_fsum(x, z);
  Hacl_EC_Curve25519_recursive_fdifference(z, origx);
  for (uint32_t i = 0; i < 5; i++) origxprime[i] = xprime[i];
  //  memcpy(origxprime, xprime, (uint32_t )5 * sizeof(uint64_t));
  ...
  for (uint32_t i = 0; i < 5; i++) origxprime[i] = xxprime[i];
  //memcpy(origxprime, xxprime, (uint32_t )5 * sizeof(uint64_t));
...
{
  uint64_t a00 = a[(uint32_t )0];
  uint64_t a10 = a[(uint32_t )1];
  uint64_t a20 = a[(uint32_t )2];
  uint64_t a30 = a[(uint32_t )3];
  uint64_t a40 = a[(uint32_t )4];
  uint64_t b00 = b[(uint32_t )0];
  uint64_t b10 = b[(uint32_t )1];
  uint64_t b20 = b[(uint32_t )2];
  uint64_t b30 = b[(uint32_t )3];
  uint64_t b40 = b[(uint32_t )4];
  uint64_t swap = (uint64_t )0 - iswap;
  uint64_t x0 = swap & (a00 ^ b00);
  uint64_t a0 = a00 ^ x0;
  uint64_t b0 = b00 ^ x0;
  uint64_t x1 = swap & (a10 ^ b10);
  uint64_t a1 = a10 ^ x1;
  uint64_t b1 = b10 ^ x1;
  uint64_t x2 = swap & (a20 ^ b20);
  uint64_t a2 = a20 ^ x2;
  uint64_t b2 = b20 ^ x2;
  uint64_t x3 = swap & (a30 ^ b30);
  uint64_t a3 = a30 ^ x3;
  uint64_t b3 = b30 ^ x3;
  uint64_t x = swap & (a40 ^ b40);
  uint64_t a4 = a40 ^ x;
  uint64_t b4 = b40 ^ x;
  a[(uint32_t )0] = a0;
  a[(uint32_t )1] = a1;
...
)
{
  ...
  for (uint32_t i = 0; i < 5; i++) nqpqx[i] = q[i];
  //memcpy(nqpqx, q, (uint32_t )5 * sizeof(uint64_t));
  Hacl_EC_Curve25519_recursive_cmult_big_loop(n,
    nqx,
    nqz,
    nqpqx,
    ...
    q,
    (uint32_t )32);
  for (uint32_t i = 0; i < 5; i++) resultx[i] = nqx[i];
  //memcpy(resultx, nqx, (uint32_t )5 * sizeof(uint64_t));
  for (uint32_t i = 0; i < 5; i++) resultz[i] = nqz[i];
  //memcpy(resultz, nqz, (uint32_t )5 * sizeof(uint64_t));
}
...
)
{
  ...
  memcpy(e, secret, (uint32_t )32 * sizeof(uint8_t));
