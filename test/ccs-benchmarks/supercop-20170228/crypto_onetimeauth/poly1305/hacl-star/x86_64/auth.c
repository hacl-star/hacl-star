...
inline static void Hacl_Bignum_Fsum_fsum_(uint64_t *a, const uint64_t *b, uint32_t i)
{
  if (i == (uint32_t )0)
    return;
  else
...
inline static void
Hacl_Bignum_Fproduct_copy_from_wide_(uint64_t *output, const FStar_UInt128_t *input, uint32_t ctr)
{
  if (ctr == (uint32_t )0)
    return;
  else
...
inline static void Hacl_Bignum_Fmul_fmul_(uint64_t *output, uint64_t *input, const uint64_t *input2)
{
  FStar_UInt128_t t[3];
...
inline static void Hacl_Bignum_Fmul_fmul(uint64_t *output, const uint64_t *input, uint64_t const *input2)
{
  uint64_t tmp[3] = { 0 };
...
inline static void
Hacl_Bignum_AddAndMultiply_add_and_multiply(uint64_t *acc, const uint64_t *block, const uint64_t *r)
{
...
)
{
  ...
  else
  {
    ...
    *new_log = (Hacl_Impl_Poly1305_64_poly1305_update((void *)(uint8_t )0, st, m) , (void *)0);
    ...
    const uint8_t *m0 = m + (uint32_t )16;
    ite =
...
)
{
  ...
  const uint8_t *partial_input = input;
  const uint8_t *last_block = input + (uint32_t )16 * (uint32_t )len16;
  uint64_t buf[6] = { 0 };
  uint64_t *r = buf;
  uint64_t *h = buf + (uint32_t )3;
  Hacl_Impl_Poly1305_64_poly1305_state st = { .x00 = r, .x01 = h };
  const uint8_t *key_r = k;
  const uint8_t *key_s = k + (uint32_t )16;
  uint64_t *x0 = Hacl_Impl_Poly1305_64___proj__MkState__item__r(st);
  const uint8_t *x1 = key_r;
