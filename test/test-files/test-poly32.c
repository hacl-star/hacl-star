...
int32_t test_poly()
{
  ...
  Hacl_Poly1305_32_crypto_onetimeauth(mac, plaintext, 34, key);
  TestLib_compare_and_print("HACL Poly1305", expected, mac, MACSIZE);
  Hacl_Poly1305_32_crypto_onetimeauth(mac, plaintext, len_, key);
...
int32_t perf_poly() {
  ...
  for (int i = 0; i < ROUNDS; i++){
    Hacl_Poly1305_32_crypto_onetimeauth(macs + MACSIZE * i, plain, len, key);
  }
