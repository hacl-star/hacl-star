...
assume val file_content: FStar.HyperStack.mem -> file_stat -> GTot (seq h8)
...
assume val file_open_read_sequential: n:uint8_p -> fb:buffer file_handle -> Stack fresult
...
assume val file_open_write_sequential: file_stat -> fb:buffer file_handle -> Stack fresult
    (requires (fun h -> length fb = 1 /\ live h fb))
    (ensures  (fun h0 s h1 -> match s with 
    	      	      	    | FileOk -> 
			      live h1 fb /\
...
assume val file_next_read_buffer: fb:buffer file_handle -> len:u64 -> Stack uint8_p
...
assume val file_close: fb:buffer file_handle -> Stack bool
