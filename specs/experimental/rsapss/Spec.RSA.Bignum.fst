...
let mont_reduction_f #nBits #cBits rBits nInv_u64 n c i res =
  ...
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + (pow2 (64*i)-1) * bn_v n + bn_v n * pow2 (64*i) * bn_v qi); //from ind hypo
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + (pow2 (64*i)-1) * bn_v n + bn_v n * pow2 (64*i) * (pow2 64 - 1));
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + pow2 (64*i) * bn_v n - bn_v n + bn_v n * pow2 (64*i) * (pow2 64 - 1));
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + pow2 (64*i) * bn_v n - bn_v n + bn_v n * pow2 (64*i) * pow2 64 - bn_v n * pow2 (64*i));
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c - bn_v n + bn_v n * pow2 (64*i) * pow2 64);
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + (pow2 (64*i) * pow2 64 - 1) * bn_v n);
  pow2_plus (64*i) 64;
  assert (64*(i+1) <= rBits);
  assert (bn_v res + bn_v n * pow2 (64*i) * bn_v qi <= bn_v c + (pow2 (64*(i+1)) - 1) * bn_v n);
...
let mod_exp_f nBits rBits nInv_u64 n bBits b a0 acc0 i (a, acc) =
  ...
  assume (mod_exp_pred nBits rBits nInv_u64 n bBits b a0 acc0 (i+1) (a, acc));
  (a, acc)
...
let mod_exp #aBits nBits n a bBits b =
  ...
  assert (bn_v a_r % bn_v n == (bn_v a * pow2 rBits) % bn_v n);
  let acc_r = to_mont nBits rBits nInv_u64 n r2 (bn_const_1 #nBits) in
  assert (bn_v acc_r % bn_v n == pow2 rBits % bn_v n);
  let res_r = mod_exp_ nBits rBits nInv_u64 n a_r bBits b acc_r in
  assert (bn_v res_r % bn_v n == ((pow (bn_v a_r) (bn_v b)) * (bn_v acc_r) / pow (pow2 rBits) (bn_v b)) % bn_v n);
  admit();
