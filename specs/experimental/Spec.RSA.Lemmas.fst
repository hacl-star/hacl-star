...
let lemma_mod_exp n a a2 b b2 acc r res =
    ...
    //assert (res % n == (((pow a2 b2) % n) * acc / pow r b2) % n);
    lemma_pow_mod a2 b2 n;
    //assert ((pow a2 b2) % n == (pow (a2 % n) b2) % n);
    //assert ((pow a2 b2) % n == (pow ((a * a / r) % n) b2) % n);
    lemma_pow_mod (a * a / r) b2 n;
    //assert ((pow a2 b2) % n == (pow (a * a / r) b2) % n);
    lemma_pow_div (a * a) b2 r;
    //assert ((pow a2 b2) % n == ((pow (a * a) b2) / pow r b2) % n);
    lemma_pow_a2_b a b2;
    //assert ((pow a2 b2) % n == (pow a (2 * b2) / pow r b2) % n);
    //assert (res % n == (((pow a (2 * b2) / pow r b2) % n) * acc / pow r b2) % n);
    lemma_mod_mult_div (pow a (2 * b2) / pow r b2) acc (pow r b2) n;
    //assert (res % n == (((pow a (2 * b2) / pow r b2) * acc) / pow r b2) % n);
    lemma_mod_mult_div_1 ((pow a (2 * b2) / pow r b2) * acc) (pow r b2) n;
    //assert (res % n == ((((pow a (2 * b2) / pow r b2) * acc) % n) / pow r b2) % n);
    lemma_mult_div_mod acc (pow a (2 * b2)) (pow r b2) n;
    //assert (res % n == ((((acc * pow a (2 * b2)) / pow r b2) % n) / pow r b2) % n);
    lemma_mod_mult_div_1 ((acc * pow a (2 * b2)) / pow r b2) (pow r b2) n;
    //assert (res % n == (((acc * pow a (2 * b2)) / pow r b2) / pow r b2) % n);
    division_multiplication_lemma (acc * pow a (2 * b2)) (pow r b2) (pow r b2);
    //assert (res % n == ((acc * pow a (2 * b2)) / (pow r b2 * pow r b2)) % n);
    lemma_pow r b2 b2
    //assert (pow r b2 * pow r b2 == pow r (2 * b2))
...
let lemma_mod_exp_1 n a a2 b b2 acc acc' r res =
    ...
    assert (res % n == ((pow a (2 * b2 + 1)) * acc / pow r (2 * b2 + 1)) % n)
...
let lemma_mod_exp_2 n a a_r b acc_r r res_r =
    ...
    //assert ((pow a_r b) % n == (pow (a_r % n) b) % n);
    ...
    //assert (res_r % n == ((pow a b) * acc_r) % n);
    lemma_mod_mul_distr_l acc_r (pow a b) n;
    //assert (res_r % n == ((acc_r % n) * pow a b) % n);
    //assert (res_r % n == ((r % n) * pow a b) % n);
