...
let setup (k:key) (n:nonce) (c:counter): Tot state =
  let constants:seq vec = Spec.Compat.Loops.seq_map to_vec constants in
  let key = Spec.Compat.Loops.seq_map to_vec (uint32s_from_le 8 k) in
  let nonce = Spec.Compat.Loops.seq_map to_vec (uint32s_from_le 3 n) in
  let ctr = create 1 (initial_counter c) in
  constants @| key @| ctr @| nonce

let column (s:state) (c:nat): (m:seq U32.t{length m = 16}) = 
  Spec.Compat.Loops.seq_map (fun r -> index r c) s
  
let transpose (s:state): Tot stateT =
    let cols = createL [0;1;2;3;4;5;6;7] in
    Spec.Compat.Loops.seq_map (column s) cols
