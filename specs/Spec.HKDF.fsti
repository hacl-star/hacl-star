...
val hkdf_expand:
  ...
  -> len:size_nat{len < 255 * HA.hash_length a} ->
  Tot (lbytes len)
...
val hkdf_expand_label:
    a: HA.hash_alg
  -> secret: bytes{length secret < HA.max_input_length a}
  -> label: bytes{length label <= maxint U8}
  -> context: bytes{
    let size_hkdf_label = numbytes U16 + numbytes U8 + length label + numbytes U8 + length context in
    length context <= maxint U8
  /\ length secret + size_hkdf_label + 1 + HA.hash_length a + HA.block_length a < HA.max_input_length a}
  -> len: nat{len <= maxint U16 /\ len < 255 * HA.hash_length a} ->
  Tot (lbytes len)
...
val hkdf_expand_derive_secret:
  ...
  -> label: bytes{length label <= maxint U8}
  -> context: bytes{
    let size_hkdf_label = numbytes U16 + numbytes U8 + length label + numbytes U8 + HA.hash_length a in
    length context <= maxint U8
  /\ length secret + size_hkdf_label + 1 + HA.hash_length a + HA.block_length a < HA.max_input_length a} ->
  Tot (lbytes (HA.hash_length a))
