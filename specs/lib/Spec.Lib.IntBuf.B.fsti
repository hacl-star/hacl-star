...
let preserves_live (h0:mem) (h1:mem) : GTot Type =
  (forall (a:Type0) (len:size_t) (b:lbuffer a len). {:pattern (live h0 b)} (live h0 b) ==> (live h1 b))
...
val upd: #a:Type0 -> #len:size_t -> b:lbuffer a len -> i:size_t{i < len} -> v:a -> Stack unit
  ...
  (ensures (fun h0 r h1 -> preserves_live h0 h1 /\ modifies1 b h0 h1
                      /\ as_lseq #a #len b h1 == LSeq.upd #a #len (as_lseq #a #len b h0) i v
                      /\ (forall j. (j <> i /\ j < len ==> LSeq.index (as_lseq b h1) j == LSeq.index (as_lseq b h0) j))))
...
val repeat_range: #a:Type -> #b:Type -> #lift:(mem -> b -> GTot (option a)) ->
  ...
  impl:(i:size_t{start <= i /\ i < fin} -> x:b ->
  Stack unit
...
val iteri: #a:Type -> #len:size_t -> n:size_t -> spec:(i:size_t{i < n}  -> LSeq.lseq a len -> Tot (LSeq.lseq a len)) ->
  impl:(i:size_t{i < n}  -> x:lbuffer a len ->
  Stack unit
  (requires (fun h0 -> live h0 x))
  (ensures  (fun h0 _ h1 -> preserves_live h0 h1 /\
				            modifies1 x h0 h1 /\
...
val iter_range: #a:Type -> #len:size_t -> start:size_t -> fin:size_t{start <= fin} ->
  ...
  impl:(i:size_t{start <= i /\ i < fin}  -> x:lbuffer a len ->
  Stack unit
  (requires (fun h0 -> live h0 x))
  (ensures  (fun h0 _ h1 -> preserves_live h0 h1 /\
				             modifies1 x h0 h1 /\
...
val lemma_live_sub: #a:Type0 -> #len:size_t -> h:mem -> b:lbuffer a len -> start:size_t -> n:size_t{start + n <= len} -> Lemma
  (requires (live h b))
  (ensures (live h (sub b start n)))
  [SMTPat (live h (sub b start n))]

val lemma_live_super: #a:Type0 -> #len:size_t -> h:mem -> b:lbuffer a len -> start:size_t -> n:size_t{start + n <= len} -> Lemma
  (requires (live h (sub b start n)))
  (ensures (live h b))
  [SMTPat (live h (sub b start n))]

val lemma_disjoint_self: #a:Type0 -> #len:size_t -> b:lbuffer a len -> Lemma
  (requires (True))
  (ensures (~ (disjoint b b)))
  [SMTPat (disjoint b b)]

val lemma_disjoint_sub1: #a1:Type0 -> #a2:Type0 -> #len1:size_t -> #len2:size_t -> b1:lbuffer a1 len1 -> b2:lbuffer a2 len2 -> start1:size_t -> n1:size_t{start1 + n1 <= len1} -> Lemma
...
val lemma_disjoint_sub2: #a:Type0 -> #len:size_t -> b:lbuffer a len -> start1:size_t -> n1:size_t{start1 + n1 <= len} -> start2:size_t -> n2:size_t{start2 + n2 <= len} -> Lemma
...
/// If a buffer B1 is disjoint with buffer B2 then
/// any sub-buffer of B1 is disjoint from B2.
val lemma_disjoint_sub3: #a1:Type0 -> #a2:Type0 -> #len1:size_t  -> #len2:size_t -> b1:lbuffer a1 len1 -> b2:lbuffer a2 len2 -> start1:size_t -> n1:size_t{start1 + n1 <= len1} -> Lemma
  (requires (disjoint b1 b2))
  (ensures  (disjoint (sub b1 start1 n1) b2))
  [SMTPat (disjoint (sub b1 start1 n1) b2)]

val lemma_as_lseq_sub: #a:Type0 -> #len:size_t -> h:mem -> b:lbuffer a len -> start:size_t -> n:size_t{start + n <= len} -> Lemma
  (requires (live h b))
  (ensures (as_lseq (sub b start n) h == LSeq.sub (as_lseq b h) start n))
  [SMTPat (as_lseq (sub b start n) h)]

val lemma_preserves_live: #a:Type0 -> #len:size_t -> b:lbuffer a len -> h0:mem -> h1:mem -> Lemma
  (requires (preserves_live h0 h1 /\ live h0 b))
  (ensures  (live h1 b))
  [SMTPat (preserves_live h0 h1 /\ live h0 b)]

val lemma_creates1:  #a1:Type0 -> #a2:Type0 -> #len1:size_t -> #len2:size_t -> b1:lbuffer a1 len1 -> b2:lbuffer a2 len2 -> h0:mem -> h1:mem -> Lemma
  (requires (live h0 b1 /\ creates1 b2 h0 h1))
  (ensures  (live h1 b1 /\ disjoint b1 b2 /\ disjoint b2 b1))
  [SMTPat (creates1 b2 h0 h1 /\ live h0 b1)]

val lemma_modifies1_transitivity: (#a:Type0) -> (#len:size_t) -> (b:lbuffer a len) -> (h0:mem) -> (h1:mem) -> (h2:mem) -> Lemma
  (requires (modifies1 b h0 h1 /\ modifies1 b h1 h2))
  (ensures (modifies1 b h0 h2))
  [SMTPat (modifies1 b h0 h1); SMTPat (modifies1 b h1 h2)]

val lemma_modifies1:  #a1:Type0 -> #a2:Type0 -> #len1:size_t -> #len2:size_t -> b1:lbuffer a1 len1 -> b2:lbuffer a2 len2 -> h0:mem -> h1:mem -> Lemma
...
val lemma_modifies2:  #a1:Type0 -> #a2:Type0 -> #a3:Type0 -> #len1:size_t -> #len2:size_t -> #len3:size_t -> b1:lbuffer a1 len1 -> b2:lbuffer a2 len2 -> b3:lbuffer a3 len3 -> h0:mem -> h1:mem -> Lemma
  (requires (disjoint b1 b2 /\ disjoint b1 b3 /\ live h0 b1 /\ modifies2 b2 b3 h0 h1))
  (ensures  (live h1 b1 /\ as_lseq b1 h1 == as_lseq b1 h0))
  [SMTPat (disjoint b1 b2 /\ modifies2 b2 b3 h0 h1)]

val lemma_modifies3:  #a1:Type0 -> #a2:Type0 -> #a3:Type0 -> #a4:Type0-> #len1:size_t -> #len2:size_t -> #len3:size_t -> #len4:size_t ->
...
val lemma_modifies_super: #a:Type0 -> #len:size_t -> b:lbuffer a len -> start:size_t -> n:size_t{start + n <= len} -> h0:mem -> h1:mem -> Lemma
  (requires (modifies1 (sub b start n) h0 h1))
