...
inline_for_extraction val sub: #a:Type0 -> #len:size_t -> b:lbuffer a len -> start:size_t -> n:size_t{v start + v n <= v len} -> Tot (lbuffer a n)
 let slice #a #len (b:lbuffer a len) (start:size_t) (fin:size_t{v fin <= v len /\ v start <= v fin}) = sub #a #len b start (sub_mod #SIZE fin start)
...
inline_for_extraction val createL: #a:Type0 -> init:list a{List.Tot.length init <= max_size_t} -> StackInline (lbuffer a (size (List.Tot.length init)))
		      (requires (fun h0 -> True))
		      (ensures (fun h0 r h1 -> preserves_live h0 h1 /\ creates1 r h0 h1 /\ modifies1 r h0 h1 /\ as_lseq #a #(size (List.Tot.length init)) r h1 == LSeq.createL #a init))
...
inline_for_extraction val index: #a:Type0 -> #len:size_t -> b:lbuffer a len -> i:size_t{v i < v len} -> Stack a 
		      (requires (fun h0 -> live h0 b))
		      (ensures (fun h0 r h1 -> h0 == h1 /\ r == LSeq.index #a #len (as_lseq #a #len b h0) i)) 

inline_for_extraction val upd: #a:Type0 -> #len:size_t -> b:lbuffer a len -> i:size_t{v i < v len} -> x:a -> Stack unit 
		      (requires (fun h0 -> live h0 b /\ v len > 0))
		      (ensures (fun h0 r h1 -> preserves_live h0 h1 /\ modifies1 b h0 h1 /\ as_lseq #a #len b h1 == LSeq.upd #a #len (as_lseq #a #len b h0) i x))
...
val blit: #a:Type -> #len:size_t -> i:lbuffer a len -> start1:size_t -> 
	  ...
	  num:size_t{v start1 + v num <=  v len /\ v start2 + v num <= v len} -> 
	  Stack unit 
...
val repeat_range: #a:Type -> #b:Type -> #lift:(mem -> b -> GTot (option a)) -> 
	    start:size_t -> fin:size_t{v start <= v fin} -> spec:(i:size_t{v start <= v i /\ v i < v fin} -> a -> Tot a) -> 
...
val repeati: #a:Type -> #b:Type -> #lift:(mem -> b -> GTot (option a)) -> 
	    n:size_t -> spec:(i:size_t{v i < v n} -> a -> Tot a) -> 
...
val iteri: #a:Type -> #len:size_t -> n:size_t -> spec:(i:size_t{v i < v n}  -> LSeq.lseq a len -> Tot (LSeq.lseq a len)) -> 
...
val iter_range: #a:Type -> #len:size_t -> start:size_t -> fin:size_t{v start <= v fin} ->
         spec:(i:size_t{v start <= v i /\ v i < v fin}  -> LSeq.lseq a len -> Tot (LSeq.lseq a len)) -> 
...
val uints_from_bytes_le: 
  #t:inttype -> #len:size_t{v len `op_Multiply` numbytes t <= max_size_t} -> 
  o:lbuffer (uint_t t) len ->
  i:lbuffer uint8 (mul_mod #SIZE len (size (numbytes t))) -> 
  Stack unit 
...
val uint32s_from_bytes_le: 
  #len:size_t{v len `op_Multiply` 4 <= max_size_t} -> 
  o:lbuffer uint32 len ->
  i:lbuffer uint8 (mul_mod #SIZE len (size 4)) -> 
  Stack unit 
	(requires (fun h0 -> live h0 o /\ live h0 i))
	(ensures (fun h0 _ h1 -> preserves_live h0 h1 /\
			      modifies1 o h0 h1 /\
...
val uint32s_to_bytes_le: 
  #len:size_t{v len `op_Multiply` 4 <= max_size_t} -> 
  o:lbuffer uint8 (mul_mod #SIZE len (size 4)) -> 
