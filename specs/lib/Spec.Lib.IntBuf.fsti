...
inline_for_extraction val alloc: #a:Type0 -> #b:Type0 -> len:size_t -> init:a -> 
		 ...
		 impl:(buf:lbuffer a (v len) -> Stack b
...
val map: #a:Type -> #len:size_nat -> f:(a -> Tot a) -> b:lbuffer a (len) -> Stack unit 
		      (requires (fun h0 -> live h0 b))
...
val blit: #a:Type -> #len:size_nat -> i:lbuffer a (len) -> start1:size_t -> 
          o:lbuffer a (len) -> start2:size_t -> 
	  num:size_t{v start1 + v num <=  len /\ v start2 + v num <= len} -> 
	  Stack unit 
  		  ...
  		  (ensures (fun h0 _ h1 -> preserves_live h0 h1 /\ 
					...
					as_lseq o h1 == 
					   LSeq.update_sub (as_lseq o h0) 
						(v start1) (v num)
						(LSeq.sub (as_lseq i h0) (v start2) (v num))))
