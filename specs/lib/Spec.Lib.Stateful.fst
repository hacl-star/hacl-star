...
let apply2_ext_st #d (#a:Type0) (#len:size_t) (ext:lseq a len) (k1:d.key) (k2:d.key) (f:lseq a len -> state_seq d k1 -> state_seq d k2) : stateful d unit = 
  fun s -> (), d.put s k2 (f ext (d.get s k1))

(* Useful functions *)
let repeat_stateful #d (n:size_t) (f:stateful d unit) : stateful d unit =
  fun s -> (),repeat n (fun s -> snd (f s)) s

let repeat_range_stateful #d (min:size_t) (max:size_t{min <= max}) (f:i:size_t{i >= min /\ i < max} -> stateful d unit) : stateful d unit =
