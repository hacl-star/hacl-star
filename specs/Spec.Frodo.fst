...
let params_q:size_nat = pow2 params_logq
...
let frodo_gen_matrix_cshake n seedLen seed =
  ...
  (fun i res ->
    ...
    (fun j res ->
      ...
      mset res i j (numeric (uint_to_nat res_ij % modulus elem_t))
    ) res
  ) res
...
let crypto_kem_dec ct sk =
  ...
  let bcond = (lbytes_eq d dp) && (matrix_eq bp_matrix bpp_matrix) && (matrix_eq c_matrix cp_matrix) in
