...
let mix4 (s0:elem) (s1:elem) (s2:elem) (s3:elem) = 
	 (s0 `fmul` two) `fadd`
         (s1 `fmul` three) `fadd`
	 s2 `fadd` s3
...
let rounds (key:lseq elem (9 * 16)) (state:block) =
  repeati 9 (fun i -> aes_enc (sub key (16*i) 16)) state

let block_cipher (key:lseq elem (11 * 16)) (input:block) =
  let state = input in
...
let aes_keygen_assist (rcon:elem) (s:block) : block =
...
let aes128_keygen_assist (rcon:elem) (s:block) : block =
...
let aes128_key_expansion (key:block) : (lseq elem (11 * 16)) =
