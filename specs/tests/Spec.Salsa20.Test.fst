...
let test_quarter_round () =
  ...
  (slice st1 0 4) = of_list (List.Tot.map u32_from_UInt32 [0ul;0ul;0ul;0ul]) &&
  (slice st2 0 4) = of_list (List.Tot.map u32_from_UInt32 [0x08008145ul;0x80ul;0x010200ul;0x20500000ul])
...
let test_row_round () =
  ...
  st = of_list (List.Tot.map u32_from_UInt32
...
let test_column_round2 () =
  let st = of_list (List.Tot.map u32_from_UInt32
...
let test_salsa20_core () =
  let st = uints_from_bytes_le #U32 #16 (of_list (List.Tot.map u8_from_UInt8
  ...
  st = uints_from_bytes_le #U32 #16 (of_list (List.Tot.map u8_from_UInt8
