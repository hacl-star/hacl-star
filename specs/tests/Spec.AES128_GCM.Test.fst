...
let key_length = AEAD.size_key Spec.AES.AES128
...
let test_aesgcm text_len text aad_len aad n_len n k expected i =
  ...
  IO.print_string " ================================\n";
  let output = AEAD.aes128gcm_encrypt k n text aad in
  let ciphertext = sub (to_lseq output) 0 (length output - 16) in
  let mac = sub (to_lseq output) (length output - 16) 16 in
  let decrypted = AEAD.aes128gcm_decrypt k n aad ciphertext mac in
