...
let test () =
  let x : lbytes key_length = of_list test2_gmul_hash_key in
  let x_int = nat_from_bytes_be x in
  let y : lbytes key_length = of_list test2_gmul_ciphertext in
  ...
  let output = GF.gmul (of_list test1_gmul_ciphertext) (of_list test1_gmul_hash_key) in
  let result = for_all2 (fun a b -> uint_to_nat #U8 a = uint_to_nat #U8 b) output (of_list test1_gmul_expected) in
  IO.print_string   "Expected hash: ";
  let test_expected : lbytes key_length = of_list test1_gmul_expected in
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list test_expected);
  IO.print_string "\nComputed hash: ";
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list output);
  if result then IO.print_string "\nSuccess!\n"
  else IO.print_string "\nFailure :(\n";

  let output = GF.gmul (of_list test2_gmul_ciphertext) (of_list test2_gmul_hash_key) in
  let result = for_all2 (fun a b -> uint_to_nat #U8 a = uint_to_nat #U8 b) output (of_list test2_gmul_expected) in
  IO.print_string   "Expected hash: ";
  let test_expected : lbytes key_length = of_list test2_gmul_expected in
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list test_expected);
  IO.print_string "\nComputed hash: ";
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list output);
  if result then IO.print_string "\nSuccess!\n"
  ...
  let output = GF.gmul (of_list test3_gmul_ciphertext) (of_list test3_gmul_hash_key) in
  let result = for_all2 (fun a b -> uint_to_nat #U8 a = uint_to_nat #U8 b) output (of_list test3_gmul_expected) in
  IO.print_string   "Expected hash: ";
  let test_expected : lbytes key_length = of_list test3_gmul_expected in
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list test_expected);
  IO.print_string "\nComputed hash: ";
  List.iter (fun a -> IO.print_string (UInt8.to_string (u8_to_UInt8 a));  IO.print_string ":") (to_list output);
  if result then IO.print_string "\nSuccess!\n"
