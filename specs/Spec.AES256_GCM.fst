...
let ghash text aad gf_key tag_key =
  ...
  let acc = GF.gf128_update ((to_lseq aad) @| aad_pad) acc r in
  let acc = GF.gf128_update ((to_lseq text) @| text_pad) acc r in
  let acc = GF.gf128_update (aad_len_bytes @| text_len_bytes) acc r in
  let tag = GF.gf128_finish tag_key acc in
  tag

val gcm:
    k: AES.aes_key AES.AES256
  -> n: nonce
...
let gcm k n m aad =
  ...
  let tag_key = AES.aes_ctr_key_block1 AES.AES256 k nlen n in
  let gf_key = AES.aes_ctr_key_block0 AES.AES256 k size_nonce (create size_nonce (u8 0)) in
...
let aead_encrypt k n m aad =
  ...
  let c = AES.aes256_ctr_encrypt_bytes k size_nonce nonce 2 m in
...
let aead_decrypt k n c tag aad =
  ...
  if lbytes_eq computed_tag tag then
    Some (AES.aes256_ctr_encrypt_bytes k size_nonce nonce 2 c)
  else None
