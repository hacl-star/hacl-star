...
let aead_decrypt k n_len n len c aad_len aad =
  let iv_len = if n_len = 12 then 12 else blocksize in
  let iv: lbytes iv_len = if n_len = 12 then n else (
    let n_padding:size_nat = (blocksize - (n_len % blocksize)) % blocksize in
    let n_padding = n_padding + 8 in
  ...
  let computed_mac = gcm k iv_len iv len c aad_len aad in
  let expected_mac = sub c (len - blocksize) blocksize in
  let result = for_all2 (fun a b -> eq a b) computed_mac expected_mac in
  if result then
    AES.aes128_encrypt_bytes k iv_len iv 2 (len - blocksize) c
  else
    create blocksize (u8 0)
