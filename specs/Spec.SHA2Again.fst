...
let k_w      a = m:Seq.seq (word a) {Seq.length m = size_k_w a}
let ws_w     a = m:Seq.seq (word a) {Seq.length m = size_ws_w a}
let block_w  a = m:Seq.seq (word a) {Seq.length m = size_block_w}
let counter = nat

//#reset-options "--using_facts_from '* -Spec.SHA2Again.Constants'"
#set-options "--z3refresh --log_queries"

(* Define word based operators *)
let words_to_be: a:hash_alg -> Tot (len:nat -> s:Seq.seq (word a){Seq.length s = len} -> Tot (Spec.Lib.lbytes (size_word a * len))) = function
  | SHA2_224 | SHA2_256 -> Spec.Lib.uint32s_to_be
  | SHA2_384 | SHA2_512 -> Spec.Lib.uint64s_to_be

let words_from_be: a:hash_alg -> Tot (len:nat -> b:Spec.Lib.lbytes (size_word a * len) -> Tot (s:Seq.seq (word a){Seq.length s = len})) = function
...
let word_shift_right: t:hash_alg -> Tot (a:word t -> s:U32.t -> Pure (word t)
  (requires (U32.v s < 8 * size_word t))
...
let k0: a:hash_alg -> Tot (m:Seq.seq (word a) {Seq.length m = size_k_w a}) = function
  | SHA2_224 -> C.k224_256
  | SHA2_256 -> C.k224_256
  | SHA2_384 -> C.k384_512
  | SHA2_512 -> C.k384_512

let h0: a:hash_alg -> Tot (m:Seq.seq (word a) {Seq.length m = size_hash_w}) = function
  | SHA2_224 -> C.h224
  | SHA2_256 -> C.h256
  | SHA2_384 -> C.h384
  | SHA2_512 -> C.h512
...
let shuffle_core (a:hash_alg) (block:block_w a) (hash:hash_w a) (t:counter{t < size_k_w a}) : Tot (hash_w a) =
  ...
  // JP: was a Seq.create: understand if the post-condition was really needed
  Seq.seq_of_list [ (word_add_mod a t1 t2); a0; b0; c0; (word_add_mod a d0 t1); e0; f0; g0 ]
...
let update (a:hash_alg) (hash:hash_w a) (block:bytes{Seq.length block = size_block a}) : Tot (hash_w a) =
...
let rec update_multi (a:hash_alg) (hash:hash_w a) (blocks:bytes{Seq.length blocks % (size_block a) = 0}) : Tot (hash_w a) (decreases (Seq.length blocks)) =
