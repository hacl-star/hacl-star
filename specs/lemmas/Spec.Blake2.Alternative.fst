...
val lemma_shift_update_last:
  ...
  -> s:state a ->
  Lemma (
    blake2_update_last a last_node 0 rem (b `Seq.append` d) s ==
    blake2_update_last a last_node (size_block a) rem d s
  )

let lemma_shift_update_last a last_node rem b d s =
...
val repeati_update1:
  ...
  Lemma (
    ...
    repeati (nb - 1) (blake2_update1 a (size_block a) d) (blake2_update_block a false false (size_block a) b s)
  )
...
let repeati_update1 a b d nb s =
  ...
  let s' = blake2_update_block a false false (size_block a) b s in
...
val lemma_unfold_update_blocks:
  ...
  -> s:state a ->
  Lemma (
    blake2_update_blocks a last_node 0 (b `Seq.append` d) s ==
    blake2_update_blocks a last_node (size_block a) d (blake2_update_block a false false (size_block a) b s)
  )
...
let lemma_unfold_update_blocks a last_node b d s =
  let nb, rem = split a (length (b `Seq.append` d)) in
  let nb', rem' = split a (length d) in
  ...
  lemma_shift_update_last a last_node rem b d s_int

#push-options "--fuel 0 --ifuel 0 --z3rlimit 1000"
let lemma_spec_equivalence_update a last_node kk k d s =
  let ll = length d in
  ...
  if kk = 0 then begin
    ...
    calc (Seq.equal) {
      blake2_update a last_node kk k d s;
    (Seq.equal) {}
      blake2_update_key a last_node kk k ll s;
    (Seq.equal) {}
      blake2_update_block a true last_node (size_block a) (blake2_key_block a kk k) s;
    (Seq.equal) { Lib.LoopCombinators.eq_repeati0 nb (blake2_update1 a 0 (blake2_key_block a kk k)) s }
      blake2_update_blocks a last_node 0 (blake2_key_block a kk k) s;
    (Seq.equal) { Seq.append_empty_r (blake2_key_block a kk k) }
      blake2_update_blocks a last_node 0 (blake2_key_block a kk k `Seq.append` Seq.empty) s;
    (Seq.equal) { Seq.lemma_empty d }
      blake2_update_blocks a last_node 0 (blake2_key_block a kk k `Seq.append` d) s;
    (Seq.equal) { }
      blake2_update' a last_node kk k d s;
    };
    ()
    ...
    calc (Seq.equal) {
      blake2_update a last_node kk k d s;
    (Seq.equal) {}
      blake2_update_blocks a last_node (size_block a) d (blake2_update_key a last_node kk k ll s);
    (Seq.equal) {}
      blake2_update_blocks a last_node (size_block a) d (blake2_update_block a false false (size_block a) (blake2_key_block a kk k) s);
    (Seq.equal) { lemma_unfold_update_blocks a last_node (blake2_key_block a kk k) d s }
      blake2_update_blocks a last_node 0 (blake2_key_block a kk k `Seq.append` d) s;
    }
#pop-options
