  ...
  if not (verifyQValidCurvePointSpec #c publicJacobian) then false
    ...
    if not (checkCoordinates #c r s) then false
      ...
      begin
      	...
      	let u1 = pow s (order - 2) * hashM % order in
      	let u2 = pow s (order - 2) * r % order in  
      	
      	let sumPoints = pointAdd #c (point_mult #c u1 (basePoint #c)) (point_mult #c u2 publicJacobian) in
      	let x, _, _ = _norm #c sumPoints in
      	if isPointAtInfinity #Jacobian sumPoints then false else x % order = r
    end
  end
...
let ecdsa_signature c alg mLen m privateKey k =
  ...
  assert_norm (pow2 32 < pow2 125);
  let order = getOrder #c in 
  let baseMultK = scalar_multiplication k (basePoint #c) in 
