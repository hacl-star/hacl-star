...
val lemma_pointAddToSpecification: 
  ...
  Lemma(requires (    
    ...
    let rD, hD = fromDomain_ r, fromDomain_ h in    
...
let lemma_pointAddToSpecification x1D y1D z1D x2D y2D z2D x3 y3 z3  u1 u2 s1 s2 h r = 
  ...
  let hD, rD = fromDomain_ h, fromDomain_ r in 
...
noextract       
val lemma_pointAdd_Mixed_ToSpecification: 
  pxD: nat {pxD < prime256} -> pyD: nat{pyD < prime256} -> pzD: nat {pzD < prime256} -> 
  qxD: nat {qxD < prime256} -> qyD: nat {qyD < prime256} ->
  x3: nat -> y3: nat -> z3: nat -> 
  u1: nat -> u2: nat -> s1: nat -> s2: nat -> 
  h: nat -> r: nat -> 
  Lemma(requires (    
    ...
    u1 == toDomain_ (pxD % prime256) /\
    u2 == toDomain_ (pzD * pzD * qxD % prime256) /\
    s1 == toDomain_ (pyD % prime256) /\
    s2 == toDomain_ (pzD * pzD * pzD * qyD % prime256) /\
    ...
    if isPointAtInfinityAffine (qxD, qyD) then 
      ...
      fromDomain_ x3 == qxD /\  fromDomain_ y3 == qyD /\ fromDomain_ z3 == 1 
    else
      x3 == toDomain_ ((rD * rD - hD * hD * hD - 2 * hD * hD * u1D) % prime) /\
      y3 == toDomain_ (((hD * hD * u1D - x3D) * rD - s1D * hD * hD * hD) % prime) /\
      z3 == toDomain_ ((pzD * hD) % prime)))
  )
  (ensures (    
    let x3D, y3D, z3D = fromDomain_ x3, fromDomain_ y3, fromDomain_ z3 in 
...
let lemma_pointAdd_Mixed_ToSpecification x1D y1D z1D x2D y2D x3 y3 z3 u1 u2 s1 s2 h r = 
  let open FStar.Tactics in 
  let open FStar.Tactics.Canon in 
  ...
  let pxD, pyD, pzD = x1D, y1D, z1D in 
  let qxD, qyD = x2D, y2D in 
  
  let (xN, yN, zN) = point_add_mixed (x1D, y1D, z1D) (x2D, y2D) in 
  ...
  let u1 = x1D % prime256 in
  let u2 = x2D * z1z1 % prime256 in

  let s1 = y1D % prime256 in
  let s2 = y2D * z1D * z1z1 % prime256 in
  ...
  assert_by_tactic ((h * h * u1 - x3_) == (u1 * (h * h) - x3_)) canon;
  assert_by_tactic ((r * (h * h * u1 - x3_) - s1 * (h * h * h)) = ((h * h * u1 - x3_) * r - s1 * h * h * h)) canon; 
  assert_by_tactic (z1D * hD = hD * z1D) canon
