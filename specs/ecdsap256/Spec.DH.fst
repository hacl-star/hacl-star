...
(* Initiator *)
val ecp256_dh_i: #c: curve -> s:lbytes (getCoordinateLen c) -> tuple3 (lbytes (getCoordinateLen c)) (lbytes (getCoordinateLen c)) uint64

let ecp256_dh_i #c s =
  let xN, yN, zN = secret_to_public #c s in
  if isPointAtInfinity (xN, yN, zN) then
    nat_to_bytes_be (getCoordinateLen c) xN, nat_to_bytes_be (getCoordinateLen c) yN, u64 (pow2 64 - 1)
  else
    nat_to_bytes_be (getCoordinateLen c) xN, nat_to_bytes_be (getCoordinateLen c) yN, (u64 0)


(* Responder *)
val ecp256_dh_r: #c: curve -> x:lbytes (getCoordinateLen c) -> y:lbytes (getCoordinateLen c) -> s:lbytes (getCoordinateLen c)
