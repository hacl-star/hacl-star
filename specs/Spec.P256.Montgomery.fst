...
val mult_one_round2: t: nat -> co: nat  -> Lemma
(requires (t % prime == co % prime))
(ensures (let result = (t + (t % pow2 s) * prime) / pow2 64 % prime in result == (co * modp_inv2 (pow2 64)) % prime))
...
val lemma_less_than_primes : a: elem -> b: elem -> Lemma (ensures (
  let t = a * b in
...
let lemma_less_than_primes a b = admit()



#reset-options "--z3rlimit 200"

val mult_4: a: elem -> b: elem -> Tot (r: nat { r < prime /\ r % prime = a * b * modp_inv2 (pow2 256) % prime})
...
let mult_4 a b =
  ...
  let tU = t3 / pow2 s in
    ...
    assert(t % prime ==  (a * b * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) % prime));
    let rp : nat = a * b * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) in
    assert(t % prime ==  rp % prime);
    mult_one_round2 t rp;

    assert(tU % prime == (a * b * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) * modp_inv2 (pow2 64) * modp_inv2 (pow2 64)) % prime);
    lemma_decrease_pow (a * b);
    assert(tU % prime == (a * b * modp_inv2 (pow2 256)) % prime);

    if tU >=prime then begin
      lemma_less_than_primes a b;
      assert(tU < 2 * prime);
      lemma3 tU;
      assert(tU - prime < prime);
      tU - prime
      end
    else
      tU
