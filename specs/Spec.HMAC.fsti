...
val init: (a:Hash.algorithm) -> (len:size_nat{len < Hash.max_input a}) -> (key:lbytes len) -> Tot (tuple2 (Hash.hash_w a) (lbytes (Hash.size_block a)))
...
val update_multi: (a:Hash.algorithm) -> (n:size_nat{n * Hash.size_block a <= max_size_t}) -> (data:lbytes (n * Hash.size_block a)) -> (hash:Hash.hash_w a) -> Tot (h:Hash.hash_w a)

val update_last: (a:Hash.algorithm) -> (n:size_nat) -> (len:size_nat{len < Hash.size_block a /\ len + n * Hash.size_block a <= Hash.max_input a}) -> (last:lbytes len) -> (h:Hash.hash_w a) -> Tot (h:Hash.hash_w a)
