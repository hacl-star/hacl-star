...
abstract type partialSwap (h0:heap) (h1:heap) (is_swap:s64) (ctr:nat{ctr<=norm_length})
...
val swap_is_on_curve: h0:heap -> h3:heap -> a:point -> b:point{distinct a b} -> 
  is_swap:s64{v is_swap = pow2 platform_size - 1 \/ v is_swap = 0} -> Lemma
...
#reset-options
val swap_conditional_lemma: h0:heap -> h1:heap -> h2:heap -> h3:heap -> a:point -> b:point{distinct a b} -> is_swap:s64{v is_swap = pow2 platform_size -1 \/ v is_swap = 0} ->
...
val on_curve_lemma: h0:heap -> h1:heap -> a:point -> mods:Ghost.erased (TSet.set abuffer) -> Lemma
  (requires (onCurve h0 a 
    ...
    (* /\ TSet.intersect (reveal mods) (refs a) = TSet.empty *)))
