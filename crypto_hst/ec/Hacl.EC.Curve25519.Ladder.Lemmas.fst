...
val formula_0: scalar:erased nat -> byte:s8 -> GTot (erased nat)
let formula_0 scalar byte = 
  hide (reveal scalar * pow2 8 + S8.v byte)

val formula_1: n:FStar.Ghost.erased nat -> b:s8 -> GTot (z:erased nat{reveal z = 2*FStar.Ghost.reveal n+S8.v b})
let formula_1 n b = 
  hide (2*Ghost.reveal n+S8.v b)

assume val formula_2: n:nat -> b:s8 -> Tot (z:FStar.Ghost.erased nat{FStar.Ghost.reveal z = 2*n+S8.v b})

assume val eformula_2: n:erased nat -> b:s8 -> Tot (z:FStar.Ghost.erased nat{FStar.Ghost.reveal z = 2*reveal n+S8.v b})
...
let mk_mask x =
  ...
  S64.sub_mod (Hacl.Cast.uint64_to_sint64 0uL) y
...
val nth_bit: byt:s8 -> idx:u32{w idx < 8} -> Tot (b:s8{S8.logand (S8.shift_right byt (7ul-|idx)) (uint8_to_sint8 1uy) == b /\ (b == uint8_to_sint8 0uy \/ b == uint8_to_sint8 1uy) })
let nth_bit byte idx =
  let bit = S8.logand (S8.shift_right byte (7ul-|idx)) (Hacl.Cast.uint8_to_sint8 1uy) in
...
val small_step_core_lemma_3: h0:heap -> h:heap -> h2:heap -> h1:heap -> pp:point -> ppq:point{distinct pp ppq} -> p:point{distinct p pp /\ distinct p ppq} -> pq:point{distinct pq pp /\ distinct pq ppq /\ distinct pq p} -> q:point{distinct q pp /\ distinct q ppq /\ distinct q p /\ distinct q pq} ->    n:erased nat -> ctr:nat{ctr<8} -> byt:s8 -> scalar:erased nat{reveal n = reveal scalar * (pow2 ctr) + (S8.v byt / (pow2 (8-ctr)))} ->  Lemma
  (requires (
    ...
    /\ ( bit == uint8_to_sint8 0uy ==> ((pointOf h p) = (pointOf h0 p) /\ (pointOf h pq) = (pointOf h0 pq)) )
    /\ ( bit == uint8_to_sint8 1uy ==> ((pointOf h pq) = (pointOf h0 p) /\ (pointOf h p) = (pointOf h0 pq)) )
    /\ ( (pointOf h2 pp) = (Math.Curve.add (pointOf h p) (pointOf h p)) /\ (pointOf h2 ppq) = (Math.Curve.add (pointOf h p) (pointOf h pq)))
    /\ (bit == uint8_to_sint8 0uy ==> ((pointOf h1 pp) = (pointOf h2 pp) /\ (pointOf h1 ppq) = (pointOf h2 ppq)))
    /\ (bit == uint8_to_sint8 1uy ==> ((pointOf h1 pp) = (pointOf h2 ppq) /\ (pointOf h1 ppq) = (pointOf h2 pp))) ))
...
let gsmall_step_core_lemma_3 h0 h h2 h1 pp ppq p pq q n ctr byt scalar =
  ...
  if (bit = uint8_to_sint8 0uy) then (
  ...
  if (bit = uint8_to_sint8 1uy) then (
  ...
  (* lemma_8 (reveal n); *)
  cut (bit == uint8_to_sint8 0uy \/ bit == uint8_to_sint8 1uy); 
  cut (bit == uint8_to_sint8  0uy ==> nTimesQ (hide (2*(reveal n)+S8.v bit)) q0 h1 pp ppq);
  cut (bit == uint8_to_sint8 1uy ==> nTimesQ (hide (2*(reveal n)+S8.v bit)) q0 h1 pp ppq);
  cut (nTimesQ (hide (2*(reveal n)+S8.v bit)) q0 h1 pp ppq);
  cut (True /\ nth_bit byt ctr = bit);
  cut (True /\ reveal (formula_1 n (nth_bit byt ctr)) = (2*reveal n+S8.v bit));
...
val small_step_core: 
   ...
   p_plus_q:point{distinct p_plus_q two_p /\ distinct p_plus_q two_p_plus_q /\ distinct p_plus_q p} -> 
   q:point{distinct q two_p /\ distinct q two_p_plus_q /\ distinct q p /\ distinct q p_plus_q} -> 
   n:erased nat -> ctr:u32{w ctr<8} -> byt:s8 -> scalar:erased nat{reveal n = reveal scalar * (pow2 (w ctr)) + (S8.v byt / (pow2 (8-w ctr)))} -> 
   ST unit
     (requires (fun h -> 
       (live h two_p) /\ (live h two_p_plus_q) /\ (onCurve h p) /\ (onCurve h p_plus_q) /\ (onCurve h q)
       /\ (nTimesQ n (pointOf h q) h p p_plus_q) 
...
val small_step: two_p:point -> two_p_plus_q:point{distinct two_p two_p_plus_q} ->
   ...
   p_plus_q:point{distinct p_plus_q two_p /\ distinct p_plus_q two_p_plus_q /\ distinct p_plus_q p} ->
   q:point{distinct q two_p /\ distinct q two_p_plus_q /\ distinct q p /\ distinct q p_plus_q} ->
   n:erased nat -> ctr:u32{w ctr<=8} -> b:s8 ->
   scalar:erased nat{reveal n = reveal scalar * (pow2 (w ctr)) + (S8.v b / (pow2 (8-w ctr)))} -> STL unit
...
val big_step_lemma_1: h0:heap -> h1:heap ->
  n:bytes -> pp:point{distinct2 n pp} -> ppq:point{distinct2 n ppq /\ distinct pp ppq} ->
   ...
   q:point{distinct2 n q /\ distinct q pp /\ distinct q ppq /\ distinct q p /\ distinct q pq} -> ctr:nat{ctr<=bytes_length-1} -> b:s8 ->
  Lemma
    (requires (
...
val big_step_lemma_2: h0:heap -> h1:heap -> h2:heap ->
  n:bytes -> pp:point{distinct2 n pp} -> ppq:point{distinct2 n ppq /\ distinct pp ppq} ->
   p:point{distinct2 n p /\ distinct p pp /\ distinct p ppq} ->
   pq:point{distinct2 n pq /\ distinct pq pp /\ distinct pq ppq /\ distinct pq p} ->
   q:point{distinct2 n q /\ distinct q pp /\ distinct q ppq /\ distinct q p /\ distinct q pq} ->  ctr:nat{ctr<=bytes_length-1} -> b:s8 -> Lemma
    (requires (
      (live h0 pp) /\ (live h0 ppq) /\ (onCurve h0 p) /\ (onCurve h0 pq) /\ (onCurve h0 q)
      /\ (nTimesQ (formula_4 h0 n ctr) (pointOf h0 q) h0 p pq)
      (* /\ modifies (refs pp +++ refs ppq +++ refs p +++ refs pq) h0 h1 *)
...
let montgomery_ladder res n q =
  ...
  // Point at infinity
  let inf_x = create (Hacl.Cast.uint64_to_sint64 0uL) (nlength+|1ul) in
  upd inf_x 0ul (Hacl.Cast.uint64_to_sint64 1uL);
  let inf_y = create (Hacl.Cast.uint64_to_sint64 0uL) nlength in
  let inf_z = create (Hacl.Cast.uint64_to_sint64 0uL) (nlength+|1ul) in
