...
val mk_mask: x:s8 -> Tot (y:s64{v y < pow2 platform_size /\ (x = 0uy ==> v y = 0) /\ (x = 1uy ==> v y = pow2 platform_size - 1)})
let mk_mask x =
  admit(); // OK
...
val nth_bit: byt:s8 -> idx:u32{w idx < 8} -> Tot (b:s8{S8.logand (S8.shift_right byt (7ul-|idx)) (1uy) = b /\ (b = 0uy \/ b = 1uy) })
...
let gsmall_step_core_lemma_3 h0 h h2 h1 pp ppq p pq q n ctr byt scalar =
  ...
  (* lemma_8 (reveal n); *)
  cut (bit = 0uy \/ bit = 1uy); 
  cut (bit = 0uy ==> nTimesQ (hide (2*(reveal n)+S8.v bit)) q0 h1 pp ppq);
  cut (bit = 1uy ==> nTimesQ (hide (2*(reveal n)+S8.v bit)) q0 h1 pp ppq);
