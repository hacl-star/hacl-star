...
val hacl_secretbox: alg:aead_alg -> output:ciphertext -> input:bytes -> len:nat{len = length input} -> n:nonce{length n = aeadRealIVSize alg} -> k:key{length k = aeadKeySize alg}
  -> STL (retcode:int)
...
val hacl_box_keypair: ec:ec_curve -> pkey:ec_point_bytes -> skey:ec_key_bytes 
  -> STL (retcode:int)
...
val hacl_box: ec:ec_curve -> alg:aead_cipher -> output:ciphertext -> input:bytes -> len:nat{len = length input} -> n:nonce{length n = aeadRealIVSize alg} -> pkey:ec_point_bytes -> skey:ec_key_bytes
  -> STL (retcode:int)
...
val hacl_box_open: ec:ec_curve -> alg:aead_cipher -> output:bytes -> input:ciphertext -> len:nat{len = length ciphertext} -> n:nonce{length n = aeadRealIVSize alg} -> pkey:ec_point_bytes -> skey:ec_key_bytes
  -> STL (retcode:int)
        (requires (fun h -> live h output /\ live h input /\ live h n /\ live h pkey /\ live h skey))
        (ensures  (fun h0 r h1 -> live h1 output /\ live h1 input /\ live h1 n /\ live h1 pkey /\ live h1 skey
                             /\ modifies_1 output h0 h1
