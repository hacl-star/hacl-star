...
In F* top-level declarations can have both a `val` declaration, and a `let` declaration. Intuitively the `val` declares the type of the object, while the `let` gives its definition.
...
In the example, the effect is `Stack`. It basically means that the function is:
- effectful,
- only performs stack allocations (no heap allocation).

It specifies a monadic computation that implicitely carries the memory of the program.
Hence the `requires` and `ensures` clauses which respectively take the program's memory before the function's execution, and the memory before execution, the result, and the memory after the execution to fully specify the stateful computation.
