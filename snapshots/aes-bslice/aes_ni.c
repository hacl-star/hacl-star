...
void rounds(state_t st, state_t key) {
  for (int i = 0; i < 9; i++)
    aes_enc(st,key+i);
}

void block_cipher(state_t out, state_t key) {
  state_t k0 = key;
  state_t k = key + 1;
  state_t kn = key + 10;
...
void key_expansion_step(state_t next, state_t prev){
...
void key_expansion(state_t out, uint8_t* key) {
...
void aes128_block(uint8_t* out,state_t kex, uint8_t* n, uint32_t c) {
  memcpy(out,n,12);
  out[12] = (uint8_t)(c >> 24);
  out[13] = (uint8_t)(c >> 16);
  out[14] = (uint8_t)(c >> 8);
