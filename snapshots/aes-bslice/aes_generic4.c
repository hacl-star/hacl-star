...
static  void to_transpose_block_copy(state_t out, uint8_t* in) {
...
static  void to_transpose_block(state_t out, uint8_t* in) {
  uint64_t fst = load64_le(in);
  uint64_t snd = load64_le(in+8);
  fst = transpose64(fst);
  snd = transpose64(snd);
...
static  void to_transpose_block_high(state_t out, uint8_t* in) {
  uint64_t fst = load64_le(in);
  uint64_t snd = load64_le(in+8);
  fst = transpose64(fst);
  snd = transpose64(snd);
...
static void from_transpose(uint8_t* out, state_t in) {
...
static  void addRoundKey(state_t st, key1_t k) {
  for (int i = 0; i < 8; i++)
    st[i] ^= k[i];
}

static  void aes_enc(state_t st, key1_t k) {
  subBytes(st);
  shiftRows(st);
  mixColumns(st);
  addRoundKey(st,k);
...
static  void aes_enc_last(state_t st, key1_t k) {
  subBytes(st);
  shiftRows(st);
  addRoundKey(st,k);
}
...
static void block_cipher(state_t out, keyex_t key) {
  uint64_t* k0 = key;
  uint64_t* k = key + 8;
...
static void aes_keygenassist(state_t next, uint8_t rcon) {
  subBytes(next);
  for (int i = 0; i < 8; i++) {
...
static void key_expansion(uint64_t* out, uint8_t* key) {
  ...
  for (int i = 1; i < 11; i++) {
    uint8_t* next = out + 8*i;
    uint8_t* prev = out + (8*i - 8);
    memcpy(next,prev,64);
    aes_keygenassist(next,rcon[i]);
    key_expansion_step(next,prev);
  }
}

static void aes128_block(uint64_t* st, uint64_t* kex, uint64_t* nt, uint32_t c) {
  uint8_t ctr[16] = {0};
  for (int i = 0; i < 4; i++) 
    store32_be(ctr+(4*i),c + i);
  to_transpose_block_high(st,ctr);
  for (int i = 0; i < 8; i++) {
    st[i] = st[i] ^ nt[i];
  }
  block_cipher(st,kex);
...
static void aes128_ctr(uint8_t* out, uint8_t* in, int in_len, uint8_t* k, uint8_t* n, uint32_t c) {
  ...
  for (int i = 0; i < blocks64; i++) {
    ...
    from_transpose(kby,kb);
    for (int j = 0; j < 64; j++) {
      out[64*i+j] = in[64*i+j] ^ kby[j];
    }
  }
