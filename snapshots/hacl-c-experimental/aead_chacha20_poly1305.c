                                             ...
                                             uint8_t *iv,         uint8_t *plaintext)
{
  ...
  // Encrypt the plaintext using the symmetric cipher
  Hacl_Symmetric_Chacha20_chacha_encrypt_bytes(ctx_chacha, ciphertext, plaintext, ciphertext_len);
  ...
  // Process Ciphertext
  i = 0;
  while (ciphertext_len > 16){
    Hacl_Impl_Poly1305_64_poly1305_update(NULL, ctx_poly, ciphertext + i * 16);
    ciphertext_len -= 16;
    i++;
  }
  uint64_t ciphertext_last[16] = {0};
  blit(ciphertext + i * 16, ciphertext_len, (uint8_t*)ciphertext_last, 0);
  Hacl_Impl_Poly1305_64_poly1305_update(NULL, ctx_poly, (uint8_t*)ciphertext_last);
