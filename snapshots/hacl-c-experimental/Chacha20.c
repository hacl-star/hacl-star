...
inline void Hacl_Symmetric_Chacha20_chacha20_update(uint32_t *ctx, uint8_t *m, uint8_t *c)
{
  Hacl_Symmetric_Chacha20_chacha_encrypt_bytes_core(ctx, m, c);
  uint32_t ctr = ctx[12];
  uint32_t one = (uint32_t )1;
...
{
  Hacl_Symmetric_Chacha20_chacha_ietf_ivsetup(ctx, k, ctr);
  return;
}

void Chacha20_chacha_encrypt_bytes(uint32_t *ctx, uint8_t *m, uint8_t *c, uint32_t l)
{
  Hacl_Symmetric_Chacha20_chacha_encrypt_bytes(ctx, m, c, l);
  return;
}
