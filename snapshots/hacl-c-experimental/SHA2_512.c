...
static void load64s_be(uint64_t *buf_64, uint8_t *buf_8, uint32_t len_8)
{
  if (len_8 == (uint32_t )0)
    return;
  else
  {
    ...
    load64s_be(buf_64, buf_8, len_8 - (uint32_t )8);
    return;
  }
}

static void store64s_be(uint8_t *buf_8, uint64_t *buf_64, uint32_t len_64)
{
  ...
  else
  {
    ...
    store64_be(x_8, x_64);
    store64s_be(buf_8, buf_64, len_64 - (uint32_t )1);
    return;
  }
}
...
static void Hacl_Hash_SHA2_512_update(uint64_t *state, uint8_t *data)
{
  ...
  load64s_be(data_w, data, (uint32_t )128);
  uint64_t *hash_w = state + (uint32_t )160;
  uint64_t *ws_w = state + (uint32_t )80;
  uint64_t *k_w = state;
  Hacl_Hash_SHA2_512_ws(ws_w, data_w, (uint32_t )0);
  KRML_CHECK_SIZE((uint64_t )(uint32_t )0, (uint32_t )8);
  uint64_t hash_0[8];
  for (uintmax_t _i = 0; _i < (uint32_t )8; ++_i)
    hash_0[_i] = (uint64_t )(uint32_t )0;
...
static void Hacl_Hash_SHA2_512_update_last(uint64_t *state, uint8_t *data, uint64_t len)
{
  ...
  K___uint32_t_uint8_t_ uu____1129;
  if (len < (uint64_t )111)
    uu____1129 = ((K___uint32_t_uint8_t_ ){ .fst = (uint32_t )1, .snd = blocks + (uint32_t )128 });
  else
    uu____1129 = ((K___uint32_t_uint8_t_ ){ .fst = (uint32_t )2, .snd = blocks });
  K___uint32_t_uint8_t_ scrut = uu____1129;
  ...
  FStar_UInt128_t
  encodedlen =
    FStar_Int_Cast_uint64_to_uint128((n1 * (uint64_t )(uint32_t )128 + len)
      * (uint64_t )(uint32_t )8);
  buf1[0] = (uint8_t )0x80;
...
{
  uint64_t *hash_w = state + (uint32_t )160;
  store64s_be(hash1, hash_w, (uint32_t )8);
  return;
}
...
uint32_t size_hash = (uint32_t )64;

uint32_t size_block = (uint32_t )128;

uint32_t size_state = (uint32_t )169;
