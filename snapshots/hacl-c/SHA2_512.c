...
static void Hacl_Hash_SHA2_512_update_last(uint64_t *state, uint8_t *data, uint64_t len)
{
  ...
  if (len < (uint64_t )112)
    ...
    final_blocks = blocks;
  memcpy(final_blocks, data, Hacl_Hash_SHA2_512_u64_to_u32(len) * sizeof data[0]);
  uint64_t n1 = state[168];
  uint8_t *padding = final_blocks + Hacl_Hash_SHA2_512_u64_to_u32(len);
  FStar_UInt128_t
  encodedlen =
  ...
  uint32_t
  pad0len =
    ((uint32_t )256 - (Hacl_Hash_SHA2_512_u64_to_u32(len) + (uint32_t )16 + (uint32_t )1))
    % (uint32_t )128;
