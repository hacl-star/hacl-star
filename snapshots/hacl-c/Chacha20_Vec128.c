...
Hacl_Impl_Chacha20_Vec128_State_state_to_key_block(uint8_t *stream_block, vec *k)
{
  ...
  *a = stream_block;
  uint8_t
  *b = stream_block + (uint32_t )16;
  uint8_t
  *c = stream_block + (uint32_t )32;
  uint8_t
  *d = stream_block + (uint32_t )48;
  vec_store_le(a, k0);
  vec_store_le(b, k1);
  vec_store_le(c, k2);
  vec_store_le(d, k3);
...
Hacl_Impl_Chacha20_Vec128_line(vec *st, uint32_t a, uint32_t b, uint32_t d, uint32_t s)
{
  ...
  sa1 = vec_add(sa, sb);
  vec
  sd1 = vec_rotate_left(vec_xor(sd, sa1), s);
  st[a] = sa1;
  st[d] = sd1;
...
Hacl_Impl_Chacha20_Vec128_store_4_vec(uint8_t *output, vec v0, vec v1, vec v2, vec v3)
{
  ...
  *o0 = output;
  uint8_t
  *o1 = output + (uint32_t )16;
  uint8_t
  *o2 = output + (uint32_t )32;
  uint8_t
  *o3 = output + (uint32_t )48;
  vec_store_le(o0, v0);
  vec_store_le(o1, v1);
  vec_store_le(o2, v2);
  vec_store_le(o3, v3);
...
{
  vec
  p0 = vec_load_le(plain);
  vec
  p1 = vec_load_le(plain + (uint32_t )16);
  vec
  p2 = vec_load_le(plain + (uint32_t )32);
  vec
  p3 = vec_load_le(plain + (uint32_t )48);
  vec
  k0 = st[0];
  vec
  k1 = st[1];
...
{
  uint8_t
  *out_block = output + (uint32_t )192 * i;
  uint8_t
  *plain_block = plain + (uint32_t )192 * i;
...
)
{
  ...
  len3 = len / (uint32_t )3;
  uint32_t
  rest3 = len % (uint32_t )3;
  uint8_t
  *plain_ = plain;
  uint8_t
  *blocks1 = plain + (uint32_t )192 * len3;
  uint8_t
  *output_ = output;
  uint8_t
  *outs = output + (uint32_t )192 * len3;
  ...
  if (rest3 == (uint32_t )2)
  {
    ...
    *block0 = blocks1;
    uint8_t
    *block1 = blocks1 + (uint32_t )64;
    uint8_t
    *out0 = outs;
    uint8_t
    *out1 = outs + (uint32_t )64;
...
{
  uint32_t
  blocks_len = len >> (uint32_t )6;
  uint32_t
  part_len = len & (uint32_t )0x3f;
  uint8_t
  *output_ = output;
  uint8_t
  *plain_ = plain;
  uint8_t
  *output__ = output + (uint32_t )64 * blocks_len;
  uint8_t
  *plain__ = plain + (uint32_t )64 * blocks_len;
