...
{
  //KB changed for auto-vec. we should get Kremlin or GCC to unroll these loops
  /*
...
)
{
  KRML_CHECK_SIZE((uint32_t )0, (uint32_t )16);
  uint32_t k[16] = { 0 };
  Hacl_Impl_Chacha20_chacha20_core((void *)(uint8_t )0, k, st, ctr);
  KRML_CHECK_SIZE((uint32_t )0, (uint32_t )16);
  uint32_t ib[16] = { 0 };
  KRML_CHECK_SIZE((uint32_t )0, (uint32_t )16);
  uint32_t ob[16] = { 0 };
  Hacl_Impl_Chacha20_uint32s_from_le_bytes(ib, plain, (uint32_t )16);
  for (uint32_t i = (uint32_t )0; i < (uint32_t )16; i = i + (uint32_t )1)
  {
    uint32_t _0_35 = ib[i];
    uint32_t _0_34 = k[i];
  ...
  Hacl_Impl_Chacha20_uint32s_to_le_bytes(output, ob, (uint32_t )16);
}
