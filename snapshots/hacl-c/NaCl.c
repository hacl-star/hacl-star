...
)
{
  ...
  *subkey = tmp;
  uint8_t
  *mackey = tmp + (uint32_t )32;
  uint8_t
  *mackey_ = tmp + (uint32_t )64;
  uint8_t
  *cmac = tmp + (uint32_t )96;
  Salsa20_hsalsa20(subkey, k1, n1);
...
)
{
  ...
  cmac[16] = { 0 };
  uint32_t
  res = Hacl_SecretBox_ZeroPad_crypto_secretbox_detached(c, cmac, m, mlen, n1, k1);
...
)
{
  ...
  *k1 = key;
  uint8_t
  *subkey = key + (uint32_t )32;
  uint8_t
  *hsalsa_n = key + (uint32_t )64;
...
)
{
  ...
  cmac[16] = { 0 };
  uint32_t
  z = Hacl_Box_ZeroPad_crypto_box_detached_afternm(c, cmac, m, mlen, n1, k1);
  memcpy(c + (uint32_t )16, cmac, (uint32_t )16 * sizeof cmac[0]);
  return z;
...
)
{
  ...
  cmac[16] = { 0 };
  uint32_t
  res = Hacl_Box_ZeroPad_crypto_box_detached(c, cmac, m, mlen, n1, pk, sk);
  memcpy(c + (uint32_t )16, cmac, (uint32_t )16 * sizeof cmac[0]);
  return res;
...
)
{
  ...
  *mac = c;
  uint32_t
  t = Hacl_Box_ZeroPad_crypto_box_open_detached_afternm(m, c, mac, mlen, n1, k1);
  return t;
