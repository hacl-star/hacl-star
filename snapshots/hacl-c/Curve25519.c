...
)
{
  ...
  *xprime = pq;
  uint64_t
  *zprime = pq + (uint32_t )5;
  uint64_t
  buf[40] = { 0 };
  uint64_t
  *origx = buf;
  uint64_t
  *origxprime = buf + (uint32_t )5;
  uint64_t
  *xxprime0 = buf + (uint32_t )25;
  uint64_t
  *zzprime0 = buf + (uint32_t )30;
...
static void Hacl_EC_Ladder_cmult(uint64_t *result, uint8_t *n1, uint64_t *q)
{
  ...
  *nq = point_buf;
  uint64_t
  *nqpq = point_buf + (uint32_t )10;
  uint64_t
  *nq2 = point_buf + (uint32_t )20;
  uint64_t
  *nqpq2 = point_buf + (uint32_t )30;
