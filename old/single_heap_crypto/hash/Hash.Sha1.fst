...
let k_init () =
  let k = SBuffer.create #32 SInt.UInt32.zero 80  in
  k_init_inner k (ST.alloc 0);
  k
...
val store : (wdata :buffer 32) ->
            (pdata :buffer 8 { length pdata = (Prims.op_Multiply 4 (length wdata)) /\ disjoint wdata pdata }) ->
            (plen  :nat        { length pdata = plen /\ plen = (Prims.op_Multiply 4 (length wdata)) })
            -> ST unit
...
let rec wsched_define ws wblock t =
  if !t < 16 then begin
    ...
    let w = (SInt.UInt32.logxor _t3
                                 (SInt.UInt32.logxor _t8
                                                      (SInt.UInt32.logxor _t14 _t16)))
    in
    let v = SInt.UInt32.rotate_left w 1 in
    SBuffer.upd ws !t v;
    t := !t + 1;
    wsched_define ws wblock t end
  else ()
...
let rec update_inner_loop ws whash t t1 k =
  if !t < 80 then begin
    ...
    let _rotl5 = SInt.UInt32.rotate_left _a 5 in
    let _rotl30 = SInt.UInt32.rotate_left _b 30 in
    let _ft = _F !t _b _c _d in
    let _kt = index k !t in
    let _wt = index ws !t in

    let _T = SInt.UInt32.add _rotl5
                              (SInt.UInt32.add _ft
                                                (SInt.UInt32.add _e
                                                                  (SInt.UInt32.add _kt _wt)))
    in
...
let rec update_step ihash wdata ws rounds i t1 k =
  if !i < rounds then begin
    ...
    let whash = create #32 SInt.UInt32.zero 5 in
...
let update whash wdata rounds =
  ...
  (* Scheduling function *)
  let ws = create #32 SInt.UInt32.zero 80 in
  (* Initialize constant *)
  let k = k_init () in
  (* Perform function *)
