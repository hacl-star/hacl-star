...
bool
print_result(uint8_t* comp, uint8_t* exp, int len)
{
  return compare_and_print(len, comp, exp);
}

bool
print_test1(uint8_t* in, int in_len, uint8_t* exp256, uint8_t* exp512)
{
  uint8_t comp256[32] = { 0 };
  uint8_t comp512[64] = { 0 };

  Hacl_Streaming_SHA2_hash_256(comp256, in, in_len);
  printf("NEW SHA2-256 (32-bit) Result:\n");
  bool ok = print_result(comp256, exp256, 32);

  Hacl_Streaming_SHA2_hash_512(comp512, in, in_len);
            ...
            uint8_t* exp3)
{
  ...
  Hacl_SHA2_Vec128_sha256_4(
    comp, comp1, comp2, comp3, in_len, in, in1, in2, in3);
            ...
            uint8_t* exp7)
{
  ...
  bool ok = true;
#if defined(HACL_CAN_COMPILE_VEC256)
  Hacl_SHA2_Vec256_sha256_8(comp,
                            comp1,
                            comp2,
                            comp3,
                            comp4,
...
main()
{
  ...
  ok &= print_test4(vectors_mb[0].input,
                    vectors_mb[1].input,
                    vectors_mb[2].input,
                    vectors_mb[3].input,
                    vectors_mb[0].input_len,
                    vectors_mb[0].tag_256,
                    vectors_mb[1].tag_256,
                    vectors_mb[2].tag_256,
                    vectors_mb[3].tag_256);

  if (EverCrypt_AutoConfig2_has_avx2()) {
    ok &= print_test8(vectors_mb[0].input,
                      vectors_mb[1].input,
                      vectors_mb[2].input,
                      vectors_mb[3].input,
                      vectors_mb[4].input,
                      ...
                      vectors_mb[2].tag_256,
                      vectors_mb[3].tag_256,
                      vectors_mb[4].tag_256,
                      vectors_mb[5].tag_256,
                      vectors_mb[6].tag_256,
                      vectors_mb[7].tag_256);

    ok &= print_test4_512(vectors_mb[0].input,
                          vectors_mb[1].input,
                          vectors_mb[2].input,
                          vectors_mb[3].input,
                          vectors_mb[0].input_len,
                          vectors_mb[0].tag_512,
                          vectors_mb[1].tag_512,
                          vectors_mb[2].tag_512,
  ...
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_Streaming_SHA2_hash_256(plain, plain, SIZE);
  }
...
#if defined(HACL_CAN_COMPILE_VEC128)
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_SHA2_Vec128_sha256_4(plain,
                              plain + 32,
                              plain + 64,
                              plain + 96,
                              SIZE,
  ...
  if (EverCrypt_AutoConfig2_has_avx2()) {
    for (int j = 0; j < ROUNDS; j++) {
      Hacl_SHA2_Vec256_sha256_8(plain,
                                plain + 32,
                                plain + 64,
                                plain + 96,
                                plain + 128,
  ...
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_Streaming_SHA2_hash_512(plain, plain, SIZE);
  }
  ...
  if (EverCrypt_AutoConfig2_has_avx2()) {
    for (int j = 0; j < ROUNDS; j++) {
      Hacl_SHA2_Vec256_sha512_4(plain,
                                plain + 64,
                                plain + 128,
                                plain + 192,
                                SIZE,
  ...
  print_time(count, tdiff2n, cdiff2n);

#if defined(HACL_CAN_COMPILE_VEC128)
  printf("VEC4 SHA2-256 (32-bit) PERF: %d\n", (int)res);
  print_time(count, tdiff2v, cdiff2v);
#endif

#if defined(HACL_CAN_COMPILE_VEC256)
  if (EverCrypt_AutoConfig2_has_avx2()) {
    printf("VEC8 SHA2-256 (32-bit) PERF: %d\n", (int)res);
    print_time(count, tdiff2v8, cdiff2v8);
  }
#endif
  printf("\n\n");
