...
typedef struct Hacl_Streaming_Poly1305_256_state_t poly1305_state;
...
main()
{
  ...
  if (EverCrypt_AutoConfig2_has_avx2()) {
    poly1305_state* s = Hacl_Streaming_Poly1305_256_malloc(v->key);
    ...
    Hacl_Streaming_Poly1305_256_digest(s, tag);
    ok &= compare_and_print(16, tag, v->tag);

    v++;
    Hacl_Streaming_Poly1305_256_reset(v->key, s);
