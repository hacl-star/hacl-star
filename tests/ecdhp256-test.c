...
test_nist()
{
  ...
  for (int i = 0; i < sizeof(i_vectors) / sizeof(ecdhp256_tv_i); i++) {
    ...
    ok =
      ok && compare_and_print(32, result + 32, i_vectors[i].expectedPublicKeyY);
  }
...
#define Hacl_P256_decompression_not_compressed_form                            \
...
test_compression()
{
  ...
  static uint8_t point_compressed[64] = {
    ...
    0xf6, 0xb4, 0x2c, 0xe7, 0xcc, 0x83, 0x88, 0x33, 0xd2, 0x87, 0xdb,
    0x71, 0xe5, 0x09, 0xe3, 0xfd, 0x9b, 0x06, 0x0d, 0xdb, 0x20, 0xba,
    0x5c, 0x51, 0xdc, 0xc5, 0x94, 0x8d, 0x46, 0xfb, 0xf6, 0x40, 0xdf,
...
test_wycheproof()
{
  ...
  for (int i = 0; i < sizeof(w_vectors) / sizeof(ecdhp256_w_i); i++) {
    ...
    bool success = Hacl_P256_dh_responder(
      result, decompressedPoint, w_vectors[i].privateKey);

    if (w_vectors[i].flag != 1) {
      // We start with the check that the point was decompressed successfully
      // and that the execution of the dh_r function was successul. If one of
      // them has failed, we consider the result of the test to be false. This
      // result is further compared with the expected flag. If the execution was
      // correct, we also check the results.

      if (!flagDecompressSuccessful)
        success = false;

      // if the flag that the test returns states that the execution was
      // INCORRECT but the flag we got is opposite - we consider the test to
      // fail
  ...
  for (int i = 0; i < sizeof(w_ecdsa_vectors) / sizeof(ecdsap256_w_i); i++) {
    ...
    bool verificationSuccessful =
      Hacl_P256_ecdsa_verif_p256_sha2(mLen,
                                      w_ecdsa_vectors[i].message,
                                      pk,
                                      w_ecdsa_vectors[i].r,
                                      w_ecdsa_vectors[i].s);
