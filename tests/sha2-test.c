...
void
ossl_sha2(uint8_t* hash, uint8_t* input, int len)
{
  SHA256_CTX ctx;
  SHA256_Init(&ctx);
           ...
           uint8_t* exp512)
{
  ...
  Hacl_Streaming_SHA2_hash_224(comp, in, in_len);
  printf("SHA2-224 (32-bit) Result:\n");
  bool ok = print_result(comp, exp224, 28);

  Hacl_Streaming_SHA2_hash_256(comp, in, in_len);
  ...
  Hacl_Streaming_SHA2_hash_384(comp, in, in_len);
  printf("SHA2-384 (32-bit) Result:\n");
  ok = print_result(comp, exp384, 48) && ok;

  Hacl_Streaming_SHA2_hash_512(comp, in, in_len);
...
main()
{
  ...
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_Streaming_SHA2_hash_224(plain, plain, SIZE);
  }
  t1 = clock();
  a = cpucycles_begin();
  for (int j = 0; j < ROUNDS; j++) {
  ...
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_Streaming_SHA2_hash_384(plain, plain, SIZE);
  }
  t1 = clock();
  a = cpucycles_begin();
  for (int j = 0; j < ROUNDS; j++) {
  ...
  print_time(count, tdiff2a, cdiff2a);
