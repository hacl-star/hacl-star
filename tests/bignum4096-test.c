...
)
{
  ...
  Hacl_Bignum4096_mod_exp(n, a, bBits, b, res);
  Hacl_Bignum4096_bn_to_bytes_be(res, resBytes);
...
)
{
  ...
  bool ok = mod_exp_bytes_be_precompr2(nBytes, aBytes, bBits, bBytes, resBytes);
...
int main() {
  ...
  for (int i = 0; i < sizeof(vectors_be)/sizeof(bignum4096_bytes_be_test_vector); ++i) {
    ok &= print_test_bytes_be_precompr2(vectors_be[i].nBytes, vectors_be[i].aBytes, vectors_be[i].bBits, vectors_be[i].bBytes, vectors_be[i].resBytes);
  }

  for (int i = 0; i < sizeof(vectors_be)/sizeof(bignum4096_bytes_be_test_vector); ++i) {
    ok &= print_test_bytes_be(vectors_be[i].nBytes, vectors_be[i].aBytes, vectors_be[i].bBits, vectors_be[i].bBytes, vectors_be[i].resBytes);
  }
