...
int main(int argc, char *argv[]) {
  ...
  for (uint64_t k = 0; k < num_elts; k++) {
    uint32_t sz = mt_get_path(mt, k, path, root);

    printf("Path from k=%lu:\n", k);
    uint8_t *tmp = init_hash();
    memcpy(tmp, path->vs[0], hash_size);
    for (uint32_t l = 0; l < path->sz; l++) {
      print_hash("  elem", path->vs[l]);
      if (l > 0) {
        hash_2(tmp, path->vs[l], tmp);
        print_hash("  tmp ", tmp);
      }
    }
    free_hash(tmp);
    ...
    bool verified = mt_verify(mt, k, sz, path, root);
...

  {
    ...
    for (uint64_t k = flush_to; k < num_elts; k++) {
      ...
      uint8_t buffer[2048];
      uint32_t spsz = mt_serialize_path(path, mt, buffer, 2048);
      assert(spsz > 0);
      hash_vec **dpath = mt_deserialize_path(buffer, 2048);
      assert(dpath != NULL);
