...
print_test(int in_len, uint8_t* in, uint8_t* key, uint8_t* exp)
{
  ...
  Hacl_MAC_Poly1305_Simd128_mac(comp, in, in_len, key);
...
main()
{
  ...
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_MAC_Poly1305_Simd128_mac(plain, plain, SIZE, key);
  }

  t1 = clock();
  a = cpucycles_begin();
  for (int j = 0; j < ROUNDS; j++) {
    Hacl_MAC_Poly1305_Simd128_mac(tag, plain, SIZE, key);
    res ^= tag[0] ^ tag[15];
  }
  b = cpucycles_end();
  t2 = clock();
  ...
  printf("Poly1305 (128-bit) PERF: %"PRIu64"\n", res);
